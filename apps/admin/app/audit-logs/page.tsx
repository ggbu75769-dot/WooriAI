...
/**
 * before/after 스냅샷 상세. **봉투를 그대로 JSON으로 편다** — 이 화면은 축을 골라 그리지 않는다.
 *
 * ⚠️ 그래서 *"무엇이 여기 서는가"* 는 전부 **쓰기 경로**가 정한다. 조회 API의 마스킹
 * (`admin/audit-logs.service.ts`)은 `password|token|secret…` 같은 **키 이름**으로만 도는
 * 방어막이라, 자유 문자열(품목명·판매처·메모)은 그 그물에 걸리지 않는다.
 *
 * 라운드 107 트랙 A(정찰 S1-1): 그 구멍으로 지출 감사 봉투가 사용자 자유 문자열을 원문으로
 * 이 화면과 CSV로 내보내고 있었다. 고친 자리는 여기가 아니라 봉투를 만드는 쪽이다
 * (`api/src/onboarding/store-shared.ts`의 `toExpenseAuditSnapshot`) — 그리고 옛 행은 파기 잡
 * phase 12가 씻는다. **이 화면에서 축을 숨겨 고치려 하지 말 것**: 숨겨도 CSV·API 응답에는
 * 그대로 남고, 봉투에 없는 값은 여기서도 저절로 사라진다.
 *
