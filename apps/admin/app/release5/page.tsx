    ...
    <div className={styles.page}>
      ...
      {!pilot || !recalls ? <p className={styles.emptyState}>준비 상태를 불러오는 중...</p> : <>
        <section className={styles.card}>
          ...
          <p>후보 {pilot.counts.candidates} · 준비 완료 {pilot.counts.ready} · 근거 누락 {pilot.counts.missingEvidence} · domain 승인 누락 {pilot.counts.missingDomainApproval}</p>
          {readyItems.length === 0 ? <p className={styles.emptyState}>현재 manifest에 넣을 수 있는 승인 완료 저위험 품목이 없어요.</p> : <div className={styles.tableWrap}><table className={styles.table}><thead><tr><th>선택</th><th>품목</th><th>revision</th><th>근거</th><th>domain 승인</th></tr></thead><tbody>{readyItems.map((item) => <tr key={item.id}><td><input aria-label={`${item.nameKo} pilot 선택`} type="checkbox" checked={selectedPilotIds.includes(item.id)} disabled={!canDraft || pendingAction !== null} onChange={() => setSelectedPilotIds((current) => current.includes(item.id) ? current.filter((id) => id !== item.id) : [...current, item.id])} /></td><td>{item.nameKo}<br /><span className={styles.hint}>{item.code}</span></td><td>{item.contentVersion}</td><td>{item.evidenceReady ? "완료" : "누락"}</td><td>{item.domainApproved ? "완료" : "누락"}</td></tr>)}</tbody></table></div>}
