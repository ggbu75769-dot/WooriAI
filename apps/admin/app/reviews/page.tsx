      ...
      {selectedId ? (
        <section className={styles.card}>
          ...
          {detail ? (
            ...
            /* 라운드 106 트랙 T5 — **실패한 조회를 "불러오는 중"이라고 부르지 않는다.**
               종전에는 상세 조회가 실패해도 이 갈래가 그대로 서서, 오류 배너 바로 아래에
               "불러오는 중..."이 **영영** 남았다(다시 부르는 자리가 없으므로 그 문장은
               끝내 사라지지 않는다). 운영자는 아직 기다리면 되는 화면인지 이미 끝난
               실패인지 알 수 없었다. 이 화면의 형제 자리들이 이미 쓰는 판정
               (`x === null && !loadError`)을 그대로 가져온다 — 새 상태도 새 문구도 0건이고,
               실패하지 않은 로딩에서는 종전과 같은 문장이 같은 자리에 선다. */
