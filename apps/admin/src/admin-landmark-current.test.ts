...
const BLIND_SPOTS: readonly BlindSpot[] = [
  ...
  },
  {
    ...
    reason:
      ...
      "⚠️⚠️ **두 시점** — 라운드 92 트랙 C에서 그 결정이 **섰다**: 짝을 지는 계약은 " +
      "`src/admin-route-surface.test.ts`(어드민 앱 **안**의 스윕이고 저장소 그물이 아니다)이고, " +
      "그 계약이 오늘 *없는 페이지를 가리키는 href 0건 · 내비에 없는 페이지 0건*을 양방향으로 물며 " +
      "평면 전제(동적 세그먼트 0건)까지 함께 진다. **이 칸은 그날 이후에도 짝을 세지 않는다** — " +
      "여기서 재는 것은 여전히 표면의 크기 하나이고, 달라진 것은 *그 짝을 누가 지는가*가 " +
      "P3에서 옆 파일로 옮겨 갔다는 사실뿐이다",
    measure: () => routePagePaths().length,
    today: 11,
    resumeCondition:
      "재개 조건(결정형 · 손은 저장소 안): 라우트 표면과 내비 표의 짝을 어느 계약이 지는지를 P3가 " +
      "정하는 날 — 오늘 그 축은 이 스윕이 아니라 라우트 표면 그물의 것이다. " +
      "⚠️⚠️ 두 시점: **그 결정이 라운드 92 트랙 C에서 섰고**(옛 문장은 그 이전의 시점이라 지우지 " +
      "않는다), 그날 이후의 재개 조건은 **사건형**으로 바뀐다 — `src/admin-route-surface.test.ts`가 " +
      "어드민에서 사라지거나 그 축이 다른 파일로 옮겨 가는 날, 이 칸이 가리키는 자리가 거짓이 되므로 " +
      "이 문장을 세 번째 시점으로 다시 적어야 한다"
  },
  {
