  ...
  const HAND_LIST_BLIND_SPOTS: readonly HandListBlindSpot[] = [
    ...
    },
    {
      ...
      reason:
        ...
        "계약 파일 가운데 이 바늘이 **보지 않는** 파일 수(오늘 43 = 전수 44 − 자기 하나). " +
        "⚠️ 두 시점: 라운드 94 트랙 C 시점의 값은 41(전수 42)이었다. 라운드 106 트랙 T5가 계약을 " +
        "하나 세우면서(`admin-control-name.test.ts` — 입력칸의 접근 가능한 이름) 이 줄이 먼저 " +
        "빨개졌고, 이 문단이 요구하는 대로 그 라운드가 **그 파일도 손 목록을 지녔는가**를 보았다: " +
        "**아니다** — 그 스윕의 모집단은 파일 전수 파생이고(`readdirSync`), 파일에 선 배열 상수는 " +
        "뿌리 둘(`app`·`src`)·태그 셋·갈래 표·빈 면제 목록뿐이라 경로꼴 문자열 셋 이상이 서는 " +
        "자리가 없다. 그래서 옮길 손 목록도 0건이다. " +
        "⚠️ 두 시점(라운드 107 트랙 D): 42 → 43. 계약이 하나 더 섰다 — " +
        "`admin-sponsor-label.test.ts`(스폰서 표시 문구 칸이 링크 폼에 실제로 서는가 · DNC-011). " +
        "이 문단이 요구하는 대로 **그 파일도 손 목록을 지녔는가**를 보았다: **아니다** — 그 파일에는 " +
        "배열 상수가 0건이고(경로는 `readSource(\"…\")` 인자로 한 번씩 쓰인다) 모집단을 순회하는 " +
        "자리도 없다. 그래서 옮길 손 목록은 이번에도 0건이다. " +
        "⚠️ 두 시점(라운드 107 트랙 I): 43 → 44. 계약이 또 하나 섰다 — " +
        "`src/lib/admin-auth-rejection.test.ts`(서버가 토큰을 거절했을 때 어드민이 무엇을 말하고 " +
        "어디로 보내고 무엇을 지우는가 · 401 넷 + 403 넷). 이 문단이 요구하는 대로 **그 파일도 손 " +
        "목록을 지녔는가**를 보았다: **아니다** — 배열 상수 선언이 0건이고(모집단은 전부 `for...of` " +
        "인라인) 경로꼴 문자열도 0건이라 옮길 손 목록이 없다. " +
        "⚠️ 두 시점(라운드 107 트랙 J): 44 → 45. 계약이 또 하나 섰다 — " +
        "`src/lib/admin-session-notice.test.ts`(세션이 끝난 이유가 로그인 화면에서 문장이 되는가 · " +
        "권한 부족 403의 영문이 조회 한 벌에서도 서지 않는가 · 세션 확인 실패가 로그아웃이 아닌가 · " +
        "그리고 로그인·MFA·비밀번호의 401 넷이 그 새 채널을 타지 않는가). 이 문단이 요구하는 대로 " +
        "**그 파일도 손 목록을 지녔는가**를 보았다: **처음에는 예였고, 그래서 옮겼다** — 초안은 " +
        "화면 소스 열세 경로를 배열 상수로 손에 들고 있었는데(경로꼴 문자열 셋 이상이 서는 바로 그 꼴), " +
        "그 목록은 새 화면이 붙는 날 조용해서 그 트랙이 세우려는 스윕 자체가 반쪽이 된다. " +
        "뿌리 둘(`app`·`src/components`)을 걷는 전수 파생으로 바꿨고 지금 그 파일의 손 목록은 0건이다. " +
        "배치도 값이다: 지시는 `app/` 아래였으나 그러면 `packages/test-utils`의 comment-tolerant " +
        "대장이 *`apps/admin/app` 아래에는 오늘 계약 파일이 0건이다*(`toEqual([])`)로 빨개지는데 그 " +
        "파일은 이 라운드가 읽기 전용으로 다루는 자라, 고칠 수 있는 쪽이 이 한 줄인 `src/lib/`를 골랐다. " +
        "⚠️ 두 시점(라운드 110 트랙 1): 45 → 46. 계약이 또 하나 섰다 — " +
        "`src/admin-validation-reason.test.ts`(400 봉투의 `details.fields` 사유가 화면 문장까지 " +
        "닿는가 · 영문 필드명이 그 길에서 걸러지는가). 이 문단이 요구하는 대로 **그 파일도 손 " +
        "목록을 지녔는가**를 보았다: **아니다** — 경로는 `readAdminSource(\"…\")`·" +
        "`readRepoSource(\"…\")` 인자로 한 번씩 쓰이고, 그 파일의 배열 상수는 응답 본문 모양 " +
        "여섯을 담은 `shapes`뿐이라 경로꼴 문자열이 0건이다(순회하는 인라인 배열 둘도 라벨·패키지 " +
        "이름이라 같은 이유로 밖이다). 그래서 옮길 손 목록은 이번에도 0건이다. " +
