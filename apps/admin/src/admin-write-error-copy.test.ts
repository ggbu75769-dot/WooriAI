...
describe("트랙 B의 무접촉 계약", () => {
  it("조회 쪽 대장은 열여섯 그대로다 (라운드 75가 닫은 자리 · 늘지도 줄지도 않는다)", () => {
    ...
    /**
     * ⚠️ 두 시점(라운드 107 트랙 J) — **이 줄은 바이트 그대로 남는다. 남긴 것이 결정이다.**
     *
     * 그 라운드는 조회 한 벌에도 "한글이 한 자도 없는 서버 문장은 화면에 세우지 않는다"는
     * 겹을 세워야 했고(역할 강등 직후의 GET이 `ADMIN_FORBIDDEN` / 영문으로 돌아와 한국어
     * 화면 열여섯 자리에 그대로 섰다), 첫 안은 이 파일의 술어를 내보내 조회가 부르는 것이었다.
     * 그러면 조회 한 벌이 이 파일의 **이름**을 지니게 되어 이 줄이 빨개진다.
     *
     * 이 줄을 고치는 대신 **설계를 바꿨다**: 조회 한 벌은 문자 범위를 자기 파일에 따로 적고
     * (그 상수의 주석이 사본인 이유를 진다), 두 사본이 갈리지 않는지는
     * `src/lib/admin-session-notice.test.ts` ⓑ의 마지막 단언이 문다 — 같은 문장 넷에 두 한 벌이
     * 같은 답을 내는가. ⚠️ 이 줄의 바이트는 `packages/test-utils`의 주석 관용 앵커 대장이
     * **면제의 증명**으로 들고 있어서, 고치면 그 대장이 함께 빨개진다(읽기 전용 자리다).
     */
