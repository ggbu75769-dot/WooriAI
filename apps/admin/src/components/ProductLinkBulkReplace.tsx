...
// 시드 링크를 CSV 한 번 업로드로 실제 제휴 딥링크로 교체한다(플랜 A 전환).
// 두 시점: 라운드 82 B가 링크 0건 품목 넷을 채워 58 → 62, 라운드 83 A가 스폰서 링크만 있던
// 다섯에 비스폰서 링크를 더해 62 → 67(당시 example.com 플레이스홀더 · URL 문자열로 86곳).
// 그 뒤 출시 트랙 LP-A(플랜 B)가 67건 전부를 일반(비제휴) 쿠팡 검색 링크로 교체해
// example.com은 시드에서 0건이다 — 이 도구의 남은 일은 플랜 A 딥링크 투입이고, 그때 쓸
// 템플릿은 docs/5차/plan-a-affiliate-links-template.csv다(활성 링크 62건 = 행 62).
// ⚠️ 링크 수를 세는 자리는 docs/5차/day1-deploy-runbook.md A-5의 실행되는 인용이고
