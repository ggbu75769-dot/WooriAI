  ...
  async setPreparedItems(user: AuthenticatedUser, childId: string, itemTemplateIds: string[]) {
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      if (appliedItemTemplateIds.length > 0) {
        ...
        //
        // ⚠️ **덮어쓰기 성질이 종전과 같다 — 단, 그 동치는 "이 요청 하나" 기준이다.** 오늘의
        // upsert도 조건 없이 `prepared`를 쓰므로 `gifted`/`not_needed` 행을 덮었고,
        // `updateMany`가 하는 일이 정확히 그것이다(여기는 사용자가 준비템 화면에서 **직접 고른**
        // 경로다 — 지출 기록이 자동으로 표시하는 경로만 이미 정리된 상태를 보존한다:
        // store-shared.ts의 `markLinkedItemPrepared`). 요청에 없는 항목의 행은 양쪽 다 건드리지
        // 않는다.
        //
        // ⚠️ 라운드 82 리뷰 L-11 — **동치의 범위를 값으로 좁힌다.** 두 문장 **사이**에는 창이 하나
        // 있다: 그 순간 다른 트랜잭션이 같은 `(childId, itemTemplateId)` 행을 **새로 만들어**
        // 커밋하면(예: 다른 기기에서 그 품목을 "선물받음"으로 눌러 행이 처음 생기는 경우),
        // `updateMany`는 그 행을 보지 못했고 `createMany({ skipDuplicates: true })`는 그 행을
        // 건너뛴다 — 그래서 그 한 항목만 종전 upsert와 달리 **덮이지 않는다**. 종전 upsert는 단일
        // 문장(`INSERT ... ON CONFLICT DO UPDATE`)이라 그 창이 없었다.
        //
        // 그래서 이 배치의 동치 주장은 **단일 요청 기준**이다(동시 쓰기가 없는 창에서 상태 전이가
        // 종전과 한 칸도 다르지 않다 — e2e 동치 계약이 그것을 문다). 그 창까지 닫으려면
        // `createMany` **뒤에 `updateMany`를 한 번 더** 두면 되고(문장 넷 고정), 채택하지 않은
        // 이유는 둘이다: ① 그 창에서 살아남는 값은 사용자가 **방금 직접 누른** 더 구체적인 상태
        // (`gifted`)라, 온보딩 목록의 일괄 `prepared`가 그것을 되덮는 쪽이 오히려 나쁘다. ② 이
        // 자리의 문장 수 셋은 문서 넷이 인용하는 값이라(확인의 표 `#141` ⓔ 등) 넷으로 바꾸는 것은
        // 계약 변경이다. ⚠️ 동시 쓰기 창까지 동치로 만들어야 하는 라운드는 ①을 먼저 재정할 것.
        //
