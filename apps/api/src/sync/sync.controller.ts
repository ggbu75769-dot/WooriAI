...
import { SyncChangesQueryDto, SyncChangesV2QueryDto } from "./dto/sync-query.dto";
...
export class SyncController {
  ...
  async changesV2(
    @Req() request: AuthenticatedRequest,
    @Query(createDtoValidationPipe(SyncChangesV2QueryDto)) query: SyncChangesV2QueryDto
  ) {
    return await this.sync.getChangesV2(
      request.user!,
      query.householdId,
      query.cursor,
      query.limit
