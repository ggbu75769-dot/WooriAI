...
describe("라운드 82 C — 준비템 저장의 왕복 수 계약", () => {
  /**
   * 비교의 작은 쪽/큰 쪽 항목 수. 큰 쪽이 작은 쪽의 **세 배**라는 것 말고 다른 뜻은 없다
   * (정찰이 적은 그대로: "항목 수를 늘려도 문장 수가 그만큼 늘지 않을 것").
   * ⚠️ 카탈로그의 크기를 뜻하지 않는다 — 그 수는 시드 값이지 계약이 아니고, 여기서는
   * **측정점 둘**을 고르는 데만 쓴다.
   * ⚠️ 라운드 82 리뷰 L-10: 종전 40에서 60으로 벌렸다 — 측정점 사이가 넓어야 상수 오버헤드의
   * 흔들림(아래 허용치)과 비례 신호가 자릿수로 갈린다(가져오기 쪽 100/400과 같은 이유).
   */
  const SMALL_ITEMS = 20;
  const LARGE_ITEMS = 60;

  /**
   * 상수 오버헤드의 흔들림 허용치(문장 수).
   *
   * ⚠️ 라운드 82 리뷰 L-10: 종전 단언은 여유를 `(LARGE - SMALL) / 10`으로 적었는데, 측정점이
   * 20/40이라 그 값이 **2**였다 — 즉 증가분이 0이나 1일 때만 초록이었다. 바로 옆 주석은
   * "상수 오버헤드가 요청마다 한두 문장씩 흔들릴 수 있다"고 적어 두었으므로, **주석이 허용한
   * 흔들림이 곧 단언을 깨는** 상태였다(가져오기 쪽은 같은 식이 100/400이라 여유가 30이다).
   *
   * 그래서 여유를 **행 수에서 파생시키는 대신 상수 오버헤드의 크기로** 적는다. 이 수가 뜻하는
   * 것은 "요청 하나가 항목 수와 무관하게 흘릴 수 있는 문장 수"이고, 비례/비비례를 가르는 신호
   * (측정점 차이 = 40)와는 자릿수가 다르다 — 그 대소를 아래에서 단언으로 함께 고정한다.
   */
  const STATEMENT_JITTER_TOLERANCE = 5;
  ...
  it("고른 항목 수를 세 배로 해도 문장 수는 그만큼 늘지 않는다", async () => {
    const accessToken = await login(app);
    ...
    // ⓑ 항목 수를 세 배로 늘렸을 때의 **증가분**이 상수 오버헤드의 흔들림 안에 있다.
    //    비례하는 구현에서는 증가분이 늘어난 항목 수와 같아지므로(40) 반드시 빨개진다.
    //    ⚠️ 라운드 82 리뷰 L-10: 여유를 측정점 차이에서 파생시키던 종전 식은 20/40에서 값이 2라
    //    "한두 문장 흔들림"을 허용한다고 적어 놓고도 그 흔들림에 깨졌다(위 상수 주석).
    expect(largeStatements - smallStatements).toBeLessThanOrEqual(STATEMENT_JITTER_TOLERANCE);
    //    여유가 신호를 잡아먹지 않는다는 것도 함께 고정한다 — 이 대소가 깨지면 ⓑ는 계약이 아니다.
    expect(STATEMENT_JITTER_TOLERANCE * 2).toBeLessThan(LARGE_ITEMS - SMALL_ITEMS);
  });
});

/**
 * ⚠️ 라운드 82 리뷰 L-11 — **이 스위트가 재는 동치의 범위는 "단일 요청"이다.**
 * `updateMany` → `createMany({ skipDuplicates: true })` 사이에 다른 트랜잭션이 같은
 * `(childId, itemTemplateId)` 행을 **새로 만들어 커밋하는** 창에서는 그 한 항목이 종전 upsert와
 * 달리 덮이지 않는다(근거와 채택하지 않은 대안은 `onboarding-core.service.ts`의 `setPreparedItems`
 * 주석). 아래 단언들은 그 창이 없는 요청 하나의 상태 전이를 문다 — 동시 쓰기 창을 계약으로
 * 만들려면 그 주석의 ①(어느 값이 살아남아야 하는가)을 먼저 정해야 한다.
 */
