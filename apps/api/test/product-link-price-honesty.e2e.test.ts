...
describe("상품 링크 가격 정직 계약 (라운드 51 #9)", () => {
  ...
  it("수집 가격과 확인 시각을 한 쌍으로 시드한다", async () => {
    expect(productLinkSeeds.length).toBeGreaterThan(0);
    const activeSeeds = productLinkSeeds.filter((seed) => seed.active);
    expect(activeSeeds.every((seed) => seed.priceSnapshotKrw !== null && Boolean(seed.priceCheckedAt))).toBe(true);

    const seeded = activeSeeds[0];
    ...
    expect(link.priceCheckedAt?.toISOString()).toBe(seeded.priceCheckedAt);
  });
