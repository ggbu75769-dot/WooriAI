...
export default function HomeScreen() {
   ...
   * 그 반대를 했다. 게이트의 달을 **그 알림이 키를 태우는 달**로 맞추면 그 창이 닫힌다. 배너·
   * 진행바는 화면의 달을 계속 본다 — 그쪽은 라이브 표면이라 기기 달력이 맞는 단위다.
   *
   * ⚠️ **라운드 81 두 시점 — 위 괄호가 틀렸다.** 종전 이 자리는 그 문장 뒤에 "(둘이 갈리지 않는
   * 대다수 경로에서는 값이 같다)"고 적었다. **두 값은 형식이 달라 같은 날에도 같은 문자열이 아니다**:
   * `thisYearMonth`는 `seoulToday.slice(0, 7)` = `"2026-09"`이고, `monthly.yearMonth`는 서버의
   * `getSeoulMonthRange(...).yearMonth` = `"2026-09-01"`이다(계약 `dateOnlySchema`가 그 모양을
   * 고정한다). 그래서 라운드 80 B가 달을 바꾼 순간 게이트의 `startsWith` 비교가 무너져 **한 달의
   * 30일치 대기 행이 게이트 밖으로 빠졌다**(1일자 행만 통과 — 실측은
   * src/notifications/budget-gate-month-format.test.ts). 이제 그 술어가 양쪽을 앞 7자 달 키로
   * 맞춘다(src/home/budget-pace.ts가 같은 필드에 이미 쓰는 정규화 관례 그대로). **여기서는 자르지
   * 않는다** — 정규화는 판정하는 순수 모듈의 몫이고(이 화면은 값만 흘린다), 여기서 자르면 그
   * 술어를 부르는 다른 호출부는 여전히 죽은 게이트를 받는다. 위 문장에서 참인 것은 **달이 같다**
   * 는 사실뿐이고, 그것이 이 게이트에 필요한 전부다.
   *
