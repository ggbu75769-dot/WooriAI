 ...
 * 이미 채워진 react-query 캐시에서 읽는다:
 *  - 사용액: `["home", childId]`(홈 화면이 채운다) `monthly.usedAmountKrw` — 선물·환불 제외
 *    서버 집계(DNC-015).
...
export default function BudgetEditScreen() {
  ...
  const usageLine = buildBudgetUsageLine({
    ...
    usedKrw: cachedHome?.monthly.usedAmountKrw
  });
  // 합산 술어는 기록 탭·홈 월 합계와 같은 countsTowardMonthlyTotal 한 곳에서만 온다(DNC-015).
  const lastMonthActualKrw = sumLastMonthActualKrw(cachedLastMonth?.expenses ?? null);
