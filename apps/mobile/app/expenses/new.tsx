...
export default function NewExpenseScreen() {
   ...
   *
   * ⚠️⚠️ **라운드 93 트랙 B(AG-4) — 위 마지막 한 줄은 거짓이고, 두 시점으로 적는다.** 지우지 않는
   * 이유는 *언제 무엇이 거짓이었는지*가 이 자리의 값이기 때문이다(AE-3의 규율).
   *
   *  · **시점 ①(라운드 92 리뷰 H-2 · `bbf1d97` · 2026-08-31)** — 이 문단이 쓰인 날. 앞의 두 줄은
   *    그날도 오늘도 참이다(이 ref는 `applyMerchantSuggestion`의 `blur()` 한 자리에서만 쓰인다).
   *    **거짓인 것은 마지막 한 줄, 곧 *다른 자리를 인용한 부분*이다.**
   *  · **왜 거짓이 됐나 — 뒤에 바뀐 것이 아니라 쓰인 날 이미 거짓이었다.** `itemNameInputRef`는
   *    DSN-053 P2-C(`32e1648` · 2026-08-28)에서 **태어날 때부터 `.focus()` 호출 둘**을 달고 있었다.
   *    인용한 손이 인용당한 자리를 열어 보지 않았을 뿐, 그 사이에 그 자리가 바뀐 적은 없다.
   *  · **오늘의 사실(2026-08-31 워킹트리)** — 두 ref의 관례는 같기는커녕 **서로 반대**다.
   *    `merchantInputRef`: `blur()` 하나 · `focus()` **0건**.
   *    `itemNameInputRef`: `focus()` **둘** · `blur()` **0건** —
   *    `startCustomItem`의 `requestAnimationFrame(() => itemNameInputRef.current?.focus())` 와
   ...
   *
   * ⚠️ **그 구멍이 메워지는 조건은 하나다 — 지난달 캐시가 이미 받아져 있을 때**(위 GAP-058 #6이
   * 형제 둘에 대해 적어 둔 그 조건과 같다: 홈·기록 탭이 "지난달 같은 시점 대비" 한 줄을 위해
   * 채워 두는 캐시를 **읽기만** 하고, 없으면 undefined다 — 여기서 새 요청을 하지 않는다).
   * 그러니 이 배열은 **콜드 스타트 직후 기록 시트로 바로 들어온 1일 아침에는 여전히 비어 있을
   * 수 있고**, 그때의 동작은 종전과 같다(2순위 사전, 그것도 안 걸리면 미선택). 원천을 넓힌 것과
   * 원천을 만들어 내는 것은 다른 일이고, 이 라운드가 한 것은 앞엣것이다.
   *
