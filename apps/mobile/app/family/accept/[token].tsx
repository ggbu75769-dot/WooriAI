...
export default function AcceptInviteScreen() {
  ...
  async function finishHouseholdJoin(result: AcceptInviteResponse) {
    ...
    try {
      ...
      //
      // 라운드 107 트랙 C(delegate): 이 가구에는 볼 아이가 없지만 **이 사용자에게는 아이가
      // 있는** 갈래. 목적지가 "/"라 안내를 읽어 주는 처리는 위 둘과 같고, 홈 도달 표시는
      // 세우지 않는다 -- 그 판단(완료→탭 · 중단→ONB-006)은 서버 진행도를 받은 app/index.tsx의
      // 몫이고, 여기서 미리 세우면 라운드 99가 서버에 넘긴 판정을 다시 기기가 가로챈다.
