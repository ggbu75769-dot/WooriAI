...
export default function NotificationsScreen() {
  ...
  useEffect(() => {
    ...
    return useNotificationStore.persist.onFinishHydration((state) => {
      ...
      /**
       * ⚠️ 두 시점(라운드 107) — **이 갈래에서는 읽음 처리가 통째로 유실됐다.**
       *
       * 종전 이 콜백은 "새 소식" 스냅샷만 떴다. 그때는 참이었다: 읽음 처리는 아래 포커스
       * 효과의 몫이고, 그 효과는 화면이 뜰 때 반드시 한 번 돈다.
       *
       * 그런데 **rehydrate 전에 포커스가 오면** 그 한 번은 빈 목록 위에서 돈다 —
       * `markAllNotificationsRead([])`는 아무것도 하지 않고, 효과는 포커스가 다시 올 때까지
       * 되돌아오지 않는다(딥링크·앱 잠금 해제 직후처럼 저장소 읽기가 첫 포커스보다 늦게
       * 끝나는 콜드 경로). 그러면 사용자는 알림 목록을 **다 보고 나왔는데** 홈 종 아이콘의
       * 배지는 그대로 N을 달고 있다 — 배지와 목록이 같은 스토어를 읽으면서도 서로 다른
       * 사실을 말하는 유일한 자리였고, 배지는 "안 본 소식이 N건 있다"는 거짓을 말한다.
       *
       * 그래서 복구가 끝나는 **바로 그 자리**에서 포커스 효과가 하려던 일을 마저 한다. 순서는
       * 아래와 같다(스냅샷 먼저, 읽음 처리 나중) — 뒤집으면 이번에 새로 온 것이 무엇인지가
       * 그 자리에서 영영 사라진다(I-7이 포커스 효과에 세운 것과 같은 순서 규칙이다).
       *
       * 액션을 `getState()`로 부르는 이유는 이 효과가 `[]` deps로 마운트 1회만 구독해야 하기
       * 때문이다(구독을 다시 매는 순간 이미 끝난 복구를 놓친다). 이 파일이 같은 효과 안에서
       * `useNotificationStore.getState()`·`.persist`를 이미 그렇게 읽고 있다.
       *
       * 이 콜백은 화면이 마운트돼 있는 동안에만 온다(정리 함수가 구독을 끊는다). 그리고 이
       * 갈래에서 화면이 그리고 있던 것은 **빈 목록**이라, 여기서 읽음 처리되는 항목은 바로
       * 다음 프레임에 이 화면에 그려질 그 항목들이다.
       */
