...
export default function PrivacySettingsScreen() {
  ...
  /**
   * 라운드 99 M-2 — **미리보기는 그 대상의 것이다: 대상이 바뀌면 미리보기를 접는다.**
   *
   * 종전에는 미리보기(뮤테이션 결과)가 대상 전환을 살아남았다: A 아이로 "삭제 전 확인하기" →
   * 아이 관리에서 B로 전환 → 돌아오면 **A 시점의 미리보기와 삭제 버튼이 그대로** 서 있고,
   * 그 버튼의 뮤테이션은 지금 값(`childId`)을 읽으므로 누르면 **B가 삭제된다** — 화면에 보이는
   * 영향 목록·confirmationText는 A의 것인데 실제 대상은 B인 어긋남이다(라운드 63 #2가 이름
   * 표기로 좁힌 그 위험이, 미리보기가 살아남는 한 그대로 남아 있었다).
   *
   * 그래서 대상 id가 바뀌는 순간 미리보기를 리셋한다 — 확인 단계가 다시 그 대상의 미리보기부터
   * 시작된다(서버 미리보기가 곧 이 화면의 확인 단계다: PreviewSummary·DangerButton이 전부
   * `preview.data`에 걸려 있다). 가구 탈퇴도 같은 모양이다: 대상(`householdId`)은 선택 아이·
   * 파라미터·기본 가구에서 파생되므로 아이 전환·목록 도착으로 조용히 바뀔 수 있고, 그때 종전
   * 가구의 미리보기가 남으면 같은 어긋남이 된다. 마운트 직후의 리셋은 idle 뮤테이션에 no-op이라
   * 첫 렌더 동작은 종전 그대로다. 계정 삭제는 대상이 세션 그 자체라 전환 축이 없다 — 리셋하지
   * 않는다(지우면 멀쩡한 미리보기를 잃는다).
   */
  useEffect(() => {
    childPreview.reset();
