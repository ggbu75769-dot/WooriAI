...
export default function WeeklyBriefingScreen() {
  ...
  if (briefing.isError) return <AppScreen><ScreenHeader onBack={() => router.back()} title="가족 주간 브리핑" /><EmptyStateCard title="브리핑을 불러오지 못했어요" actionLabel="다시 시도" onPress={() => briefing.refetch()} /></AppScreen>;
  return (
    <AppScreen>
      <ScreenHeader eyebrow="이번 주 한눈에" onBack={() => router.back()} title="가족 주간 브리핑" subtitle={briefing.data ? `${briefing.data.weekStart} 기준` : "데이터를 모으고 있어요"} />
      {briefing.data ? <>
