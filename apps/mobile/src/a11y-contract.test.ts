...
const ANNOUNCE_LEDGER_VERDICT_BLIND_SPOTS: ReadonlyArray<string> = [
  ...
  "⚠️ 라운드 88 리뷰 L-1 — **갈래를 하나만, 글자로만 본다.** `announce`로 세려면 ⓐ 자리를 감싸는 갈래 중 **최내곽 하나**(`enclosingJsxGuards(...)[0]`)가 ⓑ effect 층 배선의 `if` 조건과 **문자열이 완전히 같아야** 한다. 그래서 자리를 갈래 둘이 겹쳐 감싸는 모양(바깥 갈래가 진짜 조건인 경우)이나, 같은 뜻을 다르게 적은 조건(`save.isError` ↔ `saveMutation.isError`, 부정 순서 바뀜, 괄호·공백 밖의 표기 차이)은 배선이 실재해도 `live-region`으로 떨어진다. ⚠️ 오차의 방향은 **거짓 빨강(안전)**이다 — 침묵을 announce로 세지 않고, 그 항목에 이유를 요구해 사람이 다시 보게 만든다. 오늘 실측(2026-08-31 워킹트리)으로 이 한계가 실제로 낸 피해는 **0건**이다: 대장 아홉이 덮는 자리 **스물둘** 중 감싸는 갈래가 하나인 자리 열셋 · 둘인 자리 여덟 · 없는 자리 하나이고, **중첩 여덟은 전부 최내곽 갈래가 배선 조건과 글자로 맞아 `announce`로 떨어졌다**(오늘 `live-region` 하나는 갈래가 하나인 자리이고, 그 이유가 값으로 서 있다 — ⚠️ 라운드 89 트랙 A가 끝난 초대 카드에 배선을 걸어 종전 둘 중 하나를 닫았고, **그 배선의 `if` 조건을 카드의 최내곽 갈래와 글자로 맞춘 것**이 이 사각의 첫 소비자다). 판정 로직을 넓히는 것은 이 한계가 실제로 자리를 잘못 세우는 날의 일이다."
];
