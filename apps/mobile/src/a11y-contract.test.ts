...
describe("GAP-065 #7 공유 프리미티브 터치 타깃 계약 (크기 + 2×hitSlop ≥ theme.touchTarget)", () => {
  ...
  it("카테고리 칩: 38 + 세로 5+5 = 48. 가로는 **실측한 칩 사이 간격**이 천장이라 3 그대로다", () => {
    ...
    // ⚠️ 두 시점(라운드 100 T3 — 자기 변경 동반 핀 이관): 이 값은 18이었다. 커스텀 품목 입력
    // 시트(src/items/CustomItemSheet.tsx)가 시기 밴드·필수도 칩 줄 **둘**을 더했다 — 둘 다
    // 목록 화면의 기존 칩 줄과 같은 gap 6이라 아래 "가장 좁은 줄" 판정(6)과 가로 합 천장은
    // 한 값도 움직이지 않는다(전량 스캔이 새 줄을 자동으로 데려온다는 이 스윕의 목적 그대로).
    expect(rows.length, "칩이 서는 자리 수").toBe(20);
