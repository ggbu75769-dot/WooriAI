...
export const API_ERROR_MESSAGES: Readonly<Record<string, string>> = {
  ...
  // 서버 원문("탈퇴한 계정이에요.")에 **다시 가입할 수 있는 시점**을 덧붙인다. 탈퇴 계정은
  // 재시도로 절대 풀리지 않으므로, 사용자가 다음에 할 수 있는 일을 아는 것이 유일한 도움이다.
  ...
  // 이 문구가 재시도를 권하지 않는 이유이기도 하다 — 사실만 말하고 "다시 시도해 주세요"를 붙이지
  // 않는다.
  USER_WITHDRAWN: "탈퇴한 계정이에요. 삭제 후 30일이 지나면 다시 가입할 수 있어요.",
  ...
  FORBIDDEN: "권한이 없어 처리하지 못했어요. 가족에서 내 역할을 확인해 주세요.",
 ...
 * "가족에서 내 역할을 확인해 주세요."인데, 아직 로그인도 못 한 사람에게 가족 이야기를 하는 것은
