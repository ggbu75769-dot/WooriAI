...
export const API_ERROR_MESSAGES: Readonly<Record<string, string>> = {
   ...
   *
   * ⚠️ 두 시점 (라운드 102 리뷰 L-10) — 위 메모가 **빠뜨렸던 경로 하나**와, 그 경로가 닫힌 사실:
   * 예산 화면은 칩 대장에 없는 기존 예산 행을 "끼워 유지"해 replace-set에 함께 싣는다(§6.5).
   * 그래서 운영자가 예산 있는 분류를 숨긴 뒤에는 **평범한 저장(총액만 고치는 저장 포함)**이
   * 전부 이 코드로 떨어졌다 — 낮은 도달성이 아니라 그 아이·그 달의 상시 실패였다. 라운드 102
   * 리뷰 H가 서버 검증을 "새로 서는 id만 active 요구"로 좁혀 그 경로를 닫았으므로, 오늘 남는
   * 도달 경로는 위 문단의 셋(오래된 캐시로 **새 행**을 세우려는 저장 · 구버전/우회 클라이언트 ·
   * 해제한 뒤 같은 숨긴 id를 다시 세우려는 저장)뿐이다.
   */
