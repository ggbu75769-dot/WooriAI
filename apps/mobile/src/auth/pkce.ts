 ...
 *
 * PKCE uses expo-crypto's native cryptographic randomness and fails closed if it is unavailable.
 * The S256 challenge is computed with the local pure-JS SHA-256 (./sha256.ts).
 */
 ...
 *
 * Legacy app-lock salt helper only. PKCE no longer uses this fallback.
 * Do not use this helper for authentication secrets, session tokens or PKCE verifiers.
 */
...
export async function createPkcePair(): Promise<PkcePair> {
  ...
  if (bytes.length !== 32) throw new Error("PKCE_RANDOM_UNAVAILABLE");
