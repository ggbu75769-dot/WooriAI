...
describe("라운드 102 예산 화면 카테고리 카드 배선", () => {
  ...
  it("카테고리 카드 구간에 경고색이 없다 — 문장이 의미를 진다 (DNC-018 · 경고색 남발 금지)", () => {
    const src = budgetScreen();
    ...
    // 상한·합 관측 줄이 실제로 그 구간 안에 있다(빈 구간 방지 — 표식 실재 확인).
    expect(cardSlice).toContain('testID="budget-category-form-error"');
    expect(cardSlice).toContain('testID="budget-category-sum-notice"');
