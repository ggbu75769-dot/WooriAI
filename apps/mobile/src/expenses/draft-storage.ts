...
export type QuickExpenseDraft = {
  ...
  isGift: boolean;
  /**
   * 라운드 99 F3 L-1 — **판매처**. 가산 필드이고 **선택 사항**이다(childId와 같은 규율).
   *
   * 왜 이제야 합류하나: 판매처 입력칸(라운드 49 C-03)은 이 초안 계약이 서고 한참 뒤에 생겼고,
   * 그때 자동 저장·복원에 합류시키는 걸음을 아무도 걷지 않았다. 그래서 전화 한 통에 시트를
   * 닫으면 품목명·금액·메모는 살아 돌아오는데 **판매처만 조용히 사라졌고**, 판매처부터 친
   * 상태(다른 칸이 전부 빈 상태)는 자동 저장의 "친 것 있음" 판정에도 안 걸려 초안 자체가
   * 남지 않았다(배선은 app/expenses/new.tsx의 autosave effect — hasTypedInput).
   *
   * 값이 없을 수 있는 경우가 전부 정상이다: 이 필드가 생기기 전에 쓰인 초안(구 blob —
   * 마이그레이션 없음, 종전 그대로 복원), 판매처를 안 적은 초안(대부분의 기록이 그렇다 —
   * 빈 문자열을 적어 두는 대신 **키 자체를 싣지 않는다**, categoryId·childId와 같은 규율).
   *
   * ⚠️ 결제 수단은 이번에도 싣지 않는다(결정 주석): 시트의 결제 수단은 라운드 96 T3부터
   * **최근값 추정**(resolveDefaultPaymentMethod — 가장 최근 기록의 결제 수단으로 시작)이
   * 보완하고 있어, 초안이 나르지 않아도 복원 화면의 시작값이 대개 사용자가 쓰던 그 값이다.
   * 판매처는 그런 보완 장치가 없는 자유 텍스트라(추정으로 지어낼 수 없는 값) 초안이 유일한
   * 생존 경로다 — 두 필드의 대접이 갈리는 근거가 이것이다.
   */
  merchant?: string;
  /**
