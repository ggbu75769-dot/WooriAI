...
/*
 * 라운드 69 트랙 A(P3) — `FAILED_ROW_OTHER_CHILD_NOTICE`는 이 파일을 떠났다.
 *
 * 그 문구가 여기 있던 이유는 라운드 59 트랙 A가 `src/offline/messages.ts`를 소유해 같은 라운드에서
 * 충돌했기 때문이고, 그때부터 "옮기는 것은 다음 라운드의 몫"이라고 적혀 있었다(열 라운드 이월).
 * 동기화 상태 화면의 문구 단일 소스는 그 파일이므로 값 그대로 그리로 옮겼다 — 이 모듈이 지는
 * 것은 **프리필 판정**이지 화면 문구가 아니다. 시트 쪽 짝인 `FAILED_ROW_PREFILL_CHILD_MISMATCH_NOTICE`
 * 도 처음부터 그 파일에 있었으므로, 같은 어긋남을 말하는 두 문장이 이제 한자리에 선다.
 */
