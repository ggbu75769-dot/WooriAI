 ...
 *
 * 일반 저장의 OFFLINE_SAVED_MESSAGE("기기에 저장했어요. 연결되면 자동으로 반영할게요.")를 쓰지
 * 않는 이유: 그 문장은 **화면을 떠나기 직전** 마지막 인사라 뒷말이 필요 없다. 여기서는 화면이
 * 그대로 남고 칸이 비워지므로, 입력이 사라진 것이 오류가 아니라 다음 항목을 위한 것임을 같은
 * 줄에서 말해 줘야 한다. 저장 사실 자체는 동기화 상태(기록 탭 배지)가 계속 책임진다.
 */
export const CONTINUE_RECORDING_SAVED_MESSAGE = "저장했어요. 이어서 다음 항목을 기록해 보세요.";
