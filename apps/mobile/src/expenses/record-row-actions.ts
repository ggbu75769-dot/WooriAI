...
//
// 라운드 63 C(#8) — **두 번째 예외**: 기록 탭 달력에서 **빈 날 칸**을 누르면 그 날짜를 실어
// 시트를 연다(app/(tabs)/records.tsx → `spentOn` 파라미터, 파싱은 위 "고쳐서 다시 보내기"와
// 같은 `resolveFailedRowPrefillDate` 한 곳이라 새 파싱 규칙은 0건이다).
//
// 원칙("또 기록은 오늘에서 시작한다")은 그대로다. 갈리는 자리는 여전히 **누가 그 날짜를
// 골랐는가** 하나다:
//  - "같은 내용으로 또 기록"에서 앱은 사용자가 언제 샀는지 모른다. 과거 행의 날짜를 베끼면
//    앱이 모르는 사실을 지어내는 것이라, 오늘에서 시작하고 사용자가 필요하면 뒤로 옮긴다.
//  - "고쳐서 다시 보내기"의 날짜는 사용자가 **이미 골라 둔 사실**이다(라운드 58 #5).
//  - 달력 빈 칸의 날짜도 사용자가 **방금 직접 고른 사실**이다 -- 8월 6일 칸을 눌러 놓고 시트가
//    8월 28일에서 열리면, 그건 사용자가 가리킨 날이 아니라 앱이 고른 날이다. 게다가 이 동선의
//    출발점은 "며칠 동안 기록이 없어요"라는 기록 리마인더의 착지이므로(라운드 56 D#10), 그 며칠을
//    채우러 온 사람에게 오늘 날짜를 내미는 것은 목적지를 오늘로 되돌리는 것과 같다.
// 즉 예외의 기준은 "날짜를 아느냐"가 아니라 **"사용자가 그 날짜를 지목했느냐"**이고, 세 동선이
// 그 한 문장으로 갈린다. 프리필 파라미터를 늘리면서 이 문단을 적지 않으면 원칙이 사실상
// 사라지므로 여기에 함께 남긴다(규칙은 두 벌이 되지 않는다 -- 이 파일이 유일한 자리다).
//
// 미래 날짜는 이 예외에 들어오지 않는다: 달력이 미래 칸을 애초에 비대화형으로 두고
// (records-calendar.ts `resolveCalendarCellAction`, DNC-013), 링크로 억지로 들어와도
// `resolveFailedRowPrefillDate`가 오늘로 물러선다.
//
