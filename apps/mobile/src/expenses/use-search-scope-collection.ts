 ...
 *    ⚠️ 리뷰 M-A2: `retry: 1`을 명시한다 — 전역 기본에 맡기지 않는 이유는 이 루프가 21개월을
 *    직렬로 걷기 때문이다(달마다 재시도가 길면 수집 전체가 그 배수로 늘어진다). 한 번의 재시도
 *    뒤에도 실패한 달은 결과의 failedMonths로 남아 화면의 부분 실패 고지 + [다시 시도] 몫이다.
 *
      ...
      const collected = await collectSearchScopeMonths<Expense>(
        ...
        (yearMonth) =>
          queryClient.ensureQueryData({
            ...
            // 리뷰 M-A2: 직렬 21개월 루프라 달당 한 번만 재시도한다 — 그 뒤의 실패는
            // failedMonths로 남아 화면의 부분 실패 고지 + [다시 시도] 몫이다(머리말 참고).
            retry: 1
          }),
        (done, total) => {
