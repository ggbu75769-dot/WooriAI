 ...
 *
 *   라운드 106 T6 실측: 라운드 103이 **커스텀 지출 분류**를 더하면서 `카테고리` 열에도 사용자
 *   자유 문자열이 실리기 시작했는데(그 전까지 이 열은 시드 12분류 + 8타일의 고정 라벨뿐이었다),
 *   그 열 역시 아래 `csvCell`을 지나므로 **가드는 이미 걸려 있었다** — 새로 막을 것은 없었다.
 *   값으로 확인한 사실 세 가지를 옆 테스트가 못 박는다:
 *   1. 순서가 `sanitize → escape`다. 뒤집으면 `=1,2` 같은 이름이 `"=1,2"`가 되어 따옴표 **안**이
 *      수식으로 남는다. 지금은 `"'=1,2"`로, 따옴표 안쪽 첫 글자가 `'`다.
 *   2. 위험 문자 집합은 서버 파서와 **같은 6개**여야 한다(=, +, -, @, 탭, CR). 옆 테스트가 그
 *      리터럴을 apps/api 소스에서 읽어 대조한다 — 헤더 키워드 표를 읽는 것과 같은 관례다.
 *   3. `'`는 **왕복에서 한 번 눌어붙는다**. 재가져오기의 `sanitizeText`는 첫 글자가 `'`면 아무
 *      것도 더하지 않으므로, 내보낸 파일을 그대로 다시 올리면 품목명이 `'=SUM(...)`으로 저장되고
 *      이후 왕복에서는 더 늘지 않는다(멱등). 이 한 글자를 없애려면 가져오기 쪽에서 선행 `'`를
 *      벗겨야 하는데, 그러면 **사용자가 직접 `'`로 시작하는 이름을 적은 경우**와 구별할 수 없다 —
 *      위 "왕복의 알려진 한계"와 같은 성질의 손실이라 여기서 정하지 않고 값으로만 적어 둔다.
 *      **측정 범위**: 여기서 값으로 본 것은 내보낸 원본 파일을 곧장 다시 올리는 경로뿐이다.
 *      엑셀·구글시트를 거쳐 다시 저장한 파일이 이 `'`를 어떻게 담는지는 이 저장소에서 잴 수
 *      없어(스프레드시트 실물이 필요하다) 여기 적지 않는다.
 */
