 ...
 *              말하기 때문이다.
 *  - `all`   : 날짜를 보지 않고 전부 센다. 대기 행은 정의상 서버에 없어서, "전체"가 어디까지
 *              거슬러 올라가든 그 행은 CSV에 담기지 않는다. 날짜가 깨진 행도 마찬가지라
 *              여기서만은 `spentOn`을 요구하지 않는다.
 *
...
export function exportPendingNoticeText(count: number): string {
  return `${SYNC_ROW_PENDING_LABEL} 중인 기록 ${count}건은 아직 서버에 없어 이 파일에 담기지 않아요.`;
}
 ...
 * 토스트에도 같은 문장이 붙는다 — 두 경우 모두 **CSV에 담기지 않았다**는 같은 사실이다.
 */
export function exportPendingToastSuffix(count: number): string {
  if (count <= 0) return "";
  return ` ${SYNC_ROW_PENDING_LABEL} 중인 기록 ${count}건은 아직 서버에 없어 CSV에 담기지 않았어요.`;
}
