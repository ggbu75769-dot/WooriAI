 ...
 *
 * 끝난 초대를 여는 사람 중에는 "이미 수락해 둔 초대 링크를 한 번 더 누른 사람"이 섞여 있다
 * (초대는 1회용이라 수락한 뒤의 같은 링크는 이 카드로 온다). 그 사람에게 "새 링크를 요청해
 * 주세요"만 남기면, 이미 들어와 있는 가족을 다시 초대받으라고 말하는 셈이다.
 *
 * ⚠️ **오라클이 아니다.** 이 줄의 근거는 토큰이 아니라 **내 세션 상태**다 — 서버가 이 토큰에
 * 대해 무엇을 말했는지와 무관하게, 로그인한 사람이면 누구에게나 같은 문장이 선다(내 가족을
 * 앱에서 볼 수 있다는 사실은 토큰과 아무 관계가 없다). 그래서 비로그인 방문자의 화면은 이
 * 줄이 없는 **종전 그대로**이고(호출부의 `authToken` 축), 토큰의 존재 여부는 여전히 어떤
 * 갈래에서도 새어 나가지 않는다. 단정하지 않는 것도 같은 이유다 — "이미 참여했어요"가 아니라
 * "이미 참여한 가족이라면"이다.
 */
export const INVITE_UNAVAILABLE_ALREADY_JOINED_HINT = "이미 참여한 가족이라면 앱에서 바로 확인할 수 있어요.";
