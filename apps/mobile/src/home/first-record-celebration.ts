...
export const FIRST_RECORD_CELEBRATION_BODY = "이제 이번 달 총액이 여기 쌓여요.";
...
export const useFirstRecordCelebrationStore = create<FirstRecordCelebrationState>()((set) => ({
  ...
  observe: (childId, hasAnyRecord) =>
    set((state) => {
      ...
      if (!celebrate) return { ...state, observedHasRecord };
      return {
        ...state,
        observedHasRecord,
  ...
  reset: () => set({ observedHasRecord: {}, celebratedChildIds: {}, activeChildId: null })
}));
