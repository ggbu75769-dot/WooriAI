...
describe("라운드 43 리뷰 M-1: 고지는 링크 집합이 정한다", () => {
  ...
  it("일반 링크뿐이면 고지를 그리지 않는다 (허위 수수료 고지 제거)", () => {
    ...
    // 받는다고 말하는 화면이었다(일반 링크가 라운드 43 당시 58개 중 34개 · 라운드 83 A 이후
    // 67개 중 43개 — 제휴도 스폰서도 아닌 링크를 센 값이다).
...
describe("라운드 64 #1: 전폭 구매 CTA도 같은 판정을 지난다 (DNC-011)", () => {
  ...
  it("전부 스폰서면 전폭 CTA가 사라진다 — 죽은 버튼도, 강조된 광고도 만들지 않는다", () => {
    // 판정 쪽: **라운드 82 시드의 다섯 품목**(유일한 링크가 스폰서)이 여기에 해당했다 —
    // 라운드 83 A가 그 다섯에 일반 링크를 하나씩 더해 **오늘 시드에는 0건**이다(그 계약은
    // apps/api/test/seed-data.test.ts의 `ITEM_CODES_WITHOUT_NON_SPONSORED_LINK` 대장·래칫 0).
    // 판정 자체는 시드와 무관하게 남는다: 운영에서 비스폰서 링크가 전부 비활성화되면 그날
    // 다시 이 갈래가 열리고, 그때 화면은 죽은 버튼도 강조된 광고도 만들지 않는다.
