 ...
 * "제휴 링크"라고 말하는, 서로 어긋나는 두 주장이 한 줄에 있었다(시드 62개 품목에 달린 링크 중
 * `isAffiliate=false`가 **라운드 43 당시 58개 중 39개 · 라운드 83 A 이후 67개 중 48개**다).
 * 제휴가 아닌 링크에 제휴 고지를 붙이는 것도, 제휴인 링크의 고지를 빼는 것도 둘 다 사실과 다르다.
 *
 * ⚠️ 라운드 82 리뷰 M-5 — **이 줄과 아래 `productLinksDisclosureText` 머리말은 세는 정의가
 * 다르다.** 여기 39/48은 `isAffiliate=false`(스폰서 포함)이고, 아래 34/43은
 * `isAffiliate=false ∧ isSponsored=false`, 즉 이 모듈이 "일반"이라고 부르는 배지 그대로다
 * (스폰서 다섯이 그 차이다). 두 수를 같은 이름으로 인용하지 말 것.
 *
 * 규칙:
 ...
 *    (라운드 43 당시 58개 중 34개 · 라운드 83 A 이후 67개 중 43개 — 위 머리말의 39/48과 세는
 *    정의가 다르다: 이 수는 스폰서 다섯을 뺀 값이다). 그런 링크만
 ...
 *
 * ⚠️ 링크를 목록에서 감추는 일과는 무관하다 — 이 판정이 정하는 것은 **앱 밖으로 나가는 사본**
 * 하나뿐이고, 여는 URL·링크 목록·개수·정렬은 종전 그대로다.
 *
 * ⚠️ 두 시점(COM-105 후속). ① 종전 이 줄은 *"감추거나 「깨졌어요」라고 적는 일과는 무관하다"*
 * 였고 그때는 참이었다(앱 DTO에 헬스가 없었다). ② 이제 화면 문구는 **있다** — 다만 "깨졌어요"
 * 라는 단정이 아니라 우리 관찰의 보고 한 줄이고, 판정은 아래 `purchaseLinkHealthNotice`
 * 한 자리다. **감추지 않는다는 판정은 그대로**이고 이 함수는 한 글자도 바뀌지 않았다.
 *
...
export type ProductLinkHealthInput = Pick<ProductLink, "healthStatus">;

/**
 * COM-105 후속 — **우리가 마지막으로 눌러 봤을 때 무엇을 봤는가.**
 *
 * 종전(라운드 68 C #4 · 그때는 참): 앱은 `health_status`를 한 글자도 받지 못했다. 서버가 아는
 * 도달 실패가 앱에서 받는 대우는 ⓐ 정렬 강등과 ⓑ 공유 URL 미발급 둘뿐이었고, **화면 문구는
 * 0줄**이었다. 그래서 우리가 도달 실패를 아는 링크를 사용자는 아무 말도 못 들은 채 눌러 앱
 * 밖에서 404를 만났다.
 *
 * 이제: 서버가 실패 두 값만 내려주고(`ProductLink.healthStatus` — `ok`·미확인은 키 자체가
 * 없다), 이 함수가 그것을 문장 하나로 바꾼다. **판정도 문구도 여기 한 자리**다 — 화면은
 * 그리기만 한다.
 *
 * 어투(DNC-018 · 관찰형): 세 문장 모두 **과거 시제로 우리 관찰만** 말하고 링크나 판매처를
 * 단정하지 않는다. "이 링크는 죽었어요"는 우리가 아는 것보다 많은 말이다 — 판정은 최대
 * 24시간 묵을 수 있고, HEAD에 403으로 답하는 정상 쇼핑몰도 `broken`이 된다(워커는 405에서만
 * GET으로 재시도한다). 그래서 "마지막으로 확인했을 때는 …"으로 시간을 못 박아, 지금은 다를 수
 * 있다는 사실까지 문장이 스스로 담게 한다.
 *
 * 두 값을 **한 문장으로 합치지 않는다**: 4xx·리디렉트 초과(확정된 도달 실패)와 5xx·타임아웃
 * (다음 회차에 바로 재확인되는 일시 실패)은 서로 다른 사실이라, 같은 말을 하면 그중 하나는
 * 틀린 말이 된다.
 *
 * ⚠️ **부재는 "확인됨"이 아니다.** 값이 없으면 이 함수는 `undefined`를 돌려주고 화면은 종전
 * 그대로 아무 줄도 그리지 않는다 — 미확인을 "확인됨"으로 바꿔 말하는 자리는 어디에도 없다
 * (긍정 배지는 계약 유니온에 `ok`가 없어 타입상 만들 수조차 없다).
 *
 * ⚠️ **링크를 막지도 감추지도 않는다.** 이 함수는 문자열 하나만 돌려주고, 그 링크의 버튼·
 * 순서·개수는 이 판정과 무관하다(DNC-009 무접촉 — 수수료율은 여기 들어가지 않는다). 우리
 * 판정이 틀렸을 때 사용자의 구매 경로를 우리가 대신 닫아 버리지 않는다는 뜻이다.
 *
 * ⚠️ DNC-010 무접촉: 이 줄은 제휴 고지(`AffiliateDisclosure`)를 대체하지도 가리지도 않는다 —
 * 고지는 종전 그대로 구매 CTA 바로 위에 서고, 이 줄은 판매처 행 안에서 따로 산다.
 */
export function purchaseLinkHealthNotice(link: ProductLinkHealthInput): string | undefined {
  if (link.healthStatus === "broken") return "마지막으로 확인했을 때는 이 링크가 열리지 않았어요.";
  if (link.healthStatus === "unstable") return "마지막으로 확인했을 때는 연결이 고르지 않았어요.";
  return undefined;
}
