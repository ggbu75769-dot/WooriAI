 ...
 *
 * 1. **지어내지 않는다.** 값이 없으면 그 조각을 그리지 않고 **종전 문자열로 그대로 돌아간다**
 *    (그때 제목·낭독 라벨은 이 트랙 이전과 바이트가 같다). 라운드 87 당시에는 마스터 토글이
 *    만드는 등록 경로(`registerDevice`)가 `osVersion`을 보내지 않아 그 경로로 생긴 행이 늘
 *    `null`이었고, 라운드 88 트랙 B가 그 경로에도 같은 한 벌(`buildDeviceRegistrationBody`)을
 *    물려 이제 두 경로가 같은 값을 보낸다 — 그래도 옛 행·값 없는 응답은 남으므로 이 규율은
 *    그대로 산다.
