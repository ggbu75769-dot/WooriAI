...
export const NOTIFICATION_TYPE_OPTIONS: readonly NotificationTypeOption[] = [
  {
    ...
    label: "예산 80% 알림",
    /**
     * 라운드 106 T7 — ⚠️ 두 시점: 종전 설명은 "이번 달 예산의 80%를 쓰면 알림함에 남겨요." 한
     * 줄이었다. 그런데 판정(generators.ts의 `budgetNotifications`)은 두 경계 중 **하나만**
     * 만든다: 한 번의 지출로 80% 아래에서 100%를 넘어서면 후보는 budget_100 하나뿐이다. 그래서
     * 100% 알림만 꺼 둔 사용자는 이 스위치를 **켜 둔 채로 그 달에 아무 알림도 받지 못했다** —
     * 스위치가 약속한 것과 실제 발화가 갈리는 자리다(허위 표시 금지).
     *
     * 판정을 바꾸지 않는 이유: 110%를 쓴 사람에게 "80%를 사용했어요"라고 말하면 그것이 허위
     * 데이터다. 고칠 것은 문장 쪽이고, 문장은 옆 스위치의 **이름**으로 갈 곳을 가리킨다(짝
     * 테스트가 그 이름을 값으로 문다 — 두 곳에 손으로 적힌 라벨이 갈리지 않게).
     */
    description: "이번 달 예산의 80%를 쓰면 알림함에 남겨요. 100%를 넘어선 뒤에는 예산 100% 알림이 대신 남아요."
  },
  {
    type: "budget_100",
    label: "예산 100% 알림",
  ...
  },
  {
    ...
    label: "시기 변화 알림",
    /**
     * 라운드 106 T7 — ⚠️ 두 시점: 종전 설명은 "아이가 새로운 시기에 들어서면 알림함에
     * 남겨요." 한 줄이었고, 그것은 **전환 당일의 알림 하나**만 말한다. 그 뒤 트랙 T-F가 같은
     * 종류로 **D-7 예고**를 세웠고(stage-preview-d7.ts — 새 종류를 만들지 않고
     * `stage_transition`을 재사용한 근거는 그 머리말), 이 스위치는 그때부터 **둘**을 함께
     * 끄고 켠다. 그래서 설명만 켜 두면 사용자는 자기가 켠 적 없다고 생각하는 알림을 받는다
     * (임신 갈래에서는 출산 예정일 D-7 한 줄이다) — 약속과 실제가 갈리는 자리다.
     *
     * 스위치를 둘로 쪼개지 않는다: 같은 주제(시기 변화)를 두 축으로 나누면 저장본 검증·아이콘
     * 맵·목적지 관례가 함께 갈라진다(그 머리말의 판단 그대로). 대신 문장이 두 가지를 다 말한다.
     * 창의 일수(7일)는 짝 테스트가 판정 함수에서 세어 이 문장과 대조한다 — 창이 바뀌면 숫자가
     * 여기 남아 조용히 거짓말하지 않게.
     */
    description: "아이가 새로운 시기에 들어서면 알림함에 남겨요. 다음 시기가 7일 안으로 다가와도 미리 한 번 남겨요."
  },
  {
  ...
  },
  {
    ...
    label: "기록 리마인더",
    /**
     * 라운드 106 T7 — ⚠️ 두 시점: 종전 설명은 "며칠 동안 기록이 없으면 한 번 알려드려요."였다.
     * 그 문장이 세는 것은 **기록한 행위**인데, 판정(generators.ts의 `recordGapNotification`)이
     * 세는 것은 마지막 **지출 날짜**(`spentOn`)다. 라운드 54 P1-3이 알림 제목에서 정확히 이
     * 어긋남을 고쳤고("N일 동안 기록이 없어요" → "마지막 지출 기록이 N일 전이에요" — 3주 전
     * 영수증을 오늘 적은 사람에게 기록이 없다고 말하던 거짓 단언), 스위치 설명만 종전 축에
     * 남아 있었다. 소급 입력은 가계부에서 가장 흔한 입력이라 그 사람은 매번 "나는 오늘 적었는데"
     * 하고 이 스위치를 읽게 된다. 제목이 말하는 축을 설명도 말한다.
     */
    description: "마지막 지출 날짜로부터 며칠이 지나면 한 번 알려드려요."
  },
  {
