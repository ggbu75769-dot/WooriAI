...
describe("라운드 74 D: 옛 실패 리터럴 부정 단언 스윕", () => {
   ...
   *
   * 그래서 **횟수**를 고정한다. 오늘 살아 있는 자리는 **둘**이고(⚠️ 라운드 86 리뷰 L-9: 이 머리말은
   * 라운드 74의 "셋"을 그대로 이고 있었는데, 라운드 86 트랙 B가 온보딩 준비물 화면을 배선하면서
   * 아래 표에서 한 줄이 빠졌다 — 표는 줄었는데 산문이 옛 수를 말하고 있었다), 둘 다 "그 문장이어야
   * 하는 이유"가 있는 자리다. 그 이유를 여기 값으로 적는다 — 자리가 하나라도 늘면 이 단언이 먼저
   * 빨개진다. ⚠️ 그리고 이 산문의 수는 아래 마지막 단언이 **이름 전수로** 다시 센다(표에 이름이
   * 있는 화면은 정확히 그 둘이다) — 산문만 낡는 일을 그 줄이 막는다.
   */
