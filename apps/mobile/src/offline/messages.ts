...
/**
 * F4 — **묘비 충돌(서버 current가 deleted)의 두 줄.**
 *
 * 종전에는 이 자리의 문장이 `app/sync-status.tsx`에 인라인 리터럴 한 줄
 * ("다른 기기에서 이 기록을 삭제했어요.")뿐이었고, 그때는 참이었다: 화면이 상정한 묘비 충돌은
 * **로컬 수정 vs 서버 삭제** 하나였고 그 문장은 그 상황을 정확히 말한다.
 *
 * → 이제 두 가지를 더 말한다. 근거는 각각 다음과 같다.
 *
 *  ① 엔진 주석이 인용해 둔 설계 문서 §3.4 — *"current가 deleted면 이 옵션은 새 기록으로
 *    재생성임을 안내"* — 가 이행된 적이 없었다(저장소 전체에 "새 기록"·"재생성"·"다시 만들"
 *    0건). 화면은 살아 있는 충돌과 **글자 그대로 같은 두 버튼**만 그렸고, 그래서 사용자는
 *    "내 변경 다시 적용"이 **원본을 되살리는 것이 아니라 새 기록을 만든다**는 사실을 누른 뒤에야
 *    알 수 있었다.
 *  ② **삭제 vs 삭제**(양쪽이 같은 지출을 지웠다)에서는 재생성 자체가 없다 — 두 선택지가 같은
 *    결과(삭제 확정)로 모인다(sync-engine.ts `resolveConflictReapplyMine`의 F3 갈래). 버튼이
 *    둘인 채로 아무 말도 하지 않으면, 사용자는 한쪽이 무언가를 되돌린다고 읽는다.
 *
 * 값을 받아 갈래를 고르는 **함수**인 이유는 라운드 95 공통 금지(모바일 새 `export const` 0건)
 * 그대로다. 버튼 라벨을 문장에 보간하지 않는 것도 의도적이다 — 라운드 93 트랙 B의 조사 계약
 * (`src/korean-particle-guard.test.ts`)이 세는 모집단은 **보간 뒤 조사**이고, 이 두 줄은 그
 * 모집단에 새 자리를 만들지 않는다. 해요체·관찰형이고 사용자를 탓하지 않는다(DNC-018).
 */
export function deletedConflictRowCopy(pendingDelete: boolean): {
  /** 서버에서 무슨 일이 있었나 — 사실 한 줄. */
  fact: string;
  /** 두 선택지를 고르면 무엇이 되나 — 고지 한 줄. */
  outcome: string;
} {
  if (pendingDelete) {
    return {
...
/**
 * 라운드 104 B-5 — **복구 버튼이 실패했을 때 그 자리에 남는 한 줄.**
 *
 * ## 종전에는 화면이 아무 말도 하지 않았다
 *
 * 동기화 상태 화면(app/sync-status.tsx)은 막힌 기록을 푸는 **유일한** 화면인데, 그 안의 복구
 * 동작 열넷(충돌 3지선다 · 행 재시도·버리기 · 준비템 재시도·버리기 · 일괄 둘)이 전부
 * fire-and-forget이었다(파일 전체 `catch` 0개). 그 함수들은 하나같이 `await getOfflineStore()`
 * 로 시작하므로, 부팅 뒤 저장소가 죽으면(디스크 가득 참, SQLite I/O 오류) 호출은 reject하고
 * **목록은 그대로 남는다.** 사용자는 눌러도 아무 일이 없는 버튼을 다시 누른다.
 *
 * 그 상태에서 저장소 미가용 고지도 뜨지 않았다: 그 고지는 목록이 **빌 때만** 그려졌는데
 * (`publishStorageUnavailableSnapshot`은 행과 건수를 일부러 그대로 둔다 — sync-controller.ts),
 * 행이 남아 있으면 목록은 비지 않는다. 즉 두 자리 모두가 침묵했다.
 *
 * ## 이제
 *
 * 라운드 62 #2가 대기 행 버리기 거절에 세운 그 패턴을 그대로 쓴다(바로 위
 * `SYNC_STATUS_DISCARD_PENDING_BLOCKED_MESSAGE`): **거절·실패는 그 행 안에서 한 줄로 말한다.**
 * 이 화면에는 기록 탭 같은 화면 전역 플래시 자리가 없고, 어차피 특정 행(또는 특정 섹션의 일괄
 * 버튼)에 대한 답이라 그 자리가 제자리다. 다시 누르면 지워지고, 그 사이 행이 확정돼 사라지면
 * 컴포넌트와 함께 사라진다.
 *
 * 문장은 원인을 단정하지 않는다(저장소일 수도, 그 밖일 수도 있다 — 화면이 아는 것은 "거절됐다"
 * 하나다). 대신 **다음에 무엇을 하면 되는지**를 둘 말한다: 다시 누르기, 그래도 안 되면 앱 재시작
 * (저장소 재오픈 게이트가 실제로 그때 한 번 더 연다 — store-open-gate.ts). 해요체·책망 없음이고
 * 사용자를 재촉하지 않는다(DNC-018).
 */
export function syncStatusActionFailedMessage(): string {
  return "방금 누른 것을 처리하지 못했어요. 잠시 뒤 다시 누르거나 앱을 다시 켜 주세요.";
}
...
export const LOGOUT_UNCOUNTED_TEARDOWN_STORES: Readonly<Record<string, string>> = {
  ...
  // 홈 빠른 기록 칩 핀(라운드 102 F6b): 최근 검색어와 같은 범주다 — 품목명(개인 텍스트)이지만
  // 잃어서 되돌릴 수 없는 기록이 아니라 길게 눌러 다시 고정하면 그만인 입력 보조이고(최대 3개,
  // 기록 자체는 서버에 그대로 있다), 남기면 다음 계정의 홈 칩에 이전 계정의 품목명이 고정된 채 뜬다.
  useQuickRecordPinsStore: "길게 눌러 다시 고정하면 그만인 입력 보조이고, 남기면 다음 계정의 홈 칩에 이전 계정의 품목명이 뜬다",
  // 예산 100% 경고 햅틱의 (아이, 월, 경계) 클레임(라운드 102 리뷰 M-1): 사용자가 적은 값이 아니라
  // "이 경계에 이미 진동으로 알렸다"는 관찰 이력이고(안내 닫음 플래그·축하 이력과 같은 범주),
  // 남기면 다음 계정의 아이 id에 걸린 클레임이 그 계정이 받아야 할 첫 경고 진동을 삼킨다.
  useBudgetWarningHapticStore: "진동으로 이미 알렸다는 관찰 이력이고, 남기면 다음 계정의 첫 경고 진동이 삼켜진다",
  // 선택된 아이 id(라운드 110): 사용자가 적은 값이 아니라 "지금 어느 아이를 보고 있는가"라는
  // 포인터 하나이고, 아이 자체는 서버에 그대로 있어 다음 화면이 다시 고르면 그만이다(MOB-116
  // 복구가 GET /children으로 자동으로 한다). 앱 잠금 PIN·통계 동의와 같은 "잃는 값이 아니라
  // 계정 경계에서 반드시 지워야 하는 값" 범주다 — 남기면 다음 계정의 네 탭이 이전 계정의
  // 아이 id로 조회한다.
  useSelectedChildStore: "다시 고르면 그만인 포인터이고, 남기면 다음 계정의 탭이 이전 계정의 아이를 조회한다"
};
