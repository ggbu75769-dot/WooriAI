...
describe("FIX-119B/F2: 결정적 5xx는 시도 상한에서 'failed'로 승격돼 큐를 놓아준다", () => {
  it(`연속 5xx ${MAX_SERVER_ERROR_ATTEMPTS}회 -> 'failed' 승격, 그 pass에서 곧바로 뒤의 기록이 진행된다`, async () => {
    ...
    for (let attempt = 1; attempt < MAX_SERVER_ERROR_ATTEMPTS; attempt += 1) {
      ...
      expect(summary).toEqual({ synced: 0, createdSynced: 0, failed: 0, conflicted: 0, itemStatusSynced: 0, itemStatusFailed: 0, stoppedForNetwork: true });
    ...
    expect(escapeSummary).toEqual({ synced: 2, createdSynced: 2, failed: 1, conflicted: 0, itemStatusSynced: 0, itemStatusFailed: 0, stoppedForNetwork: false });
    expect(sent).toEqual([
  ...
  it("승격 이후의 자동 flush는 그 행을 건드리지 않고, 사용자의 '재시도'가 정상 탈출구다", async () => {
    ...
    expect(idle).toEqual({ synced: 0, createdSynced: 0, failed: 0, conflicted: 0, itemStatusSynced: 0, itemStatusFailed: 0, stoppedForNetwork: false });
    expect(sent).toHaveLength(MAX_SERVER_ERROR_ATTEMPTS);
