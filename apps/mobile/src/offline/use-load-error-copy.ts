 ...
 *
 * ## ⚠️ 알려진 한계 — **연속 실패에서는 판정이 다시 서지 않는다** (라운드 86 리뷰 L-14)
 *
 * deps가 `[isError]` 하나라, 실패 카드가 떠 있는 동안 사용자가 [다시 시도]를 눌러 **또 실패하면**
 * (react-query에서 `isError`는 그동안 계속 참이다) effect가 다시 돌지 않는다 — 그 두 번째 실패의
 * 문구는 **첫 실패 때 잰 연결 상태**를 그대로 쓴다. 터널에 들어간 뒤의 실패가 *"잠시 후 다시"* 로,
 * 빠져나온 뒤의 실패가 *"지금은 오프라인이에요"* 로 읽힐 수 있다.
 *
 * ⚠️ **오늘 고치지 않는 이유**(그리고 그 이유를 여기 값으로 적는 이유): 재판정 트리거로 쓸 값
 * (`errorUpdatedAt`·`failureCount`)은 **이 훅이 받지 못한다**. 받으려면 시그니처를 늘려야 하고,
 * 그러면 이 배선을 쓰는 **열다섯 화면 + 저장·가져오기 자리 전부**가 그 값을 골라 넘겨야 한다
 * (그 호출 문자열은 `messages.test.ts`가 글자로 물고 있다). 선택 인자로 열어 두는 길은 오늘
 * 넘기는 화면이 0건이라 **호출부 없는 인자**가 되고, 이 저장소는 그런 판정을 라운드 86 A에서
 * 이미 한 번 걷어 냈다. 그래서 한계로 남기고 자리·폭·재개 조건을 문서에 적는다
 * (`docs/operations/known-limitations.md` AA-R 한계 ①). ⚠️ **이 한계는 오늘 배선된 열다섯 화면
 * 공통이고, 라운드 86이 만든 것이 아니다**(라운드 52 C-07의 cancelled 패턴 이래 같은 모양이다).
 */
