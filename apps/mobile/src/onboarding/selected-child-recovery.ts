 ...
 *  - **재연결(offline→online)**: 연결 사실을 관측하는 자리는 **둘뿐**이다 -- 실패 카드로 전환될
 *    때의 폴 한 번, 그리고 위 AppState 전이마다 한 번.
 *
 *    ⚠ 그래서 이 전이는 **포그라운드에 머무는 동안에는 관측되지 않는다**: 카드를 띄운 채 앱을
 *    켜 두고 있다가 와이파이가 돌아오면, 다음 관측(=앱을 나갔다 돌아오는 전이)이 오기 전까지
 *    아무 일도 일어나지 않고 사용자가 [다시 시도]를 누르는 것이 그대로 정상 경로다. 이 갈래가
 *    실제로 발화하는 것은 "오프라인에서 카드를 본 뒤 앱을 나갔다가, 연결이 돌아온 뒤 돌아온"
 *    순서다. 그 공백을 메우려면 폴러가 필요한데, **새 15초 폴러를 돌리지 않는다**: 이 저장소의
 *    재연결 감시자(`startConnectivityWatcher`)는 아웃박스 flush의 소유물이고(sync-controller.ts),
 *    같은 것을 하나 더 켜는 것은 이 트랙의 금지 사항이다. 잃는 것은 "가만히 두면 알아서 낫는"
 *    편의 하나이고, 사용자에게는 언제나 [다시 시도] 버튼이 있다.
 *
 * ## 재시도가 몇 번인가 -- 실제 보장 수준
 *
 * 이 판정이 보장하는 것은 **전이 하나에 최대 한 번**이다(같은 값이 다시 들어오면 두 조건 모두
 * 거짓이므로 루프가 될 수 없다). "앱 세션당 한 번"은 아니다 -- 재시도가 또 실패해 카드가 다시
 * 서면 그때의 사실로 관측이 처음부터 다시 시작되고, 다음 전이는 다시 한 번을 얻는다.
 *
 * 두 전이가 같은 순간에 도착하는 경우(호출부 useEffect의 AppState 콜백은 `observe`로 appState를
 * 먼저 반영하고 곧이어 연결을 폴한다)는 대개 첫 재시도가 상태를 "error"에서 내려 두 번째
 * 관측이 취소되지만, 그 폴이 먼저 착지하면 **fetch가 한 번 더 나갈 수 있다**. 그래도 루프는
 * 되지 않는다: 두 전이는 각자 한 번씩만 참이고, 그다음 같은 값에는 이 판정이 false다.
 */
