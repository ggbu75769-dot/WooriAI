 ...
 * 대응하는 짝이 없어, **안드로이드에서만** 소리가 났다 — 검색을 건 사람이 iOS에서는
 ...
 *
 * ⚠️ **낭독이 나는 시점은 "제출"이 아니라 350ms 디바운스마다다**(라운드 90 리뷰 L-2). 화면의
 * 낭독 effect는 `activeSearchQuery`가 바뀔 때 도는데, 그 값을 내보내는 것은 위
 * `pendingSearchSubmission`을 쓰는 디바운스 effect다 — **입력이 350ms 멎을 때마다** 한 번씩
 * 나간다(키보드 [검색] 버튼은 그 경로 중 하나일 뿐이다). 그래서 이 문장은 *제출한 사람*이 아니라
 * *검색을 건 사람*이 듣는 문장이고, 실기기 확인의 짝 문서 `#164` ⓐ가 그 시점을 값으로 적는다.
 * ⚠️ 접근성 표 `A-31 #106`의 같은 칸은 아직 *"제출"* 이라고 적는다(라운드 90 리뷰 L-2가 고친
 * 모집단은 이 파일·화면 주석·짝 문서 셋이다) — 그 한 자리는 다음 문서 걸음의 몫이다.
 *
