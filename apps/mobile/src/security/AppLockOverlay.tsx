 ...
 *   않는다(단방향 래치). 그 뒤의 잠금/해제 왕복에서는 노드가 그대로 있고 두 prop만 바뀌므로
 *   재마운트가 **한 번도** 일어나지 않는다. 래치를 되돌리지 않는 것도 같은 이유다 — 설정에서
 *   잠금을 꺼도, 비활성 노드 하나를 없애자고 화면 전체를 다시 그릴 이유가 없다.
 * - **단, 래치가 처음 켜지는 그 한 번은 재마운트다**(라운드 59 통합리뷰 P2-5 — 정직 고지).
 *   그 렌더에서 자식의 자리가 `<>{children}</>`에서 `<View>{children}</View>`로 바뀌므로 뒤 트리가
 *   언마운트→재마운트되고, 화면 컴포넌트의 로컬 상태(쓰다 만 입력 등)는 그때 사라진다. 실제로
 *   일어나는 순간은 둘이다: ① 설정에서 잠금을 **켠 바로 그 순간**(그 화면은 설정 화면이고, 뒤
 *   트리에 남아 있던 다른 탭의 입력 상태가 초기화된다), ② 콜드 스타트에서 SecureStore 기록을
 *   다 읽어 `enabled`가 참이 되는 순간(부팅 직후라 아직 아무것도 입력되지 않았다). 잠금을 켜지
 *   않은 사용자에게는 영영 일어나지 않는다. 이 한 번을 없애려면 방패를 **모든** 사용자에게 상시
 *   마운트해야 하고(수용 기준 2 위반), 그 대가가 위에서 물린 재마운트보다 크다 — 기록되는 것은
 *   트레이드오프이고, 코드는 그대로 둔다.
...
/**
 * 레이아웃 통과용. 뒤 트리가 있던 자리를 그대로 채운다.
 *
 * 라운드 59 통합리뷰 P2-6: 종전 주석은 부모를 "SafeAreaProvider의 flex:1 뷰"라고 적었는데 이
 * 앱에는 SafeAreaProvider가 없다(저장소 전체에 그 컴포넌트를 쓰는 자리가 한 곳도 없다). 실제
 * 부모는 `app/_layout.tsx`의 `ErrorBoundary`이고 그것은 Fragment를 돌려주므로, 이 뷰의 네이티브
 * 부모는 RN 루트 컨테이너(flex:1)다. 그래서 `flex: 1` 하나로 `<Stack>`이 차지하던 자리를 그대로
 * 채운다 — 방패가 서기 전후로 레이아웃이 달라지지 않아야 한다는 것이 이 상수의 전부다.
 * (노치 여백은 이 자리가 아니라 화면마다 `ScreenScaffold`의 RN `SafeAreaView`가 잡는다 — 방패
 * 훨씬 아래다.)
 */
