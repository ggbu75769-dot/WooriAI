 ...
 * 값이 없으면 행 자체가 서지 않는다). **지우지 않는다** — 위 머리말이 적었듯 이 파일은
 * `consent/legal-links.ts`에서 형식만 가져왔고, 그쪽 쌍둥이와 함께 판정하지 않으면 그 관례가
 * 반쪽으로 남는다.
 */
...
/**
 * 열지 못했을 때 뜨는 알림. **조용히 넘기지 않는다** — 아무 일도 일어나지 않는 행을 남기면
 * 그것이 곧 가짜 버튼이다(app/settings/privacy.tsx의 관례).
 *
 * ## 라운드 72 리뷰 M-1 — **재시도를 권하지 않고, 원인도 단정하지 않는다**
 *
 * 종전 본문은 "잠시 후 다시 시도해 주세요."였다. 그런데 `openExternalUrl` 한 벌이 이 알림을
 * 띄우는 경우는 둘뿐이고(열 수 있는지 물었을 때 false · 여는 호출이 던짐) **둘 다 기다려서
 * 풀리지 않는다** — 몇 번을 눌러도 같은 답이 온다. 다시 눌러도 결과가 같은 실패에 기다리라고
 * 말하지 않는 것이 이 저장소의 규율이다(라운드 70 B의 저장 실패 · 라운드 71 A의 가져오기 실패).
 *
 * ⚠️ 그 규율을 지킨다고 **원인을 대신 지어내지도 않는다.** 두 경우는 같은 `catch`로 들어오므로
 * 이 자리에서 앱이 아는 것은 "열리지 않았다"까지다 — "브라우저가 없어요" 같은 단정은 잘못된
 * 주소에도 그대로 뜬다(허위 표시). 그래서 본문은 **관찰된 사실 한 줄**이고, 링크 실패 네 자리
 * (더보기 · 설정 · 개인정보 · 로그인)가 같은 결의 문장을 쓴다.
 */
export const SUPPORT_LINK_FAILED_TITLE = "링크를 열지 못했어요";
export const SUPPORT_LINK_FAILED_MESSAGE = "이 기기에서 링크가 열리지 않았어요.";
