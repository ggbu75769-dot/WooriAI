...
const text = {
  ...
  tertiary: "#7A716B",
  /**
   * 입력칸 플레이스홀더 전용 잉크 — **라운드 106 정찰 S2 발견 3의 고침**.
   *
   * 종전(그때는 참): 플레이스홀더 일곱 자리가 `gray300`(#E5DFDB)을 썼다. 그 값이 테두리·트랙·
   * 구분선의 색이기도 해서 "연한 회색 하나"로 같이 쓰였고, 그때는 그것이 자연스러운 재사용이었다.
   * → 이제: 플레이스홀더는 자기 토큰을 갖는다. 근거는 실측이다(WCAG 2.x 상대휘도 공식, 오늘 재측정):
   *   · 종전 #E5DFDB on `cream.surface`(#FFFFFF) = **1.32:1** — 본문 기준 4.5:1은커녕 큰 글자 3:1도
   *     못 넘긴다. 플레이스홀더는 비활성 컨트롤이 아니라 **살아 있는 입력칸이 무엇을 받는지 말하는
   *     안내문**이라 WCAG 1.4.3의 비활성 예외가 걸리지 않는다.
   *   · 오늘 값 #756C66 = 흰 배경 **5.13:1** · `cream.bg`(#FFFDFC) 5.06:1 · `cream.surfaceAlt`
   *     (#F8F6F4) 4.76:1 — 앱의 세 배경 어디에서도 4.5:1을 넘긴다.
   *
   * ⚠️ 왜 `gray600`(#5F5854 · 6.98:1)이 아닌가 — 이것이 이 토큰의 진짜 거래다. 대비만 보면
   * gray600이 더 높지만, 플레이스홀더는 **입력된 값과 구별되어야** 한다(구별을 잃으면 "이미 뭔가
   * 적혀 있다"고 읽혀 사용자가 지우려 든다). 입력값의 색은 이 자리들 전부 `text.primary`(#211E1C)다:
   *   · gray600 ↔ text.primary = **2.38:1** — WCAG가 "구별된다"고 부르는 최소선(3:1)에 못 미친다.
   *   · #756C66 ↔ text.primary = **3.23:1** — 그 선을 넘는다.
   * 즉 4.5:1(읽힘)과 3:1(구별됨)을 **둘 다** 만족하는 창 안의 값이고, 그 두 수치를
   * src/placeholder-contrast.test.ts가 리터럴 기대값으로 문다.
   */
  placeholder: "#756C66"
} as const;
