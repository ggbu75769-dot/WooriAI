 ...
 * 반응이 없다"로 보인다. 저장소는 그 비용을 이미 알고 있었다 — 판매처 자동완성 칩을 `onBlur`로
 * 접지 않는 이유가 통째로 그 설명이었다(app/expenses/new.tsx의 `merchantFocused` 주석).
 * 이식된 스캐폴드(src/design-system/components/ScreenScaffold.tsx)는 이미 `"handled"`다.
 *
 * ⚠️⚠️ **두 시점 — 위 인용은 오늘 그대로 읽으면 안 된다**(라운드 92 리뷰 M-3 · 주석만 고친다).
 *  · **라운드 65 시점**: 그 주석은 *"첫 탭이 통째로 먹히니 blur에 칩을 접으면 두 번째 탭이 맞을
 *    자리가 없다"* 를 이유로 들었고, 그때 이 문단이 인용한 것이 그 문장이었다.
 *  · **오늘(라운드 92 A + 리뷰 H-2)**: 그 화면의 칩 줄들이 저마다 `"handled"`를 명시해 첫 탭은
 *    이제 칩에 닿는다. 그래서 인용의 **결론**(`onBlur`로 접지 않는다)만 살아 있고 **이유**가
 *    바뀌었으며, 사라진 자동 blur는 `applyMerchantSuggestion`이 손으로 되돌린다
 *    (`merchantInputRef.current?.blur()`). 오늘의 사실은 그 파일의 ①②③ 세 시점 주석에 있다.
 * ⚠️ 이 스크롤러 자신의 판정과 아래 `"always"`를 고르지 않은 이유는 **그 갈림과 무관하게** 옳다.
 *
