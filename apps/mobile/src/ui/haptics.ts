 ...
 *
 * ⚠️ 채택 지점은 전부 **핸들러/콜백 또는 effect 안**이다(저장 onSuccess · 상태 체크 확정 ·
 * PIN 오류 · 홈 빠른 기록 칩 핀 토글 · 홈 예산 100% 경고 배너의 등장).
 * 렌더 경로에서 부르지 말 것 — EXP-001/ITEM-001 비세션 렌더 무접촉 계약과 무관해야 하고,
 * 렌더마다 진동이 나는 화면은 그 자체로 결함이다.
 *
 * ⚠️ 두 시점 (라운드 102 리뷰 L-10) — 라운드 101 트랙 B 시점의 이 문단은 *"전부 핸들러/콜백
 * 안"* 이라고 적었다. 라운드 102 F6b가 예산 경고를 **effect**에서 채택하며 그 목록에 넷째
 * 종류가 생겼다: 값이 "사람이 누른 순간"이 아니라 **화면에 사실이 나타난 순간**에 나는 신호는
 * 핸들러가 없어 effect가 유일한 자리다. 규율의 본체는 그대로다 — 금지되는 것은 "핸들러가
 * 아닌 곳"이 아니라 **렌더 경로**이고, effect는 렌더가 끝난 뒤에 돈다. 그런 effect 채택은
 * 발화 기억을 **화면 밖(persist 클레임)**에 두어야 한다는 조건이 따라붙는다(리뷰 M-1: ref는
 * 마운트마다 새로 서므로 앱을 열 때마다 울린다).
 */
