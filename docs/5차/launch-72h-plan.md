...
- [ ] `pnpm --filter api prisma:deploy && pnpm --filter api seed` (마이그레이션은 **개수를 세지 않는다** — `prisma migrate deploy`가 미적용분을 전부 적용하고 개수를 인자로 받지 않는다. 적용 확인은 `prisma migrate status`가 "up to date"인가로 한다. ⚠️ **두 시점**: 이 자리는 2026-08-21(라운드 13, DOC-114)까지 "마이그레이션 **13개**"라고 적었고 그때는 참이었다 — 당시 `apps/api/prisma/migrations/`가 `000001`~`000013`이었다. 오늘 2026-09-07에는 `000001`~`000026`으로 26개이고, 손으로 적은 수는 마이그레이션을 더할 때마다 낡으므로 다시 적지 않는다(⚠️ **세 번째 시점**: 이 자리는 라운드 106 F5 시점에 **25개**라고 적었고 그때도 참이었다 — 라운드 107 트랙 E가 `000026_product_links_seed_key`를 더했다. 그래서 이 자리가 결론으로 적은 "세지 않는다"가 다시 한 번 옳았다). 개수의 단일 소스는 그 디렉터리 자신이다. 시드: 카테고리 12·준비템 62·링크 **67** — 두 시점: 라운드 82 B가 링크 0건 품목 넷을 채워 58 → 62, 라운드 83 A가 스폰서 링크만 있던 다섯에 비스폰서 링크를 더해 62 → 67. 수를 세는 자리는 §5 참고)
...
- ⚠️ **두 시점(라운드 107 트랙 E) — 이 전환이 다음 배포에 살아남는가**: **종전**에는 살아남지 못했다.
  시드가 매 배포마다 `product_links`를 전량 덮어써서 CSV로 심은 `url`·`affiliateUrl`·`isAffiliate`가
  **비제휴 검색 링크로 되돌아갔고**, 자연키에 `title`이 들어 있어 제목까지 바꿨다면 되돌림이 아니라
  링크가 하나 더 생겼다(실측 67 → 68행). **오늘**은 시드가 있는 행을 고치지 않고 안정 키
  `product_links.seed_key`(마이그레이션 000026)로 자기 행을 찾으므로, **CSV 일괄 교체 결과가 그대로
  남는다**(`apps/api/prisma/seed.ts:33`·`:48`, 회귀 고정 `apps/api/test/seed-boundary.db.test.ts`).
  전환 뒤 배포 로그의 `[시드] 구매 링크: 신규 N · 유지 N · 덮어씀 N` 줄에서 **덮어씀 0**을 확인하면 된다.
