...
- [ ] `pnpm --filter api prisma:deploy && pnpm --filter api seed` (마이그레이션은 **개수를 세지 않는다** — `prisma migrate deploy`가 미적용분을 전부 적용하고 개수를 인자로 받지 않는다. 적용 확인은 `prisma migrate status`가 "up to date"인가로 한다. ⚠️ **두 시점**: 이 자리는 2026-08-21(라운드 13, DOC-114)까지 "마이그레이션 **13개**"라고 적었고 그때는 참이었다 — 당시 `apps/api/prisma/migrations/`가 `000001`~`000013`이었다. 오늘 2026-09-07에는 `000001`~`000025`로 25개이고, 손으로 적은 수는 마이그레이션을 더할 때마다 낡으므로 다시 적지 않는다. 개수의 단일 소스는 그 디렉터리 자신이다. 시드: 카테고리 12·준비템 62·링크 **67** — 두 시점: 라운드 82 B가 링크 0건 품목 넷을 채워 58 → 62, 라운드 83 A가 스폰서 링크만 있던 다섯에 비스폰서 링크를 더해 62 → 67. 수를 세는 자리는 §5 참고)
