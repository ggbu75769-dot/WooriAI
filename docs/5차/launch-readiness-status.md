# 출시 준비 현황 (2026-08-21 기준, 라운드 12까지)
...
| 테스트 | **api 342 · mobile 550 · admin 68 · domain 28 · contracts 35 (총 1,023)**, 릴리즈 게이트 11/11 (라운드 12 후 재인증) |
| 실서버 검증 | HTTP 스모크 28/28 (워커 헬스 포함) · 어드민 브라우저 E2E 7/7 · 백업 복구 드릴 6/6 |
| 정밀 리뷰 | 4라운드 32건 + 커버리지 표적 테스트가 찾은 실버그 6건·잠재 이슈 4건 — 전량 수정 |
| 성능 | DB 인덱스 5종 실측(파기 16ms→0.07ms) · /home 이중 조회 제거 · 기록 탭 FlatList 가상화 · 부하 p95 기준선 |
