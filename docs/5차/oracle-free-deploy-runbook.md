...
- **재실행해도 어드민에서 고친 콘텐츠가 살아남습니다** ⚠️ **두 시점(라운드 107 트랙 E)**. **종전**: 위 보장은 `admin_users` **하나에만** 있었고, 이 스크립트의 8단계 시드가 매 실행마다 카테고리·준비템·준비템 단계·고지 문구·구매 링크 다섯 표를 시드 값으로 **되돌렸습니다**(구매 링크는 되돌림이 아니라 **증식**이었습니다 — 자연키에 어드민 편집 축인 제목이 들어 있어, 링크 이름을 고치면 다음 실행이 살아 있는 구매 CTA를 하나 더 만들었습니다. 실측 67 → 68행). **오늘**: 다섯 표 전부 *"없는 행을 만들 뿐, 있는 행은 고치지 않는다"* 이고, 구매 링크는 안정 식별자 `product_links.seed_key`(마이그레이션 000026)로 자기 행을 찾습니다 — 그래서 **부트스트랩 재실행이 실제로 멱등**입니다(그 판단과 경계 표는 `scripts/deploy/oracle-bootstrap.sh` 8단계 주석 · `apps/api/prisma/seed.ts:33`·`:42~49`에 값으로 있고, 회귀는 `apps/api/test/seed-boundary.db.test.ts`가 고정한다). 되돌리는 길은 명시적 opt-in `SEED_OVERWRITE_CONTENT=1` 하나뿐이고 **이 스크립트는 그 값을 절대 설정하지 않습니다**. 재실행 뒤 로그의 `[시드] … 신규 N · 유지 N · 덮어씀 N` 줄과 `product_links 중복:` 경고를 한 번 읽어 주세요(경고가 있으면 시드는 아무것도 지우지 않았고, 어느 행을 남길지는 사람이 어드민에서 정합니다).
