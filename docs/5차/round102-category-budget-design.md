...
- **커스텀 카테고리 부재 전제**: 사용자는 분류를 만들 수 없다 — 모집단은 운영 시드뿐이므로 상한 30(§1.4)과 칩 대장 재사용이 성립한다. 이 전제가 깨지는 라운드는 이 절을 재론해야 한다.
  - ⚠️ **두 시점 — 이 전제는 라운드 103에서 깨졌고, 그 라운드가 요구대로 이 절을 재론했다**: `docs/5차/round103-custom-expense-category-design.md` §6.3. 재론 결과 위 두 문장은 **그대로 참이다** — 커스텀 상한을 15로 정해 12 + 15 = 27 ≤ 30을 유지했고(그 문서 §1.7 — 이쪽 숫자를 맞춘 것이지 상한 30을 움직이지 않았다), 커스텀 칩의 `matchIds`는 자기 id 하나라 가족 합류 규칙에 새 축이 들어가지 않는다(§2.2). 아래 활성 판정 규칙(신규 거부·기존 유지)도 커스텀의 **보관**(`active:false`)에 그대로 적용된다. 서버에서 실제로 바뀐 자리는 예산 검증의 가구 스코프 하나다(그 문서 §1.9 #9 · 라운드 103 T1).
