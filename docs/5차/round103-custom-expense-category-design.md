...
| 시드 21행 | 정식 12(`categorySeeds`, **랜덤 UUID**) + 모바일 퀵타일 별칭 8(`mobileCategoryAliasSeeds`, 고정 UUID·`mobile_*` code·`selectable:false`) + 가져오기 스텁 1(`import_stub_default`). ⚠️ **정정(T1 실측)**: 이 칸은 원래 *"전 행이 `is_system=true`"* 라고 적었으나 거짓이다 — 별칭 8 + 스텁 1은 `isSystem: false`로 시드된다(dev DB 실측 `true`=12 / `false`=9). 이 오기가 §1의 CHECK를 양방향 등호로 적게 했고(그대로였으면 마이그레이션이 실패한다) §2.2가 표식을 `isSystem`으로 적게 했다. 세 트랙이 이 표를 근거로 읽으므로 여기서 고친다 | `prisma/seed-data.ts:78,1183,1201`, `prisma/seed.ts:23~60` |
