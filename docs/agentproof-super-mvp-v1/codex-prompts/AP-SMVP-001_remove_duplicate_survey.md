# AP-SMVP-001 — 중복 파일럿 설문 필수 경로 제거
...
```text
AP-SMVP-001 — 중복 파일럿 설문 필수 경로 제거 작업을 수행해줘.
대상 저장소는 agentproofKR/agentproofKR.github.io 하나뿐이다.
목표: 무료 진단 후 `/proof/pilot-design/questions`를 필수로 거치지 않게 만들고, questions route는 고급 설정/legacy route로만 유지한다.
