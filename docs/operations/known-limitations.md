...
- ⚠️⚠️ **갱신 (2026-08-30 · 라운드 79) — 기각이 아니라 *전제를 정정한 뒤 보류*다.** 위 근거(*"`readOnly`가
  ...
  HTML 명세상 `<select>`의 옵션 텍스트는 선택 가능한 텍스트 노드가 아니라 **활성 상태에서도 드래그 복사가
  되지 않고**(체크박스에는 애초에 복사할 텍스트가 0건이다), 그래서 `disabled`가 실제로 앗아가는 것은
  **복사가 아니라 도달**이다(탭 순서에서 빠지고 · 목록을 열어 다른 값을 볼 수 없고 · 스크린리더 포커스가
  닿지 않는다). ⚠️ **그러면 대안의 값이 재배열된다**: ⓑ는 **복사를 되살리지 못하고 포커스만 되살리며**,
