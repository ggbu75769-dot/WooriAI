...
| 실 제휴 링크 | 시드는 **비제휴 실 쿠팡 검색 링크**다 — `https://example.com/dev/...` 플레이스홀더는 **오늘 0곳**이고(2026-09-07 다시 셈: `grep -c 'https://example.com' apps/api/prisma/seed-data.ts` → 0), 링크 67건의 `url` 67이 전부 `https://www.coupang.com/np/search…`이며 `affiliateUrl`을 가진 행은 0건이다(쿠팡 파트너스 승인 전). ⚠️ **세 시점(낡은 값을 지우지 않는다)**: ① 라운드 82 B 이후 **62건 / example.com 81곳** → ② 라운드 83 A가 비스폰서 링크 다섯을 더한 뒤 **67건 / 86곳** — **여기까지가 이 칸이 종전에 적던 값이고 그때는 참이었다** → ③ 출시 트랙 LP-A(72h 계획 §5 플랜 B)가 그 86곳을 전부 실 쿠팡 검색 링크로 교체해 **67건 / 0곳**이 됐고, 이 칸은 라운드 106 F5가 그 사실로 맞췄다. ⚠️ 이 수를 **세는 자리**는 `docs/5차/day1-deploy-runbook.md` A-5의 실행되는 인용이다(`packages/test-utils/src/repo-self-description.test.ts`가 그 명령을 실제로 돌린다) — 여기 적힌 것은 그 답의 사본이고, ②가 ①을 사본으로 이어받았다가 원본과 갈렸던 것이 이번 정정의 이유다(X-4) | 제휴 계약 + 관리자 CMS(`apps/api/src/admin/product-link-bulk.controller.ts` 포함)에서 실 URL 등록 |
...
- **`ProductComparisonRow`의 `caption` 기본값 `"무료배송"`은 실사용자에게 도달하지 않는다.**
  ...
  (`apps/mobile/app/items/[itemTemplateId].tsx:1069` — ⚠️ **이 좌표를 옮기는 손은 F이고 시점은
  *그 파일이 밀린 것을 라운드가 처음 재는 날*이다) 그 자리는 `caption={hasSession ? … : undefined}`
  라 **기본값이 서는 것은 비세션 분기뿐**이다(`apps/mobile/src/ui.tsx`의 `caption = "무료배송"`),
  ⚠️⚠️ **두 시점(2026-08-31 · 라운드 93 F)** — 라운드 70이 적고 라운드 71~92가 그대로 둔 좌표는
  `apps/mobile/app/items/[itemTemplateId].tsx:997`이었고, **그 좌표는 오늘 거짓이다**(그 줄에
  `caption={hasSession`이 없다 · 파일이 자라며 **72줄** 밀렸다). **옛 좌표를 지우지 않는 이유는
  트랙 E의 사각 ⓐ가 다음 라운드에 얼마로 줄었는지를 셀 수 있게 하기 위해서다** —
  근거 자체(실호출부 하나 · 비세션 분기뿐)는 그때도 오늘도 참이고, 밀린 것은 줄 번호뿐이다,
...
- **알림 벨 🔔도 같은 판정이다.** 같은 manifest의 home `bodyText`에 실재하고(1건), 그리는 노드는
  `apps/mobile/src/notifications/NotificationBell.tsx:22`이며(2026-08-31 재확인 — 그 줄이 오늘도
  `<Text style={bellGlyphStyle}>🔔</Text>`다) HOME-001 픽셀락 분기가 그것을 그린다
  (`apps/mobile/app/(tabs)/index.tsx:2142`의 `action={<NotificationBell />}` — ⚠️ **이 좌표를
  옮기는 손은 F이고 시점은 *그 줄이 밀린 것을 라운드가 처음 재는 날*이다).
  ⚠️⚠️ **두 시점(2026-08-31 · 라운드 93 F)** — 라운드 70이 적고 라운드 71~92가 그대로 둔 좌표는
  `apps/mobile/app/(tabs)/index.tsx:2105`였고, **그 좌표는 오늘 거짓이다**(`action={<NotificationBell`이
  그 줄에 없다 · **37줄** 밀렸다). 옛 좌표는 지우지 않는다 — **밀린 것은 줄 번호뿐이고 판정은
  그때도 오늘도 같다.**
...
- ⚠️⚠️ **명시적 제외 셋은 오늘 하나도 남아 있지 않다 — 두 시점으로 적는다**(2026-08-31 · 라운드 93 F ·
  ⚠️ **이 줄은 좌표를 옮겨 고치는 것이 아니라 문장 자체가 오늘의 사실이 아니라서 다시 쓴다**).
  - **종전(라운드 72~73 시점, 이 줄이 처음 쓰인 때)**: *"명시적 제외 셋은 그대로다:
    `app/settings/privacy.tsx:150`의 `loadFailedText` · 검수 화면(`app/import/[importJobId].tsx:131`)의
    조회 실패 둘 · 지출 상세(`app/expenses/[expenseId].tsx:791`)의 실패 카드. 셋 다 [다시 시도]가
    실제로 통하는 실패라는 판정이 유지된다(라운드 72 트랙 B의 금지 목록이 다시 명시했다)."*
  - ⚠️⚠️ **오늘(2026-08-31 · HEAD 실측)**: **셋 다 공용 조회 훅으로 배선됐고**(라운드 74 트랙 D ·
    GAP-074 #4) `OFFLINE_AWARE_LOAD_ERROR_SCREENS`에 이름으로 서 있다. 그래서 **인용된 심볼
    `loadFailedText`는 `app/settings/privacy.tsx`에도 `app/import/[importJobId].tsx`에도 오늘
    0건**이다 — 좌표가 밀린 것이 아니라 **무는 대상 자체가 사라졌다**. 오늘 그 자리에 서는 것은
    `useLoadErrorCopy(...)`의 반환값이고(개인정보 화면 **넷** · 검수 화면 **둘** · 지출 상세 **하나**),
    `OFFLINE_AWARE_LOAD_ERROR_EXEMPT_SCREENS`는 **빈 객체**다(라운드 86 트랙 B가 마지막 한 자리를
    닫았다 — **조회 제외 0건**).
  - ⚠️⚠️ **그리고 계약과 문서가 정확히 반대를 말하고 있었다**:
    `apps/mobile/src/settings/destructive-flow-messages.test.ts`가
    `expect(privacySource).not.toContain("const loadFailedText =")` 로 **그 심볼의 *없음*을 계약으로
    물고 있는데**, 이 줄은 그것이 *있다*고 적고 있었다. **계약은 라운드 74부터 옳았고 문서만 낡았다** —
    라운드 93 트랙 E가 좌표를 세는 자를 세우고 나서야 그 갈림이 보였다(판정 **AH-4**).
  - ⚠️ **판정 자체는 그때도 오늘도 참이다** — 셋 다 *[다시 시도]가 실제로 통하는 실패*이고, 오늘
    그 사실을 지는 것이 손으로 적은 제외 목록이 아니라 **공용 훅 + 목록 스윕**이라는 것만 달라졌다.
    지출 상세의 실패 카드는 오늘도 그 자리다 — `app/expenses/[expenseId].tsx:791`의 이유 문단이
    그 판정을 적고, 카드는 그 아래 `useLoadErrorCopy(expense.isError)`에서 값을 받는다.
    ⚠️ **이 좌표를 옮기는 손은 F이고, 시점은 *그 줄이 밀린 것을 라운드가 처음 재는 날*이다.**
...
#### L-2 갱신 (2026-09-07 · 라운드 103 리뷰 M-3) — **열다섯 → 열여섯 · 카드 아닌 자리 일곱 → 여덟**

- 라운드 103이 세운 지출 분류 관리 화면(`app/settings/categories.tsx`)이 **저장 쪽에만** 배선돼
  있었다. 조회 쪽은 훅도 부르지 않고 옛 실패 리터럴도 쓰지 않아, `messages.test.ts`의 스윕이
  세는 두 집합이 **일치한 채 통과**했다 — 라운드 73 L-2가 이 절의 사각으로 미리 적어 둔
  바로 그 자리다(`offline-aware-screens.ts` 머리주석이 같은 문장을 값으로 갖고 있다).
- 사각이 실제로 무엇을 덮었나: 비행기 모드나 500에서 그 화면은 분류가 열다섯 개 있어도
  **"아직 직접 추가한 분류가 없어요."** 하나만 그렸다. 재시도 버튼도 오프라인 고지도 없었고,
  목록이 비니 중복·상한 판정의 모집단까지 비어 **이미 있는 이름을 다시 쳐도 [분류 추가]가
  활성**이었다(서버 400으로만 막혔다).
- 오늘의 값은 **배선 열여섯 · 조회 제외 0 · 카드가 아닌 자리 여덟**이다(저장 쪽은 종전 그대로
  배선 여섯 · 제외 0). 여덟째가 카드 아닌 자리로 등재된 사유는 값으로 적혀 있다 —
  `LoadErrorCard`라 프롭 이름이 `message`·`retryLabel`이고, `["children"]` 조회가
  `householdId == null`로 접혀 조회 중으로 읽히므로 둘째 훅을 부르지 않는다.
- ⚠️ **래칫을 양방향으로 실증했다**(라운드 86 갱신이 값을 옮기기만 한 것과 다른 점): 대장 등재만
  되돌리면 4건이, 훅만 벗기면 같은 스윕이 **반대 방향으로** 4건이 빨개진다. 즉 이 화면에 대해서는
  "훅도 리터럴도 없어 양쪽이 일치한 채 통과"하던 갈래가 닫혔다. 다른 화면에 대한 그 사각은
  여전히 열려 있다 — 이 절이 그 사실을 계속 진다.
...
- ⚠️⚠️ **고르지 않은 대안과 그 이유가 이 절의 재사용 가능한 값이다.** *"배너와 같은 수를 훅에 넘긴다"*
  ...
  `app/(tabs)/index.tsx:1519`에서 만들어지고 훅 호출은 그보다 위라 **둘 중 하나를 옮겨야 한다.**
  ⚠️⚠️ **두 시점 — 이 자리가 이 저장소에서 *이동 의무*가 곁에 적힌 유일한 좌표였고, 그 의무는
  라운드 79부터 라운드 92까지 지켜지지 않았다**(2026-08-31 · 라운드 93 F).
  - **종전(라운드 79~92)**: *"`monthlyUsed`는 `app/(tabs)/index.tsx:1482`에서 만들어지고 훅 호출은
    `:1260`이라 둘 중 하나를 옮겨야 한다."*
  - ⚠️⚠️ **오늘(HEAD 실측)**: `const monthlyUsed`가 서는 자리는 **`:1519`**이고(**37줄** 밀렸다),
    `:1260`은 **훅 호출이 아니라 대기 행(`pendingRecordRows`) 주석의 한 줄**이다 — 그 좌표는
    줄이 밀린 것이 아니라 **가리키는 것의 갈래가 달라졌다**(오늘 그 줄에 `use…(` 꼴이 0건이다).
    ⚠️ **두 번째 좌표는 다시 숫자로 적지 않는다** — 훅 호출은 `monthlyUsed`보다 **위**에 있다는
    사실만 적고 좌표를 빼는 것이 오늘의 답이다(그 자리는 이 절이 무엇을 말하는지에 필요하지
    않고, 숫자로 적는 순간 다음 라운드에 또 거짓이 된다).
  - ⚠️⚠️ **의무 이행 명기** — 이 문서에서 *누가 언제 옮기는가*가 곁에 적힌 자리는 라운드 92까지
    **이 하나**였고(트랙 E 실측 · `MOVE_OBLIGATION_FLOOR = 1`), **그 하나가 자기 의무를 지키지
    않았다.** 오늘 라운드 93 F가 그 의무를 **처음 이행한다** — 그리고 같은 걸음에 **의무를 옮기는
    손과 시점을 이 자리에 값으로 적는다: 옮기는 손은 F이고, 시점은 *`app/(tabs)/index.tsx`가
    밀려 이 좌표가 거짓이 된 것을 라운드가 처음 재는 날*이다.** ⚠️ **라운드 93 트랙 E가 세운
    좌표 계약이 그 실재를 매 라운드 다시 센다**(`packages/test-utils/src/repo-self-description.test.ts`).
...
- ⚠️ **갱신 (2026-08-31 · 라운드 90 트랙 F) — 답한 자리를 되짚는다.** 라운드 90 **AE절 머리말**의
  답 다섯 중 **첫째**가 전수로 답했다(수와 그 수를 낸 방법은 그 절이 든다).
  ⚠️ **판정을 여기 옮겨 적지 않는다**(O-3·X-4) — 어느 절이 답했는지만 가리킨다.
...
- ⚠️ **갱신 (2026-08-31 · 라운드 90 트랙 F) — 답한 자리를 되짚는다.** 라운드 90 **AE절 머리말**의
  답 다섯 중 **둘째**가 전수로 답했다(수와 그 수를 낸 방법은 그 절이 든다).
  ⚠️ **판정을 여기 옮겨 적지 않는다**(O-3·X-4) — 어느 절이 답했는지만 가리킨다.
...
- ⚠️ **갱신 (2026-08-31 · 라운드 90 트랙 F) — 답한 자리를 되짚는다.** 라운드 90 **AE절 머리말**의
  답 다섯 중 **셋째**가 전수로 답했다(수와 그 수를 낸 방법은 그 절이 든다).
  ⚠️ **판정을 여기 옮겨 적지 않는다**(O-3·X-4) — 어느 절이 답했는지만 가리킨다.
...
- ⚠️⚠️ **정정 (2026-08-31 · 라운드 89 적대 리뷰 M-2·M-3) — 위 두 문장의 수가 어긋났다. 판정은
  그대로이고, 어긋난 수만 두 시점으로 고친다.**
  - ⓐ **자리는 둘이 아니라 셋이었다.** *당시*: 이 절도 짝 계약도 C-3 경과 수를 말하는 자리를
    **둘**(접근성 표의 C절 행 · 수동 증거 절)로 셌다 — 모집단을 **한 파일 안**으로 그었기 때문이다.
    *오늘*: 리뷰가 **셋째 자리**를 찾았다 — `docs/qa/runtime-verification-required.md:367` §0-1의
    🔒 줄이 같은 경과 수를 한 벌 더 적고 있었고, **어느 계약도 그 자리를 물지 않았다.** 오늘 짝 계약이
    그 자리까지 함께 물도록 넓혀졌다(같은 파서 재사용 · ⓐ 수사 · ⓑ 접두 · ⓒ 끝 라운드 · ⓓ 목록 모양).
    ⚠️ **이 절이 스스로 말한 그 질문**(*"같은 사실을 말하는 자리가 몇인가"*)에 **이 절 자신이 낸 답이
    하나 모자랐다는 사실**이 여기 값으로 남는다 — 세는 그물을 세울 때 모집단을 *파일*로 긋는 것과
    *사실*로 긋는 것은 다른 일이다.
  - ⓑ **갈림 상한은 셋이 아니라 0이다.** *당시*: 짝 계약이 세워질 때의 실측 갈림이 셋(88 − 85)이라
    상한도 3이었고, 위 문장이 *"셋을 넘지 않는가"* 라고 적은 것은 그 시점의 사실이다. *오늘*:
    **같은 라운드의 F(이 절)가 두 자리를 함께 89로 올려 갈림이 0이 됐는데 상한만 3으로 남아**,
    *"C절만 올리기"* 한 걸음이 다시 조용히 통과하는 상태였다. 리뷰가 상한을 오늘의 실측 갈림(**0**)으로
    내렸고, **두 자리를 함께 올린 라운드는 상한도 그날의 실측 갈림으로 함께 내린다**는 규율이 그
    상수의 주석에 값으로 박혔다. ⚠️ 위 문단의 *"하한·정합만 물어야 한다"* 는 판정은 그대로다 —
    여기서 내린 것은 **갈림의 상한**이지 문서가 자라는 방향의 상한이 아니고, 갈림에는 하한이 없어
    내리는 쪽은 언제나 안전하다.
- ⚠️ **갱신 (2026-08-31 · 라운드 90 트랙 F) — 답한 자리를 되짚는다.** 라운드 90 **AE절 머리말**의
  답 다섯 중 **넷째**가 전수로 답했고, 위 정정 ⓑ가 내린 그 상수를 문서가 옮겨 적은 두 자리는
  같은 라운드의 F가 고쳤다. ⚠️ **판정을 여기 옮겨 적지 않는다**(O-3·X-4) — 어느 절이 답했는지만
  가리킨다.
...
- ⚠️ **갱신 (2026-08-31 · 라운드 90 트랙 F) — 답한 자리를 되짚는다.** 라운드 90 **AE절 머리말**의
  답 다섯 중 **다섯째**가 전수로 답했다(답은 **0**이고, 그 0이 낳은 관찰은 그 절과 **AE-5**가 든다).
  ⚠️ **판정을 여기 옮겨 적지 않는다**(O-3·X-4) — 어느 절이 답했는지만 가리킨다.

## AE. 라운드 90에서 확정한 판정 (2026-08-31 · GAP-090 트랙 F)

라운드 89가 물은 것이 **판정이 값으로 선 다음에는 무슨 일이 일어나는가**(답: 아무 일도 일어나지
않는다 · AD-1)였다면, 라운드 90의 물음은 그 **바로 옆 칸**에 있다 — **그물이 자기 손으로
*"여기는 어느 스윕도 세지 않는다"* 고 적어 둔 자리는 누가 집는가.** 세어 보니 답은 **아무도 집지
않는다**였고, 그 자리 가운데 하나는 **핵심 루프 안에서 사람이 겪는 침묵**이었다. 축은 라운드
81~89와 같이 **사용자 가치**였고(준비템 검색 · 운영자 도구 · 저장소가 자기 문서와 자기 그물을 세는
자리), 다섯 판정 다 K~AD절과 같이 **결함 보고가 아니라 다음 결정의 입력**이며 2026-08-31 소스에서
확인됐다(라운드 90 트랙 A·B·C·D·E 머지 후). ⚠️⚠️ **이번 라운드는 세 라운드 만에 핵심 루프 안의
화면을 하나 열었다** — 다만 연 것은 렌더가 아니라 **낭독 한 줄**이고, 렌더·문구·가격·링크는 바이트
불변이다.

⚠️⚠️ **이번 라운드의 가장 값진 관측: 판정 칸이 지목한 자리를 고친 라운드의 다음 질문은 사각 칸이
지목한 자리이고, 그 둘은 같은 요구의 다른 모양이다**(AE-1). 라운드 89는 **판정 칸**(프롭 대장의
`crossPlatform`)이 지목한 자리를 고쳤고, 오늘 세어 보니 **사각 칸**이 이름으로 지목한 자리가
열하나였으며 **그중 하나가 사람이 겪는 침묵**이었다 — a11y 그물이 스스로 *"live region 단독은 어느
스윕도 세지 않는다"* 라고 적어 둔 넷째 사각이 가리킨 여섯 자리다. ⚠️ **그 여섯을 열어 보니 오늘
침묵인 것은 둘이고 나머지 넷은 결함이 아니었다** — 그리고 *넷이 결함이 아니라는 판정*이 이 트랙의
값 절반이다. ⚠️⚠️ **판정 칸은 라운드 88이 세워 라운드 89가 집었고, 사각 칸은 라운드 79~89가 여럿
세웠는데 오늘까지 한 번도 배정된 적이 없었다** — 뒤쪽이 더 오래 서 있었고, 더 오래 서 있었다는
사실을 아무도 세지 않았다.

⚠️⚠️ **두 번째 관측: 한 앱에서 열 라운드에 걸쳐 대장이 된 규율이 형제 앱에서는 마흔일곱 중 한
자리에만 서 있었고, 그 한 자리는 같은 파일 두 줄 위에 있었다**(AE-2). 모바일은 라운드 79~89가
*저장 실패 문장은 낭독되는 노드 안에 선다*를 대장·판정 칸·출구 셋으로 세웠는데, 어드민에서 사람에게
**조작의 결과**를 말하는 자리 **마흔일곱** 가운데 소리로 닿는 것은 **하나**였다. ⚠️ **그리고 그
하나의 두 줄 위(실패 배너 `:269`)와 열여섯 줄 아래(성공 배너 `:287`)가 둘 다 0건이었다** — *옳은 형식이 같은
저장소의 형제 자리에 이미 있다*(AA-2)의 **가장 좁은 판**이고, 그 형제가 **같은 파일 안**에 있었다.
⚠️⚠️ **AB-1이 이름 붙인 *한 층에서 닫은 규율은 옆 층에서 다시 세어지지 않는다*의 형제 앱 판이고,
이번에는 그 층이 앱 경계였다.**

⚠️⚠️ **세 번째 관측: 계약이 조여져도 그 계약을 설명한 문장은 조여지지 않는다 — 같은 라운드
안에서도**(AE-3). 라운드 89 리뷰 M-2가 접근성 짝 계약의 갈림 상한을 **3에서 0으로** 내리며 그
규율을 상수 주석에 박았는데, **그 상수를 인용한 문서 두 자리는 오늘까지 셋이라고 적고 있었다.**
⚠️ **두 자리 다 자기 안에서는 말이 되고 계약은 초록이었다** — 그 계약이 무는 것은 세 자리의 *수*이지
문서가 옮겨 적은 *상수*가 아니기 때문이다. ⚠️⚠️ **AD-4가 이름 붙인 *낡은 수는 자기가 낡았다고 말하지
않는다*의 즉시 재발이고, 이번에 낡은 쪽은 *문서가 인용한 계약 상수*다.** ⚠️ **오늘 F가 그 두 자리를
고쳤고, 그 갈림을 세는 계약은 오늘도 0건이다** — 그 사실이 트랙 D의 사각에 재개 조건과 함께 값으로
있고, **그 조건이 다음 라운드의 배정표다**(AD-3이 세운 그 형식 그대로).

⚠️⚠️ **네 번째 관측: 그물을 세는 자리 자신이 손 목록으로 서 있었고, 그 목록끼리의 정합을 세는 계약이
0건이었다**(AE-4). 두 대장이 각각 *"내 앞의 그물 열셋"*·*"내 앞의 그물 열넷"* 을 **손으로** 적고
`+1`로 자기 수를 내는데, 두 계약은 **자기 수만** 등호로 못 박았다. 오늘 그 둘의 정합은 참이었지만
**그것을 세는 자리가 없어 참인 것은 오늘뿐이었다** — 열여섯째 그물이 서는 날 그 트랙은 열다섯 이름을
손으로 다시 옮겨 적고, 순서가 바뀌거나 이름 하나가 빠져도 모든 계약이 초록이다.
⚠️ **AB-4가 이름 붙인 *대장에 이름이 있다는 사실이 세어졌다로 읽힌다*가 **한 겹 위에서**, 그러니까
*그물을 세는 자리 자신*에게서 다시 난 모양이다.**

⚠️⚠️ **다섯 번째 관측: 조건을 좁혀 적으면 규율이 막던 자리를 선택이 막게 되고, 그 둘은 다르게
처방된다**(AE-5). 라운드 89 AD-5는 *"a11y 그물을 여는 다음 트랙이 서는 날"* 이라는 자기모순 조건을
*"그 스윕 자체가 한 트랙의 축이 되는 라운드가 서는 날"* 로 좁혀 다시 적었다. ⚠️⚠️ **오늘 그 조건은
집힐 수 있었다 — 그런데 집히지 않았다.** 이번 라운드의 a11y 트랙(A)이 축을 **다르게 골랐을 뿐**이고,
**막은 것은 규율이 아니라 배정이다.** ⚠️ **그 상태는 겉으로 같아 보인다**(둘 다 *미도래*로 읽힌다) —
**다른 것은 처방이다: 규율이 막으면 조건을 고쳐야 하고, 선택이 막으면 배정하면 된다.**
**AD-5의 처방이 실제로 통했다는 첫 증거가 이 구별이고, 그 구별은 조건을 고쳐 본 라운드의 다음
라운드에만 보인다.**
...
- **이 스캐너가 쿼리로 분류한 자리의 낭독** — 재실측 상태 변화 0, A-20 #85 선행 → **U절 머리말**
  (⚠️ **접점 0건이다 — 두 라운드 연속**이고, 다섯 트랙 중 그 여섯 화면을 여는 것도 그 계약 파일을
  여는 것도 없다).
- **`monthly_wrapup`의 달 이동 구멍** — 게이트가 읽는 것은 여전히 대기 행의 바뀐 뒤 날짜 하나 → **U-3**
  (⚠️⚠️ **`src/notifications/**`가 이번에도 접점 0건이고, 이것으로 세 라운드 연속이다**).
- **S-3(어드민 `disabled`)** — 판정 종전 그대로, 브라우저 확인 `#130` 선행 → **U절 머리말**.
  ⚠️⚠️ **그리고 이 줄에는 오늘 구별이 하나 더 붙는다**: `app/items/page.tsx`와 `app/links/page.tsx`가
  **두 라운드 연속으로 열렸는데 열린 축이 서로 다르다** — 라운드 89는 **표 이름**이었고 오늘은
  **상태 낭독**이다. **역할 게이트·[수정] 토글·폼 필드·저장 경로·`disabled` 처리는 두 번 다 바이트
  불변**이고, S-3은 두 번 다 손대지 않았다. ⚠️ **그러므로 오늘의 정확한 문장은 *"두 라운드 연속으로
  그 두 파일이 열렸고, 열린 축이 서로 달랐으며, S-3은 두 번 다 손대지 않았다"* 이다** —
  *"열지 않았다"* 와 *"열었지만 그 축이 아니다"* 는 다른 문장이고(라운드 87이 세운 구별),
  ⚠️⚠️ **오늘 그 구별에 한 칸이 더해진다: *열린 축이 라운드마다 다르다*는 것도 따로 세어야 한다.**
- **`withdrawn_at`** — 저장소 전체 셋 · 파일 둘, 컬럼 신설은 여전히 별도 결정 → **U절 머리말**
  (⚠️ **이번 라운드도 이 수를 다시 재지 않았다** — 어느 트랙도 그 두 파일을 열지 않았고, 라운드 88이
  자기 정정으로 확정한 값을 여기서 옮겨 적지 않는다 · **두 라운드 연속 재측정 대상이 아니다**).
- **AA-R ① 연속 실패 재판정** — deps는 오늘도 `[isError]` 하나 · 자동 재시도·폴러 **0건** → **AA-R ①**
  (⚠️ 어느 트랙도 그 훅도 그 훅의 호출부 화면도 열지 않았다).

**다섯 다 2026-08-31 재실측이고 상태 변화 0이다**(`withdrawn_at`은 재실측 대상이 아니었다는 사실이
그 줄의 값이다). ⚠️⚠️ **그리고 이번 라운드의 접점 지도가 라운드 89와 다른 점은 하나뿐이다: 라운드
89가 *네 라운드 연속 0건이던 S-3의 두 파일이 열렸다*를 값으로 적었는데, 오늘 그 두 파일이 **다시**
열렸고 **축이 또 달랐다.** 접점의 유무도, 접점의 축도, 그 축이 라운드마다 같은지도 서로 다른
사실이다.**

⚠️ **N-4의 두 문턱은 오늘로 13라운드 연속 미발동이고, 준비템 탭 비가상화는 이번에도 제안하지
않는다** — ⚠️⚠️ **그 두 수는 화면이 세므로 이 절도 옮겨 적지 않고**(O-3 · 갱신 이력은 N-4에 있다),
⚠️⚠️ **이번 라운드도 그 수를 아예 재지 않았다**(활성 카탈로그 수는 실 PostgreSQL을 요구하는데 다섯
트랙 중 `apps/api/**`를 여는 것도 준비템 탭의 목록 렌더를 여는 것도 0건이었다 — 트랙 A가 그 탭에서
연 것은 **검색 결과 한 줄의 낭독**이지 목록 렌더가 아니다). **"오늘 재지 않았다"가 이 자리의 값이고,
라운드 88·89의 수를 인용하지 않는 이유도 그것이다.**

⚠️⚠️ **AD-1~AD-5가 남긴 질문 다섯 전수와 오늘의 답이다 — 다섯 다 답했고 셋이 발동했다**(AD-1·AD-3·AD-4).
⚠️ **수치는 여기 옮겨 적되 *그 수를 어떻게 냈는지*(모집단·바늘·사각)를 반드시 함께 가리킨다**
(AC-5·AD-4가 세운 그 규율 — 모집단과 바늘을 적지 않은 수는 다음 라운드가 전수로 다시 읽는다).
⚠️⚠️ **그리고 이 답들은 계약이 아니라 손으로 잰 수다** — 계약이 세는 수와 한 낱말로 적지 않는다.

- **AD-1**(**판정 칸·사각 칸이 값으로 지목한 자리** 중 아직 고쳐지지 않은 것이 몇이고, 그중 재개
  조건이 **자기를 고칠 트랙**을 이름으로 적은 것이 몇인가) — ⚠️⚠️ **발동했고 오늘의 트랙 하나가 그
  답에서 나왔다**(A). **답**: 사각 칸 **마흔다섯** · 판정 칸 **둘** · 그중 **자리를 이름으로 지목한
  것 열여섯** · **오늘 고쳐진 것 셋** · **남은 열셋** · 그중 **자기 축을 적은 것 여덟** · **전제가
  참이고 손이 저장소 안인 것 넷**. **수를 낸 방법**(모집단 = 저장소의 계약·대장 파일이 지닌 **사각 칸
  배열**과 **판정 칸** 전수 · 바늘 = `*BLIND_SPOTS` 꼴 배열의 항목과 `crossPlatform` 판정 칸 ·
  자리별 실측: `dead-export-ledger.ts` **일곱**(+`CLOSED_BLIND_SPOTS` **셋**) ·
  `comment-tolerant-anchor-ledger.ts` **일곱** · `resume-condition-ledger.ts` **여섯** ·
  `accessibility-checklist-shape.test.ts` **여섯** · `contract-net-ledger.test.ts` **셋** ·
  `a11y-contract.test.ts` **열여섯**(판정 사각 여섯 + 모듈 스윕 다섯 + 반쪽 프롭 스윕 다섯) ·
  ⚠️ **사각 둘 = 배열 꼴이 아닌 산문 사각(`dnc-*.ts` 셋)을 못 보고, `floor`+`measure` 쌍 없이 문장만
  든 사각은 오늘도 참인지 기계가 세지 않는다 → **마흔다섯은 하한이다**):
  `docs/5차/round90-scout.md`의 **선행 확인 4**와 이 절의 트립와이어 표.
  ⚠️⚠️ **정찰과 갈린 자리를 값으로 적는다** — 정찰은 사각 칸을 **서른다섯**으로 셌고 오늘은
  **마흔다섯**이다. **부채가 열 늘어난 것이 아니라 세는 자리가 열 늘었다**: 트랙 A가 반쪽 프롭
  스윕에 다섯, 트랙 D가 접근성 짝 계약에 둘(`좌표-내용`·`문서가-옮겨-적은-계약-상수`), 트랙 E가 새
  짝 계약에 셋을 적었다. ⚠️ **이름으로 지목한 열여섯의 내역도 함께 적는다** — 정찰이 센 **열하나**에
  트랙 D의 둘(`좌표-내용`·`문서가-옮겨-적은-계약-상수`)과 트랙 E의 셋(`실재-미확인`·`손-목록`·
  `apps-뿌리-0건`)이 더해진 수다. ⚠️ **그리고 고쳐진 셋 가운데 하나가 오늘 이 절을 쓰는 손의 것이다** —
  문자열 리터럴 축(트랙 C) · live region 단독(트랙 A) · **문서가 옮겨 적은 계약 상수(F)**.
  ⚠️ **재개 조건(사건형): 사각 칸이 이름으로 지목한 자리가 하나 더 배정되는 날** — 그날 물을 것은
  *몇이 남았는가*가 아니라 **그 자리가 몇 라운드를 서 있었는가**다(오늘 고친 셋은 각각 두·세·한
  라운드를 서 있었다).
- **AD-2**(**접근성 이름이 필요한 구조**(표 말고도 `role="group"`·랜드마크·목록·다이얼로그) 중
  **이름이 0건인 것**이 몇이고, 그중 **바로 위 제목에서 파생 가능한 것**이 몇인가) — ⚠️ **답했고
  오늘의 트랙이 되지 않았다.** **답**: 어드민 `<table>` **17 → 이름 17**(라운드 89 B가 닫았고 오늘
  그대로) · `<section>` **34 → 이름 0건**(그중 **29**가 바로 안에 제목을 지녀 파생 가능) ·
  `<main>` **다섯 중 중첩 셋** · `aria-current` **0건** · `<nav>` 1·`<form>` 1(이름 0건) ·
  `role="img"` 2·`role="group"` 1은 **이미 이름 있음** · 다이얼로그·목록 **0건**. 모바일은 이름 축의
  결함이 오늘도 **0건**이다(아이콘만 있는 버튼 0 · 라벨 없는 `TextInput` 0). **살아 있는 결함은
  어드민의 중첩 `<main>` 셋뿐**이고, **`<section>` 34에 이름을 주지 않는 것이 판정이다**(이름 없는
  `<section>`은 랜드마크가 아니라 제네릭이라 오늘 결함이 아니고, 34에 이름을 주면 랜드마크 서른넷이
  생겨 소리로 훑는 운영자에게 소음이 된다). **수를 낸 방법**(모집단 = 어드민·모바일의 비테스트 `.tsx`
  전수 · 바늘 = 여는 태그의 속성 구간을 괄호·따옴표 균형으로 잘라 이름 속성의 유무를 세고 텍스트
  자식이 이름을 대신하는지 본문까지 잘라 확인 · 사각 = **런타임에 합성되는 이름**을 소스로만
  추정하므로 오차의 방향이 **결함을 놓치는 쪽**이다): `docs/5차/round90-scout.md`의 **선행 확인 5**.
  ⚠️⚠️ **오늘 이 답에 값이 하나 붙었다 — 그 둘을 이제 기계가 센다.** 트랙 B의 앱 내 스윕이
  `<main>` **다섯**과 `aria-current` **0건**을 **부정 단언으로** 못 박아, 그 판정이 낡으면
  (중첩 main이 늘거나 `aria-current`가 붙으면) **그 계약이 먼저 빨개진다.** ⚠️ **판정은 그대로이고
  바뀐 것은 그 판정이 조용히 낡을 수 있는가다.**
  ⚠️ **재개 조건(사건형): 어드민의 *랜드마크와 현재 위치 표기*가 한 트랙의 축이 되는 라운드가 서는 날**
  — 곁다리로 적으면 이 조건은 매 라운드 진다(AD-5의 처방을 그대로 지킨 표기다).
- **AD-3**(**도래한 채로 넘어간 조건**이 몇이고, 그중 **그날 무엇을 먼저 해야 하는지가 적혀 있는 것**이
  몇인가) — ⚠️⚠️ **발동했고 오늘의 트랙 하나가 그 답에서 나왔다**(C). **답**: **하나 중 하나였고,
  오늘 그 하나가 닫혀 남은 것은 0이다.** 라운드 89가 도래시킨 채 넘긴 `string-literal-references`는
  조건 문장 안에 *"문자열 마스킹과 템플릿 `${…}` 갈래를 **한 번에**"* 를 적고 있었고, 트랙 C가 그
  순서대로 집어 `CLOSED_BLIND_SPOTS`의 **셋째**가 됐다 — **실측: 사문 전수 40 → 44 · 대장의 줄
  22 → 22(+0) · 면제 18 → 22 · 모듈 6 그대로.** ⚠️⚠️ **라운드 89 C가 넘기며 적어 둔 예상
  (*"대장의 줄은 0이 는다"*)이 실측과 갈리지 않았다** — **순서를 적은 조건은 집히고, 값까지 적은
  조건은 집었을 때 계약이 비어 있지 않다**(AD-3의 두 번째 배당). **답 2 — 오늘 새로 도래한 채 넘어간
  조건은 0건이다.** ⚠️ **문턱에 가장 가까운 미도래 하나**: `derived-exemptions`의 *"면제 수가 사문
  전수의 절반을 넘는 날"* — 오늘 **44 중 22로 정확히 절반**이고 조건은 *"넘는 날"* 이라 **아직
  아니다**(정찰은 *"둘 남았다"* 고 적었고 오늘은 **하나**다 — 다음 라운드가 `export const` 하나를 더
  계약 전용으로 만들면 그날 이 판정은 좁혀져야 한다). **수를 낸 방법**(모집단 = 위 AD-1의 사각 칸
  마흔다섯 + 아래 기각 열여섯의 재개 조건 · 바늘 = 조건 문장이 말하는 사건을 **대장을 실행해서** 오늘
  값으로 재고 참이면 *도래*로 센다 · 사각 = **대장 안에 사는 조건**만 세므로 판정 문서의 산문 조건
  145는 형이 없어 기계가 도래 여부를 가를 수 없다 → **하나는 하한이었다**).
  ⚠️⚠️ **그리고 닫으면서 반대 방향의 사각 하나가 열렸다** — 문자열을 지우는 그물은 **문자열 열쇠로만
  닿는 참조**를 보지 못하고(오늘 **55**), 그 오차는 **거짓 빨강**이다. 닫힌 사각이 다음 사각을 낳았고,
  그 사실이 `CLOSED_BLIND_SPOTS` 셋째 줄에 값으로 있다.
  ⚠️ **재개 조건(사건형): 도래한 채 넘어가는 조건이 다시 하나 생기는 날** — 오늘의 답이 0이므로,
  다음 답은 *몇인가*가 아니라 **0이 몇 라운드 이어지는가**다.
- **AD-4**(판정 문서·확인 문서에서 **같은 사실을 두 자리 이상에서 말하는 곳**이 몇이고, 그중
  **계약이 그 정합을 세는 것**이 몇인가) — ⚠️⚠️ **발동했고 오늘의 트랙 둘이 그 답에서 나왔다**(D·E).
  **답**: **아홉 중 여섯**이다(라운드 89의 셋에서 셋이 늘었다). **수를 낸 방법**(모집단 = 판정 문서
  하나와 확인 문서 둘, 그리고 그 문서들이 **소스의 값을 옮겨 적은 자리** · 바늘 = 같은 사실을 말하는
  자리를 손으로 짝지어 세고 그 짝을 무는 단언이 있는지 소스에서 찾는다 · 사각 = 모집단을 **문서 셋**
  으로 그었으므로 소스가 문서를 인용하는 자리와 `docs/dev/**`·`docs/5차/**`는 이 수에 들지 않는다 →
  **아홉은 하한이다**):

  | # | 같은 사실 | 말하는 자리 | 계약이 세는가(2026-08-31) |
  | --- | --- | --- | --- |
  | 1 | C-3 경과 수 | 접근성 C절 행 · 수동 증거 절 · 짝 문서 §0-1 (**셋**) | ✔ 종전 그대로(오늘 세 자리를 함께 올렸다) |
  | 2 | 짝 문서 §0의 여섯 수 | 머리말 · §0 표 · 셸 블록 주석 | ✔ 종전 그대로 |
  | 3 | N-4 문턱 200 | known-limitations · 어드민 카드 | ✔ 종전 그대로 |
  | 4 | **접근성 표 → 짝 문서 좌표** | 자리 **91** · 인용 번호 **95** · 서로 다른 번호 **91** | ✔ ⚠️⚠️ **오늘 섰다**(트랙 D — 하나에서 전수로) |
  | 5 | **좌표에 딸린 표면 값** | 표면까지 함께 적은 참조 **32** | ✔ ⚠️ **오늘 섰다**(같은 계약 · 어긋남 0) |
  | 6 | **짝 계약의 갈림 상한** | 계약 상수 `MAX_DIVERGENCE = 0` ↔ 접근성 표 **두 자리** | ✖ **0건 · 오늘 갈려 있었고 F가 문서 쪽을 고쳤다** |
  | 7 | **계약 그물의 목록·수** | 두 대장의 손 목록(**열셋**·**열넷**)과 각자의 수 | ✔ ⚠️⚠️ **오늘 섰다**(트랙 E — 접두·자기 수·하한) |
  | 8 | 사문 대장 항목·래칫·사각 일곱 | 소스 ↔ 이 절의 트립와이어 표 | ✖ 0건(트립와이어는 라운드마다 다시 잰다고 스스로 적는다) |
  | 9 | 앵커 대장 다섯 수 | 소스 ↔ 트립와이어 표 | ✖ 0건(같은 이유) |

  ⚠️⚠️ **6이 오늘의 결함이었고, 그것을 고친 손이 이 절을 쓰는 손이다** — 그래서 그 축을 무는 계약은
  오늘 세우지 않았다(오늘 빨간 계약을 세우면 *계약이 문서를 지킨다*가 *문서가 계약을 맞춘다*로
  뒤집힌다 · 라운드 88이 이 후보를 집지 않은 이유가 정확히 그 순환이었다).
  ⚠️ **재개 조건(사건형): 문서가 인용한 계약 상수의 정합이 한 트랙의 축이 되는 라운드가 서는 날** —
  **그날 F가 오늘 고친 두 자리가 그 계약의 첫 모집단이다**(트랙 D의 사각이 이 조건을 자기 축과 함께
  적어 두었고, F가 절을 먼저 쓰고 다음 라운드가 계약을 세우는 그 순서다).
- **AD-5**(재개 조건 가운데 **자기가 누구의 축이 되어야 하는지를 적은 것**이 몇이고, ***다른 트랙이
  서는 날* 꼴로 적혀 규율에 막히는 것**이 몇인가) — ⚠️ **답은 0이고, 0도 재어 본 값이다.**
  **답**: 판정 문서의 재개 조건 자리 **252** 중 자기 축을 적은 살아 있는 조건 **열다섯**(정찰의
  열셋에 트랙 D·E가 새로 적은 둘이 더해졌다) · ⚠️⚠️ **규율에 막히는 것 0건**(라운드 89 AD-5가 그
  하나를 좁혀 고쳤고, C-12는 소진됐다). **수를 낸 방법**(모집단 = `known-limitations.md` +
  `apps/**`·`packages/**`의 `.ts(x)` + `docs/qa/**`의 `.md` · 바늘 = *"재개 조건|재개 트리거"* 지점부터
  다음 줄까지를 이어 붙여 **조건 문장 하나**를 자르고 그 안에서 ⓐ *축이 되/축으로 서/자기 축* 를,
  ⓑ *…트랙이 서는 날 · …라운드가 오는 날* 꼴을 찾는다 · 사각 = 조건 문장을 **줄+다음 줄**로 자르므로
  **세 줄 넘게 접힌 조건**은 못 본다(재개 조건 대장의 `folded-notation`이 오늘 **둘**로 그 크기를
  값으로 든다) → **열다섯은 하한이다**).
  ⚠️⚠️ **그리고 그 0이 오늘 두 번째로 새 관찰을 낳는다** — 낭독 라벨 모집단은 **오늘도 집힐 수
  있었는데 집히지 않았고**, 막은 것은 **두 라운드 연속으로 규율이 아니라 선택**이다. **한 번은 그
  라운드의 배정이지만 두 번은 *조건이 옳게 적혀 있어도 배정이 없으면 오지 않는다*의 증거다** — AD-1이
  판정 칸에 대해 말한 문장이 **조건 문장에서 그대로 다시 참이 됐다**(→ AE-5).
  ⚠️ **재개 조건(사건형 · 자기 축을 적음 — 종전 그대로): 그 스윕 자체가 한 트랙의 축이 되는 라운드가
  서는 날.**

⚠️⚠️ **AA-3의 이행 — 결정형 재개 조건 전수와 오늘의 처분이다. 라운드 89의 열하나가 정찰 시점에
열둘이 됐고, 오늘 HEAD에서 다시 세니 열셋이다.**
⚠️⚠️ **열둘 → 열셋도 새 부채가 아니라 *세는 자리가 늘었다*는 뜻이다**: 라운드 89가 집어 든 둘이 그날
닫혀 아홉이 됐고, 라운드 89의 기각이 새로 결정형이라고 이름 붙인 셋이 정찰의 표에 들어와 열둘이
됐으며, **오늘 트랙 E가 자기 사각에 결정형 하나를 새로 적어** 열셋이 됐다. **기각과 사각이 조건을
값으로 적으면 그 조건은 다음 라운드의 표에 선다** — V-2가 세운 규율이 모집단으로 되돌아오는 자리다.

| # | 결정형 조건 | 사는 곳 | 손 | 오늘의 처분(2026-08-31 실측) |
| --- | --- | --- | --- | --- |
| 1 | 막대를 포커스 가능하게 | AA-2 | 안 | **집지 않는다 — 값 0.** 두 화면 다 `trend.showTable` 갈래의 표가 값을 텍스트로 주고, 그 표들은 라운드 89 B에서 이름까지 얻었다(재실측 · 변화 0) |
...
| 5 | 감사 뷰의 대상 축 필터 | AA절 기각 | **밖**(서버 DTO 0건) | 집지 않는다 — ⚠️ **어느 트랙도 `apps/api/**`를 쓰기로 열지 않았다**(다섯 라운드 연속) |
| 6 | 타일 안 배지 | AA-1 | **밖**(디자인 승인) | 집지 않는다. 변화 0 |
| 7 | C-3 잠금 오버레이 TalkBack 투과 | 접근성 표 C-3 | **밖**(사람·기기·날짜) | **트랙 F의 소유 밖이다** — 오늘로 **스물네 라운드째 미확인**. ⚠️ **오늘 이 줄에 붙은 값은 확인이 아니라 *그 줄이 인용한 계약 상수가 낡아 있었다*는 사실이고, F가 그 인용 둘을 고쳤다**(경과 수 세 자리는 함께 올렸다) |
| 8 | JSX 사용을 참조로 세는 판정 | `dead-export-ledger.ts`(소스) | 안 | **집지 않는다 — 그물 배정**(트랙 C가 그 파일을 열었고 *한 트랙이 한 그물에 축 둘을 얹지 않는다* · **네 라운드 연속 같은 이유**). 전제는 참(`.tsx`의 `export function` **141** — 문자열 축을 켠 뒤에도 그대로) |
| 9 | 재조회 실패 창에서 옛 목록을 남길지 | AC절 기각 | 안 | **집지 않는다 — 화면 구조 결정이고 오늘 값 0건**(어느 트랙도 `src/offline/**`의 화면 구조를 열지 않았다) |
| 10 | 앵커 대장의 `helper-named-reader` | `comment-tolerant-anchor-ledger.ts`(소스) | 안 | **집지 않는다 — 래칫이다.** 모집단이 164만큼 넓어지면 `comment-tolerant` 래칫 **70**이 첫날부터 전수로 다시 적혀야 한다. ⚠️⚠️ **오늘 그 대장은 다섯 자리 전부 불변이고, 이것으로 네 번 재어 한 자리도 다르지 않다**(라운드 88·89·정찰·오늘) |
| 11 | 재개 조건 대장의 `prose-only` | `resume-condition-ledger.ts`(소스) | 안(**F**) | **집지 않는다.** 조건이 *"관례를 소급해 적용할지를 F가 정하는 날"* 인데, 소급 적용은 **판정 문서의 산문 조건 145 자리를 F가 손으로 고치는 일**이라 한 라운드의 F가 질 수 없다. ⚠️ **그리고 이 수는 줄지 않고 는다**(당시 119 → 정찰 145 → 이 절을 쓰면 더) |
| 12 | 재개 조건 대장의 `round-notes` | `resume-condition-ledger.ts`(소스) | 안 | **집지 않는다 — 조건이 *"라운드 노트가 판정 문서로 승격되는 관례가 서는 날"* 이고 오늘 그런 관례는 0건이다.** ⚠️ 그 수는 정찰의 **222**에서 오늘 **265**가 됐는데, 늘어난 것은 부채가 아니라 **round90-scout.md 한 파일**이다 |
| 13 | **그물 파일이 자기를 이름으로 선언하는 표식** | `contract-net-ledger.test.ts`(소스 · **오늘 신설**) | 안 | **집지 않는다 — 오늘 선 조건이다.** 트랙 E가 자기 사각에 *"손 목록이 소스에서 파생하지 않는다"* 를 값으로 적으며 **표식을 세울지 정하는 날**을 조건으로 걸었다(오늘 표식 **0건** · 손으로 적힌 이름 **스물일곱**) |

⚠️⚠️ **이 표가 라운드 89의 표와 다른 점 하나: 오늘도 집는 것이 0건이다.** 열셋 중 손이 안인 여덟
(1·2·8·9·10·11·12·13) 가운데 **전제가 참인 것은 8·10·11·12 넷**이고, **넷 다 집지 않는 이유가
*조건이 미도래*가 아니라 *배정·래칫·손의 크기*다.** ⚠️ **그래서 이번 라운드의 트랙 다섯 가운데
결정형 표에서 나온 것은 0건이고, 셋은 *사각 칸이 지목한 자리*에서(A·C·D), 둘은 *질문의 답*에서
나왔다(B·E)** — **AD-3이 세운 *순서를 적은 조건은 집힌다*의 반대편 값이 두 라운드 연속으로 참이다:
순서가 적혀 있어도 그 조건을 집을 트랙이 다른 축을 이미 지고 있으면 그 라운드에는 집히지 않는다.**

⚠️⚠️ **이번 라운드가 실측하고 기각한 열여섯을 값으로 남긴다 — 전부 재개 조건과 함께**(V-2가 세운
규율: 조건 없는 보류는 이유가 적혀 있다는 이유로 재론되지 않는다). ⚠️ **그중 다섯은 재개 조건이
*결정형*이라는 사실과 그 결정의 손이 어디에 있는지를 함께 적고**, 열다섯째 줄(손이 밖인 다섯)은
다섯 조건을 한 줄에 묶어 **전부 결정형 · 손은 저장소 밖**이라고 적는다(Z-4·AA-3·AB-3·AC-3의 이행).
⚠️ **아래 마지막 두 줄은 그 열여섯에 들지 않는다** — 라운드 62~89가 남긴 **상태 변화 0의 표식**과
**제외 목록 준수 확인**이고, 재론 대상이 아니라 스윕이 그것들을 다시 줍지 않게 하는 자리다.

- **어드민의 랜드마크와 현재 위치 — 재었고 실재하는데 집지 않는다.** `AdminShell.tsx`의 `<main>`
  안에 `app/page.tsx`·`app/error.tsx`·`app/not-found.tsx`가 `<main>`을 한 번 더 세우고(전체 다섯 중
  중첩 **셋**), 활성 nav 링크는 CSS 클래스로만 표시된다(`aria-current` 저장소 전수 **0건**).
  ⚠️ **집지 않는 이유는 파일 배타다** — 그 파일들을 트랙 B가 **상태 낭독 축으로** 열었고,
  *한 트랙이 한 그물에 축 둘을 얹지 않는다.* ⚠️⚠️ **다만 오늘 그 두 수는 기계가 세기 시작했다**
  (트랙 B의 앱 내 스윕이 `<main>` 다섯과 `aria-current` 0을 부정 단언으로 못 박는다 — **판정은
  그대로이고 바뀐 것은 그것이 조용히 낡을 수 있는가다**).
  ⚠️ **재개 조건(사건형 · 자기 축을 함께 적는다): 어드민의 *랜드마크와 현재 위치 표기*가 한 트랙의
  축이 되는 라운드가 서는 날** — 곁다리로 적으면 이 조건은 매 라운드 진다.
- **어드민 `<section>` 34에 이름을 주지 않는다 — 재었고 그것이 판정이다.** 34 중 이름 0건이고
  **29가 바로 안에 제목을 지녀 파생 가능**하지만, 이름 없는 `<section>`은 랜드마크가 아니라 제네릭
  이라 오늘 결함이 아니고 34에 이름을 주면 **랜드마크 서른넷**이 생겨 소리로 훑는 운영자에게 소음이
  된다. ⚠️ **라운드 89 B의 표 열일곱과 성질이 다르다**(표는 이름이 없으면 *"표"* 로 들리고, section은
  이름이 없으면 들리지 않는다).
  ⚠️ **재개 조건(결정형 · 손은 저장소 안): 한 화면의 카드가 너무 많아 랜드마크로 건너뛰는 길이
  필요해지는 날** — 그날 정할 것은 *이름*이 아니라 **어느 카드가 랜드마크인가**다.
- **행마다 갈리는 낭독 라벨의 모집단 — ⚠️⚠️ 두 라운드 연속으로 규율이 아니라 선택이 막는다.**
  라운드 89 AD-5가 조건을 *"a11y 그물을 여는 트랙의 축이 바로 이 모집단인 라운드가 서는 날"* 로
  좁혀 다시 적었고, **그 좁힘 덕에 이 조건은 오늘도 집힐 수 있었다** — 이번 라운드의 a11y 트랙(A)이
  축을 **반쪽 프롭**으로 골랐을 뿐이다. ⚠️ **재실측**: 모바일 비테스트 `.tsx`에서 낭독 라벨을 식으로
  만드는 자리 **162** · 목록 안의 낭독 라벨 **39** 중 **행마다 갈리지 않는 상수 문자열 0건** ·
  규율을 지키는 것은 여전히 **손 핀 스물두 개 남짓**이다(정찰과 변화 0).
  ⚠️ **재개 조건(사건형 · 자기 축을 적음 — 그대로): 그 스윕 자체가 한 트랙의 축이 되는 라운드가
  서는 날.** ⚠️⚠️ **그리고 두 라운드 연속 *선택*이 막았다는 사실이 이 줄의 새 값이다**(→ AE-5).
- **`dead-export-ledger.ts`의 결정형 8(JSX 사용을 참조로 세는 판정) — 전제는 참인데 집지 않는다.**
  `.tsx`의 `export function` **141**이 오늘도 모집단 밖이고, 문자열 축을 켠 뒤에도 그 수는 움직이지
  않았다. ⚠️ **집지 않는 이유는 그물 배정이다**(트랙 C가 그 파일을 열었다 · **네 라운드 연속 같은
  이유**). ⚠️ **재개 조건(결정형 · 손은 저장소 안): 그 판정이 서는 날 — 그날 141이 모집단으로
  들어온다.**
- **앵커 대장의 사각 `helper-named-reader` 164 — 재었고 다섯 자리 전부 불변이며 집지 않는다.**
  ...
  unanchored **0**) · 면제 **8** — ⚠️⚠️ **라운드 88 → 89 → 정찰 → 오늘, 네 번 재어 한 자리도 다르지
  않다.** ⚠️ **집지 않는 이유는 래칫이다**: 모집단이 넓어지면 `comment-tolerant` 래칫 70이 첫날부터
  전수로 다시 적혀야 한다. ⚠️ **재개 조건(결정형 · 손은 저장소 안): 저장소가 소스 리더 이름을 한
  벌로 모으는 날, 또는 이 대장이 헬퍼 정의를 따라가 루트를 푸는 법을 배우는 날.**
- **재개 조건 대장의 `prose-only` 145 · `round-notes` 265 — 재었고 둘 다 집지 않는다.**
  형 표기가 없는 산문 조건은 **당시 119 → 오늘 145**로 늘었고(가장 큰 사각은 줄지 않는다), 라운드
  노트는 정찰의 **222**에서 오늘 **265**가 됐는데 늘어난 것은 부채가 아니라 **round90-scout.md 한
  파일**이다(일부러 모집단 밖이다). ⚠️ **재개 조건은 둘 다 결정형 · 손은 저장소 안이고 종전 그대로다**
  (소급 적용을 F가 정하는 날 · 라운드 노트가 판정 문서로 승격되는 관례가 서는 날).
- **재개 조건 대장의 `sibling-documents` 아홉 — 재었고 미도래.** 짝 문서 둘의 재개 조건 자리는 오늘도
  **아홉**이고, **형을 괄호로 밝힌 것은 하나(C-12의 사건형) · 결정형 0건**이다(⚠️ **F가 오늘 두 짝
  문서에 쓴 문장에도 결정형 표기를 세우지 않았다 — 세우는 순간 그 대장의 모집단 판단이 바뀐다**).
  ⚠️ **재개 조건(사건형): 짝 문서 중 하나에 결정형 표기가 처음 서는 날.**
- **소스에 사는 재개 조건 표기의 둘째 파일 — ⚠️⚠️ 재었고 오늘 도래했다.** 괄호로 형을 밝힌 표기를
  지닌 소스는 정찰 시점까지 `dead-export-ledger.ts` **하나**뿐이었는데, **오늘 트랙 E의
  `contract-net-ledger.test.ts`가 둘째 파일이 됐다**(*"재개 조건(결정형 · 손은 저장소 안)"* 을 자기
  사각에 적고, **자기 소스를 읽어 그 문장이 실재하는지까지 확인한다**). ⚠️ **그런데 그 축을 지는
  대장(`resume-condition-ledger.ts`)의 `SOURCE_AXIS_FILES`는 오늘도 한 파일만 든다** — **조건은
  도래했고 처분은 다음 라운드 몫이다**(그 대장은 이번 라운드에 어느 트랙의 것도 아니었다).
  ⚠️ **재개 조건(사건형): 이미 도래했다 — 남은 것은 배정이고, 그날의 일은 소스 축의 뿌리 목록을
  손 하나에서 전수로 바꾸는 것이다.**
- **`derived-exemptions`의 절반 문턱 — 재었고 하나 남았다.** 사문 전수 **44 중 면제 22**로 **정확히
  절반**이고, 조건은 *"면제 수가 사문 전수의 절반을 **넘는** 날"* 이라 **오늘은 아직 아니다.**
  ⚠️⚠️ **정찰은 *둘 남았다*(40 중 18)고 적었고 트랙 C가 축을 켠 뒤 실측은 *하나 남았다*(44 중 22)로
  갈렸다 — 라운드 89 C가 적어 둔 *"대장의 줄은 0이 는다"* 는 참이었지만 *면제도 0이 는다*는 아니었다.**
  ⚠️ **재개 조건(사건형): 면제가 사문 전수의 절반을 넘는 날 — 그날 이 판정은 다시 좁혀져야 한다.**
- **AC-1의 일곱(형제가 실재하는 계약) — 오늘 다시 돌리지 않았다.** 어느 트랙도 형제 화면이 갈리는
  사건을 만들지 않았고, 한 화면만 무는 계약을 두 화면 루프로 바꾸는 것은 **오늘 고칠 어긋남이
  0건**이라 *판정을 맞추는 일*이 아니라 *계약의 모양을 바꾸는 일*이 된다.
  ⚠️ **재개 조건(사건형): 그 일곱 중 한 자리에서 형제 화면이 실제로 갈리는 날.**
- **AC-2의 0(호출부 둘 이상의 필드 갈림) — 오늘 다시 돌리지 않았다.** 어느 트랙도
  `client.ts`·`admin-api.ts`의 시그니처를 늘리지 않았다. ⚠️ **재개 조건(사건형): 본문에 새 필드가
  붙는 날, 또는 그 스윕을 다시 돌리는 날** — ⚠️ **다시 돌릴 때는 변수로 만든 본문을 따라가는 바늘을
  함께 지고 돌려야 한다.**
- **조회 실패 창의 모집단 · 응답 필드 축(모바일) · `HouseholdMember.joinedAt` · 기록 탭 검색의 분류
  갈래 · 막대 포커스 가능화 — 다섯 다 라운드 89의 판정 그대로이고 상태 변화 0.** 어느 트랙도
  `src/offline/**`의 화면 구조를 열지 않았고, 응답 타입 선언을 늘리지 않았으며, 구성원 행은 이름으로
  이미 갈리고, 기록 탭 검색은 **다섯 라운드 연속 어긋남 0건**이며, 두 차트의 표는 값을 텍스트로 준다.
  ⚠️ **재개 조건 다섯 다 종전 그대로다**(둘은 결정형 · 손은 저장소 안 — 재조회 실패 창의 결정과
  placeholder가 분류를 약속하는 날 · 나머지는 사건형).
- **이월 다섯(U절 셋 · S-3 · `withdrawn_at`) — 판정 종전 그대로이고, 접점만 오늘 갱신한다**(위 이월
  목록). ⚠️⚠️ **S-3의 줄에는 오늘의 구별이 함께 선다**: *"두 라운드 연속으로 그 두 파일이 열렸고,
  열린 축이 서로 달랐으며(표 이름 → 상태 낭독) S-3은 두 번 다 손대지 않았다."* ⚠️ **재개 조건은
  다섯 다 종전 그대로다.**
- **성능 넷 — ⓐ·ⓒ·ⓓ는 재었고 미도래, ⓑ는 오늘 재지 않았다.** ⓐ **첫 페인트**(⚠️ 어느 트랙도
  `useQuery` 선언을 늘리지 않았다 — 트랙 A가 더한 것은 `useEffect` **두 벌**과 ref 하나다) ·
  ⓒ **번들**(⚠️ **새 런타임 의존성 0건 — 다섯 트랙 다 기존 import만 쓴다**) · ⓓ **api의 루프**
  (⚠️ **`apps/api/**` 쓰기 0건 · 다섯 라운드 연속**). ⓑ **렌더 비용**은 활성 카탈로그 수가 실
  PostgreSQL을 요구해 **오늘 재지 않았다**(라운드 88·89의 수를 인용하지 않는다 — 옮겨 적은 수는 계약
  밖의 사본이 된다 · O-3). ⚠️ **재개 조건 넷 다 종전 그대로다.**
- **손이 밖인 다섯 — 넷은 다른 사람이 내릴 결정이고 하나는 배정이다**(위 표의 3~7). 준비템 분류 필수
  입력 · 서버의 중복 대기 초대 방지 · 감사 뷰의 대상 축 필터 · 타일 안 배지는 **카탈로그 정책 · 초대
  정책 · 서버 계약 · 디자인 승인**이 각각 선행이고, C-3은 **사람·기기·날짜 배정**이다(오늘로
  **스물네 라운드 연속 미확인**). ⚠️ **재개 조건(전부 결정형 · 손은 저장소 밖): 그 결정이 서는 날** —
  ⚠️ **넷과 하나를 한 줄에 적지 않는 이유가 그 칸이다**(넷은 결정을 기다리고, 하나는 시간을 기다린다).
- **`${seller}에서 구매하기` 둘 — 제외 확인이지 제안이 아니다.** 준비템 판매처 1:1과 구매 확인 판매처
  라벨은 **영구 기각** 축이다(라운드 62~89가 남긴 그대로). ⚠️ **재개 조건: 없다 — 이 줄은 재론 대상이
  아니라 스윕이 그 둘을 다시 줍지 않게 하는 표식이다.**
- **감사 뷰 CSV · 콘텐츠 리비전 편집 표면 · 어드민 손 미러 · `SYNC_STATUS_RETRY_ALL_LABEL` · 지출 입력
  두 화면의 조립 · CSV 왕복 다섯 열 손실 · `refund` 생성 불가 · `link_health`의 `errors` 카운터 ·
  서버 알림 층 · 홈의 손 폴 다섯 · 공유 카드 왕복 · 어드민 카탈로그 전량 조회 · 미출처 틴트 둘 ·
  첫돌 이후 마일스톤 고착 · api 하네스 동시 실행 구멍 · 서버 중복 아이 가드 부재 · 크래시 파이프라인
  부재 · `Share.share`의 catch 없는 `void` 둘 · 결제 수단 기본값 — 라운드 62~89가 남긴 그대로이고
  상태 변화 0.**
- **제외 목록 준수 확인**: 준비템 목록 **가격 표시**(잠금 · ⚠️ **다섯 트랙 어디에도 가격·링크 수가
  들어가지 않았다**) · 오프라인 로컬 아이 복구 · 외부 계정/키/자산 · **C-3 잠금 오버레이 낭독** ·
  **P-2 법무 대조** · P-3 테스트 건수 자동화 · **표기 방언 통일** · S-4 파기 `targetId` · 40주 초과
  달력 · `onBudgetRelevantChange` · 4가구/`viewedHouseholdId` · **지출→리포트 정확성 축** ·
  **공유 왕복 축** · **준비템 완료율 동기부여**(⚠️ **트랙 A가 연 것은 검색 결과 한 줄의 낭독이고
  진행률 카드·완료율 문구는 바이트 불변이다**) · **준비템 탭 비가상화** · **홈 수치 정합** —
  **전부 오늘도 제안하지 않았다.**

⚠️⚠️ **트립와이어 대조 — 라운드 89가 남긴 래칫·하한을 오늘 HEAD에서 다시 잰 값이다.**
⚠️ **정찰의 수를 옮겨 적지 않는다** — 정찰의 실측은 **트랙 다섯이 머지되기 전**의 값이고, 아래는
**A~F가 전부 머지된 뒤** 대장을 실행해 다시 낸 값이다(라운드 89 F가 정찰의 수를 그대로 옮겼다면 그
절이 전부 틀렸을 것이라는 기록이 이 표의 관례다).
⚠️ **라운드 90 리뷰 M-2가 그 시점을 *한 시점으로* 맞췄다** — 종전에는 *"A~E 머지 뒤"* 라고 적어
두고 표 안에 F 이후 값(짝 문서 165 · 접근성 표 A-31 · `MAX_DIVERGENCE` 문서 쪽 0)과 F 이전
값(좌표 전수 · 재개 조건 대장)이 섞여 있었다. 오늘 남은 예외는 **아래 *재개 조건 대장의 사각
여섯* 한 줄뿐**이고, 그 줄은 자기가 F 이전 값이라는 사실을 스스로 적는다.
⚠️⚠️ **오늘 다시 재니 스무 자리 중 열이
움직였고, 여덟은 그대로이며, 둘은 오늘 처음 세어졌다**(움직였다 **10** · 그대로 **8** · 오늘 처음
**2** = **20** — 아래 표의 행을 다시 센 값이다). ⚠️ **그대로인 여덟 가운데 하나는 이 저장소에서
처음 있는 일이다 — 계약 그물 수가 늘지 않은 첫 라운드다.**
⚠️⚠️ **그리고 이 문단 자체가 AE-3이 이름 붙인 그 모양의 재발이었다**(라운드 90 리뷰 H-2): 표는
스무 행인데 이 산문은 *"열여섯 자리 중 아홉"* 이라고 적었고, 커밋 메시지는 또 *"열아홉"* 이라고
적어 **한 절 안에서 자리 수가 셋으로 갈렸다** — *계약(여기서는 표)이 자라도 그것을 설명한 문장은
함께 자라지 않는다*가, 그 판정을 세운 바로 그 절에서 다시 참이 된 것이다. **오늘 고친 것은 이
문단뿐이고, AE-3 본문의 판정은 한 글자도 다시 쓰지 않았다.**
⚠️ **그리고 이 표는 리뷰 자신의 걸음을 담지 않는다**(시점이 하나여야 하므로): 같은 리뷰가 사문
대장에 사각 하나를 새로 열었으므로(`jsx-apostrophe-string-masking` — 스캐너가 JSX 텍스트의
어포스트로피 짝을 문자열로 읽어 코드를 지울 수 있다 · 표면 105 · 실피해 0건) 아래 *사문 대장의
사각 일곱* 행은 **다음 라운드의 트립와이어에서 여덟이 된다.** 그 사실을 여기 적어 두는 것이,
같은 절에서 두 번 재발한 그 병을 세 번째로 반복하지 않는 방법이다.

| 자리 | 라운드 89가 남긴 값 | 2026-08-31 HEAD 재실측(**A~F 머지 뒤** · 리뷰 커밋 전) | 움직였는가 |
| --- | --- | --- | --- |
| 사문 대장 항목 / 래칫 | 22 / 22 | **22 / 22** | **그대로다** — ⚠️ 축이 하나 넓어졌는데도 줄이 늘지 않았다(라운드 89 C의 예상이 적중했다) |
| 사문 대장의 사각 일곱 | 18 · 0 · 226 · 20 · 55 · 141 · 0 | **22 · 0 · 226 · 20 · 55 · 141 · 0** | ⚠️ **움직였다 — 그리고 다섯째의 *이름*이 바뀌었다**(`string-literal-references`가 닫히고 그 자리에 반대 방향의 `string-keyed-dynamic-access`가 섰다 · 값은 우연히 같은 55다) |
| 사문 전수 / 면제 / 모듈 | 40 / 18 / 6 | **44 / 22 / 6** | ⚠️ **움직였다** — 문자열 마스킹이 넷을 더 잡았고 **넷 다 자리 표 축이 면제**해 대장의 줄은 0이 늘었다 |
| 참조가 전부 문자열뿐인 export | 넷 | **넷** | 그대로다 — ⚠️ **바뀐 것은 수가 아니라 그 넷이 모집단 안에 있다는 사실이다** |
| `CLOSED_BLIND_SPOTS` | 둘 | **셋** | ⚠️ **움직였다** — 문자열 리터럴 축이 셋째로 닫혔고, **닫힌 사각이 새 사각 하나를 낳았다**는 사실까지 그 줄이 든다 |
| 사문 대장의 갈래(이름·소스·대장) | 5 / 15 / 2 | **5 / 15 / 2** | 그대로다 |
| 앵커 대장(앵커 전수 · code-only · comment-tolerant/래칫 · comment-only/래칫 · unanchored) | 698 · 620 · 70/70 · 8/8 · 0 | **698 · 620 · 70/70 · 8/8 · 0** | **그대로다 — 네 번 재어 한 자리도 다르지 않다** |
| 앵커 대장의 면제 | 8 | **8** | 그대로다 |
| 재개 조건 대장(자리 · 괄호 표기 · 손의 위치 · 결정형 · 손 누락 · 소스 표기/괄호 전수) | 252 · 80 · 18 · 19 · 1 · 5/7 | **293 · 97 · 23 · 24 · 1 · 5/8** | ⚠️⚠️ **다섯이 움직였다** — ⚠️ **이 줄도 A~E 머지 시점의 값(252 · 80 · 18 · 19)으로 서 있었다**(라운드 90 리뷰 M-2가 F 머지 뒤로 다시 쟀다): 이 절 자신이 AE절 다섯과 결정형 열셋을 더하며 문서의 재개 조건이 **44자리** 늘었다 — **판정을 쓰는 걸음이 곧 이 대장의 모집단을 키우는 걸음**이라 이 행은 구조적으로 F 이후에 재실측해야 한다. 손 누락은 **1 그대로**이고, 소스 표기/괄호는 **5/8 그대로**다(F는 소스를 0바이트 고쳤다 — 인용을 포함한 넓은 바늘이 여덟이 된 것은 트랙 E의 새 계약이 그 관례를 인용했기 때문이다) |
| 재개 조건 대장의 사각 여섯 | 145 · 2 · 222 · 9 · 2 · 12 | **145 · 2 · 265 · 9 · 3 · 12** | ⚠️ **셋째와 다섯째가 움직였다** — 라운드 노트 하나(round90-scout.md)가 늘었고 인용된 소스 조건이 하나 늘었다. ⚠️ **이 줄은 F가 이 절을 쓰기 *전*의 값이다**(이 절 자체가 재개 조건을 더하므로 커밋 뒤의 실측은 이보다 크고, **하한 설계 덕에 계약은 그대로 초록이다**) |
| 계약 그물 수 | 열다섯 | **열다섯** | ⚠️⚠️ **그대로다 — 그물 수가 늘지 않은 첫 라운드다.** 트랙 B의 앱 내 스윕(앱 하나만 걷는다) · 트랙 D의 모집단 확대(같은 파일 같은 축) · 트랙 E의 정합 짝 계약(**세는 자이지 그물이 아니다**)이 셋 다 그 수에 들지 않고, **왜 들지 않는지를 각자의 머리말이 값으로 적는다** |
| 프롭 대장 아홉의 출구 / `crossPlatform`이 비어 있지 않은 항목 | {announce 21 · live-region 1} / 하나 | **{announce 21 · live-region 1} / 하나** | 그대로다 — ⚠️ **트랙 A가 연 것은 이 대장이 아니라 그 대장이 자기 사각으로 지목한 *밖*이다** |
| 반쪽 프롭(그 사각이 지목한 자리) | **세는 자리 0건** | **일곱**(live region 단독 **6** · role 단독 **1**) · 판정 **wired 2 · already-cross-platform 1 · unreachable 3 · silent 0** | ⚠️⚠️ **오늘 처음 세어졌다** — 사각 문장이 가리키던 자리가 모집단이 됐다 |
| 어드민의 상태 문장 | **세는 자리 0건**(출구 하나) | 자리 **47** · 파일 **14** · 출구 **45**(`role="alert"` **34** · `role="status"` **11**) · 면제 **2** · 문장 대장 **47줄**(sha256) | ⚠️⚠️ **오늘 처음 세어졌다.** ⚠️ **두 수를 한 낱말로 적지 않는다** — *종전에 소리로 닿지 않던 자리*는 **마흔여섯**(47 − 이미 서 있던 하나)이고, *오늘 출구를 지닌 자리*는 **마흔다섯**(47 − 면제 둘)이다. 둘 다 참이고 세는 것이 다르며, **새로 출구를 얻은 자리는 마흔넷**이다 |
| 어드민의 표 이름 / 무명 표 | 17 / 0(`app/**`) | **17 / 0**(`app/**`) · ⚠️ 그 스윕 **밖**에 무명 표 **하나**(`src/components/ProductLinkBulkReplace.tsx`) | 그대로다 — ⚠️ **오늘 그 하나가 값으로 동결됐다**(늘면 새 무명 표, 줄면 누가 이름을 준 것이다) |
| 접근성 표 → 짝 문서 좌표 | **무는 자리 하나**(C-3의 `#26`) | 자리 **92** · 인용 번호 **96** · 서로 다른 번호 **92** · 표면까지 적은 참조 **33** · 역방향 **8** · 어긋남 **0** | ⚠️⚠️ **움직였다** — ⚠️ **정찰의 *서로 다른 번호 87*·*역방향 7*을 트랙 D가 91·8로 정정했다**(자리마다 첫 번호만 세면 87이고, 줄바꿈을 넘는 역방향 하나를 더 찾았다). ⚠️ **이 행은 라운드 90 리뷰 M-2가 F 머지 뒤로 다시 잰 값이다** — D 커밋 시점은 자리 91 · 번호 95 · 서로 다른 번호 91 · 표면 32였고, F가 접근성 표에 A-31(`#106`)을 더하며 넷이 하나씩 늘었다(역방향과 어긋남은 그대로) |
| 짝 문서(행 수 · 실기기 · 브라우저 · 서버 · 작업) | 163 · 135 · 18 · 9 · 1 | **165 · 136 · 19 · 9 · 1** | ⚠️ **움직였다** — `#164`(실기기 · 트랙 A) · `#165`(브라우저 · 트랙 B) |
| 접근성 표 | A-30까지(#105) | **A-31까지(#106)** | ⚠️ **움직였다** |
| 접근성 표 C-3 경과 수(세 자리) | **스물셋 · 스물셋 · 스물셋**(갈림 0) | **스물넷 · 스물넷 · 스물넷**(갈림 0) | ⚠️ **움직였고, 세 자리를 함께 올렸다** — 한 자리만 올리면 짝 계약이 빨개진다 |
| `MAX_DIVERGENCE` ↔ 문서가 옮겨 적은 그 상수 | **0** ↔ 문서 두 자리 **셋** | **0** ↔ 문서 두 자리 **0** | ⚠️⚠️ **문서 쪽이 움직였다 — 오늘의 결함이 여기 있었고 F가 고쳤다**(상수는 한 바이트도 건드리지 않았다) |

⚠️ **이 표가 라운드 89의 트립와이어 관례에 더하는 것이 둘이다.** ⓐ **세는 자리가 0건이던 축 둘이
오늘 처음 수를 얻었다**(반쪽 프롭 · 어드민 상태 문장) — **트립와이어는 값이 움직인 자리뿐 아니라
*세어지기 시작한 자리*도 기록해야 한다.** ⓑ ⚠️⚠️ **정찰의 수 셋이 트랙의 재실측으로 정정됐다**
(서로 다른 좌표 번호 **87→91** · 역방향 **7→8** · 반쪽 프롭의 *도달 0건 셋* 가운데 하나의 **근거**
— `LoadingState`는 배럴 간접 경로가 실재해 판정의 근거가 import 그래프에서 제목 정규식 문지기로
바뀌었다) — **네 라운드 연속으로 정찰의 전제가 낡았고, 다섯 번째인 오늘도 낡았다.** **정찰이 grep과 손 판정으로 낸 수는 하한이고, 갈리면 그 수가
값이라는 관례가 다섯 라운드째 참이다.**

**이 라운드가 짝 문서에 남긴 것.** 확인의 표에 **#164~#165 둘**이 서고 §0의 여섯 숫자와 머리말의
재계산 줄이 파싱으로 다시 세어졌으며, 접근성 표에는 **A-31 #106 하나**와 **C-3의 세 자리 갱신**,
그리고 ⚠️⚠️ **낡은 인용 둘의 수리**가 섰다.
⚠️⚠️ **표면 배분은 라운드 88·89와 같다 — 실기기 하나 · 브라우저 하나이고, 이번으로 세 라운드
연속이다. 그리고 그 이유가 §0의 수보다 값이 크다**: 줄어든 것은 **확인을 미뤄서가 아니라 다섯 트랙 중
셋이 화면을 한 곳도 열지 않았기 때문**이다(사문 대장의 문자열 리터럴 축 · 접근성 표의 좌표 축 ·
계약 그물 목록 정합 짝 계약 — 셋 다 **화면 0건 · 문구 0건 · 렌더 0건**이라 사람이 밟을 자리를 만들지
않는다). ⚠️ **라운드 84가 `실기기` 0건에 대해 세운 그 구분**(*"확인할 것이 없다"* 가 아니라 *"폰에
보이는 동작을 한 곳도 바꾸지 않았다"*)**의 다섯째 판이고, 세 라운드 연속이라는 사실이 오늘 처음
값이 된다** — 한 번은 그 라운드의 모양이고 두 번은 되풀이지만, **세 번은 이 저장소의 트랙이 서는
층이 옮겨 갔다는 뜻이다.**
⚠️⚠️ **다만 라운드 88·89와 다른 점 하나를 함께 적는다 — 이번 라운드의 화면 트랙 둘은 둘 다 *계약이
자기 사각으로 적어 둔 자리*에서 나왔다.** 트랙 A의 모집단은 a11y 그물의 **넷째 사각**이고, 트랙 B의
모집단은 *모바일이 열 라운드에 걸쳐 세운 규율이 형제 앱에 0건*이라는 사실이다. **앞의 두 라운드는
화면 트랙이 새로 발견한 자리에서 나왔고, 오늘의 둘은 이미 적혀 있던 자리에서 나왔다 — 표가 세는
수는 같은데 그 수가 어디서 왔는지가 다르다.**
⚠️ **A-31이 A-30과 다른 점 하나는 그 절의 머리말이 진다**: 라운드 89의 하나는 *판정 칸이 값으로 선
자리를 실제로 고쳤는가*를 물었고, **이번 하나는 *사각 칸이 이름으로 지목한 자리를 실제로 고쳤는가*를
묻는다.** ⚠️ **기기 조건은 A-30과 같은 모양이다** — **서로 다른 플랫폼 한 대씩**이고(결함이 있던
쪽이 iOS이므로), 재료는 준비템 카탈로그가 있는 계정 하나면 된다.
⚠️⚠️ **그리고 그 행이 라운드 89 리뷰 L-3·L-4의 두 물음을 다시 진다** — 라이브 리전과 겹쳐 **두 번
읽히지 않는가** · 같은 검색어로 다시 렌더될 때 **다시 읽지 않는가**. **A-30 #105 ⓑ가 처음 진 그
두 물음이 한 라운드 만에 다른 화면에서 그대로 다시 섰고, 그 되풀이 자체가 이 표가 아직 답을 받지
못했다는 뜻이다.**
⚠️ **트랙 B의 어드민 항목은 종전 판정대로 행이 아니라 문단으로 적었다**(브라우저 화면은 그 표의
조건 밖이다) — ⚠️ **다만 그 트랙이 고친 것이 정확히 접근성 축이라는 사실은 문단이 진다**: 운영자가
저장·삭제·검토 요청·CSV 내보내기·일괄 교체를 누른 결과를 **마흔일곱 자리 가운데 마흔여섯**에서 듣지
못했다. ⚠️ **트랙 C·D·E는 소스 계약이라 두 표 어디에도 행이 서지 않는다.**
⚠️⚠️ **C-3(잠금 오버레이 TalkBack 투과)은 오늘로 스물네 라운드째 미확인**이고, ⚠️ **이번 라운드가
그 줄에 더하는 값은 경과 수가 아니라 *그 줄이 인용한 계약 상수가 낡아 있었다*는 사실과, **그 갈림을
세는 계약이 오늘도 0건**이라는 사실이다.

### AE-1. **판정 칸이 지목한 자리를 고친 라운드의 다음 질문은 사각 칸이 지목한 자리이고, 그 둘은 같은 요구의 다른 모양이다** — 뒤쪽이 더 오래 서 있었고, 오래 서 있었다는 사실을 아무도 세지 않았다

- **사실.** 라운드 88이 프롭 대장에 **판정 칸**을 세웠고 라운드 89가 그 칸이 지목한 자리를 고쳤다
  (AD-1). ⚠️⚠️ **오늘 세어 보니 그 판정 칸 바로 옆에 같은 모양이 하나 더 있었다** — a11y 그물의
  `MODULE_ANNOUNCE_SWEEP_BLIND_SPOTS` 넷째 문장이 *"live region 단독은 어느 스윕도 세지 않는다"* 라고
  **자기 손으로** 적어 둔 자리다. **판정 칸은 *이 자리는 이렇다*를 말하고, 사각 칸은 *이 자리는 세지
  않는다*를 말한다 — 둘 다 *여기를 보라*는 같은 요구이고, 뒤쪽은 오늘까지 한 번도 배정된 적이 없었다.**
- **그 문장이 가리킨 자리를 열어 보니 여섯이었고, 오늘 사람이 겪는 침묵은 둘이었다.**
  `src/preparation/PreparationListParity.tsx`의 **검색 결과 개수**(*"‘…’ 검색 결과 N개"*)와
  `app/index.tsx`의 **콜드 스타트 대기 안내**다. 둘 다 `accessibilityLiveRegion="polite"`가 짝 없이
  걸려 있었고 같은 파일에 `announceForA11y`가 **0건**이라 **안드로이드에서만 소리가 나고 iOS에서는
  아무 일도 일어나지 않은 것처럼** 들렸다. ⚠️⚠️ **앞의 하나는 핵심 루프 안이다** — 준비템 탭에서
  검색을 제출한 사람이 **몇 개가 남았는지** 듣지 못했다.
- ⚠️⚠️ **나머지 넷이 결함이 아니라는 판정이 이 트랙의 값 절반이다.** `app/settings/app-lock.tsx`는
  이미 두 플랫폼에 낭독하고(모양만 보면 결함으로 셌을 자리), design-system 셋은 **제품 화면 도달
  0건**이다. **반쪽 프롭이라는 *모양*이 아니라 *effect 배선과 도달*까지 봐야 답이 맞는다.**
  ⚠️ **그리고 그 셋 중 하나에서 정찰의 전제가 뒤집혔다** — `LoadingState`는 배럴 재수출을 타는
  간접 경로가 **실재했고**, 도달 0건의 근거가 import 그래프가 아니라 **제목 정규식 문지기**로
  정정됐다(정찰이 *"이 노트에서 가장 뒤집히기 쉬운 판정"* 이라고 이름 붙인 그 자리다).
- **오늘의 값 — effect 두 벌과 문구 함수 하나, 그리고 사각 칸이 모집단이 된 것이다.** ⓐ 두 자리에
  `announceForA11y` effect가 서고 ⚠️ **검색 쪽의 `if` 조건은 그 줄을 세우는 최내곽 JSX 갈래와 글자로
  같다**(라운드 88 리뷰 L-1이 이름 붙인 사각의 둘째 소비자). ⓑ 낭독 문장은 **화면이 이미 그리는 두
  값**에서 문구 모듈이 짓는다(화면에 새 한국어 리터럴 0건 · 눈과 귀가 다른 말을 하지 않는다).
  ⓒ **프롭은 한 바이트도 건드리지 않았다.** ⓓ **재낭독 가드는 값이 아니라 문장을 기억한다** —
  사람에게 닿는 단위가 문장이기 때문이고, 질의가 달라졌는데 개수만 같은 창에서는 다시 읽어야 한다.
  ⓔ ⚠️⚠️ **그리고 그 사각 문장이 모집단이 됐다**: 반쪽 자리 **일곱**을 전수로 파생하고 자리마다
  판정 하나를 소스에서 낸다(`wired` 2 · `already-cross-platform` 1 · `unreachable` 3 · `silent` **0**).
  **손으로 적은 여섯이 아니라 전수가 그 수를 정한다.**
- ⚠️ **그리고 이 판정은 라운드 79~89를 나무라지 않는다.** 사각을 값으로 적는 것은 규율이고(AA-4),
  적어 두지 않았으면 오늘 이 자리는 보이지도 않았다. ⚠️⚠️ **문제는 사각 칸에 *누가 언제 집는가*가
  붙어 있지 않았다는 것이다** — 판정 칸에 대해 라운드 89가 발견한 그 공백이 **사각 칸에서 한 번 더,
  더 오래 서 있는 채로** 있었다.
- **일반형.** **판정 칸과 사각 칸은 같은 요구의 다른 모양이고, 그물이 자기 사각을 값으로 적는 관례가
  자리 잡은 저장소에서는 *다음 라운드의 배정표*가 이미 소스 안에 쓰여 있다.** ⚠️ **그 배정표를 읽는
  비용은 낮다** — 사각 칸은 이름으로 자리를 지목하고 대개 `measure`까지 지니고 있어 **전제가 참인지
  싸게 잴 수 있다.** ⚠️⚠️ **그런데 사각 칸에는 판정 칸에 없는 함정이 하나 더 있다: 사각은 *세지
  않는다*를 말할 뿐 *그 자리가 결함인가*는 말하지 않는다.** 오늘 여섯 중 넷이 결함이 아니었고, **그
  가름은 열어 보기 전에는 알 수 없었다.**
  ⚠️ **다음 라운드가 먼저 세어 볼 만한 것**: 이 저장소의 사각 칸 가운데 **`floor`+`measure` 쌍을
  지닌 것이 몇이고 문장만 든 것이 몇인가**, 그리고 **문장만 든 사각이 오늘도 참인지를 세는 자리가
  몇인가**(오늘 마흔다섯 중 a11y 그물의 열여섯이 문장만 들고 있고, 그 열여섯은 기계가 다시 재지
  않는다 — 오늘의 답은 **0**이다).
  ⚠️ **되짚기(라운드 91 트랙 F)**: 이 질문에 답한 것은 **AF절 머리말의 답 표 `#1`**이고,
  그 답이 낳은 판정은 **AF-3**이다(⚠️ **수와 판정은 여기 옮겨 적지 않는다** — O-3·X-4).

### AE-2. **한 앱에서 열 라운드에 걸쳐 대장이 된 규율이 형제 앱에서는 마흔일곱 중 한 자리에 서 있었고, 그 한 자리는 같은 파일 두 줄 위에 있었다** — 옆 자리를 보면 보이는 것을 아무도 보지 않았다

- **사실.** 모바일은 라운드 79~89에 걸쳐 *저장 실패 문장은 낭독되는 노드 안에 선다*를 **대장 ·
  판정 칸 · 출구 셋**으로 세웠다. ⚠️⚠️ **오늘 어드민을 같은 눈으로 세니 사람에게 조작의 결과를
  말하는 자리가 마흔일곱(파일 열넷)이었고, 소리로 닿는 것은 하나였다.** 그 하나는
  `ProductLinkBulkReplace.tsx`의 대량 교체 경고(`role="status"` · 라운드 84)이고,
  ⚠️ **그 두 줄 위의 실패 배너와 열여섯 줄 아래의 성공 배너는 둘 다 0건**이었다
  (`ProductLinkBulkReplace.tsx:269` 실패 배너 · `:271` 그 하나 · `:287` 성공 배너 — ⚠️ 라운드 90
  리뷰 L-3이 *"세 줄"* 을 실측 **두 줄**로 고쳤다. 정찰이 좌표(`:269`)는 옳게 적고 거리만
  한 줄 더 셌다).
- ⚠️⚠️ **운영자가 누른 결과를 하나도 듣지 못했다.** 마흔일곱은 전부 조건부로 뜨는 자리라 —
  저장·삭제·검토 요청·CSV 내보내기·일괄 교체를 누른 **직후에** 선다 — 스크린리더로 일하는 운영자는
  **성공했는지 실패했는지**를 화면을 다시 훑어야만 알았다. **값은 화면에 다 있었고 소리로 가는 길만
  없었다.**
- ⚠️ **관례를 발명하지 않고 인용했다는 점이 이 판정의 절반이다.** 저장소가 이미 고른 답은 *성공·경고
  쪽이 `status`* 였고(그 하나가 그렇게 서 있었다), 이 트랙이 새로 고른 것은 **실패 쪽 하나**뿐이다
  (`role="alert"` — `status`의 assertive 짝이고, 실패한 창에서는 하려던 조작이 이미 무의미하다).
  ⚠️ **`aria-live`가 아니라 `role`을 고른 이유도 본보기다** — `role`은 공손함만이 아니라 **역할**까지
  준다(스크린리더가 *"경고"* 와 *"알림"* 으로 갈라 읽는다).
- **오늘의 값 — 자리 마흔다섯과 면제 둘, 그리고 앱 내 스윕 하나다.** ⓐ 마흔일곱 중 **마흔다섯**이
  출구를 얻었다(`alert` **34** · `status` **11**). ⓑ **면제 둘은 이유를 값으로 든다** —
  `AdminShell` 헤더의 복구 코드 안내는 **조작의 답이 아니라 헤더 크롬**이고, 라이브 영역으로 만들면
  다시 마운트될 때마다 남은 장수를 제목 위에 덮어 읽는다(**정보가 아니라 소음이다**). ⚠️ **자리는
  둘인데 여는 태그가 하나(삼항)라 판정도 하나여야 한다** — 한쪽만 출구를 주면 같은 문장이 남은 장수에
  따라 들리다 말다 한다. ⓒ ⚠️ **문구는 0글자 바뀌었고 픽셀도 0이다** — 더한 것은 여는 태그의 속성
  하나뿐이고, **그 사실을 계약이 자리마다 sha256으로 문다**(속성을 벗기면 종전 바이트여야 한다).
  ⓓ **앱 내 스윕이 섰다**(모집단 47 · 파일 14 · 출구 래칫 45 · 면제 2) — ⚠️ **계약 그물 열다섯에
  들지 않는다: 앱 하나만 걷는다.**
- ⚠️⚠️ **그리고 이 트랙은 옆 축 둘을 값으로 동결했다.** 라운드 89 B의 표 이름 스윕 **밖**에 남은
  무명 표 **하나**(`ProductLinkBulkReplace.tsx`)와, 이 앱의 `<main>` **다섯**·`aria-current` **0건**
  이다. **셋 다 이 트랙의 축이 아니라 손대지 않았지만, 값으로 두어야 다음 트랙이 그 자리를 먼저
  본다** — **동결은 고침이 아니지만 *조용히 낡는 것*은 막는다.**
- **일반형.** **한 층에서 대장으로 세운 규율은 옆 층으로 건너가지 않는다**(AB-1) — 그리고 **그 층이
  앱 경계일 때 가장 오래 안 건너간다.** 형제 앱은 같은 저장소·같은 리뷰·같은 릴리즈 게이트를 지나는데,
  **모집단을 세는 스윕이 앱 하나만 걷기 때문에 옆 앱의 0건은 어느 계약도 빨갛게 만들지 않는다.**
  ⚠️⚠️ **그래서 규율을 세운 라운드가 남겨야 하는 것은 대장만이 아니라 *이 규율의 모집단이 어디까지인가*
  이고**, 그 문장이 없으면 **옳은 형식이 같은 저장소의 두 줄 위에 있어도 열 라운드가 지나간다.**
  ⚠️ **다음 라운드가 먼저 세어 볼 만한 것**: 이 저장소의 스윕 가운데 **모집단이 한 앱으로 그어진 것이
  몇이고, 그 축이 형제 앱에도 필요한 것이 몇인가**(오늘 어드민의 앱 내 스윕이 둘이 됐고 — 표 이름 ·
  상태 낭독 — 모바일에는 같은 축의 대장이 각각 0건과 여럿이다).
  ⚠️ **되짚기(라운드 91 트랙 F)**: 이 질문에 답한 것은 **AF절 머리말의 답 표 `#2`**이고,
  그 답이 낳은 판정은 **AF-1**이다(⚠️ **수와 판정은 여기 옮겨 적지 않는다** — O-3·X-4).

### AE-3. **계약이 조여져도 그 계약을 설명한 문장은 조여지지 않는다 — 같은 라운드 안에서도** — 그리고 문서가 옮겨 적은 것이 *수*가 아니라 *상수*이면 낡아도 자기 안에서는 말이 된다

- **사실.** 라운드 89 리뷰 M-2가 접근성 짝 계약의 갈림 상한을 **3에서 0으로** 내리며 *"두 자리를 함께
  올린 라운드는 이 상한도 그날의 실측 갈림으로 함께 내린다"* 를 상수 주석에 값으로 박았다.
  ⚠️⚠️ **그런데 그 상수를 인용한 문서 두 자리는 오늘까지 *"셋을 넘지 않는가"* 라고 적고 있었다** —
  접근성 표 C절의 C-3 행과 "수동 증거" 절이다.
- ⚠️ **두 자리 다 자기 안에서는 말이 되고 계약은 초록이었다.** 그 계약이 무는 것은 **세 자리의 수**
  (경과 라운드 목록)이지 **문서가 옮겨 적은 상수**가 아니기 때문이다. ⚠️⚠️ **AD-4가 이름 붙인
  *낡은 수는 자기가 낡았다고 말하지 않는다*의 즉시 재발이고, 이번에 낡은 것은 수가 아니라 *상수의
  인용*이다** — 수는 라운드마다 자라므로 낡으면 옆자리와 갈리지만, **상수의 인용은 옆자리가 없어
  갈릴 곳조차 없다.**
- ⚠️⚠️ **그리고 이 자리를 찾은 것이 계약이 아니라 정찰이라는 사실이 이 판정의 절반이다.** 라운드 90
  트랙 D가 접근성 표의 좌표 인용을 **하나에서 전수(91)로** 넓히면서 이 갈림을 **자기 사각으로**
  적었고(`문서가-옮겨-적은-계약-상수` · 값 둘 · 하한 둘), **무는 축은 세우지 않았다** — 오늘 빨간
  계약을 세우면 그 트랙이 문서를 고쳐야 하고, **그 순간 *계약이 문서를 지킨다*가 *문서가 계약을
  맞춘다*로 뒤집힌다.**
- **오늘의 값 — 문장 둘의 수리와, 그 수리를 예고한 사각 하나다.** ⓐ 두 자리의 *"셋"* 을 오늘의
  상수(**0**)로 고쳤고, **상수는 한 바이트도 건드리지 않았다.** ⓑ ⚠️ **두 시점으로 적었다** —
  *당시*에는 셋이 실측 갈림이었고 *오늘*은 0이다(옛 문장을 지우는 것이 아니라 **언제 무엇이 참이었는지**
  를 남긴다). ⓒ ⚠️⚠️ **그 자리가 낡아 있었다는 사실과 그것을 세는 계약이 0건이라는 사실을 문서 세
  자리에 값으로 적었다**(접근성 표 머리말 · C-3 행 · 수동 증거 절 · 그리고 짝 문서 §0-1의 🔒 줄).
  ⓓ **그 축을 무는 계약은 다음 라운드다** — 트랙 D의 사각이 **자기 축과 함께** 재개 조건을 적었고
  (*문서가 인용한 계약 상수의 정합이 한 트랙의 축이 되는 라운드가 서는 날*), **오늘 F가 고친 두
  자리가 그 계약의 첫 모집단이다.**
- **일반형.** **문서가 소스의 값을 옮겨 적을 때 위험한 것은 *수*보다 *상수*다.** 수는 라운드마다 자라
  옆자리와 갈리지만, **상수는 한 번 적히면 옆자리가 없고 그 문장은 자기 안에서 영원히 정합이다.**
  ⚠️ **그래서 계약을 조이는 걸음에는 *그 계약을 설명한 문장이 어디에 몇 자리 있는가*를 함께 세는
  걸음이 붙어야 한다** — 조이는 손과 설명을 고치는 손이 다르면(오늘은 리뷰와 F였다) 그 사이가
  라운드를 건너 살아남는다. ⚠️⚠️ **그리고 그 수리를 계약으로 만드는 순서는 정해져 있다: 문서를 고치는
  손이 먼저 고치고, 그 자리를 무는 계약은 다음 라운드가 세운다**(라운드 89 D·E가 세운 그 순서이고,
  오늘 그것이 두 번째로 이행됐다).
  ⚠️ **다음 라운드가 먼저 세어 볼 만한 것**: 이 저장소의 문서가 **소스의 *상수*를 옮겨 적은 자리가
  몇이고, 그중 계약이 그 정합을 세는 것이 몇인가**(오늘 하나를 찾아 고쳤고, 세는 것은 **0건**이다 ·
  ⚠️ 오늘의 모집단은 문서 셋이었으므로 그 수는 **하한**이다).
  ⚠️ **되짚기(라운드 91 트랙 F)**: 이 질문에 답한 것은 **AF절 머리말의 답 표 `#3`**이고,
  그 답이 낳은 판정은 **AF-5**이다(⚠️ **수와 판정은 여기 옮겨 적지 않는다** — O-3·X-4).

### AE-4. **그물을 세는 자리 자신이 손 목록으로 서 있었고, 그 목록끼리의 정합을 세는 계약이 0건이었다** — 착시는 그것을 이름 붙인 자리에 가장 늦게 온다

- **사실.** `comment-tolerant-anchor-ledger.ts`는 *"내 앞의 그물 **열셋**"* 을,
  `resume-condition-ledger.ts`는 *"내 앞의 그물 **열넷**"* 을 **손으로** 적고 각자 `+1`로 자기 수를
  낸다(**14**·**15**). ⚠️⚠️ **두 계약은 자기 수만 등호로 못 박았고, 두 목록이 서로 맞는지를 세는
  자리는 0건이었다.**
- ⚠️ **오늘 그 정합은 참이었다** — 앞의 열셋은 뒤의 열넷의 **접두**이고 더해진 하나가 *"주석 관용 앵커
  대장"* 이다. ⚠️⚠️ **그런데 세는 자리가 없어 참인 것은 오늘뿐이었다**: 열여섯째 그물이 서는 날 그
  트랙은 **열다섯 이름을 손으로 다시 옮겨 적고**, 순서가 바뀌거나 이름 하나가 빠져도 **모든 계약이
  초록이다.**
- ⚠️⚠️ **AB-4가 이름 붙인 착시**(*대장에 이름이 있다는 사실이 "그 자리는 세어졌다"로 읽힌다*)**가
  한 겹 위에서, 그러니까 그물을 세는 자리 자신에게서 다시 난 모양이다.** ⚠️ **AC-4가 *처방은 처방을
  낳은 자리에 가장 늦게 온다*로 적은 그 문장이 두 번째로 참이 됐다.**
- **오늘의 값 — 손 목록을 하나 더 만들지 않은 것이다.** ⓐ 모집단은 `packages/**`·`apps/**`를 걸어
  **그 이름을 export하는 파일 전수**에서 나온다(오늘 **둘** · 걸은 파일 **906**) — ⚠️ **고치려는 병이
  *손으로 적은 목록*이므로 세 번째 손 목록을 두면 병을 옮겨 적는 것이 된다.** ⓑ 값은 소스를
  파싱하지 않고 **그 모듈을 실제로 불러** 읽는다(손으로 적힌 것을 무는 계약이 다시 텍스트만 보면 같은
  자리에서 속는다). ⓒ 묻는 것은 **접두 · 자기 수 = 목록 길이 + 1 · 가장 긴 목록 + 1이 오늘의 그물
  수(하한) · 이름의 유일성**이고, **상한도 전수 일치도 묻지 않는다** — 등호로 물면 **열여섯째 그물을
  세우는 트랙이 이 계약을 맞추게 되고, 그 순간 세는 자가 세어지는 것을 막는 자가 된다.**
  ⓓ ⚠️⚠️ **이 파일은 그 두 이름을 export하지 않는다** — 지니는 순간 자기 자신을 그물로 세게 되고
  **열다섯이 조용히 열여섯이 된다.** 그 배제는 산문이 아니라 계약이고, **자기 소스를 읽어** 확인한다.
- ⚠️ **그리고 이 계약은 자기가 못 보는 것을 값으로 든다** — **목록끼리의 정합**을 셀 뿐 *그 이름이
  가리키는 그물이 실재하는가*는 세지 않는다(오늘 열넷 중 자기 이름으로 풀리는 것 **0건**). ⚠️⚠️ **그
  사각이 새 결정형 재개 조건 하나를 낳았고, 그 조건은 자기 축을 함께 적는다**: *그물 파일이 자기를
  이름으로 선언하는 표식을 세울지 정하는 날 — 그날 이 목록은 손이 아니라 소스에서 나온다*
  (**손은 저장소 안**).
- **일반형.** **세는 자리를 세우는 라운드는 *그 세는 자리 자신이 무엇으로 서 있는가*를 함께 물어야
  한다.** 그물이 열넷·열다섯으로 늘어나는 동안 각 대장은 자기 수를 정직하게 등호로 못 박았고,
  **그 정직함이 정확히 착시의 재료였다** — 등호가 있으니 세어졌다고 읽히는데, **등호가 무는 것은
  *자기 목록의 길이*이지 *그 목록이 옳은가*가 아니다.** ⚠️ **그래서 손 목록이 둘 이상 서는 순간
  물어야 할 것은 *각자 맞는가*가 아니라 *서로 맞는가*이고**, 그 질문은 목록이 하나일 때는 존재하지
  않다가 **둘이 되는 날 조용히 생긴다.**
  ⚠️ **다음 라운드가 먼저 세어 볼 만한 것**: 이 저장소에서 **같은 사실을 손으로 적은 목록이 둘 이상
  서는 자리가 몇이고, 그중 목록끼리의 정합을 세는 것이 몇인가**(오늘 그물 목록 하나에 계약이 섰고,
  ⚠️ 그 계약이 세는 열넷 가운데 **소스에서 자기 이름으로 풀리는 것은 0건**이다 — **다음 질문은 수가
  아니라 *이름이 실재로 풀리는가*다**).
  ⚠️ **되짚기(라운드 91 트랙 F)**: 이 질문에 답한 것은 **AF절 머리말의 답 표 `#4`**이고,
  그 답이 낳은 판정은 **AF-5**이다(⚠️ **수와 판정은 여기 옮겨 적지 않는다** — O-3·X-4).

### AE-5. **조건을 좁혀 적으면 규율이 막던 자리를 선택이 막게 되고, 그 둘은 겉이 같고 처방이 다르다** — 미도래와 미배정은 구별되지 않는 채로 라운드를 건넌다

- **사실.** 라운드 89 AD-5는 *행마다 갈리는 낭독 라벨의 모집단*이 지닌 조건
  (*"a11y 그물을 여는 다음 트랙이 서는 날"*)이 **자기모순**임을 값으로 만들고 —
  조건이 참이 되는 사건이 곧 조건을 막는 사건이었다 — 그것을
  *"그 스윕 자체가 한 트랙의 축이 되는 라운드가 서는 날"* 로 **좁혀 다시 적었다.**
- ⚠️⚠️ **오늘 그 조건은 집힐 수 있었다. 그런데 집히지 않았다.** 이번 라운드에도 a11y 그물을 여는
  트랙은 하나(A)였고, 그 트랙의 축은 **반쪽 프롭**이었다 — **규율이 막은 것이 아니라 배정이 다른 곳에
  갔다.** ⚠️ **그리고 그 자리는 오늘도 값이 그대로다**(식으로 만드는 낭독 라벨 **162** · 목록 안의
  낭독 라벨 **39** 중 행마다 갈리지 않는 상수 **0건** · 손 핀 스물두 개 남짓).
- ⚠️⚠️ **그래서 오늘 처음 보이는 것이 있다: *조건이 도래할 수 없는 상태*와 *조건은 도래할 수 있는데
  배정이 없는 상태*는 문서에서 똑같이 읽힌다.** 둘 다 *미도래*로 적히고, 둘 다 다음 라운드가 그
  줄을 지나치게 만든다. ⚠️ **다른 것은 처방이다 — 앞은 조건을 고쳐야 하고(라운드 89가 했다), 뒤는
  배정하면 된다(아직 하지 않았다).** **그 구별은 조건을 고쳐 본 라운드의 *다음* 라운드에만 보인다:
  고치기 전에는 두 상태가 겹쳐 있어 어느 쪽인지 알 수 없다.**
- **오늘의 값 — 기각의 이유를 고쳐 적은 것이다.** 이 라운드가 그 줄에 더한 것은 새 수가 아니라
  **막는 것이 무엇인가**이고, 그 자리는 이제 *"두 라운드 연속으로 규율이 아니라 선택이 막는다"* 라고
  적힌다. ⚠️ **V-2가 세운 규율**(조건과 함께 기각하라)**에 라운드 89가 한 칸을 더했고**
  (조건이 도래할 수 없는 모양이면 그것도 기각의 일부로 고쳐 적는다), **오늘 한 칸이 더 붙는다:
  조건이 옳게 적혀 있는데도 집히지 않으면 *막는 것이 무엇인지*를 기각에 함께 적는다.**
- ⚠️ **같은 검산을 오늘의 다른 조건들에도 돌렸다.** 결정형 열셋 가운데 *"그 판정이 서는 날"* 꼴은
  전부 자기가 어느 트랙의 축이 되는지를 함께 적고 있어 이 모순에 걸리지 않고, **손이 저장소 밖인
  다섯**은 애초에 저장소의 규율이 막는 자리가 아니다. ⚠️⚠️ **오늘 이 모순에 걸린 조건은 0건이고,
  *배정이 막은* 조건은 넷이다**(결정형 표의 8·10·11·12 — 그물 배정 · 래칫 · 손의 크기 · 관례 부재).
  **막는 이유가 넷 다 서로 다르고, 넷 다 *미도래*가 아니다.**
- **일반형.** **재개 조건이 옳게 적혔는지는 그 조건이 참이 되는 라운드에만 확인된다** — 그리고 그
  확인의 결과가 *집혔다*가 아니라 *집힐 수 있었는데 안 집혔다*이면, **그 자리는 조건의 문제가 아니라
  배정의 문제이고 기각의 이유를 고쳐 적어야 한다.** ⚠️⚠️ **문서에서 가장 오래 사는 오류는 틀린
  문장이 아니라 *두 상태를 같은 낱말로 적은 문장*이다** — *미도래* 하나가 **아직 때가 아니다**와
  **때는 됐는데 아무도 안 집었다**를 함께 덮고 있으면, 뒤쪽은 영원히 앞쪽처럼 읽힌다.
  ⚠️ **다음 라운드가 먼저 세어 볼 만한 것**: 이 저장소의 기각 가운데 **전제가 오늘 참인 것이 몇이고,
  그중 *막는 것이 무엇인지*(규율 · 배정 · 래칫 · 손의 크기 · 다른 사람의 결정)를 값으로 적은 것이
  몇인가**(오늘 결정형 넷에 대해 답했고 넷 다 이유가 서로 달랐다 — **나머지 기각 열둘은 그 칸이 아직
  없다**).
  ⚠️ **되짚기(라운드 91 트랙 F)**: 이 질문에 답한 것은 **AF절 머리말의 답 표 `#5`**이고,
  그 답이 낳은 판정은 **AF-2**이다(⚠️ **수와 판정은 여기 옮겨 적지 않는다** — O-3·X-4).

## AF. 라운드 91에서 확정한 판정 (2026-08-31 · GAP-091 트랙 F)

라운드 90이 물은 것이 **그물이 자기 손으로 "여기는 어느 스윕도 세지 않는다"고 적어 둔 자리는
누가 집는가**(답: 아무도 집지 않는다 · AE-1)였다면, 라운드 91의 물음은 그 **바깥 한 칸**에 있다 —
⚠️⚠️ **어느 대장도, 어느 사각 칸도 이름을 준 적 없는 자리는 어떻게 찾는가.** 오늘의 답은
**저장소가 이미 고른 답을 고르지 않은 자리를 세는 것**이었고, 그렇게 찾은 자리 하나가 **핵심
루프 4단계 안에서 사람이 겪는 잘못된 수**였다. 축은 라운드 81~90과 같이 **사용자 가치**였고
(구매 링크 · 운영자 도구 · 하네스와 저장소가 자기를 세는 자리), 다섯 판정 다 K~AE절과 같이
**결함 보고가 아니라 다음 결정의 입력**이며 2026-08-31 소스에서 확인됐다(라운드 91 트랙
A·B·C·D·E 머지 후). ⚠️⚠️ **이번 라운드는 핵심 루프의 4단계를 두 라운드 만에 다시 열었고, 이번에
연 것은 낭독도 렌더도 아니라 *누른 뒤의 한 창*이다** — 링크의 표시·순서·문구·가격은 바이트
불변이다.

⚠️⚠️ **이번 라운드의 가장 값진 관측: 판정 칸도 사각 칸도 이름을 준 적 없는 자리가 남아 있고,
그것을 찾는 법은 대장을 읽는 것이 아니라 *저장소가 이미 스물일곱 자리에서 고른 답을 고르지 않은
자리*를 세는 것이다**(AF-1). 라운드 88이 **판정 칸**을 세웠고 89가 그 칸이 지목한 자리를, 90이
**사각 칸**이 지목한 자리를 집었다. **오늘의 트랙 A는 셋 중 어느 것도 아니다** — 모바일 비테스트
`.tsx`의 `useMutation` 선언 **31** 가운데 눌림을 막는 자리가 **27**이었고 막지 않는 **넷** 중
하나가 구매 링크였다. ⚠️ **그 자리를 이름으로 적어 둔 대장은 0건이었고**, 그 자리가 만드는 해악의
이름은 **서버 소스의 주석**이 이미 적어 두었다(*"집계에는 열린 적 없는 클릭이 쌓인다(**허위
수치**)"*). **가장 오래 서 있던 자리는 어느 대장에도 없었다.**

⚠️⚠️ **두 번째 관측: 배정이 막는 조건은 배정하면 열리고, 그 이행은 조건이 자기 축을 적어 둔
라운드의 *다음다음* 라운드에 왔다**(AF-2). 라운드 89 AD-5가 *"조건이 도래할 수 없는 상태"* 와
*"도래할 수 있는데 배정이 없는 상태"* 를 가르는 처방을 세웠고, 라운드 90 AE-5가 그 구별을 값으로
만들며 **막는 것이 *배정*이라고 적힌 조건 넷**을 이름으로 남겼다. **오늘 그중 셋이 배정됐다** —
어드민의 랜드마크와 현재 위치(트랙 B) · 소스 축 표기의 둘째 파일(트랙 D) · 문서가 인용한 계약
상수의 정합(트랙 E). ⚠️ **셋 다 새 관측이 필요 없었다**: 조건 문장이 *그날 무엇을 먼저 해야
하는지*까지 적어 두어서, 트랙은 재기만 하고 곧장 집었다. ⚠️⚠️ **그리고 그 이행이 AD-3이 세운
*순서를 적은 조건은 집힌다*의 두 번째 증거이자, AE-5의 처방이 실제로 통한 첫 증거다.**

⚠️⚠️ **세 번째 관측: 세는 걸음이 없으면 이미 적힌 값도 없는 것으로 읽히고, 고치는 손이 여럿이면
*고쳐진 사실*조차 낡은 채로 남는다**(AF-3). 오늘 라운드 90 F의 두 수를 정정한다 — 문장만 든
사각 열여섯 중 **오늘도 기계가 재는 것은 0이 아니라 다섯**이고, 기각 중 **막는 것을 값으로 적은
것은 넷이 아니라 여덟**이다. ⚠️ **둘 다 부채가 는 것이 아니라 세는 눈이 자기 옆을 못 본
자리다.** ⚠️⚠️ **그리고 같은 병의 세 번째 얼굴이 오늘 이 문서 밖에서 잡혔다**: 라운드 90 리뷰가
보고에만 남긴 잔존 둘(*"제출"* · *"세 줄 위"*)이 **소스와 판정 문서는 이미 고쳐진 채** 확인 문서
둘에만 살아 있었다 — **고친 사실이 낡은 것이 아니라, 그 사실을 적은 자리가 여럿이라 한 손이
전부를 옮기지 못했다.** ⚠️ **그 갈림을 세는 계약은 오늘도 0건이다.**

⚠️⚠️ **네 번째 관측: 하네스가 보는 세계와 제품이 보는 세계가 갈릴 때, 세 라운드가 "재지 않았다"로
넘긴 수는 하나가 아니라 둘이었다**(AF-4). 라운드 88·89·90이 성능 넷의 ⓑ(렌더 비용 · 활성 카탈로그
수)를 *"실 PostgreSQL을 요구해 오늘 재지 않았다"* 로 세 번 넘겼는데, 오늘 재니 **수가 둘 나왔고
둘이 서로 다른 사실**이었다: 제품이 보는 카탈로그는 **62**이고 하네스가 보는 카탈로그는
**8,706**이다(오늘 F의 재실측). ⚠️ **N-4의 문턱 200을 넘는 것은 뒤쪽 하나뿐이고 그 수는 부채가
아니라 누적이므로, N-4는 오늘도 미발동이다** — **한 낱말로 적었으면 판정이 뒤집혔을 자리다.**

⚠️⚠️ **다섯 번째 관측: 같은 관례가 여러 파일에 손으로 적히면 이름부터 갈리고, 그 갈림은 관례를
세운 자리에서 가장 늦게 보인다**(AF-5). AE-4가 *자기 배제 관례*를 **여섯 자리 · 이름 둘**로 셌는데
오늘 바늘을 넓혀 다시 세니 **열한 자리 · 이름 넷**이다(`LEDGER_SELF_FILES` 넷 ·
`SWEEP_SELF_FILES` 둘 · `SELF_PATH` 셋 · `SELF_FILE` 둘). **열한 자리가 같은 한 문장을 적는데 그
정합을 세는 자리는 오늘도 0건이고, 이름이 넷으로 갈렸다는 사실 자체가 그 증거다.** ⚠️⚠️ **그리고
오늘 그 병의 거울상 하나가 계약에서 잡혔다**: 트랙 E의 새 축이 접근성 표의 인용 자리를 **줄 번호
등호**로 물었는데, 같은 라운드의 F가 그 문서에 절 하나를 더하자 그 등호가 곧바로 빨개졌다 —
**라운드 90 트랙 D가 같은 문서의 좌표 축을 세울 때 *하한*을 골라 F의 걸음을 통과시킨 것과 정확히
반대의 선택**이고, 그 차이가 오늘 값으로 드러났다.

⚠️⚠️ **이월 다섯은 전부 보류 유지이고 재실측 값만 갱신했다 — 갱신 한 줄씩은 그 판정이 사는 절에 있다**
(다음 라운드가 같은 실측을 다시 돌리지 않도록 여기서는 자리만 가리킨다).

- **이 스캐너가 쿼리로 분류한 자리의 낭독** — 재실측 상태 변화 0, A-20 #85 선행 → **U절 머리말**
  (⚠️ **접점 0건이고 이것으로 세 라운드 연속이다** — 다섯 트랙 중 그 여섯 화면을 여는 것도 그
  계약 파일을 여는 것도 없다).
- **`monthly_wrapup`의 달 이동 구멍** — 게이트가 읽는 것은 여전히 대기 행의 바뀐 뒤 날짜 하나 → **U-3**
  (⚠️⚠️ **`src/notifications/**`가 이번에도 접점 0건이고, 이것으로 네 라운드 연속이다**).
- **S-3(어드민 `disabled`)** — 판정 종전 그대로, 브라우저 확인 `#130` 선행 → **U절 머리말**.
  ⚠️⚠️ **그리고 이 줄에 오늘 더해지는 칸은 *접점의 부재*다**: `app/items/page.tsx`와
  `app/links/page.tsx`는 라운드 89(표 이름)·라운드 90(상태 낭독)에 **두 라운드 연속으로 열렸는데
  이번 라운드에는 접점 0건이다**(트랙 B가 연 어드민 파일은 `AdminShell.tsx`와
  `app/{page,error,not-found}.tsx` 넷이고 그 둘은 그중에 없다). ⚠️ **그러므로 오늘의 정확한 문장은
  *"두 라운드 연속으로 열렸고 열린 축이 서로 달랐으며, 이번 라운드에는 열리지 않았다"* 이다** —
  *"열지 않았다"* 와 *"열었지만 그 축이 아니다"* 는 다른 문장이고(라운드 87), *"열린 축이 라운드마다
  다르다"* 도 따로 세어야 하며(라운드 90), ⚠️⚠️ **오늘 그 구별에 한 칸이 더해진다: *연속이 끊긴
  것*도 따로 세어야 한다**(끊김을 세지 않으면 *두 라운드 연속*이라는 문장이 조용히 세 라운드로 읽힌다).
- **`withdrawn_at`** — 저장소 전체 셋 · 파일 둘, 컬럼 신설은 여전히 별도 결정 → **U절 머리말**
  (⚠️ **이번 라운드도 이 수를 다시 재지 않았다** — 어느 트랙도 그 두 파일을 열지 않았고, 라운드
  88이 자기 정정으로 확정한 값을 여기서 옮겨 적지 않는다 · **세 라운드 연속 재측정 대상이 아니다**).
- **AA-R ① 연속 실패 재판정** — deps는 오늘도 `[isError]` 하나 · 자동 재시도·폴러 **0건** → **AA-R ①**
  (⚠️ 어느 트랙도 그 훅도 그 훅의 호출부 화면도 열지 않았다).

**다섯 다 2026-08-31 재실측이고 상태 변화 0이다**(`withdrawn_at`은 재실측 대상이 아니었다는 사실이
그 줄의 값이다). ⚠️⚠️ **그리고 이번 라운드의 접점 지도가 라운드 90과 다른 점은 하나뿐이다: 라운드
89·90이 *S-3의 두 파일이 열렸다*를 두 번 값으로 적었는데, 오늘 그 두 파일은 열리지 않았다.
접점의 유무도, 접점의 축도, 그 축이 라운드마다 같은지도, 그리고 그 연속이 끊겼는지도 서로 다른
사실이다.**

⚠️ **N-4의 두 문턱은 오늘로 14라운드 연속 미발동이고, 준비템 탭 비가상화는 이번에도 제안하지
않는다** — ⚠️⚠️ **그런데 이번에는 다르다: 그 수를 오늘 잴 수 있었고 두 수가 나왔다.** 트랙 C가
`apps/api/test/**`를 다섯 라운드 만에 열며 실 PostgreSQL을 요구했고, F가 같은 컨테이너에서 읽기
전용으로 다시 재니 **제품 카탈로그는 62**(시드가 만드는 그 수 · `wooriai_dev`)이고 **하네스
카탈로그는 8,706**(`wooriai_test`의 활성 준비템 · 전체 9,333 · DB 722MB)이다. ⚠️⚠️ **문턱을 넘는
것은 뒤쪽 하나뿐이고, 그 수는 사용자가 보는 목록이 아니라 하네스의 누적이다** — **N-4는 오늘도
미발동이며 이 문단이 그 판정을 바꾸지 않는다.** ⚠️ **어드민 카드가 세는 수는 여기 옮겨 적지
않는다**(O-3 · 갱신 이력은 N-4에 있다). ⚠️ **그리고 두 수를 한 낱말로 적지 않는 것이 이 문단의
값이다** — *"활성 카탈로그가 8,706이다"* 만 적으면 N-4는 오늘 발동한 것으로 읽힌다.

⚠️⚠️ **AE-1~AE-5가 남긴 질문 다섯 전수와 오늘의 답이다 — 다섯 다 답했고 셋이 발동했다**(AE-2·AE-3·AE-5).
⚠️ **수치는 여기 옮겨 적되 *그 수를 어떻게 냈는지*(모집단·바늘·사각)를 반드시 함께 가리킨다**
(AC-5·AD-4가 세운 그 규율). ⚠️⚠️ **그리고 이 답들은 계약이 아니라 손으로 잰 수다** — 계약이 세는
수와 한 낱말로 적지 않는다. ⚠️⚠️ **다섯 중 둘이 라운드 90 F의 수를 정정한다**(AE-1의 **0 → 다섯** ·
AE-5의 **넷 → 여덟**) — **둘 다 부채가 는 것이 아니라 이미 채워져 있던 칸을 F가 덜 센 것이다.**

| # | 자리 | 질문 | 2026-08-31 답(라운드 91 F) | 발동 |
| --- | --- | --- | --- | --- |
| 1 | AE-1 `:7341` | 사각 칸 중 `floor`+`measure` 쌍이 몇이고 문장만 든 것이 몇인가 · 문장만 든 사각이 오늘도 참인지 세는 자리가 몇인가 | 사각 칸 **쉰** · 쌍 **25** · 값만 **9** · 문장만 **16** · ⚠️⚠️ **문장만 든 열여섯 중 오늘도 기계가 재는 것은 *다섯*이다 — 라운드 90 F의 0을 정정한다** | ⚠️ 답했고 트랙이 되지 않았다(→ P3) |
| 2 | AE-2 `:7383` | 모집단이 한 앱으로 그어진 스윕이 몇이고 그 축이 형제 앱에도 필요한 것이 몇인가 | 디렉터리를 걸어 모집단을 파생하는 파일 **44**(모바일 **20** · 어드민 **9** · api **6** · `packages/test-utils` **9**) — ⚠️ **`apps/**`에 사는 서른다섯은 대개 자기 앱만 걷고, `packages`의 아홉이 저장소 뿌리나 두 앱 이상을 건다** · 형제에 0건인 축 둘 중 **하나를 오늘 배정했다**(연타 가드의 모바일 절반 → 트랙 A) · 남은 하나는 **어드민 라우트 표면**(`app/**`의 `page.tsx` **11** ↔ `NAV_ITEMS` href **11** · 어긋남 **0** · 세는 계약 **0건**) | ⚠️⚠️ **발동 → 트랙 A** · 라우트 표면은 P3 |
| 3 | AE-3 `:7418` | 문서가 소스의 ***상수***를 옮겨 적은 자리가 몇이고 그중 계약이 정합을 세는 것이 몇인가 | 문서 넷이 수치 상수의 **이름과 값을 함께** 적은 자리 **12**(이름 **아홉**) · ⚠️⚠️ **그중 계약이 세는 것은 오늘도 0건** · ⚠️ **다만 *이름 없이 값만* 적은 접근성 표의 세 자리는 오늘 계약이 센다**(트랙 E의 ⓖ) — **두 수를 한 낱말로 적지 않는다** | ⚠️⚠️ **발동 → 트랙 E** |
| 4 | AE-4 `:7455` | 같은 사실을 손으로 적은 목록이 둘 이상 서는 자리가 몇이고 그중 정합을 세는 것이 몇인가 | 자기 배제 관례를 손으로 적은 자리 **열하나** · 이름 **넷**(`LEDGER_SELF_FILES` 4 · `SWEEP_SELF_FILES` 2 · `SELF_PATH` 3 · `SELF_FILE` 2) — ⚠️⚠️ **AE-4가 센 *여섯 · 이름 둘*은 하한이었다**(그 바늘은 배열 export 둘만 봤다) · **정합을 세는 자리 0건** | ⚠️ 답했고 트랙이 되지 않았다(→ P3 · 결정형 13과 같은 자리) |
| 5 | AE-5 `:7490` | 기각 중 전제가 오늘 참인 것이 몇이고 그중 *막는 것이 무엇인지*를 값으로 적은 것이 몇인가 | AE절 기각 **열여섯** 중 전제가 오늘 참인 것 **열하나** · 그중 막는 것을 값으로 적은 것 **여덟**(파일 배타 · 선택 · 그물 배정 · 래칫 · 손의 크기 · 관례 부재 · 배정 · 다른 사람의 결정) — ⚠️⚠️ **라운드 90 F가 적은 *넷*은 하한이었다** · ⚠️⚠️ **그리고 *배정*이 막는다고 적힌 넷 가운데 셋을 오늘 배정했다** | ⚠️⚠️ **발동 → 트랙 B·D·E** |

**답 1을 낸 방법.** **모집단** = 저장소의 계약·대장 파일이 지닌 **사각 칸 배열** 전수(여섯 배열).
**바늘** = `*BLIND_SPOTS` 꼴 배열의 항목마다 ⓐ 하한(`floor`)과 ⓑ 오늘 다시 재는 자(`measure`)의
유무를 가른다. **자리별 실측**(전부 오늘 실행값): `dead-export-ledger.ts` **여덟**(22 · 0 · 226 ·
20 · 55 · 105 · 141 · 0 → 쌍 여섯 · 값만 둘) · `comment-tolerant-anchor-ledger.ts` **일곱**
(자기 `measure`가 0건이라 쌍으로 세지 않는다) · `resume-condition-ledger.ts` **여덟**
(라운드 91 D가 소스 축을 넓히며 둘을 새로 졌다 → 쌍 여덟) · `accessibility-checklist-shape.test.ts`
**여덟**(라운드 91 E가 ⓖ 축과 함께 둘을 새로 졌다 → 쌍 여덟) · `contract-net-ledger.test.ts` **셋** ·
`a11y-contract.test.ts` **열여섯**(문장만).
**합 쉰 · 쌍 25 · 값만 9 · 문장만 16.** ⚠️ **라운드 90의 마흔다섯에서 다섯이 늘었는데 부채가 는
것이 아니라 *세는 자리*가 늘었다**(리뷰가 하나 · 트랙 D가 둘 · 트랙 E가 둘). ⚠️⚠️ **정정의 값은 마지막 칸에 있다** — 라운드 90 F는
*"그 열여섯은 기계가 다시 재지 않는다 · 오늘의 답은 0"* 이라고 적었는데, 소스를 열어 보니 **다섯은
잰다**(`VERDICT` ②③④⑥ · `MODULE` ④ — 옆의 단언이 그 사각이 가리킨 실재를 다시 잰다). **F가 자기
그물의 옆 단언을 세지 않았다.** ⚠️ **사각 둘**: ⓐ 배열 꼴이 아닌 산문 사각(`dnc-*.ts` 셋)을 못 본다 ·
ⓑ `CLOSED_BLIND_SPOTS` 셋은 닫힌 자리라 넣지 않았다. **쉰은 하한이다.**

**답 2를 낸 방법.** **모집단** = `apps/**`·`packages/**`의 `.ts(x)` 전수. **바늘** = `readdirSync`를
부르는 파일(디렉터리를 걸어 모집단을 파생하는 자리) — 오늘 **44**다. ⚠️ **정찰은 같은 축을 더 좁은
바늘로 셌고**(계약 파일 395 중 **35** · 한 앱뿐인 것 **27**) 이 답은 그보다 넓다 — **두 수를 한
낱말로 적지 않는다.** ⚠️⚠️ **어느 바늘로 재도 이 답의 값은 수가 아니라 방향에 있다: 라운드 91이
더한 스윕 셋이 전부 한 앱짜리다**(트랙 A의 모바일 · 트랙 B의 어드민 · 트랙 C의 api) — **저장소는
앱 내 스윕을 더 잘 세우게 됐지 형제 앱으로 넓히게 되지 않았다.** ⚠️ **그리고 형제에 0건이던 축
둘 가운데 하나가 오늘 닫혔다**(연타 가드 — 다만 닫힌 것은 **모바일 절반**이고, 어드민의 절반은
`<button>` 여는 태그 **68** 중 `disabled=`를 지닌 것 **44**라는 값과 재개 조건이 트랙 A의 사각에
있다 · ⚠️ **정찰의 29는 하한이었다**). ⚠️ **사각**: `readdirSync` 없이 손 목록으로 모집단을 적은
계약은 이 바늘 밖이고 그런 계약이 훨씬 많다. **마흔넷은 하한이다.**

**답 3을 낸 방법.** **모집단** = 판정 문서 하나 + 확인 문서 둘 + `docs/dev/do-not-change.md`.
**바늘** = 소스에서 `const <대문자> = <정수>` 꼴로 선언된 수치 상수의 **이름과 값을 함께** 적은
문서 줄. ⚠️⚠️ **그리고 오늘 이 답에는 칸이 하나 더 붙는다**: 접근성 표가 `MAX_DIVERGENCE`를
*이름 없이 값만* 적은 **세 자리**는 이 바늘에 걸리지 않는데, **그 세 자리를 오늘 트랙 E가
계약으로 만들었다.** 즉 *"계약이 세는 것 0건"* 은 **이 바늘의 열두 자리**에 대한 답이고,
**그 밖에서 세어지기 시작한 자리가 셋 있다** — 두 수를 한 낱말로 적으면 이 축이 오늘 닫힌 것으로
읽힌다. ⚠️ **사각**: 문자열 상수의 **값**(문구 전문)은 이 바늘 밖이다(이름만으로도 **321**이다).
**열둘은 하한이다.**

**답 4를 낸 방법.** **모집단** = `apps/**`·`packages/**`의 `.ts(x)` 전수. **바늘** =
*"이 대장/스윕 자신의 파일은 모집단 밖"* 이라는 **같은 한 문장**을 손으로 적은 선언
(`<이름> = ` 꼴). ⚠️⚠️ **AE-4는 그 관례를 배열 export 둘의 이름으로만 셌고(여섯), 바늘을 선언
전수로 넓히니 열한 자리 · 이름 넷이다** — 스칼라로 적은 자리(`SELF_PATH`·`SELF_FILE`)가 다섯 더
있었다. **열한 자리가 같은 사실을 적는데 그 정합을 세는 자리는 0건이고, 이름이 넷으로 갈렸다는
사실 자체가 아무도 안 센다는 증거다.** ⚠️ **사각**: 이 바늘은 **선언 없이 인라인으로 자기를
거르는 자리**(`if (file === __filename)` 꼴)를 못 본다. **열하나는 하한이다.**

**답 5를 낸 방법.** **모집단** = AE절이 값으로 남긴 기각 **열여섯**. **바늘** = 줄마다 ⓐ 전제를
오늘 실측으로 다시 재고 ⓑ *막는 것*(규율 · 배정 · 래칫 · 손의 크기 · 관례 부재 · 다른 사람의
결정)을 **낱말로** 적었는지 본다. ⚠️⚠️ **라운드 90 F는 *"오늘 결정형 넷에 대해 답했고 나머지 기각
열둘은 그 칸이 아직 없다"* 고 적었는데, 세어 보니 여덟이다** — **기각을 쓰는 걸음이 이미 그 칸을
여덟 자리에 채우고 있었고 F는 자기가 채운 칸을 세지 않았다.** ⚠️ **AE-5가 이름 붙인 병의
거울상이다**(값이 이미 적혀 있는데 그 값을 세는 걸음이 없으면 없는 것으로 읽힌다).
⚠️ **사각**: *막는 것*을 **낱말로** 적었는지만 보므로, 산문이 그 뜻을 말하고 낱말을 안 쓴 자리는
세지 않는다. **여덟은 하한이다.**

⚠️⚠️ **AA-3의 이행 — 결정형 재개 조건 전수와 오늘의 처분이다. 라운드 90의 열셋에서 닫힌 것은 0건이고,
오늘 HEAD에서 다시 세니 열다섯이다.**
⚠️⚠️ **열셋 → 열다섯도 새 부채가 아니라 *세는 자리가 늘었다*는 뜻이다**: 트랙 C가 자기 사각에
*"공유 테스트 DB를 언제 무엇으로 비울지"* 를, 트랙 D가 소스 축의 새 사각에 *"조건의 도래를 값으로
가르는 관례"* 를 **결정형으로 이름 붙여** 표에 들어왔다. **사각이 조건을 값으로 적으면 그 조건은
다음 표에 선다** — V-2가 세운 규율이 모집단으로 되돌아오는 자리이고, 이것으로 세 라운드 연속이다.
⚠️ **트랙 A·B·E가 새로 적은 재개 조건은 전부 사건형이라 이 표에 오지 않는다**(A 셋 · E 둘 ·
⚠️⚠️ **B는 재개 조건을 한 줄도 적지 않았다** — 라운드 91의 신설 스윕 셋 가운데 유일하고, 그
사실을 아래 기각이 값으로 진다).

| # | 결정형 조건 | 사는 곳 | 손 | 오늘의 처분(2026-08-31 실측) |
| --- | --- | --- | --- | --- |
| 1 | 막대를 포커스 가능하게 | AA-2 | 안 | **집지 않는다 — 값 0.** 두 화면 다 `trend.showTable` 표가 값을 텍스트로 준다(재실측 · 변화 0) |
| 2 | 기록 탭 검색의 분류 갈래 | Z절 | 안 | **집지 않는다 — 어긋남 0건 · 일곱 라운드 연속 같은 답**(placeholder·낭독 라벨이 상수에서 파생하므로 갈릴 자리가 구조적으로 없다) |
| 3 | 준비템 분류 필수 입력 | Z-3 | **밖**(카탈로그 정책) | 집지 않는다. 변화 0 |
| 4 | 서버의 중복 대기 초대 방지 | AA-4 | **밖**(초대 정책) | 집지 않는다. 변화 0 |
| 5 | 감사 뷰의 대상 축 필터 | AA절 기각 | **밖**(서버 DTO 0건) | 집지 않는다 — ⚠️ **다만 이번 라운드는 `apps/api/**`를 다섯 라운드 만에 쓰기로 열었다**(트랙 C · `test/**` 둘) · **서버 소스와 DTO는 여전히 0바이트다** |
| 6 | 타일 안 배지 | AA-1 | **밖**(디자인 승인) | 집지 않는다. 변화 0 |
| 7 | C-3 잠금 오버레이 TalkBack 투과 | 접근성 표 C-3 | **밖**(사람·기기·날짜) | **F의 소유 밖이다** — 오늘로 **스물다섯 라운드째 미확인**(67~91). ⚠️ **오늘 이 줄에 붙는 값은 확인이 아니라 *그 줄이 인용한 계약 상수를 이제 계약이 센다*는 사실이다**(트랙 E의 ⓖ) |
| 8 | JSX 사용을 참조로 세는 판정 | `dead-export-ledger.ts` | 안 | **집지 않는다 — 다섯 라운드 연속이고 이번에는 이유가 다르다.** 종전 넷은 *그물 배정*(다른 트랙이 그 파일을 열었다)이었는데 **오늘 그 파일은 아무 트랙의 것도 아니다** — 막는 것은 **크기와 문턱의 겹침**이다(`.tsx`의 `export function` **141**이 모집단으로 들어오면 같은 걸음에 `derived-exemptions`의 **여유 0** 문턱이 함께 흔들려 한 트랙이 판정 둘을 진다). 전제는 참(**141**) |
| 9 | 재조회 실패 창에서 옛 목록을 남길지 | AC절 기각 | 안 | **집지 않는다 — 화면 구조 결정이고 오늘 접점 0건**(어느 트랙도 `src/offline/**`를 열지 않았다) |
| 10 | 앵커 대장의 `helper-named-reader` | `comment-tolerant-anchor-ledger.ts` | 안 | **집지 않는다 — 래칫이다.** ⚠️⚠️ **다만 오늘 이 줄의 사실 하나가 갈렸다**: 앵커 다섯 자리(**698 · 620 · 70/70 · 8/8 · 0**)와 면제 **8**은 **여섯 번째로 한 자리도 다르지 않은데**, 그 사각의 크기는 **164에서 165**가 됐다(라운드 91의 새 테스트 하나가 헬퍼 이름으로 소스를 읽는다). **"다섯 번 재어 불변"은 앵커 수의 이야기이지 사각의 이야기가 아니었다** |
| 11 | 재개 조건 대장의 `prose-only` | `resume-condition-ledger.ts` | 안(**F**) | **집지 않는다.** 소급 적용은 산문 조건 **184** 자리를 F가 손으로 고치는 일이라 한 라운드의 F가 질 수 없다. ⚠️ 그 수는 줄지 않고 는다(119 → 145 → 165 → **오늘 184** — ⚠️ **늘린 것의 절반이 이 절 자신이다**) |
| 12 | 재개 조건 대장의 `round-notes` | `resume-condition-ledger.ts` | 안 | **집지 않는다 — 관례 0건.** ⚠️ 그 수는 정찰의 **265**에서 오늘 **319**인데, 갈림의 자리가 **정찰 노트 자신**이다(`round91-scout.md` 54자리 · 265 + 54 = 319) — **정찰은 자기가 쓰고 있던 문서를 세지 못했다**(일부러 모집단 밖이다) |
| 13 | 그물 파일이 자기를 이름으로 선언하는 표식 | `contract-net-ledger.test.ts` | 안 | **집지 않는다 — 관례를 새로 정하는 일이다.** ⚠️⚠️ **그 옆에 오늘 모집단 하나가 더 섰다**: 자기 배제 관례를 손으로 적은 자리가 **열하나 · 이름 넷**이고 정합을 세는 자리는 0건이다(AE-4의 답을 넓혀 다시 센 값 — 위 답 4) |
| 14 | **공유 테스트 DB를 언제 무엇으로 비울지** | `apps/api/test/harness-catalog-cost.test.ts`(**오늘 신설**) | 안 | **집지 않는다 — 오늘 선 조건이다.** 트랙 C가 자기 사각에 *"비우는 순간 다른 스위트가 누적 위에서 초록인가가 처음 시험된다"* 를 값으로 적으며 그날을 조건으로 걸었다(오늘 `global-setup.ts`의 정리 **0건**을 **부정 단언**으로 못 박아, 지우는 걸음이 생기는 날 그 자리가 먼저 빨개진다) |
| 15 | **조건의 *도래*를 값으로 가르는 관례** | `resume-condition-ledger.ts`(**오늘 신설**) | 안 | **집지 않는다 — 오늘 선 조건이다.** 트랙 D가 소스 축을 넓히며 *"이 축은 표기의 실재만 세고 그 조건이 오늘 참인가는 묻지 않는다"* 를 사각으로 적고, 그 관례를 세우는 것은 대장이 아니라 라운드의 결정이라고 못 박았다 |

⚠️⚠️ **이 표가 라운드 90의 표와 다른 점 둘.** ⓐ **오늘도 결정형에서 집는 것은 0건이다** — 열다섯 중
손이 안인 열(1·2·8·9·10·11·12·13·14·15) 가운데 전제가 참인 것은 **8·10·11·12·13·14·15 일곱**이고
**일곱 다 *미도래*가 아니다**(크기·래칫·손의 크기·관례 부재·오늘 선 조건). ⓑ ⚠️⚠️ **그런데 이번
라운드의 다섯 트랙 가운데 셋이 *사건형이면서 배정만 남은 조건*에서 나왔다**(B·D·E) — 라운드 90은
그 수가 둘이었다. **결정형 표는 다섯 라운드째 0건인데 사건형 배정은 라운드마다 늘고 있고, 그
갈림이 이 표가 오늘 내는 값이다: 조건의 *형*이 그 조건이 언제 집히는지를 정한다.**

⚠️⚠️ **이번 라운드가 실측하고 기각한 스물을 값으로 남긴다 — 전부 재개 조건과 함께**(V-2).
⚠️ **정찰이 예고한 열여덟에서 둘이 늘었다**: 트랙 A가 자기 사각으로 남긴 **둘**(어드민의 연타 가드
절반 · `useMutation` 밖의 쓰기 경로)과 **신설 스윕이 재개 조건을 0건 적은 자리** 하나가 새로 서고,
AC-1과 AC-2를 한 줄로 묶었다. ⚠️ **그중 여덟은 재개 조건이 *결정형*이라는 사실과 그 결정의 손이
어디에 있는지를 함께 적고**, 손이 밖인 다섯 줄은 다섯 조건을 한 줄에 묶어 **전부 결정형 · 손은
저장소 밖**이라고 적는다.
⚠️ **아래 마지막 두 줄은 그 스물에 들지 않는다** — 라운드 62~90이 남긴 **상태 변화 0의 표식**과
**제외 목록 준수 확인**이고, 재론 대상이 아니라 스윕이 그것들을 다시 줍지 않게 하는 자리다.

- **행마다 갈리는 낭독 라벨의 모집단 — ⚠️⚠️ 세 라운드 연속으로 규율이 아니라 선택이 막는다.**
  ⚠️ **이번 라운드는 a11y 그물(`apps/mobile/src/a11y-contract.test.ts`)을 여는 트랙이 0건이라**
  조건은 또 미도래로 읽힌다 — **그런데 막는 것은 이번에도 규율이 아니라 배정이다**(다섯 트랙 가운데
  둘이 화면이고 셋이 계약인데, 그 계약 셋이 다른 그물을 졌다). ⚠️ **그 모집단은 오늘 다시 재지
  않았다** — 그 파일을 여는 트랙이 0건이라 잴 걸음이 없었고, 정찰의 실측(식으로 짓는 낭독 라벨
  **162**)이 마지막 값이다(O-3 · 옮겨 적은 수는 계약 밖의 사본이 된다).
  ⚠️ **재개 조건(사건형 · 자기 축을 적음 — 그대로): 그 스윕 자체가 한 트랙의 축이 되는 라운드가 서는 날.**
  ⚠️⚠️ **한 번은 배정이고 두 번은 되풀이지만 세 번은 이 저장소가 그 조건을 *영원히 미도래처럼 읽고
  있다*는 뜻이다 — 다음 라운드는 이 자리를 *먼저* 배정해야 하고, 이 줄의 새 값은 수가 아니라 그 문장이다.**
- **어드민의 라우트 표면 — 재었고 실재하는데 집지 않는다.** 모바일에는 `app/**`를 전수 열거하는
  그물이 있는데(라운드 80 D), **어드민의 라우트를 세는 계약은 오늘도 0건**이다. 실측:
  `apps/admin/app/**`의 `page.tsx` **11** ↔ `AdminShell.tsx`의 `NAV_ITEMS` href **11** ·
  **없는 페이지를 가리키는 href 0건 · 내비에 없는 페이지 0건**(오늘 어긋남 0). ⚠️ **집지 않는 이유는
  파일 배타다** — 트랙 B가 `AdminShell.tsx`를 **랜드마크·현재 위치 축으로** 열었고 *한 트랙이 한
  그물에 축 둘을 얹지 않는다*. ⚠️⚠️ **그리고 오늘 그 사실이 더 아프다: 트랙 B가 그 파일에서
  `NAV_ITEMS`를 실제로 순회하는 자리를 고쳤는데도 그 목록이 라우트와 맞는지는 여전히 아무도 세지
  않는다.** ⚠️ **재개 조건(사건형 · 자기 축을 함께 적는다): 어드민의 *라우트 표면*이 한 트랙의 축이
  되는 라운드가 서는 날 — 그날의 첫 모집단은 `page.tsx` 열하나와 `NAV_ITEMS` 열하나다.**
- **어드민 `<section>` 34에 이름을 주지 않는다 — 재었고 그것이 판정이다.** 오늘도 **34 · 이름 0건**
  이고, 이름 없는 `<section>`은 랜드마크가 아니라 제네릭이라 결함이 아니다(34에 이름을 주면 랜드마크
  서른넷이 생겨 소리로 훑는 운영자에게 소음이 된다). ⚠️⚠️ **오늘 트랙 B가 연 축과 다르다** — B는
  **이미 랜드마크인 것 다섯**을 둘로 정리하고 **현재 위치**를 세웠지, 새 랜드마크를 만들지 않았다.
  ⚠️ **재개 조건(결정형 · 손은 저장소 안): 한 화면의 카드가 너무 많아 랜드마크로 건너뛰는 길이
  필요해지는 날** — 그날 정할 것은 *이름*이 아니라 **어느 카드가 랜드마크인가**다.
- **어드민의 무명 표 하나 — 재었고 동결된 채로 둔다.** 오늘 재실측 **표 18 · 이름 17**이고, 스윕
  밖의 하나는 `src/components/ProductLinkBulkReplace.tsx:318`이다. ⚠️ **집지 않는 이유는 축이다** —
  표 이름은 라운드 89 B의 스윕이 지는 축이고 트랙 B의 축은 랜드마크·현재 위치다.
  ⚠️ **재개 조건(사건형): 표 이름 스윕의 모집단이 `app/**` 밖으로 넓어지는 날.**
- **`dead-export-ledger.ts`의 결정형 8(JSX 사용을 참조로 세는 판정) — ⚠️⚠️ 전제는 참이고, 다섯
  라운드째 집지 않으며, 이번에는 막는 이유가 다르다.** `.tsx`의 `export function` **141**이 오늘도
  모집단 밖이다. ⚠️ **종전 네 라운드의 이유는 *그물 배정*이었는데 오늘 그 파일은 아무 트랙의 것도
  아니다** — 막는 것은 **크기와 문턱의 겹침**이다: 141이 들어오는 걸음에 `derived-exemptions`의
  **여유 0** 문턱이 함께 흔들려 한 트랙이 *축을 넓히는 판정*과 *문턱을 좁히는 판정* 둘을 지게 된다.
  ⚠️ **막는 것을 값으로 적는 것이 AE-5의 처방이고 이 줄이 그 이행이다.**
...
- **`derived-exemptions`의 절반 문턱 — 재었고 여유가 0이다.** 사문 전수 **44 중 면제 22**로 **정확히
  절반**이고 조건은 *"넘는 날"* 이라 **아직 아니다**. ⚠️⚠️ **정찰은 *다섯 트랙 어디에도 새
  `export const`가 없어 보이지만 그것은 예상이지 실측이 아니다*라고 적었고, 오늘 F가 다시 재니
  44/22 그대로다 — 예상이 맞았다.** ⚠️ **재개 조건(사건형): 면제가 사문 전수의 절반을 넘는 날.**
- **앵커 대장의 사각 `helper-named-reader` — ⚠️⚠️ 재었고 이 줄의 수가 여섯 라운드 만에 처음
  움직였다.** 앵커 전수 **698**(code-only **620** · comment-tolerant **70**/래칫 70 · comment-only
  **8**/래칫 8 · unanchored **0**) · 면제 **8**은 **여섯 번째로 한 자리도 다르지 않은데**,
  그 사각의 크기는 **164 → 165**다(오늘 새로 선 테스트 하나가 헬퍼 이름으로 소스를 읽는다).
  ⚠️⚠️ **정찰이 *"다섯 번 재어 불변"* 이라고 적은 것은 앵커 다섯 자리이지 사각의 수가 아니었고, F가
  둘을 한 낱말로 읽을 뻔했다** — **불변인 수와 늘어난 수가 같은 줄에 산다.** ⚠️ **집지 않는 이유는
  래칫이다**(모집단이 165만큼 넓어지면 `comment-tolerant` 래칫 70이 첫날부터 전수로 다시 적혀야 한다).
  ⚠️ **재개 조건(결정형 · 손은 저장소 안): 저장소가 소스 리더 이름을 한 벌로 모으는 날, 또는 이
  대장이 헬퍼 정의를 따라가 루트를 푸는 법을 배우는 날.**
- **재개 조건 대장의 `prose-only` 184 · `round-notes` 319 — 재었고 둘 다 집지 않는다.**
  형 표기가 없는 산문 조건은 **119 → 145 → 165 → 오늘 184**로 늘었고(가장 큰 사각은 줄지 않으며,
  ⚠️ **이번에 늘린 것의 절반이 AF절 자신이다** — 판정을 쓰는 걸음이 곧 이 사각을 키우는 걸음이다),
  라운드 노트는
  정찰의 **265**에서 오늘 **319**인데 ⚠️ **그 갈림의 자리가 정찰 노트 자신이다**(265 + 54 = 319).
  ⚠️ **트랙 D가 그 대장을 열었지만 축은 *소스 축의 뿌리*였고 이 둘은 그 트랙의 축이 아니다** —
  *한 트랙이 한 그물에 축 둘을 얹지 않는다.* ⚠️ **재개 조건은 둘 다 결정형 · 손은 저장소 안이고
  종전 그대로다**(소급 적용을 F가 정하는 날 · 라운드 노트가 판정 문서로 승격되는 관례가 서는 날).
- **재개 조건 대장의 `sibling-documents` — 재었고 열하나에서 **열다섯**이 됐으며 미도래다.**
  짝 문서 둘의 재개 조건 자리는 **A~E 머지 시점에 열하나**였고 **F가 쓴 문장들로 오늘 열다섯**이며,
  **형을 괄호로 밝힌 것은 여전히 하나(C-12의 사건형) · 결정형 0건**이다 —
  ⚠️ **오늘 F가 두 짝 문서에 쓴 문장에도 결정형 표기를 세우지 않았다**(세우는 순간 그 대장의 모집단
  판단이 바뀐다). ⚠️ **재개 조건(사건형): 짝 문서 중 하나에 결정형 표기가 처음 서는 날.**
- **그물 파일이 자기를 이름으로 선언하는 표식(결정형 13) · 자기 배제 관례 — 재었고 집지 않는다.**
  표식 **0건**이고, ⚠️⚠️ **그 옆의 모집단은 오늘 커졌다**: 같은 사실(*이 대장/스윕 자신의 파일은
  모집단 밖*)을 손으로 적은 자리가 **열하나**이고 이름이 **넷**으로 갈렸다(`LEDGER_SELF_FILES` 넷 ·
  `SWEEP_SELF_FILES` 둘 · `SELF_PATH` 셋 · `SELF_FILE` 둘) — **그 정합을 세는 자리 0건**이고, 이름이
  넷으로 갈렸다는 사실 자체가 아무도 안 센다는 증거다. ⚠️ **집지 않는 이유는 관례를 새로 정하는
  일이기 때문이다.** ⚠️ **재개 조건(결정형 · 손은 저장소 안 — 종전 그대로): 그물 파일이 자기를
  이름으로 선언하는 표식을 세울지 정하는 날 — 그날 자기 배제 관례 열하나의 이름도 함께 정해진다.**
- **공유 테스트 DB를 비우는 일 — ⚠️ 재었고 트랙 C의 밖으로 잘라 둔 채로 둔다.** 오늘 F의 재실측:
  `wooriai_test` **722MB** · `item_templates` **9,333**(활성 **8,706**) ↔ `wooriai_dev` **26MB** ·
  **62**(활성 62) · `test/global-setup.ts`의 정리 **0건**. ⚠️⚠️ **정찰의 719MB · 9,302 · 8,680은
  이 컨테이너에서 그 시점의 값이었고, 오늘 다시 재니 셋 다 늘었다 — 늘린 것은 라운드 91 자신의
  스위트 실행이다**(누적이 실증됐다는 사실이 이 줄의 새 값이고, *환경마다 다르다*는 사실 자체는
  트랙 C가 자기 사각으로 진다). ⚠️ **집지 않는 이유는 결정이다** — 무엇을 언제 지울지가 *시드 보존
  ↔ 누적 삭제*의 판단이고, **다른 스위트가 누적 위에서 초록인지 아무도 세지 않았다.**
  ⚠️ **재개 조건(결정형 · 손은 저장소 안): 하네스가 *어느 표를 언제 비우는가*를 정하는 날 —
  그날의 첫 모집단은 트랙 C가 적어 둔 여섯 표이고, 먼저 물을 것은 *시드가 만드는 62와 e2e가 만드는
  나머지가 서로 다른 표식을 지니는가*다.**
- **어드민의 연타 가드 절반 — ⚠️ 재었고 트랙 A의 밖으로 잘라 둔다.** 모바일 절반은 오늘 닫혔지만
  (선언 31 · `control-blocks` 27 · `handler-blocks` 1 · `does-not-block` 3), **어드민에는 그 축의
  스윕이 0건**이다. 오늘 실측: 어드민 비테스트 `.tsx`의 `<button` 여는 태그 **68** · `disabled=`를
  지닌 줄 **44**(⚠️ **정찰의 29는 하한이었다**). ⚠️ **집지 않는 이유는 파일 배타와 축이다** —
  트랙 B가 어드민을 랜드마크 축으로 열었고, *한 트랙이 한 앱에 축 둘을 얹지 않는다.*
  ⚠️ **재개 조건(사건형): 연타 가드가 어드민에서 한 트랙의 축이 되는 라운드가 서는 날 — 그날의 첫
  모집단은 그 `<button>` 예순여덟이다**(트랙 A가 자기 사각에 같은 조건을 적어 두었다).
- **`useMutation` 밖의 쓰기 경로 — ⚠️ 재었고 오늘의 바늘 밖이다.** 트랙 A의 모집단은 `useMutation`
  선언이라 **직접 `fetch`·`void` 호출로 서버에 쓰는 자리**는 이 스윕이 보지 못한다. ⚠️ **정찰이
  *"그런 자리가 하나라도 핵심 루프 안에 있으면 그 자리도 이 트랙의 것이다"* 라고 적었고, 트랙 A가
  그 사각을 값과 재개 조건으로 졌다.** ⚠️ **재개 조건(사건형): `useMutation` 밖의 쓰기가 핵심 루프
  안에 서는 날 — 그날 이 스윕의 바늘이 넓어진다.**
- **AC-1의 일곱(형제가 실재하는 계약) · AC-2의 0(호출부 둘 이상의 필드 갈림) — 오늘 다시 돌리지
  않았다.** 어느 트랙도 형제 화면이 갈리는 사건을 만들지 않았고 `client.ts`·`admin-api.ts`의
  시그니처를 늘리지 않았다. ⚠️ **재개 조건 둘 다 사건형이고 종전 그대로다.**
- **조회 실패 창의 모집단 · 응답 필드 축(모바일) · `HouseholdMember.joinedAt` · 기록 탭 검색의 분류
  갈래 · 막대 포커스 가능화 — 다섯 다 판정 그대로이고 상태 변화 0.** ⚠️ **재개 조건 다섯 다 종전
  그대로다**(둘은 결정형 · 손은 저장소 안).
- **이월 다섯(U절 셋 · S-3 · `withdrawn_at`) — 판정 종전 그대로이고 접점만 갱신한다**(위 이월 목록).
  ⚠️⚠️ **S-3의 줄에는 오늘 *연속의 끊김*이 값으로 선다**(두 라운드 연속 열리던 두 파일이 이번에는
  접점 0건이다). ⚠️ **재개 조건은 다섯 다 종전 그대로다.**
- **성능 넷 — ⓐ·ⓒ는 재었고 미도래, ⓑ는 ⚠️⚠️ 세 라운드 만에 값을 얻었고, ⓓ는 오늘 축이 바뀌었다.**
  ⓐ **첫 페인트**(⚠️ 어느 트랙도 `useQuery` 선언을 늘리지 않았다 — 트랙 A가 더한 것은 핸들러 안의
  조건 한 줄이다) · ⓒ **번들**(⚠️ **새 런타임 의존성 0건 — 다섯 트랙 다 기존 import만 쓴다**) ·
  ⓑ **렌더 비용**(⚠️⚠️ **오늘 두 수가 나왔다: 제품 62 ↔ 하네스 8,706** — 위 N-4 문단) ·
  ⓓ **api의 루프**(⚠️⚠️ **트랙 C가 `apps/api/test/**`를 다섯 라운드 만에 열어 전량 조회 둘을
  없앴다 — 왕복 ~229ms와 ~217ms가 SQL 한 문장으로 바뀌었고, 서버 소스는 0바이트다**).
  ⚠️ **재개 조건 넷 다 종전 그대로다.**
- **손이 밖인 다섯 — 넷은 다른 사람이 내릴 결정이고 하나는 배정이다.** 준비템 분류 필수 입력 ·
  서버의 중복 대기 초대 방지 · 감사 뷰의 대상 축 필터 · 타일 안 배지는 **카탈로그 정책 · 초대 정책 ·
  서버 계약 · 디자인 승인**이 각각 선행이고, C-3은 **사람·기기·날짜 배정**이다(오늘로 **스물다섯
  라운드 연속 미확인**). ⚠️ **재개 조건(전부 결정형 · 손은 저장소 밖): 그 결정이 서는 날** —
  ⚠️ **넷과 하나를 한 줄에 적지 않는 이유가 그 칸이다**(넷은 결정을 기다리고 하나는 시간을 기다린다).
- **신설 스윕이 재개 조건을 한 줄도 적지 않은 자리 하나 — ⚠️ 오늘 처음 세어 값으로 남긴다.**
  라운드 91이 세운 앱 내 스윕 셋 가운데 `apps/mobile/src/mutation-press-guard.test.ts`는 사건형 셋을,
  `apps/api/test/harness-catalog-cost.test.ts`는 사건형 셋과 결정형 하나를 적었는데
  **`apps/admin/src/admin-landmark-current.test.ts`는 0건**이다. ⚠️ **결함이 아니다** — 그 트랙은
  자기 축을 **다 닫고** 끝냈으므로 남길 조건이 없었다(중첩 셋 → 0 · `aria-current` 0 → 1).
  ⚠️⚠️ **그런데 *조건을 안 적은 것*과 *적을 것이 없던 것*은 문서에서 똑같이 읽힌다** — AE-5가 이름
  붙인 그 병의 세 번째 얼굴이고, 그래서 여기 값으로 적는다.
  ⚠️ **재개 조건(사건형): 새 스윕이 재개 조건 0건으로 서는 라운드가 다시 오는 날 — 그날 물을 것은
  *왜 없는가*이고, 답이 *다 닫았다*가 아니면 그것이 이 줄의 첫 결함이다.**
- **`${seller}에서 구매하기` 둘 — 제외 확인이지 제안이 아니다.** 준비템 판매처 1:1과 구매 확인
  판매처 라벨은 **영구 기각** 축이다(라운드 62~90 그대로). ⚠️ **재개 조건: 없다 — 스윕이 다시 줍지
  않게 하는 표식이다.**
- **감사 뷰 CSV · 콘텐츠 리비전 편집 표면 · 어드민 손 미러 · `SYNC_STATUS_RETRY_ALL_LABEL` · 지출 입력
  두 화면의 조립 · CSV 왕복 다섯 열 손실 · `refund` 생성 불가 · `link_health`의 `errors` 카운터 ·
  서버 알림 층 · 홈의 손 폴 다섯 · 공유 카드 왕복 · 어드민 카탈로그 전량 조회 · 미출처 틴트 둘 ·
  ...
  상태 변화 0.**
- **제외 목록 준수 확인**: 준비템 목록 **가격 표시**(잠금 · ⚠️ **다섯 트랙 어디에도 가격·링크 수가
  들어가지 않았다** — 트랙 A가 바꾼 것은 **누르는 시점의 가드**이고 링크의 표시·순서·문구는 바이트
  불변이다) · 오프라인 로컬 아이 복구 · 외부 계정/키/자산 · **C-3 잠금 오버레이 낭독**(**스물다섯
  라운드 연속**) · **P-2 법무 대조** · P-3 테스트 건수 자동화 · **표기 방언 통일** · S-4 파기
  `targetId` · 40주 초과 달력 · `onBudgetRelevantChange` · 4가구/`viewedHouseholdId` ·
  **지출→리포트 정확성 축** · **공유 왕복 축** · **준비템 완료율 동기부여** · **준비템 탭 비가상화** ·
  **홈 수치 정합** · **`getHome` 카탈로그 전량 읽기**(⚠️ **트랙 C가 연 것은 하네스의 요청이지 서버의
  읽기가 아니다**) — **전부 오늘도 제안하지 않는다.**

⚠️⚠️ **트립와이어 90 → 91 — 라운드 90이 남긴 래칫·하한을 다섯 트랙과 F가 머지된 뒤 다시 잰
대조표다.** ⚠️ **정찰의 수를 옮겨 적지 않는다** — 아래는 **오늘 HEAD에서 다시 잰 값**이고, 정찰과
갈린 자리는 그 칸이 갈렸다고 적는다(라운드 62 이후 이 표의 관례이고, 다섯 라운드 연속으로 정찰의
전제가 낡았다는 기록도 이 표가 진다).
⚠️⚠️ **시점은 하나다 — *A~F 머지 뒤 · 리뷰 전*.** 라운드 90 리뷰 M-2가 세운 그 규율을 이번에는
표 전체가 처음부터 지킨다(**예외 0건** — 재개 조건 대장의 자리 수도, 접근성 표의 좌표 수도, 짝
문서의 행 수도 전부 F가 자기 걸음을 끝낸 뒤의 값이다). **판정을 쓰는 걸음이 곧 그 대장들의 모집단을
키우는 걸음이라, 이 표는 구조적으로 F 이후에 재실측해야 한다.**

| 자리 | 라운드 90이 남긴 값 | 2026-08-31 HEAD 재실측(**A~F 머지 뒤** · 리뷰 전) | 움직였는가 |
| --- | --- | --- | --- |
| 사문 대장 항목 / 래칫 | 22 / 22 | **22 / 22** | **그대로다** — 어느 트랙도 `dead-export-ledger.*`를 열지 않았다(⚠️ 결정형 8이 다섯 라운드째 집히지 않는데, **그 파일이 아무 트랙의 것도 아닌 첫 라운드**라 막는 이유가 *그물 배정*에서 *크기와 문턱의 겹침*으로 바뀌었다) |
| 사문 대장의 사각 **여덟** | 일곱: 22 · 0 · 226 · 20 · 55 · 141 · 0 | **여덟: 22 · 0 · 226 · 20 · 55 · 105 · 141 · 0** | ⚠️⚠️ **일곱이 여덟이 됐다 — 라운드 90 리뷰 M-3의 `jsx-apostrophe-string-masking`(표면 105 · 하한 60 · 실피해 0)이고, AE절이 그 사실을 미리 적어 두었다**(그래서 AE절의 표는 자기 시점에서 참이고 한 글자도 고치지 않았다) |
| 사문 전수 / 면제 / 모듈 | 44 / 22 / 6 | **44 / 22 / 6** | 그대로다 — ⚠️ **정확히 절반이고 여유 0**이며, 정찰의 예상(*"새 `export const` 0건일 것"*)이 실측으로 맞았다 |
| `CLOSED_BLIND_SPOTS` | 셋 | **셋** | 그대로다 |
| 앵커 대장(앵커 전수 · code-only · comment-tolerant/래칫 · comment-only/래칫 · unanchored) | 698 · 620 · 70/70 · 8/8 · 0 | **698 · 620 · 70/70 · 8/8 · 0** | **그대로다 — 여섯 번 재어 한 자리도 다르지 않다** |
| 앵커 대장의 면제 | 8 | **8** | 그대로다 |
| 앵커 대장의 사각 `helper-named-reader` | 164 | **165** | ⚠️⚠️ **움직였다 — 여섯 라운드 만에 처음이다.** 라운드 91의 새 테스트 하나가 헬퍼 이름으로 소스를 읽는다. ⚠️ **정찰의 *"다섯 번 재어 불변"* 은 앵커 다섯 자리의 이야기였지 이 사각의 이야기가 아니었다**(불변인 수와 늘어난 수가 같은 줄에 산다) |
| 재개 조건 대장(자리 · 괄호 표기 · 손의 위치 · 결정형 · 손 누락 · 소스 표기/괄호 전수) | 293 · 97 · 23 · 24 · 1 · 5/8 | **333 · 111 · 29 · 30 · 1 · 11/15** | ⚠️⚠️ **움직였다** — 이 절 자신이 AF절 다섯과 결정형 열다섯을 더하며 문서의 재개 조건이 늘었고, **트랙 D가 소스 축의 뿌리를 손 하나에서 전수 파생으로 바꿔** 소스 쪽 수가 함께 늘었다. **손 누락은 1 그대로다**(라운드 86 Z-1 · `DECISIVE_HAND_EXEMPTIONS`가 면제하고 `DECISIVE_MISSING_HAND_TODAY = 1`이 그 값을 진다 — ⚠️ **F도 D도 그 줄을 건드리지 않았고, 양쪽이 함께 지켜야 그 자리가 산다**) |
| 재개 조건 대장의 사각 **여덟** | 여섯: 145 · 2 · 265 · 9 · 3 · 12 | **여덟: 184 · 2 · 319 · 15 · 4 · 21 · 11 · 56** | ⚠️⚠️ **여섯이 여덟이 됐다** — 트랙 D가 소스 축을 넓히며 새 사각 둘(`source-notation-existence` · `unmarked-source-prose`)을 값과 하한으로 졌다. ⚠️ **넓힌 축은 넓힌 만큼 새 사각을 진다**(AB-5) |
| 표기를 지닌 소스 파일 / 그 뿌리 | 하나(`dead-export-ledger.ts` · **손으로 적은 목록**) | **셋**(`dead-export-ledger.ts` · `contract-net-ledger.test.ts` · `harness-catalog-cost.test.ts` · **`apps`·`packages` 전수 파생**) | ⚠️⚠️ **움직였고 바늘이 바뀌었다** — AE절이 *"조건은 도래했고 처분은 다음 라운드 몫"* 이라고 적은 그 자리를 트랙 D가 오늘 처분했다. ⚠️ **그리고 셋째가 값이다**: `harness-catalog-cost.test.ts`는 **오늘 처음 생긴 파일**이라 손 목록이었으면 몰랐을 자리다 |
| 계약 그물 수 | 열다섯 | **열다섯** | ⚠️⚠️ **그대로다 — 그물 수가 늘지 않은 두 번째 라운드다.** 트랙 A·B의 앱 내 스윕 둘, 트랙 C의 하네스 계약, 트랙 E의 짝 계약 축이 넷 다 그 수에 들지 않고, **왜 들지 않는지를 각자의 머리말이 값으로 적는다** |
| 반쪽 프롭(짝 · role 단독 · live region 단독) | 일곱(37 · 1 · 6) | **일곱(37 · 1 · 6)** | 그대로다 — a11y 그물을 여는 트랙이 0건이다 |
| 어드민의 상태 문장 | 자리 47 / 파일 14 / 출구 45 / 면제 2 | **47 / 14 / 45 / 2** | 그대로다 — ⚠️ **트랙 B가 세 화면의 태그를 바꿨는데도 sha256 문장 대장이 초록이다**(태그 이름만 바뀌고 문장 바이트는 그대로였다) |
| 어드민의 표 이름 / 무명 표 | 17 / 0(`app/**`) · 스윕 밖 하나 | **17 / 0**(`app/**`) · 스윕 밖 **하나** | 그대로다 |
| 어드민 `<section>` / 이름 | 34 / 0 | **34 / 0** | 그대로다 — 이름을 주지 않는 것이 판정이다 |
| ⚠️⚠️ 어드민 `<main>`(중첩) / `aria-current` | 5(중첩 3) / 0 | **2(중첩 0) / 1** | ⚠️⚠️ **움직였다 — 라운드 90 P3가 자기 축을 이름으로 적어 둔 조건이 오늘 배정돼 닫혔다.** 남은 둘은 셸 하나와 `app/global-error.tsx` 하나(자기 `<html>`을 세우는 자리라 중첩이 아니다) |
| 어드민 라우트 표면(`page.tsx` ↔ `NAV_ITEMS`) | 11 ↔ 11 · 어긋남 0 · **세는 계약 0건** | **11 ↔ 11 · 어긋남 0 · 세는 계약 0건** | 그대로다 — ⚠️ **트랙 B가 그 파일의 그 목록을 도는 자리를 고쳤는데도 세는 자리는 오늘도 0건이다** |
| 접근성 표 → 짝 문서 좌표(자리 · 인용 번호 · 서로 다른 번호 · 표면 · 역방향 · 어긋남) | 92 · 96 · 92 · 33 · 8 · 0 | **93 · 97 · 93 · 34 · 8 · 0** | ⚠️ **움직였다** — F가 A-32(`#107`)를 세우며 넷이 하나씩 늘었다(역방향과 어긋남은 그대로). ⚠️ **라운드 90 D가 이 축을 *하한*으로 세운 덕에 F의 걸음이 그대로 통과했다** |
| 짝 문서(행 수 · 실기기 · 브라우저 · 서버 · 작업) | 165 · 136 · 19 · 9 · 1 | **167 · 137 · 20 · 9 · 1** | ⚠️ **움직였다** — `#166`(실기기 · 트랙 A) · `#167`(브라우저 · 트랙 B) |
| 접근성 표 | A-31까지(`#106`) · Batch 40 | **A-32까지(`#107`) · Batch 41** | ⚠️ **움직였다** |
| 접근성 표 C-3 경과 수(세 자리) | 스물넷 · 스물넷 · 스물넷(갈림 0) | **스물다섯 · 스물다섯 · 스물다섯**(갈림 0) | ⚠️ **움직였고, 세 자리를 함께 올렸다** — 한 자리만 올리면 짝 계약이 빨개진다 |
| `MAX_DIVERGENCE` ↔ 문서가 옮겨 적은 그 상수 | **0** ↔ 문서 세 자리 **0** · **세는 계약 0건** | **0** ↔ 문서 세 자리 **0** · ⚠️⚠️ **세는 계약 하나**(트랙 E의 ⓖ) | ⚠️⚠️ **값은 그대로이고 *누가 보는가*가 움직였다** — AE-3이 적어 둔 재개 조건이 **한 라운드 만에** 이행됐고, **라운드 90 F가 고친 두 자리가 그 계약의 첫 모집단이 됐다** |
| 문서가 인용한 수치 상수(이름+값) | **12자리 / 아홉 이름** · 세는 계약 **0건** | **12자리 / 아홉 이름** · 세는 계약 **0건** | 그대로다 — ⚠️ **트랙 E가 연 것은 *이름 없이 값만* 적힌 접근성 표의 세 자리이고 이 열두 자리는 그 바늘 밖이다**(두 수를 한 낱말로 적지 않는다) |
| ⚠️⚠️ 모바일 `useMutation`(선언 · 컨트롤이 막음 · 핸들러가 막음 · 안 막음) | **세는 자리 0건** | **31 · 27 · 1 · 3**(트랙 전 31 · 27 · 0 · 4) | ⚠️⚠️ **오늘 처음 세어졌다** — 어느 대장도 이름을 준 적 없던 자리가 모집단이 됐다. ⚠️ **정찰의 25는 하한이었다**(*"읽되 막지 않는다"* 로 센 둘이 한 걸음을 거쳐 막고 있었다) |
| ⚠️⚠️ 하네스의 `tab=now` 요청 자리 / *재료 목적*의 전량 조회 | **세는 자리 0건** | **28 / 0**(트랙 전 30 / 2 · 래칫 0) | ⚠️⚠️ **오늘 처음 세어졌다** — `global-setup.ts`의 정리 **0건**을 부정 단언으로 함께 못 박는다 |
| ⚠️⚠️ 공유 테스트 DB / 활성 카탈로그 ↔ 제품 카탈로그 | **오늘 재지 않았다**(세 라운드 연속) | **722MB · 9,333(활성 8,706)** ↔ **62**(`wooriai_dev` 26MB) | ⚠️⚠️ **오늘 처음 값을 얻었다.** ⚠️ **정찰의 719MB · 9,302 · 8,680에서 셋 다 늘었고, 늘린 것은 라운드 91 자신의 스위트 실행이다** — 누적이 실증됐다. **N-4의 판정은 미발동 그대로다** |
| ⚠️⚠️ 자기 배제 관례(자리 / 이름) | **여섯 / 둘**(AE-4의 답) | **열하나 / 넷** | ⚠️⚠️ **움직였다 — 부채가 아니라 바늘이 넓어졌다**(배열 export 둘에서 선언 전수로). **정합을 세는 자리는 오늘도 0건이다** |
| ⚠️⚠️ 접근성 짝 계약의 상수 인용 좌표 | **없던 축** | **`22 · 1223 · 1267`**(E 커밋 시점 `22 · 1153 · 1188`) | ⚠️⚠️ **오늘 처음 섰고, 같은 라운드 안에서 곧바로 낡았다** — 트랙 E가 그 축을 **등호**로 물었고 같은 라운드의 F가 그 문서에 절을 더하자 두 좌표가 밀렸다. **라운드 90 D가 같은 문서의 좌표 축을 *하한*으로 세워 F의 걸음을 통과시킨 것과 정확히 반대의 선택이고, F가 그 한 줄을 함께 옮겼다**(→ AF-5) |

⚠️ **이 표가 라운드 90의 트립와이어 관례에 더하는 것이 셋이다.** ⓐ ⚠️⚠️ **시점 예외가 0건인 첫
표다**(라운드 90은 한 줄이 F 이전 값이었고 그 줄이 스스로 그렇게 적었다). ⓑ **세는 자리가 0건이던
축 셋이 오늘 처음 수를 얻었다**(연타 가드 · 하네스 전량 조회 · 공유 테스트 DB) — 그중 하나는
**세 라운드가 "재지 않았다"로 넘긴 수**였다. ⓒ ⚠️⚠️ **정찰의 수 넷이 트랙과 F의 재실측으로
정정됐다**: 연타 가드의 **25 → 27**(그래서 안 막는 넷 중 하나가 오늘 셋이 됐다) · 어드민
`disabled` **29 → 44** · 라운드 노트 **265 → 319** · 앵커 사각 **164 → 165**. **여섯 라운드 연속으로
정찰의 전제가 낡았고, 정찰이 grep과 손 판정으로 낸 수는 하한이며 갈리면 그 수가 값이라는 관례가
여섯 라운드째 참이다.**

**이 라운드가 짝 문서에 남긴 것.** 확인의 표에 **`#166`~`#167` 둘**이 서고 §0의 여섯 숫자와 머리말의
재계산 줄·§1-1 머리말의 라운드 구간이 파싱으로 다시 세어졌으며, 접근성 표에는 **A-32 `#107` 하나**와
**C-3의 세 자리 갱신**, 그리고 ⚠️⚠️ **라운드 90 리뷰가 보고에만 남긴 잔존 둘의 수리**가 섰다.
⚠️⚠️ **표면 배분은 라운드 88~90과 같다 — 실기기 하나 · 브라우저 하나이고 이번으로 네 라운드
연속이다. 그리고 그 이유가 §0의 수보다 값이 크다**: 줄어든 것은 확인을 미뤄서가 아니라 **다섯 트랙
중 셋이 화면을 한 곳도 열지 않았기 때문**이다(하네스 전량 조회 비용 계약 · 재개 조건 대장의 소스
축 · 문서-상수 정합 짝 계약 — 셋 다 **화면 0건 · 문구 0건 · 렌더 0건**이라 사람이 밟을 자리를
만들지 않는다). ⚠️ **라운드 84가 세운 그 구분**(*"확인할 것이 없다"* 가 아니라 *"폰에 보이는 동작을
한 곳도 바꾸지 않았다"*)**의 여섯째 판이고, 네 라운드 연속이라는 사실이 이 문단의 값이다.**
⚠️⚠️ **다만 라운드 88~90과 다른 점 하나를 함께 적는다 — 이번 라운드의 화면 트랙 둘 가운데 하나는
*아무 대장도 이름을 준 적 없는 자리*에서 나왔다**(트랙 A). 라운드 88·89는 화면 트랙이 새로 발견한
자리에서, 라운드 90은 둘 다 *계약이 자기 사각으로 적어 둔 자리*에서 나왔는데, **오늘은 하나가
사각 칸의 바깥이다** — **표가 세는 수는 같은데 그 수가 어디서 왔는지가 또 달라졌다.**
⚠️ **A-32가 A-31과 다른 점 하나는 그 절의 머리말이 진다**: 라운드 90의 하나는 *사각 칸이 이름으로
지목한 자리를 실제로 고쳤는가*를 물었고, **이번 하나는 *어느 칸도 지목한 적 없는 자리를 어떻게
찾았는가*를 묻는다.** ⚠️ **기기 조건은 A-30·A-31과 다르다 — 폰 한 대면 되고 플랫폼을 가리지
않으며, 대신 *느린 망*이 재료다**(대기 창이 눈에 보일 만큼 길어야 두 번째 탭을 넣을 수 있다).
⚠️⚠️ **그리고 그 행이 처음 지는 물음 하나가 있다 — *가드가 영구 잠금이 아닌가*다.** 라운드 89
리뷰 L-3·L-4가 낭독에 대해 물은 *"두 번 읽히지 않는가 · 다시 읽지 않는가"* 와 같은 자리에 있는
물음이고(막는 것이 옳게 막는가), **막는 계약은 소스가 지지만 *풀리는가*는 폰이 답한다.**
⚠️ **트랙 B의 어드민 항목은 종전 판정대로 행이 아니라 문단으로 적었다**(브라우저 화면은 그 표의
조건 밖이다) — ⚠️ **다만 그 트랙이 고친 것이 정확히 접근성 축이라는 사실은 문단이 진다**: 운영자가
랜드마크 로터에서 `main`을 두 번 듣고, 내비 열한 링크 가운데 지금 어디에 서 있는지 듣지 못했다.
⚠️ **트랙 C·D·E는 소스 계약·하네스라 두 표 어디에도 행이 서지 않는다.**
⚠️⚠️ **C-3(잠금 오버레이 TalkBack 투과)은 오늘로 스물다섯 라운드째 미확인**이고, ⚠️ **이번 라운드가
그 줄에 더하는 값은 경과 수가 아니라 *그 줄이 인용한 계약 상수를 이제 계약이 센다*는 사실이다** —
**라운드 90 F가 고친 그 두 자리가 오늘 계약의 첫 모집단이 됐고, 확인은 여전히 배정을 기다린다.**

### AF-1. **판정 칸도 사각 칸도 이름을 준 적 없는 자리가 남아 있고, 그것을 찾는 법은 대장을 읽는 것이 아니라 저장소가 이미 고른 답을 고르지 않은 자리를 세는 것이다** — 가장 오래 서 있던 자리는 어느 목록에도 없었다

- **사실.** 라운드 88이 프롭 대장에 **판정 칸**을 세웠고, 89가 그 칸이 지목한 자리를 고쳤으며,
  90이 **사각 칸**이 지목한 자리를 고쳤다(AD-1 · AE-1). ⚠️⚠️ **오늘의 트랙 A는 셋 중 어느 것도
  아니다** — 모바일 비테스트 `.tsx` **58**의 `useMutation` 선언 **31** 가운데 **왕복이 끝나기 전의
  두 번째 탭을 막는 자리가 27**이었고, 막지 않는 **넷** 중 하나가 **핵심 루프 4단계**(구매 링크)였다.
  **그 자리를 이름으로 적어 둔 대장·판정 칸·사각 칸은 0건이다.**
- ⚠️⚠️ **해악은 사람이 겪는 잘못된 수였다.** 누르는 자리 둘(판매처 비교 행 · 구매 CTA)이 한
  핸들러로 모이는데, 그 핸들러는 ⓐ 분석 이벤트를 쏘고 ⓑ 서버에 클릭을 기록한 뒤 ⓒ 응답이 와야
  브라우저를 연다. **왕복이 끝나기 전에 한 번 더 누르면 ⓐ와 ⓑ가 한 번 더 돌고, 서버에는 dedupe도
  멱등 키도 없다.** ⚠️ **그 해악의 이름을 서버 소스가 이미 적어 두었다** —
  ...
  연타는 같은 문의 반대쪽이었다.** 그 수가 서는 자리는 어드민 클릭 통계의 순위 카드(DNC-009 고지가
  붙은 그 카드)와 대시보드의 `affiliateClicks7d`다.
- ⚠️⚠️ **답을 발명하지 않고 인용했다는 점이 이 판정의 절반이다.** `if (…isPending) return;`은 이
  저장소에 이미 **일곱 자리**가 서 있었고, 트랙 A가 더한 것은 **여덟째 한 줄**이다.
  ⚠️ **`disabled`를 고르지 않은 이유가 나머지 절반이다** — `PrimaryButton`은 `disabled`면 배경이
  `gray300`이 되어 **대기 창 동안 픽셀이 바뀌고**(그 버튼은 승인 캡처 ITEM-002 · DSN-053의 자리다),
  `ProductComparisonRow`에는 `disabled` 프롭이 **아예 없다**. **눌림을 막는 옳은 형식이 이 자리에서는
  컨트롤이 아니라 핸들러였고, 그 판단을 계약이 부정 단언으로 문다.**
- **오늘의 값 — 한 줄과, 그 한 줄이 옳은 형식임을 파생으로 세우는 스윕 하나다.** ⓐ 화면 변경은
  **핸들러 첫 줄 하나**이고 렌더는 **바이트 불변**이다(화면 한국어 0글자 · 픽셀 0 · 서버 0바이트).
  ⓑ 스윕은 **31**을 전수로 파생하고 자리마다 판정 하나를 **소스에서** 낸다
  (`control-blocks` **27** · `handler-blocks` **1** · `does-not-block` **3**). ⓒ ⚠️ **컨트롤이
  먼저다** — 둘 다 지닌 자리는 `control-blocks`로 세므로 **오늘 `handler-blocks`는 트랙이 더한 그
  하나뿐**이고, 그 하나가 이 트랙의 값이다. ⓓ **`does-not-block` 셋에는 이유가 반드시 붙고 그 이유를
  소스가 증명한다**(빈 이유는 계약이 길이로 막는다).
- ⚠️⚠️ **그리고 이 트랙은 자기 전제를 다시 재어 정찰을 정정했다.** 정찰은 *"막는 것 25 · 읽되 막지
  않는 것 둘"* 이라고 적었는데, 그 둘은 **한 걸음(이름 붙은 불리언)을 거쳐 막고** 있었다 —
  **27이 값이고 25는 하한이었다.** ⚠️ **정찰의 수가 낡는 것이 여섯 라운드 연속이고, 이 트랙은 그
  갈림을 자기 머리말에 값으로 적었다.**
- **일반형.** **판정 칸과 사각 칸은 *이미 본 자리*의 목록이고, 아직 아무도 본 적 없는 자리는 그
  목록 밖에 있다.** ⚠️⚠️ **그 자리를 찾는 법은 오늘 하나 값이 됐다: *저장소가 같은 물음에 이미
  여러 번 고른 답*을 먼저 찾고, **그 답을 고르지 않은 자리를 세는 것**이다.** 형식이 스물일곱 자리에
  서 있으면 그것은 규율이고, 규율에서 빠진 넷은 **의도이거나 구멍이며 둘은 열어 보기 전에는 갈리지
  않는다**(오늘 넷 중 셋은 결함이 아니었고 하나가 핵심 루프였다). ⚠️ **AA-2가 이름 붙인 *옳은 형식이
  같은 저장소에 이미 있다*의 가장 넓은 판이고, 이번에는 형제가 옆 파일이 아니라 스물일곱이다.**
  ⚠️⚠️ **그리고 이 방법에는 판정 칸·사각 칸에 없는 성질이 하나 있다 — 배정표가 필요 없다.** 사각
  칸을 읽으려면 누군가 그것을 적어 두어야 하지만, *고르지 않은 자리*는 **저장소가 스스로 답을 고를
  때마다 자동으로 생긴다.**
  ⚠️ **다음 라운드가 먼저 세어 볼 만한 것**: 이 저장소가 **같은 물음에 세 자리 이상에서 같은 답을
  고른 관례**가 몇이고, **그 관례에서 빠진 자리가 몇인가**(오늘 연타 가드 하나를 그렇게 찾았고,
  ⚠️ 그 관례를 세는 자리는 오늘 **하나**다 — **나머지는 아직 이름조차 없다**).
  ⚠️ **되짚기(라운드 92)** — 이 질문에 답한 자리는 **AG-1**이고, 같은 방법이 낸 둘째 자리는
  **AG-2**다(판정은 여기 옮겨 적지 않는다 · O-3·X-4).

### AF-2. **배정이 막는 조건은 배정하면 열리고, 그 이행은 조건이 자기 축을 적어 둔 라운드의 다음다음 라운드에 온다** — 처방이 통했다는 증거는 처방을 쓴 라운드가 아니라 그 뒤에 온다

- **사실.** 라운드 89 AD-5가 *"조건이 도래할 수 없는 상태"* 와 *"도래할 수 있는데 배정이 없는
  상태"* 를 가르는 처방을 세웠고, 라운드 90 AE-5가 그 구별을 값으로 만들며 **막는 것이 *배정*이라고
  적힌 조건 넷**을 이름으로 남겼다. ⚠️⚠️ **오늘 그중 셋이 배정됐다** — 어드민의 랜드마크와 현재
  위치(트랙 B) · 소스 축 표기의 둘째 파일(트랙 D) · 문서가 인용한 계약 상수의 정합(트랙 E).
  **AE-5의 처방이 실제로 이행된 첫 라운드다.**
- ⚠️⚠️ **셋 다 새 관측이 필요 없었다는 것이 이 판정의 값이다.** 세 조건은 *그날 무엇을 먼저 해야
  하는지*까지 자기 문장에 적어 두었다 — *"그날 그 두 수를 부정 단언에서 옮긴다"* ·
  *"그날의 일은 소스 축의 뿌리 목록을 손 하나에서 전수로 바꾸는 것이다"* ·
  *"그날 F가 이미 고친 그 두 자리가 그 계약의 첫 모집단이다"*. **트랙은 재기만 하고 곧장 집었고,
  집었을 때 계약이 비어 있지 않았다**(AD-3의 두 번째 배당이 세 번 더 참이 됐다).
- ⚠️ **트랙 B의 값은 태그 이름과 속성 하나다.** 셸의 `<main>` 안에서 다시 서던 `<main>` **셋**이
  `<div>`로 내려가고(둘 다 블록 요소 · `style`·`className`·자식 바이트 불변), 활성 내비 링크에
  `aria-current="page"` **하나**가 섰다(값은 손이 아니라 그 줄에서 이미 계산되던 `isActive`에서
  파생한다). **픽셀 0 · 문구 0 · 클래스 0**이다. ⚠️⚠️ **그리고 그 트랙은 전제를 소스로 다시 풀었다** —
  *"셸 안에서 렌더된다"* 는 파일 이름이 아니라 **렌더 트리**의 사실이라, 라우트 그룹·병렬 라우트가
  끼면 뒤집힌다. **그 전제를 주석이 아니라 단언으로 다시 재고**(레이아웃 하나 · 라우트 그룹 0 ·
  병렬 라우트 0), `global-error.tsx`가 자기 `<html>`을 세우는 자리라 모집단 밖이라는 판정도 값으로 적었다.
- ⚠️ **정정 한 줄(라운드 91 리뷰 L-8) — 정찰의 산술이 한 칸 어긋나 있었다.** 라운드 91 정찰이 이
  트랙에 붙인 제목은 *"랜드마크 **다섯이 셋으로** 정리되고 현재 위치가 선다"* 였는데, 같은 노트가
  바로 위에서 적은 사실은 *"`<main>`이 **다섯**인데 그중 **셋**이 셸의 `<main>` 안에서 다시 선다"*
  이다 — 중첩 셋을 내리면 남는 것은 셋이 아니라 **둘**이다(셸 하나 + `app/global-error.tsx` 하나).
  실제 결과도 **둘**이고 트랙 B의 커밋 제목·계약(`OPEN_MAIN_RATCHET = 2`)·A-32 절의 문단이 전부
  둘로 적었으므로, **틀린 자리는 정찰 제목 한 곳뿐이고 어느 판정도 그 수를 물려받지 않았다.**
  ⚠️ **정찰 노트 자체는 고치지 않는다** — 그 라운드의 기록이고, 옛 라운드 문단을 고치는 것은 이
  저장소가 하지 않는 일이다(AA-3 이래의 관례). 정정은 이 한 줄이 진다.
- ⚠️ **트랙 D의 값은 손 목록을 전수 파생으로 바꾼 것이다.** `SOURCE_AXIS_FILES`가 손으로 적은 한
  파일이었는데, 오늘 `apps`·`packages`를 걸어 **셋**이 나왔다 — ⚠️⚠️ **그중 하나는 같은 라운드의
  트랙 C가 오늘 만든 파일이다.** **손 목록이었으면 그 자리는 다음 라운드까지 몰랐을 자리이고, 그
  사실이 이 트랙이 왜 옳았는지의 증명이다**(AB-4가 이름 붙인 착시의 반대 방향).
- ⚠️⚠️ **그런데 오늘 배정되지 않은 넷째가 있다 — 낭독 라벨의 모집단이고, 이것으로 세 라운드
  연속이다.** ⚠️ **막은 것은 이번에도 규율이 아니라 배정이다**(다섯 트랙 중 계약 셋이 다른 그물을
  졌다). **한 번은 그 라운드의 배정이고 두 번은 되풀이지만, 세 번은 이 저장소가 그 조건을 *영원히
  미도래처럼 읽고 있다*는 뜻이다.**
- **일반형.** **조건을 옳게 적는 것과 그 조건이 집히는 것 사이에는 *배정*이라는 한 걸음이 더
  있고, 그 걸음은 문서가 아니라 라운드가 진다.** ⚠️ **오늘 값이 된 것은 그 걸음의 *지연*이다** —
  조건이 자기 축을 적은 라운드(89)가 아니라 그 다음다음(91)에 이행됐고, **셋 다 같은 지연을
  겪었다.** ⚠️⚠️ **그러므로 조건에 적어야 할 것이 하나 더 있다: *언제 배정할 것인가*.** 자기 축을
  적는 것은 *누가 집는가*를 정하고, 그것만으로는 *언제*가 비어 있어 라운드마다 다른 축이 먼저
  배정된다. **오늘의 셋은 배정되기까지 두 라운드를 기다렸고, 넷째는 세 라운드를 기다리고 있다.**
  ⚠️ **다음 라운드가 먼저 세어 볼 만한 것**: 이 문서의 재개 조건 가운데 **전제가 참이 된 뒤 몇
  라운드를 서 있었는지**를 셀 수 있는 것이 몇인가(오늘 셋은 두 라운드 · 하나는 세 라운드였는데,
  **그 수를 조건 자신은 적지 않는다 — 사람이 절을 거슬러 읽어야 나온다**).
  ⚠️ **되짚기(라운드 92)** — 이 질문에 답한 자리는 **AG-4**이고, 이 절이 이름으로 지목한 *배정되지
  않은 넷째*를 집은 자리는 **AG-2**다(판정은 여기 옮겨 적지 않는다 · O-3·X-4).

### AF-3. **세는 걸음이 없으면 이미 적힌 값도 없는 것으로 읽히고, 같은 사실을 적은 자리가 여럿이면 *고쳐진 사실*조차 낡은 채로 남는다** — 정정의 방향이 둘 다 "부채가 늘었다"가 아니었다

- **사실.** 오늘 라운드 90 F의 두 수를 정정한다. ⓐ **문장만 든 사각 열여섯 중 오늘도 기계가 재는
  것은 0이 아니라 다섯**이다(그 사각 칸 *옆*의 단언이 그 자리의 실재를 다시 잰다). ⓑ **기각 중
  *막는 것*을 값으로 적은 것은 넷이 아니라 여덟**이다(기각을 쓰는 걸음이 이미 그 칸을 여덟 자리에
  채우고 있었다). ⚠️⚠️ **둘 다 부채가 는 것이 아니라 *세는 눈이 자기 옆을 못 본* 자리다.**
- ⚠️⚠️ **그리고 같은 병의 세 번째 얼굴이 오늘 문서 밖에서 잡혔다.** 라운드 90 리뷰가 보고에만
  남긴 잔존 둘이 **소스와 판정 문서는 이미 고쳐진 채** 확인 문서에만 살아 있었다:
  ① 낭독이 나는 시점을 *"제출"* 이라고 적던 자리가 접근성 표 A-31 절에 **여섯**(소스는
  `search-draft.ts:48-50`·`PreparationListParity.tsx:459-460`에서 *"350ms 디바운스마다"* 를 값으로
  적고, 짝 문서 `#164` ⓐ도 그 정정을 지녔다) · ② 실패 배너의 거리를 *"세 줄 위"* 라고 적던 자리가
  접근성 표에 **하나**와 짝 문서 `#165`에 **하나**(실측은 **두 줄 위**이고 AE-2 본문은 이미 두 줄로
  서 있었다). **오늘 F가 그 여덟 자리를 고쳤다.**
- ⚠️⚠️ **이 셋의 공통점은 *고칠 것이 없었다*는 것이다** — 값은 이미 옳게 어딘가에 있었고, 없던
  것은 **그 값을 세는 걸음**이었다. ⓐ·ⓑ는 세는 걸음이 없어 *0*으로 읽혔고, ①·②는 고치는 손이
  여럿이라 **한 손이 자기가 아는 자리만 고쳤다**(리뷰가 소스와 판정을, F가 확인 문서를 — 그 사이가
  한 라운드였다).
- ⚠️ **그리고 오늘 그 갈림 하나가 소스에 값으로 남아 있다.** `search-draft.ts:52-53`이
  *"접근성 표 `A-31 #106`의 같은 칸은 아직 *제출*이라고 적는다 … 그 한 자리는 다음 문서 걸음의
  몫이다"* 라고 쓰는데, **오늘 그 문서 걸음이 왔다.** ⚠️⚠️ **그 주석은 이제 그 시점의 기록이고, F는
  제품 소스를 0바이트 고치므로 고쳐 적지 않는다** — **낡은 것이 아니라 *언제 무엇이 참이었는지*를
  남긴 자리다**(AE-3이 세운 두 시점 관례). ⚠️ **그 갈림을 세는 계약은 오늘도 0건이다.**
- **오늘의 값 — 문서 여덟 자리의 수리와, 그 수리가 왜 늦었는지의 판정이다.** ⚠️ **문서가 소스를
  따라가는 방향이지 그 반대가 아니라는 사실을 고치는 자리마다 값으로 적었고**(소스의 좌표를 함께
  적는다), **두 시점으로 적었다**(*"종전 이 자리는 …이었다"* — 옛 문장을 지우는 것이 아니라 언제
  무엇이 참이었는지를 남긴다).
- **일반형.** ⚠️⚠️ **한 사실이 N 자리에 적혀 있으면 그 사실을 고치는 비용은 N이고, 고치는 손이
  M명이면 실제로 고쳐지는 것은 *각 손이 아는 자리*뿐이다.** 라운드 90의 리뷰는 소스와 판정을 알았고
  F는 확인 문서를 알았는데, **그 라운드에는 둘을 잇는 자리가 없었다.** ⚠️ **그래서 *잔존*을 보고에만
  남기는 것은 기록이지 배정이 아니다** — **다음 라운드의 누가 그것을 집는지가 적혀 있지 않으면,
  그 잔존은 사각 칸과 똑같이 아무도 집지 않는다**(AE-1이 사각 칸에 대해 낸 답이 *리뷰의 잔존*에서
  그대로 다시 참이 됐다). ⚠️⚠️ **그리고 그 병은 *세는 걸음의 부재*와 같은 병이다: 값이 어딘가에
  있다는 사실이 그 값이 모든 자리에서 참이라는 뜻은 아니다.**
  ⚠️ **다음 라운드가 먼저 세어 볼 만한 것**: 이 저장소의 **리뷰가 보고에만 남긴 잔존**이 몇이고,
  그중 **다음 라운드의 누가 집는지가 적혀 있는 것이 몇인가**(오늘 둘을 F가 집었고 **둘 다 배정은
  적혀 있지 않았다** — 정찰이 그것을 다시 발견해 F에게 넘겼다).
  ⚠️ **되짚기(라운드 92)** — 이 질문에 답한 자리는 **AG-4**다(판정은 여기 옮겨 적지 않는다 ·
  O-3·X-4).

### AF-4. **하네스가 보는 세계와 제품이 보는 세계가 갈릴 때, 세 라운드가 "재지 않았다"로 넘긴 수는 하나가 아니라 둘이었다** — 한 낱말로 적었으면 판정이 뒤집혔다

- **사실.** 라운드 88·89·90이 성능 넷의 ⓑ(렌더 비용 · 활성 카탈로그 수)를 *"실 PostgreSQL을
  요구해 오늘 재지 않았다"* 로 세 번 넘겼다. ⚠️⚠️ **오늘 트랙 C가 `apps/api/test/**`를 다섯 라운드
  만에 열며 그 수를 재게 됐고, 재니 수가 *둘* 나왔으며 둘이 서로 다른 사실이었다**: 제품이 보는
  카탈로그는 **62**(시드가 만드는 수 · `wooriai_dev`)이고, 하네스가 보는 카탈로그는
  **8,706**(`wooriai_test`의 활성 준비템 · 전체 9,333 · DB 722MB)이다.
- ⚠️⚠️ **N-4의 문턱 200을 넘는 것은 뒤쪽 하나뿐이고, 그 수는 부채가 아니라 하네스의 누적이다.**
  `test/global-setup.ts`는 연결 확인 → 마이그레이션 → 시드만 하고 **정리를 0건** 하므로, 라운드마다
  e2e가 만든 준비템이 그대로 남는다. **N-4는 오늘도 미발동이고, 그 판정을 이 관측이 바꾸지 않는다** —
  ⚠️ **그런데 *"활성 카탈로그가 8,706이다"* 만 적으면 N-4는 오늘 발동한 것으로 읽힌다.**
- ⚠️ **그 누적이 오늘 실증됐다.** 정찰이 같은 컨테이너에서 읽은 값은 **719MB · 9,302 · 8,680**
  이었는데 F가 다시 재니 **722MB · 9,333 · 8,706**이다 — **늘린 것은 라운드 91 자신의 스위트
  실행이다.** ⚠️⚠️ **한 라운드의 검증 한 벌이 공유 DB를 3MB · 준비템 서른한 건만큼 키운다는 사실은
  추정이 아니라 두 시점의 실측이고, 그 사실이 이 판정의 값 절반이다.**
- **오늘의 값 — 왕복 둘과, 그 비용을 세는 계약 하나다.** ⓐ 재료 하나(품목 id)를 얻으려 전량
  카탈로그를 받던 자리 **둘**이 SQL 한 문장이 됐다(~**229ms**와 ~**217ms**의 왕복이 사라졌고,
  **단언·기대값·나머지 요청은 바이트 그대로**다 — 라운드 82가 같은 축에서 세운 그 구별이다).
  ⓑ 계약은 `tab=now` 요청 자리를 **전수로** 걸어 자리마다 *목록이 시험 대상인가*를 값으로 가르고
  (오늘 **28 / material 0**), **판정표에 없는 자리는 `material`로 센다**(fail-closed).
  ⓒ ⚠️ **`global-setup.ts`의 정리 0건을 *부정 단언*으로 못 박았다** — 지우는 걸음이 생기는 날 그
  자리가 **먼저** 빨개져 사람이 P3의 결정을 보게 된다.
- ⚠️ **그리고 이 트랙은 자기가 못 보는 것을 값으로 든다** — **DB의 크기를 세지 않는다**(대조하면
  소스 계약이 환경에 매인다) · **응답 시간을 세지 않는다**(기계의 부하에 매여 간헐적으로 빨개진다) ·
  **`?tab=` 밖의 전량 조회는 바늘 밖이다.** ⚠️⚠️ ***환경마다 다르다*는 사실 자체를 사각으로 적은
  것이 이 계약의 값이다** — 정찰이 그 위험을 이름으로 지목했고 트랙이 그대로 졌다.
- **일반형.** ⚠️⚠️ **"재지 않았다"가 라운드를 건너면 그 자리는 *하나의 미지수*로 읽히는데, 실제로
  재 보면 미지수가 여럿인 경우가 있다.** 오늘 그 여럿은 **제품과 하네스**였고, 둘은 같은 표를 보고도
  **140배 다른 세계**를 본다. ⚠️ **그리고 그 갈림은 문턱을 지닌 판정(N-4)을 정면으로 위협한다** —
  문턱은 *사용자가 보는 목록*을 재려고 세운 것인데 하네스의 수를 넣으면 조용히 발동한다.
  ⚠️⚠️ **그래서 *재지 않았다*를 적을 때 함께 적어야 하는 것은 *무엇을 재려 했는가*이고**, 그것이
  적혀 있으면 재는 날 두 수가 나와도 **어느 쪽이 그 판정의 수인지** 알 수 있다. **오늘은 적혀 있어서
  판정이 살았다.**
  ⚠️ **다음 라운드가 먼저 세어 볼 만한 것**: 이 저장소의 문턱을 지닌 판정 가운데 **그 문턱이 재는
  대상이 *제품인지 하네스인지*를 값으로 적은 것이 몇인가**(오늘 N-4 하나가 그렇게 적혀 있어 살았고,
  ⚠️ **나머지가 몇이고 어느 쪽인지는 오늘 세지 않았다**).
  ⚠️ **되짚기(라운드 92)** — 이 질문에 답한 자리는 **AG절 기각 목록의 첫 줄**(*문턱이 재는 세계*)
  이고, 그 답은 트랙이 되지 않았다(판정은 여기 옮겨 적지 않는다 · O-3·X-4).

### AF-5. **같은 관례가 여러 파일에 손으로 적히면 이름부터 갈리고, 그 갈림은 관례를 세운 자리에서 가장 늦게 보인다** — 그리고 문서를 무는 계약이 좌표를 등호로 물면 그 문서를 자라게 하는 걸음이 곧 그 계약을 깬다

- **사실.** AE-4가 *자기 배제 관례*(**이 대장/스윕 자신의 파일은 모집단 밖**)를 **여섯 자리 ·
  이름 둘**로 셌다. ⚠️⚠️ **오늘 바늘을 선언 전수로 넓혀 다시 세니 열한 자리 · 이름 넷이다** —
  `LEDGER_SELF_FILES` **넷** · `SWEEP_SELF_FILES` **둘** · `SELF_PATH` **셋** · `SELF_FILE` **둘**.
  **열한 자리가 같은 한 문장을 적는데 그 정합을 세는 자리는 오늘도 0건이고, 이름이 넷으로 갈렸다는
  사실 자체가 아무도 안 센다는 증거다.**
- ⚠️ **AE-4가 여섯으로 센 것이 틀린 것이 아니라 그 바늘이 *배열 export 둘*만 봤기 때문이다** —
  스칼라로 적은 자리 다섯이 그 밖에 있었다. ⚠️⚠️ **그리고 이 정정의 방향도 AF-3과 같다: 부채가
  는 것이 아니라 세는 눈이 넓어졌다.**
- ⚠️⚠️ **그리고 오늘 그 병의 거울상 하나가 계약에서 잡혔다.** 트랙 E가 세운 새 축(ⓖ)은 **접근성
  표가 이 계약의 상수를 산문으로 인용한 세 자리**를 문는데, 그 자리를 **줄 번호 등호**로 못 박았다
  (`22 · 1153 · 1188`). ⚠️ **같은 라운드의 F가 그 문서에 A-32 절과 잔존 수리를 적자 아래 두 좌표가
  밀렸고, 그 등호가 곧바로 빨개졌다** — **값(0)도 꼴(인용+정정 · 단언 · 단언)도 한 바이트 바뀌지
  않았는데 좌표만 움직였다.**
- ⚠️⚠️ **그 선택이 라운드 90 트랙 D의 선택과 정확히 반대라는 것이 이 판정의 값이다.** D는 **같은
  문서의 좌표 축**을 세우며 하한(`>= 91` 꼴 넷)을 골랐고, 그래서 오늘 F가 절 하나를 더해 자리가
  92 → 93으로 늘어도 **그 계약은 초록으로 남았다.** E는 등호를 골랐고, **같은 라운드 안에서 F의
  걸음에 깨졌다.** ⚠️ **어느 쪽이 옳다는 판정이 아니다** — 등호는 *자리가 사라지거나 새로 생기는
  것*까지 잡고 하한은 못 잡는다. ⚠️⚠️ **적어야 하는 것은 그 선택의 *비용*이다: 등호로 물면 그
  문서를 자라게 하는 손이 라운드마다 그 줄을 함께 옮겨야 하고, 그 의무가 계약 안에 적혀 있지
  않으면 다음 F는 그것을 *결함*으로 읽는다.** **그래서 오늘 F는 그 한 줄을 옮기며 두 시점과 그
  의무를 주석으로 함께 적었다.**
- **오늘의 값 — 좌표 한 줄과, 그 한 줄이 왜 라운드마다 움직이는지의 판정이다.** ⚠️ **F가 계약
  소스에서 만진 것은 그 배열 하나와 그 위의 주석뿐이고**(값·꼴·하한·바늘은 바이트 불변),
  **`runtime-checklist-shape.test.ts`의 라운드 트립와이어 한 줄**(`"90"` → `"91"`)과 함께
  **이 라운드의 F가 소스에서 옮긴 두 줄 전부**다.
- **일반형.** ⚠️⚠️ **손으로 적은 좌표는 손으로 적은 목록과 같은 병을 앓는다** — 목록은 *이름이
  갈리고*, 좌표는 *자리가 밀린다*. **둘 다 자기가 낡았다고 말하지 않는데, 차이는 하나뿐이다:
  좌표를 등호로 물면 계약이 대신 소리를 지르고 목록은 조용하다.** ⚠️ **그러므로 문서를 무는 계약이
  좌표를 물 때 물어야 할 것은 *그 좌표가 맞는가*가 아니라 **누가 언제 그것을 옮기는가**이고, 그
  답은 계약 안에 적혀 있어야 한다.** ⚠️⚠️ **오늘 이 저장소에는 그 두 답이 나란히 서 있다** —
  하한을 고른 축(D)은 F의 걸음을 통과시키고, 등호를 고른 축(E)은 F에게 한 줄의 의무를 지운다.
  **둘 다 옳을 수 있지만, 그 의무가 적혀 있지 않으면 뒤쪽은 다음 라운드에 *깨진 계약*으로 읽힌다.**
  ⚠️ **다음 라운드가 먼저 세어 볼 만한 것**: 이 저장소의 계약 가운데 **문서의 좌표(줄 번호·행
  번호·절 이름)를 등호로 무는 것이 몇이고, 그중 *누가 언제 그것을 옮기는지*를 값으로 적은 것이
  몇인가**(오늘 하나를 찾아 옮기며 그 의무를 적었고, ⚠️ **그런 계약이 몇인지는 오늘 세지 않았다** —
  이 문서와 짝 문서 둘을 무는 계약만 해도 셋이다).
  ⚠️ **되짚기(라운드 92)** — 이 질문에 답한 자리는 **AG-5**다(판정은 여기 옮겨 적지 않는다 ·
  O-3·X-4).

## AG. 라운드 92에서 확정한 판정 (2026-08-31 · GAP-092 트랙 F)

라운드 91이 물은 것이 **어느 대장도, 어느 사각 칸도 이름을 준 적 없는 자리는 어떻게 찾는가**
(답: 저장소가 이미 고른 답을 고르지 않은 자리를 센다 · AF-1)였다면, 라운드 92의 물음은 그 답을
**한 겹 더 들어간 자리**에 있다 — ⚠️⚠️ **저장소가 그 답을 고른 자리를 세는 계약이 *이미 있는데도*
빠진 자리가 보이지 않는 경우는 무엇인가.** 오늘의 답은 **그 계약의 모집단이 손 목록일 때**이고,
그렇게 찾은 자리 여덟이 **핵심 루프 1단계(지출 기록) 안에서 사람이 겪는 *눌렀는데 반응이 없다***
였다. 축은 라운드 81~91과 같이 **사용자 가치**였고(지출 기록의 첫 탭 · 낭독의 모집단 · 운영자
도구 · 저장소가 자기를 세는 자리), 다섯 판정 다 K~AF절과 같이 **결함 보고가 아니라 다음 결정의
입력**이며 2026-08-31 소스에서 확인됐다(라운드 92 트랙 A·B·C·D·E 머지 후).
⚠️⚠️ **이번 라운드는 핵심 루프의 1단계를 열었고, 연 것은 낭독도 렌더도 아니라 *터치가 전달되는
경로*다** — 화면 문구·픽셀·가격·링크는 바이트 불변이다.

⚠️⚠️ **이번 라운드의 가장 값진 관측: 관례를 세는 계약이 있어도 그 계약의 모집단이 손 목록이면
관례에서 빠진 자리는 구조적으로 보이지 않고, 같은 라운드의 형제 계약이 이미 그 교훈을 배운
경우에도 그렇다**(AG-1). 라운드 65는 형제 블록 **둘**을 같은 라운드에 세웠다 — GAP-065 **#6**
(스크롤 스캐폴드의 키보드 계약)과 **#7**(공유 프리미티브의 터치 타깃). ⚠️ **#7은 *"화면 목록을
사람이 적어 두지 않는다"* 를 배워 손 목록 여덟을 전수 스캔으로 바꿨고 그 파일이 스스로 그 이유를
적는다**(*"이 계약이 그 줄을 한 번도 읽지 않았다"*). **#6의 `scaffolds`는 오늘까지 손으로 적은
셋이었다.** 그 모집단 밖에 **여덟 자리**가 서 있었고, 여덟이 전부 지출 기록·지출 수정·기록 탭의
**가로 칩 줄**이며 **다섯은 타이핑 중에 누르라고 만든 줄**이다. ⚠️⚠️ **한 라운드가 같은 교훈을
형제 하나에만 적용했고, 그 갈림이 스물일곱 라운드를 살아남았다.**

⚠️⚠️ **두 번째 관측: 배정이 막는다는 진단이 옳았는지는 배정해 봐야 알고, 오늘 배정하니 고칠 것이
0건이었다**(AG-2). AF-2가 낭독 라벨의 모집단을 이름으로 지목하며 *"세 번은 이 저장소가 그 조건을
영원히 미도래처럼 읽고 있다는 뜻이다 — 다음 라운드는 이 자리를 **먼저** 배정해야 한다"* 고 적었고,
**오늘 먼저 배정했다.** ⚠️ **재어 보니 그 축은 오늘 값으로 통과했다** — 식으로 짓는 낭독 라벨
**162** 가운데 목록 콜백 안이 **35**이고, 행 변수를 직접 쓰는 것 **34** · 한 걸음 파생 **1** ·
**행마다 갈리지 않는 자리 0건**이다. ⚠️⚠️ **그러므로 세 라운드를 미룬 이유는 *고칠 것이 많아서*가
아니었고, 그 사실은 배정하기 전에는 알 수 없었다** — **진단(*막는 것은 규율이 아니라 배정이다*)이
옳았다는 증명은 배정 자체이지 배정 전의 어떤 관측도 아니다.**

⚠️⚠️ **세 번째 관측: 조건을 세우는 손에 *리뷰*가 있고, 리뷰가 세운 조건은 F의 표가 닫힌 뒤에 오므로
구조적으로 다음 라운드 정찰이 처음 센다**(AG-3). AF절의 결정형 전수표는 **열다섯**이었는데 오늘
**열여섯**이고, **열여섯째를 세운 것은 트랙도 F도 아니라 라운드 91 리뷰 L-6**이다
(`apps/admin/src/admin-landmark-current.test.ts` — *"라우트 표면과 내비 표의 짝을 어느 계약이
지는지를 P3가 정하는 날"*). ⚠️⚠️ **그리고 오늘 그 하나를 집는다** — **결정형 표에서 집는 것이 다섯
라운드 연속 0건이었고 그 연속이 오늘 끊긴다**(#16 · 트랙 C). ⚠️ **집는 결정이 작았다는 것이 이
관측의 절반이다**: 정할 것은 *어느 계약이 지는가* 하나였고 조건이 이미 답을 가리키고 있었다.
⚠️ **#14는 집지 않지만 그 조건이 *먼저 물으라*고 적은 질문에는 오늘 답이 났다** — *"시드가 만드는
62와 e2e가 만드는 나머지가 서로 다른 표식을 지니는가"* 에 `psql` 읽기로 답이 **예**로 나왔다
(**62 ↔ 9,340 · 겹침 0**). **조건이 *무엇을 먼저 물을지*를 적으면 집지 않는 라운드도 그 조건을 앞으로
민다.**

⚠️⚠️ **네 번째 관측: 조건도 주석도 자기가 몇 라운드째 서 있는지를 적지 않고, 그래서 *미도래*와
*오래 낡음*이 문서에서 같은 낯으로 읽힌다**(AG-4). 재개 조건 자리 **333** 가운데 **그 줄 자신이
경과를 적은 것은 셋**이고, ±5줄 창까지 넓혀도 **70**이며 **나머지 263은 어디에도 없다**(트랙 D
커밋 시점의 실행값). ⚠️⚠️ **그리고 이 절 자신이 조건을 더한 뒤의 오늘 값은 383 · 4 · 83 · 300이라,
두 시점을 한 낱말로 적지 않는다** — **판정을 쓰는 걸음이 곧 그 대장의 모집단을 키우는 걸음이다.**
⚠️⚠️ **그리고 같은 병의 소스판 하나가 오늘 잡혔다**: `app/expenses/new.tsx`의 `merchantFocused`
주석이 *"이 화면의 스크롤러(`src/ui.tsx` AppScreen)는 `keyboardShouldPersistTaps` **기본값
(`"never"`)** 이라…"* 라고 적는데, **그 문장은 라운드 65에 거짓이 됐고 오늘까지 스물일곱 라운드를
서 있었다**(`src/ui.tsx`는 그때부터 `"handled"`이고, **그렇게 바꾼 라운드의 계약 주석이 바로 이
`merchantFocused` 주석을 자기 근거로 인용한다** — **인용한 손이 인용당한 자리를 고치지 않았다**).
⚠️ **결론(*칩을 blur에서 접지 않는다*)은 오늘도 옳고 이유가 바뀌었다** — 첫 탭을 먹는 것은 바깥
스캐폴드가 아니라 **그 칩 줄 자신의 중첩 `ScrollView`** 다. **그 낡음이 이 자리를 *이미 해결된
것*으로 읽히게 했다.**

⚠️⚠️ **다섯 번째 관측: 문서를 무는 계약이 좌표를 등호로 물 때 적어야 하는 것은 좌표가 아니라
*누가 언제 그것을 옮기는가*이고, 그 의무가 계약 안에 값으로 서면 F의 걸음은 기억이 아니라
이행이 된다**(AG-5). AF-5가 그 처방을 적었고, 오늘 트랙 E가 짝 계약 둘의 좌표 자리를 **자기
소스에서 전수로 파생해**(절 이름 등호 **여덟** · 줄 번호 배열 **하나**) 자리마다 그 의무를 적었으며
**의무가 빈 자리 0건**을 값으로 문다. ⚠️⚠️ **그리고 그 계약이 오늘 F에게 실제로 한 줄을 지웠다** —
F가 A-33 절과 C-3의 세 자리를 올리자 `[22, 1223, 1267]`이 `[22, 1281, 1333]`이 됐고, F가 그 한 줄을
함께 옮겼다. ⚠️ **라운드 91 F의 첫 이행은 *주석이 부탁한* 걸음이었고 오늘의 두 번째 이행은 *계약이
값으로 진* 걸음이다** — **둘 다 같은 한 줄을 옮겼는데, 잊었을 때 무엇이 빨개지는지가 달라졌다.**

⚠️⚠️ **이월 다섯은 전부 보류 유지이고 재실측 값만 갱신했다 — 갱신 한 줄씩은 그 판정이 사는 절에 있다**
(다음 라운드가 같은 실측을 다시 돌리지 않도록 여기서는 자리만 가리킨다).

- **이 스캐너가 쿼리로 분류한 자리의 낭독** — 재실측 상태 변화 0, A-20 #85 선행 → **U절 머리말**.
  ⚠️⚠️ **이번 라운드에는 이 줄에 갈래가 하나 생긴다**: 트랙 B가 **a11y 그물을 열었지만 그 여섯 화면의
  축이 아니었다** — B가 연 것은 *행마다 갈리는 낭독 라벨의 모집단*이고, 이 줄이 기다리는 것은
  *쿼리로 분류한 여섯 화면의 낭독*이다. ⚠️ ***"열지 않았다"* 와 *"열었지만 그 축이 아니다"* 는 다른
  문장이고**(라운드 87이 세운 구별), **접점 0건으로는 네 라운드 연속이다.**
- **`monthly_wrapup`의 달 이동 구멍** — 게이트가 읽는 것은 여전히 대기 행의 바뀐 뒤 날짜 하나 → **U-3**
  (⚠️⚠️ **`src/notifications/**`가 이번에도 접점 0건이고, 이것으로 다섯 라운드 연속이다**).
- **S-3(어드민 `disabled`)** — 판정 종전 그대로, 브라우저 확인 `#130` 선행 → **U절 머리말**.
  ⚠️⚠️ **그리고 라운드 91이 처음 센 *연속의 끊김*이 오늘 둘이 된다**: `app/items/page.tsx`와
  `app/links/page.tsx`는 라운드 89·90에 두 라운드 이어 열렸다가 라운드 91에 접점 0건이 됐고,
  **이번 라운드에도 0건이다**(트랙 C가 연 어드민 파일은 신설 계약 하나와 사각 한 칸이고 그 둘은
  그중에 없다). ⚠️ **그러므로 오늘의 정확한 문장은 *"끊긴 채로 두 라운드"* 이고, *끊김*과 *끊긴
  기간*은 또 다른 두 칸이다.**
- **`withdrawn_at`** — 저장소 전체 셋 · 파일 둘, 컬럼 신설은 여전히 별도 결정 → **U절 머리말**
  (⚠️ **이번 라운드도 이 수를 다시 재지 않았다** — 어느 트랙도 그 두 파일을 열지 않았고, 라운드 88이
  자기 정정으로 확정한 값을 여기서 옮겨 적지 않는다 · **네 라운드 연속 재측정 대상이 아니다**).
- **AA-R ① 연속 실패 재판정** — deps는 오늘도 `[isError]` 하나 · 자동 재시도·폴러 **0건** → **AA-R ①**
  (⚠️ 어느 트랙도 그 훅도 그 훅의 호출부 화면도 열지 않았다).

**다섯 다 2026-08-31 재실측이고 상태 변화 0이다**(`withdrawn_at`은 재실측 대상이 아니었다는 사실이
그 줄의 값이다). ⚠️⚠️ **이번 라운드의 접점 지도가 라운드 91과 다른 점은 하나다: 낭독 스캐너 줄이
*접점 0건*에서 *접점은 있는데 축이 다르다*로 옮겨 갔다.** 접점의 유무도, 접점의 축도, 그 축이
라운드마다 같은지도, 연속이 끊겼는지도, 끊긴 채로 몇 라운드인지도 서로 다른 사실이다.

⚠️ **N-4의 두 문턱은 오늘로 15라운드 연속 미발동이고, 준비템 탭 비가상화는 이번에도 제안하지
않는다** — ⚠️⚠️ **오늘도 두 수가 나왔고 둘이 서로 다른 사실이다**: 제품이 보는 카탈로그는 **62**
(시드가 만드는 그 수 · `wooriai_dev` 24MB · 활성 62)이고, 하네스가 보는 카탈로그는 **8,765**
(`wooriai_test`의 활성 준비템 · 전체 **9,402** · DB **732MB**)다. ⚠️⚠️ **문턱 200을 넘는 것은 뒤쪽
하나뿐이고 그 수는 부채가 아니라 하네스의 누적이므로 N-4는 오늘도 미발동이며, 이 문단이 그 판정을
바꾸지 않는다.** ⚠️ **어드민 카드가 세는 수는 여기 옮겨 적지 않는다**(O-3 · 갱신 이력은 N-4에 있다).
⚠️⚠️ **그리고 라운드 91 F의 722MB · 9,333 · 8,706에서 셋 다 또 늘었는데, 이번에 늘린 손은 트랙이
아니라 *리뷰의 실행*이다** — 라운드 91 적대 리뷰가 스위트를 돌린 것이 공유 DB를 **10MB · 준비템
예순아홉 건**만큼 키웠다. **한 라운드의 검증 한 벌이 공유 DB를 얼마나 키우는지는 이제 세 시점의
실측이고, 그 수를 움직이는 손에 트랙 · F에 이어 리뷰가 더해졌다.**

⚠️⚠️ **AF-1~AF-5가 남긴 질문 다섯 전수와 오늘의 답이다 — 다섯 다 답했고 셋이 발동했다**(AF-1·AF-2·AF-5).
⚠️ **수치는 여기 옮겨 적되 *그 수를 어떻게 냈는지*(모집단·바늘·사각)를 반드시 함께 가리킨다**
(AC-5·AD-4가 세운 그 규율). ⚠️⚠️ **그리고 이 답들은 계약이 아니라 손으로 잰 수다** — 계약이 세는
수와 한 낱말로 적지 않는다. ⚠️⚠️ **다섯 중 하나가 라운드 91 F의 수를 정정하고, 그 정정은 이번에도
*부채가 늘어서*가 아니다**: AF-5가 센 자기 배제 관례 **열하나**는 하한이었고 정찰이 **열둘**로
넓혔으며 **오늘 HEAD에서는 열넷**이다(이름은 여전히 **넷**).

| # | 자리 | 질문 | 2026-08-31 답(라운드 92 F) | 발동 |
| --- | --- | --- | --- | --- |
| 1 | AF-1 `:7968` | 같은 물음에 **세 자리 이상**에서 같은 답을 고른 관례가 몇이고, **그 관례에서 빠진 자리**가 몇인가 | 전수로 잰 관례 **여덟** · ⚠️⚠️ **빠진 자리가 있는 것은 하나**(`keyboardShouldPersistTaps="handled"` — 트랙 전 고른 자리 **3** · 안 고른 **14** · 그중 **키보드가 뜰 수 있는 화면의 중첩 칩 줄 여덟**) · **전수 파생으로 세는 계약이 있는 것 하나**(연타 가드) · **세는 자리가 0건이던 것 하나**(행마다 갈리는 낭독 라벨 — 오늘 닫혔다) · 나머지 다섯은 빠진 자리 0건 | ⚠️⚠️ **발동 → 트랙 A · 트랙 B** |
| 2 | AF-2 `:8013` | 재개 조건 가운데 **전제가 참이 된 뒤 몇 라운드 서 있었는지**를 셀 수 있는 것이 몇인가 | 재개 조건 자리 **333**(트랙 D 커밋 시점) 중 **그 줄 자신이 경과를 적은 것 셋** · ±5줄 창까지 **70** · **어디에도 없는 것 263** — ⚠️ **정찰의 창 61은 폭 넷 언저리의 창이었고 이 대장의 창(±5줄)으로는 70이다**(갈린 것은 문서가 아니라 바늘이다) · ⚠️⚠️ **이 절 뒤의 오늘 값은 383 · 4 · 83 · 300이다** | ⚠️⚠️ **발동 → 트랙 D** |
| 3 | AF-3 `:8050` | **리뷰가 보고에만 남긴 잔존**이 몇이고, 그중 **다음 라운드의 누가 집는지**가 적혀 있는 것이 몇인가 | **라운드 91 리뷰의 잔존 0건** — M-1의 *링크 단위* 좁힘이 소스·접근성 표 `#107`·짝 문서 `#166` **세 자리 다**에 서 있다. ⚠️⚠️ **그런데 같은 병의 훨씬 오래된 판 하나가 오늘 잡혔다**: `app/expenses/new.tsx`가 인용한 사실이 **라운드 65에 고쳐진 뒤 오늘까지 낡아 있었다**(트랙 A가 두 시점으로 정정했다) · **배정이 적힌 것 0건** | ⚠️ 답했고 **트랙 A의 절반**이 됐다 |
| 4 | AF-4 `:8087` | **문턱을 지닌 판정** 가운데 그 문턱이 재는 대상이 **제품인지 하네스인지**를 값으로 적은 것이 몇인가 | 문턱·래칫 상수 **38**(⚠️ **정찰의 30에서 늘었고 늘린 것은 이번 라운드의 계약 셋이다** — 어드민 라우트 표면 다섯 · 짝 계약 둘의 이동 의무 셋) 중 재는 세계를 **낱말로** 적은 것 **넷**(전부 `harness-catalog-cost.test.ts` · *하네스*) · **제품이라고 적은 것 0건** · **소스 텍스트라고 적은 것 0건** · ⚠️⚠️ **이 저장소에서 그것은 이분법이 아니라 삼분법이다**(하네스 **4** · 제품 데이터 **8** · 소스 텍스트 **26**) | ⚠️ 답했고 트랙이 되지 않았다(→ 기각) |
| 5 | AF-5 `:8125` | **문서의 좌표를 등호로 무는 계약**이 몇이고, 그중 ***누가 언제 옮기는지*를 값으로 적은 것**이 몇인가 | **줄 번호 배열 하나 · 절 이름 등호 여덟**(⚠️ **정찰의 *여섯*은 `headingLine(…)` 꼴만 센 값이고, `startsWith("### 0-1.")` 꼴 둘이 그 밖에 있었다** — 트랙 E가 자기 소스에서 전수 파생해 여덟을 냈다) · ⚠️⚠️ **아홉 자리 다 이동 의무가 적혔고 빈 자리 0건이다**(트랙 전 **1/9**) | ⚠️⚠️ **발동 → 트랙 E** |

**답 1을 낸 방법.** **모집단** = `apps/mobile/{app,src}/**`의 비테스트 `.tsx` **58**(같은 물음이
화면마다 되풀이되는 층). **바늘** = ⓐ 저장소가 같은 물음에 **세 자리 이상**에서 같은 답을 골랐고
ⓑ 그 물음이 걸리는 자리를 **전수로 셀 수 있는** 관례만 고른다. ⚠️⚠️ **오늘 그 표의 값은 세 번째
줄과 네 번째 줄에 있다**: 세 번째(`keyboardShouldPersistTaps`)는 **세는 계약이 손 목록 셋**이라
빠진 여덟이 모집단에 들어온 적이 없었고, 네 번째(행마다 갈리는 낭독 라벨)는 **세는 계약이 0건**
이었다. **오늘 둘 다 전수 파생이 섰다** — 스크롤 컨테이너 **17**(명시 **11** · 키보드가 뜰 수 없는
화면 **6** · **미가드 0건**)과 낭독 라벨 **162**(목록 안 **35**)다.
⚠️ **사각 셋**: ⓐ 이 바늘은 **전수로 셀 수 없는 관례**(문구·색·간격처럼 자리마다 판단이 다른 것)를
못 본다 · ⓑ **모바일만** 본다(어드민의 같은 물음은 이 표 밖이고 `<button type=` 한 줄만 대조로
넣었다) · ⓒ 관례를 **여덟만** 골랐다 — 고르는 걸음 자체가 손이다. **여덟은 하한이다.**

**답 2를 낸 방법.** **모집단** = `resume-condition-ledger.ts`의 `collectDocumentSites()`가 판정
문서에서 파생하는 재개 조건 자리 — **트랙 D 커밋 시점 333 · 이 절 뒤 383**(⚠️ **키운 것이 이 절
자신이다**). **바늘** = 그 자리의 **줄 자신**에 `N 라운드 (연속|째|만에)` 꼴로 경과가 적혀 있는가 ·
없으면 **±5줄 창**까지 넓혀 다시 본다. ⚠️⚠️ **트랙 D가 이 축을 대장에 심으며 정찰의 네 수 가운데
둘을 정정했다** — 자리 전수는 정찰과 **똑같이 333**이었으므로 그 사이에 문서는 한 줄도 자라지
않았고, 그러니 창의 **61 ↔ 70**은 *언제 쟀는가*가 아니라 *무엇을 창으로 보았는가*의 차이다
(폭 셋이 56 · 폭 넷이 65 · 폭 다섯이 70).
⚠️ **사각 셋(대장이 값과 하한으로 진다 · 오늘 값)**: ⓐ **산문으로 오래됨을 말하는 창 9**(수 없이
말한 자리라 기계가 셀 수 없다) · ⓑ **경과가 적혀 있다는 것과 그 수가 오늘 참이라는 것은 다르다**
(창 **83** 전부가 이 사각 안이다) · ⓒ **짝 문서와 라운드 노트의 경과 89는 모집단 밖이다.**
**셋과 70은 둘 다 하한이다.**

**답 3을 낸 방법.** **모집단** = 라운드 91 리뷰가 소스·문서에 남긴 표식(`리뷰 [LMH]-N`)이 사는
파일 **아홉**. **바늘** = 그 표식이 가리키는 **한 사실**을 ⓐ 소스 ⓑ 판정 문서 ⓒ 확인 문서
**셋에서 각각** 다시 읽어 오늘도 같은가를 본다. **잔존 0건**이고, 라운드 90 리뷰가 확인 문서 둘을
두고 갔던 그 모양이 이번에는 나지 않았다. ⚠️⚠️ **그런데 같은 바늘을 *더 오래된 리뷰*에 돌리면
하나가 나온다** — 라운드 65 GAP-065 #6이 `src/ui.tsx`를 `"handled"`로 고치며 자기 근거로 인용한
주석이 그날부터 거짓이었고, **오늘 트랙 A가 그것을 두 시점으로 정정했다.**
⚠️ **사각**: **리뷰 보고 자체는 저장소 밖이라 전수 모집단이 없다** — 이 답은 *표식이 남은 잔존*만
세고 *표식조차 남기지 않은 잔존*은 구조적으로 못 본다. **0건은 하한이다.**

**답 4를 낸 방법.** **모집단** = `apps/**`·`packages/**`·`scripts/**`에서
`const <대문자>(RATCHET|FLOOR|CEILING|THRESHOLD)… = <정수>` 꼴로 선 상수 — **오늘 38**.
**바늘** = 그 상수가 사는 **파일 안에** 그 수가 재는 세계(**제품 데이터 · 하네스 누적 · 소스 텍스트**)를
**낱말로** 적었는가. **하네스라고 적은 것 넷** · **제품 0건** · **소스 텍스트 0건**이고, 재는 세계로
가르면 **하네스 4 · 제품 8 · 소스 텍스트 26**이다 — ⚠️⚠️ **셋째 세계가 다수인데 그 이름이 어디에도
없고, 이번 라운드가 더한 여덟은 전부 그 셋째 세계다.** ⚠️⚠️ **가장 아픈 자리는 N-4의 문턱을
*인용*하는 소스다**: `apps/admin/src/lib/catalog-size-view.ts`는 자기가 인용이라는 사실은 정확히
적는데(*"이 모듈은 판정하지 않는다. 인용한다"*) **그 수가 어느 세계의 수인지는 한 낱말도 적지
않는다** — 그 카드가 세는 수는 오늘 `wooriai_dev`에서 **62**, `wooriai_test`에서 **8,765**다.
⚠️ **사각 둘**: ⓐ **이름에 그 네 낱말이 든 상수만** 본다(`*_MAX`·`*_LIMIT`은 대개 입력 검증이라
뺐고, 빼는 걸음 자체가 손이다) · ⓑ **파일 안에 적혔는지만** 보므로 그 세계를 *판정 문서가* 적어 둔
경우(N-4 본문이 그렇다)는 이 수에 들지 않는다. **서른여덟은 하한이다.**

**답 5를 낸 방법.** **모집단** = 짝 계약 둘(`accessibility-checklist-shape.test.ts` ·
`runtime-checklist-shape.test.ts`)이 문서의 **좌표**를 무는 자리 전수 — ⚠️ **손 목록이 아니라
그 계약이 *자기 소스*에서 파생한다**(트랙 E가 세운 그 규율). **바늘** = ⓐ 문서 줄 번호를 리터럴
배열로 등호에 넣은 자리와 ⓑ 절 제목 문자열을 등호로 찾는 자리(`headingLine("…")` ·
`startsWith("### 0-1.")`)를 **갈라** 센다. **오늘 ⓐ 하나 · ⓑ 여덟**이고, ⚠️⚠️ **두 수를 한 낱말로
적으면 이 축이 닫힌 것으로 읽힌다.** ⚠️ **정찰의 여섯이 여덟이 된 것은 바늘이 넓어졌기 때문이다**
(정찰은 `headingLine(…)` 꼴만 셌다). **아홉 자리 다 의무가 적혔고 빈 자리 0건이며, 그 의무를 계약이
길이·손·시점으로 문다.**
⚠️ **사각 셋(계약이 값과 하한으로 진다)**: ⓐ **문서를 파싱해 파생한 좌표는 이 바늘 밖이다**(라운드
90 D의 축 — 오늘 그 자리 수는 **94**이고 D가 하한을 골라 오늘도 초록이다) · ⓑ **문서가 소스의
좌표를 무는 반대 방향은 이 바늘 밖이고 그 수는 훨씬 크다** · ⓒ **의무가 적혀 있다는 것과 그 의무가
지켜진다는 것은 다르다** — 이 자는 앞쪽만 세고 **뒤쪽은 라운드마다 F의 걸음이 답한다**(오늘 F가
그 한 줄을 옮겼으므로 이 라운드의 답은 *지켜졌다*이다).

⚠️⚠️ **AA-3의 이행 — 결정형 재개 조건 전수와 오늘의 처분이다. 라운드 91의 열다섯에서 닫힌 것은
0건이고, 오늘 HEAD에서 다시 세니 열여섯이다.**
⚠️⚠️ **열다섯 → 열여섯도 새 부채가 아니라 *세는 자리가 늘었다*는 뜻이고, 이번에 늘린 손은 트랙도
F도 아니라 라운드 91 *리뷰*다**(L-6이 어드민 스윕의 사각에 결정형을 붙였다). **F의 표는 리뷰보다
먼저 닫히므로 그 조건은 구조적으로 다음 라운드의 정찰이 처음 센다** — AC-3이 세운 *사각이 조건을
값으로 적으면 다음 표에 선다*의 **네 번째** 이행이고, 이번에는 그 손이 리뷰였다.
⚠️ **트랙 A·B·C·E가 새로 적은 재개 조건은 전부 사건형이라 이 표에 오지 않는다**(⚠️⚠️ **트랙 D는
경과 축의 사각 하나에 결정형을 적었는데 그 조건의 문면이 #15와 같은 결정이라 새 줄로 서지
않는다** — *조건의 도래를 값으로 가르는 관례*를 두 자리가 함께 기다린다).

| # | 결정형 조건 | 사는 곳 | 손 | 오늘의 처분(2026-08-31 실측) |
| --- | --- | --- | --- | --- |
| 1 | 막대를 포커스 가능하게 | AA-2 | 안 | **집지 않는다 — 값 0.** 두 화면 다 `trend.showTable` 표가 값을 텍스트로 준다(재실측 · 변화 0) |
| 2 | 기록 탭 검색의 분류 갈래 | Z절 | 안 | **집지 않는다 — 어긋남 0건 · 여덟 라운드 이어 같은 답**(라벨이 상수에서 파생해 갈릴 자리가 구조적으로 없다). ⚠️ **오늘 트랙 A가 그 화면을 열었지만 축이 다르다** — 바꾼 것은 분류 칩 줄의 **터치 전달 규칙**이고 라벨·placeholder는 바이트 불변이다 |
| 3 | 준비템 분류 필수 입력 | Z-3 | **밖**(카탈로그 정책) | 집지 않는다. 변화 0 |
| 4 | 서버의 중복 대기 초대 방지 | AA-4 | **밖**(초대 정책) | 집지 않는다. 변화 0 |
| 5 | 감사 뷰의 대상 축 필터 | AA절 기각 | **밖**(서버 DTO 0건) | 집지 않는다 — ⚠️⚠️ **이번 라운드는 `apps/api/**`가 다시 접점 0건이다**(라운드 91이 다섯 라운드 만에 열었고 **한 라운드 만에 닫혔다**) · **서버 소스와 DTO는 여전히 0바이트다** |
| 6 | 타일 안 배지 | AA-1 | **밖**(디자인 승인) | 집지 않는다. 변화 0 |
| 7 | C-3 잠금 오버레이 TalkBack 투과 | 접근성 표 C-3 | **밖**(사람·기기·날짜) | **F의 소유 밖이다** — 오늘로 **스물여섯 라운드째 미확인**(67~92). ⚠️ **오늘 이 줄에 붙는 값은 확인이 아니라 *세 자리를 함께 올리는 걸음이 계약이 지운 의무가 됐다*는 사실이다**(트랙 E의 ⓗ) |
| 8 | JSX 사용을 참조로 세는 판정 | `dead-export-ledger.ts` | 안 | **집지 않는다 — 여섯 라운드 이어서다.** 막는 것은 **크기와 문턱의 겹침** 그대로다(`.tsx`의 `export function` 오늘도 **141** · `derived-exemptions`는 **44 중 22 · 여유 0**). ⚠️ 그 파일은 이번에도 아무 트랙의 것이 아니다 |
| 9 | 재조회 실패 창에서 옛 목록을 남길지 | AC절 기각 | 안 | **집지 않는다 — 화면 구조 결정이고 오늘도 접점 0건**(`src/offline/**` 무접촉) |
| 10 | 앵커 대장의 `helper-named-reader` | `comment-tolerant-anchor-ledger.ts` | 안 | **집지 않는다 — 래칫이다.** 앵커 다섯 자리(**698 · 620 · 70/70 · 8/8 · 0**)와 면제 **8**은 **일곱 번째로 한 자리도 다르지 않은데**, ⚠️⚠️ **그 사각의 크기는 오늘 F가 다시 재어 165에서 166이 됐다**(트랙 A의 새 스윕이 헬퍼 이름으로 소스를 읽는다 · 대장이 지닌 하한은 **164**다 — **세 수를 한 낱말로 적지 않는다**) |
| 11 | 재개 조건 대장의 `prose-only` | `resume-condition-ledger.ts` | 안(**F**) | **집지 않는다.** 오늘 **208**(119 → 145 → 165 → 184 → 208)이고 소급 적용은 한 라운드의 F가 질 수 없다. ⚠️ **트랙 D가 그 대장을 열었지만 축은 *경과*였고 이 둘은 그 트랙의 축이 아니다** |
| 12 | 재개 조건 대장의 `round-notes` | `resume-condition-ledger.ts` | 안 | **집지 않는다 — 관례 0건.** 오늘 **377**인데 ⚠️ **늘린 자리가 정찰 노트 자신이다**(319 + `round92-scout.md` 58 = 377 · 일부러 모집단 밖이다) |
| 13 | 그물 파일이 자기를 이름으로 선언하는 표식 | `contract-net-ledger.test.ts` | 안 | **집지 않는다 — 관례를 새로 정하는 일이다.** 표식 **0건** · ⚠️⚠️ **그 옆의 모집단은 오늘 또 커졌다**: 자기 배제 관례를 손으로 적은 자리가 **열넷 · 이름 넷**이고(`LEDGER_SELF_FILES` 4 · `SWEEP_SELF_FILES` 2 · `SELF_PATH` 4 · `SELF_FILE` 4) **정합을 세는 자리 0건** — **AF-5의 열하나도 정찰의 열둘도 하한이었다** |
| 14 | 공유 테스트 DB를 언제 무엇으로 비울지 | `harness-catalog-cost.test.ts` | 안 | **집지 않는다 — 그런데 ⚠️⚠️ 그 조건이 *"먼저 물을 것"* 으로 지목한 질문에 오늘 답이 났다.** `wooriai_test`의 `item_templates` **9,402** 가운데 시드 코드와 정확히 같은 것은 **62**이고 나머지 **9,340**은 전부 생성 접두를 지닌다(**겹침 0**) — **무엇을 지울지의 술어가 값으로 섰다** |
| 15 | 조건의 *도래*를 값으로 가르는 관례 | `resume-condition-ledger.ts` | 안 | **집지 않는다 — 관례를 정하는 일이다.** ⚠️⚠️ **오늘 그 조건을 기다리는 자리가 둘이 됐다**: 트랙 D의 경과 축이 *"경과가 적혀 있다는 것과 그 수가 오늘 참이라는 것은 다르다"* 를 사각으로 적으며 **같은 결정**을 자기 재개 조건으로 걸었다(경과는 *얼마나 서 있었나*이고 도래는 *참이 됐나*다 — **두 수를 한 낱말로 적지 않는다**) |
| 16 | **라우트 표면과 내비 표의 짝을 어느 계약이 지는가** | `admin-landmark-current.test.ts`(라운드 91 리뷰 L-6이 신설) | 안 | ⚠️⚠️ **오늘 집는다 — 트랙 C.** 짝을 지는 계약은 `apps/admin/src/admin-route-surface.test.ts`이고(어드민 **앱 안**의 스윕 · 저장소 그물 열다섯 밖), 오늘 **`page.tsx` 11 ↔ `NAV_ITEMS` 11 · 어긋남 0**을 양방향으로 물며 L-7이 기댄 **평면 전제**(동적 세그먼트 0건)까지 함께 진다. ⚠️ 그 사각 칸은 **두 시점**으로 옮겨 적혔고 재개 조건은 그날 이후 **사건형**이 된다 |

⚠️⚠️ **이 표가 라운드 91의 표와 다른 점 셋.** ⓐ **열다섯이 열여섯이 됐고 그 하나를 세운 것은
리뷰다** — 트랙도 F도 아니다. ⓑ ⚠️⚠️ **결정형에서 집는 것이 다섯 라운드 이어 0건이었는데 오늘
하나를 집는다**(#16 · 트랙 C). **AF절이 *"조건의 형이 그 조건이 언제 집히는지를 정한다"* 고 적었는데,
오늘 그 문장에 한 칸이 붙는다: 결정형도 *결정이 작고 조건이 답을 가리키면* 집힌다.** ⓒ **#14는
집지 않지만 그 조건이 스스로 물으라고 적은 첫 질문에는 오늘 답이 났다** — **조건이 *무엇을 먼저
물을지*를 적으면 집지 않는 라운드도 그 조건을 앞으로 민다**(AD-3의 처방이 *집지 않는 쪽*에서 값을
낸 두 번째 자리다).

⚠️⚠️ **이번 라운드가 실측하고 기각한 스물넷을 값으로 남긴다 — 전부 재개 조건과 함께**(V-2).
⚠️ **그중 여덟은 재개 조건이 *결정형*이라는 사실과 그 결정의 손이 어디에 있는지를 함께 적고**,
손이 밖인 다섯 줄은 다섯 조건을 한 줄에 묶어 **전부 결정형 · 손은 저장소 밖**이라고 적는다.
⚠️ **아래 마지막 두 줄은 그 스물넷에 들지 않는다** — 라운드 62~91이 남긴 **상태 변화 0의 표식**과
**제외 목록 준수 확인**이고, 재론 대상이 아니라 스윕이 그것들을 다시 줍지 않게 하는 자리다.

- **문턱이 재는 세계(제품 ↔ 하네스 ↔ 소스 텍스트) — ⚠️⚠️ 오늘 처음 세어 값으로 남기고 집지 않는다.**
  문턱·래칫 상수 **38** 중 세계를 낱말로 적은 것 **넷**(전부 하네스) · **제품 0건 · 소스 텍스트 0건**
  이고, 재는 세계로 가르면 **하네스 4 · 제품 8 · 소스 텍스트 26**이다. ⚠️ **집지 않는 이유는 축과
  앱이다** — 가장 아픈 자리(`apps/admin/src/lib/catalog-size-view.ts`)는 어드민이고 이번 라운드의
  어드민 축은 **라우트 표면**이다(*한 라운드가 한 앱에 축 둘을 얹지 않는다*).
  ⚠️ **재개 조건(사건형 · 자기 축을 함께 적는다): 문턱이 재는 세계의 표기가 한 트랙의 축이 되는
  라운드가 서는 날 — 그날의 첫 모집단은 이 서른여덟이고, 먼저 물을 것은 *제품을 재는 여덟이 그
  사실을 어디에 적는가*다.**
- **자기 배제 관례 — ⚠️⚠️ 재었고 열둘이 아니라 열넷이며, 이름은 여전히 넷이다.**
  `LEDGER_SELF_FILES` **4** · `SWEEP_SELF_FILES` **2** · `SELF_PATH` **4** · `SELF_FILE` **4**이고,
  ⚠️ **늘린 손이 이번 라운드의 트랙 둘이다**(트랙 C가 `SELF_FILE` 하나 · 트랙 E가 `SELF_PATH` 하나).
  ⚠️⚠️ **그리고 트랙 E는 자기 소스에 *열두 자리 · 이름 넷*이라고 적었는데, 그 문장은 자기가 그 수를
  하나 키우는 걸음이라 커밋된 순간 이미 낡았다** — **정합을 세는 자리가 0건이라는 사실의 가장 짧은
  증명이다.** ⚠️ **집지 않는 이유는 관례를 새로 정하는 일이기 때문이다**(결정형 13).
  ⚠️ **재개 조건(결정형 · 손은 저장소 안 — 종전 그대로): 그물 파일이 자기를 이름으로 선언하는
  표식을 세울지 정하는 날 — 그날 이 열넷의 이름도 함께 정해진다.**
- **공유 테스트 DB를 비우는 일 — ⚠️⚠️ 재었고 집지 않지만, 조건이 *먼저 물으라*고 적은 질문에 답이 났다.**
  `wooriai_test`의 `item_templates` **9,402**(활성 **8,765** · DB **732MB**) 가운데 시드 코드와
  정확히 같은 것은 **62**이고 나머지 **9,340**은 전부 생성 접두를 지닌다(`admin_<epoch>…` ·
  `idem-…` · `adm008-…` · `bulk-e2e-…` · `m2-race-…` — **겹침 0건**). `wooriai_dev`는 **62**(활성
  62 · 24MB)이고 코드가 전부 사람이 읽는 이름이다. ⚠️ **그래도 집지 않는다** — *언제* 지울지와
  *다른 스위트가 누적 위에서 초록인지*는 여전히 아무도 세지 않았다.
  ⚠️ **재개 조건(결정형 · 손은 저장소 안 — 좁혀 다시 적는다): 하네스가 *어느 표를 언제 비우는가*를
  정하는 날 — **표식 질문은 오늘 닫혔으므로** 그날 먼저 물을 것은 *비운 뒤에도 초록인가*이고 그
  첫 모집단은 라운드 91 트랙 C가 적어 둔 여섯 표다.**
- **`derived-exemptions`의 절반 문턱 — 재었고 여유가 0 그대로다.** 사문 전수 **44 중 면제 22**로
  **정확히 절반**이고 조건은 *"넘는 날"* 이라 **아직 아니다**(⚠️ 다섯 트랙 어디에도 새 `export const`가
  서지 않았다 — 정찰의 예상이 실측으로 맞았다). ⚠️ **재개 조건(사건형): 면제가 사문 전수의 절반을
  넘는 날.**
- **`dead-export-ledger.ts`의 결정형 8 — 전제는 참이고 여섯 라운드째 집지 않으며, 막는 이유도 그대로다.**
  `.tsx`의 `export function` 오늘도 **141**이고, 막는 것은 **크기와 문턱의 겹침**이다.
  ⚠️ **이번에도 그 파일은 아무 트랙의 것이 아니다.** ⚠️ **재개 조건(결정형 · 손은 저장소 안 — 종전
  그대로): 그 판정이 서는 날 — 그날 141이 모집단으로 들어오고 같은 걸음에 절반 문턱을 다시 재야 한다.**
- **앵커 대장의 사각 `helper-named-reader` — ⚠️⚠️ 오늘 F가 다시 재었고 165에서 166이다.**
  앵커 전수 **698**(code-only **620** · comment-tolerant **70**/래칫 70 · comment-only **8**/래칫 8 ·
  unanchored **0**) · 면제 **8**은 **일곱 번째로 한 자리도 다르지 않은데**, 그 사각의 크기는
  **166**이고 대장이 지닌 하한은 **164**다 — ⚠️⚠️ **세 수를 한 낱말로 적지 않는다.** 늘린 것은
  **트랙 A의 새 스윕**(`keyboard-tap-guard.test.ts`가 `readSweptSource`라는 이름으로 소스를 읽는다)
  이고, ⚠️ **트랙 C의 새 스윕은 그 바늘에 걸리지 않는다**(소스 리더를 두지 않는다) — **한 라운드에
  스윕 둘이 서도 이 수는 하나만 움직일 수 있다.** ⚠️ **집지 않는 이유는 래칫이다.**
  ⚠️ **재개 조건(결정형 · 손은 저장소 안 — 종전 그대로): 저장소가 소스 리더 이름을 한 벌로 모으는
  날, 또는 이 대장이 헬퍼 정의를 따라가 루트를 푸는 법을 배우는 날.**
- **재개 조건 대장의 `prose-only` 208 · `round-notes` 377 — 재었고 둘 다 집지 않는다.**
  형 표기가 없는 산문 조건은 **119 → 145 → 165 → 184 → 208**로 늘었고(⚠️ **이번에 늘린 것의
  대부분이 이 절 자신이다** — 판정을 쓰는 걸음이 곧 이 사각을 키우는 걸음이다), 라운드 노트는
  **377**이며 그 갈림의 자리가 **정찰 노트 자신**이다(319 + 58).
  ⚠️ **재개 조건은 둘 다 결정형 · 손은 저장소 안이고 종전 그대로다.**
- **재개 조건 대장의 `unmarked-source-prose` — ⚠️⚠️ 재었고 68에서 98이 됐으며, 이번에는 늘린 손이 트랙이다.**
  라운드 91 F의 트립와이어가 적은 **56**이 정찰 시점에 **68**이었고(늘린 손이 리뷰였다) 오늘
  **98**이다 — **이번에 늘린 것은 트랙 D·E가 소스에 적은 산문 조건들**이다.
  ⚠️ **하한(10) 설계 덕에 오늘도 초록이다.** ⚠️⚠️ **그리고 이 줄이 오늘 처음 세는 사실 하나: 이
  사각의 수를 움직인 손이 이제 셋이다**(트랙 · F · 리뷰). ⚠️ **재개 조건(사건형 — 종전 그대로):
  이 수가 형 표기를 지닌 소스 조건 수를 넘는 날.**
- **어드민의 무명 표 하나 — 재었고 동결된 채로 둔다.** 오늘 재실측 **표 18 · 이름 17**이고 스윕
...
- **어드민 `<section>` 34에 이름을 주지 않는다 · `<nav>`·`<form>`의 이름 0건 — 재었고 그것이 판정이다.**
  오늘도 `<section>` **34 · 이름 0건**이고, `<nav>`는 저장소 전수 **하나** · `<header>` **하나** ·
  `<form>` **여섯**이다 — 이름 없는 `<section>`·`<form>`은 랜드마크가 아니라 제네릭이라 결함이 아니다.
  ⚠️ **재개 조건(결정형 · 손은 저장소 안 — 종전 그대로): 한 화면의 카드가 너무 많아 랜드마크로
  건너뛰는 길이 필요해지는 날.**
- **어드민의 `<button type=` — 재었고 관례가 완전하다.** 어드민 비테스트 `.tsx` **18**의 `<button`
  여는 태그 **68**이 **전부 `type=`을 지닌다**(`<form>` 안에서 `type` 없는 버튼이 submit이 되는 그
  함정이 0건이다). ⚠️ **AF-1의 방법으로 재어 *빠진 자리가 0건*으로 닫힌 관례이고, 그 사실 자체가
  값이다** — **고르지 않은 자리가 늘 있는 것은 아니다.** ⚠️ **재개 조건: 없다 — 스윕이 다시 줍지
  않게 하는 표식이다.**
...
- **`accessibilityState={{ disabled }}`의 열한 자리 — ⚠️ 재었고 결함이 아니다.**
  ...
  **AF-1의 방법이 낸 후보 가운데 열어 보니 구멍이 아니라 의도였던 자리다.**
  ⚠️ **재개 조건(사건형): 이 앱이 Pressable이 아닌 누르는 자리를 `disabled`와 함께 세우는 날.**
- **`useMutation` 밖의 쓰기 경로 — 재었고 오늘도 핵심 루프 밖이다.** 라운드 91 A의 사각이 적은
  **35**(`void <호출>(`)와 맨 `fetch(` **0건**은 오늘도 그 스윕의 바늘 밖이고, ⚠️ **핵심 루프 안의
  그런 자리는 오늘 0건이다**. ⚠️ **재개 조건(사건형 — 종전 그대로): `useMutation` 밖의 쓰기가 핵심
  루프 안에 서는 날.**
- **연타 스윕의 사각 `control-verdict-is-file-scoped`(리뷰 M-2) — 재었고 전제가 참이며 미도래다.**
  `useMutation` 선언 **31** · `control-blocks` **27** 그대로다(하한 1). ⚠️ **이 라운드의 어느 트랙도
  그 파일을 열지 않았다.** ⚠️ **재개 조건(사건형 — 종전 그대로): 한 파일 안에서 뮤테이션 둘 이상이
  서로 다른 누르는 자리를 지고 그중 하나만 `disabled`를 받는 자리가 처음 발견되는 날.**
- **반쪽 프롭 일곱 — 재었고 오늘도 일곱이다.** 낭독 프롭 짝 **37** · role 단독 **1** ·
  live region 단독 **6**. ⚠️⚠️ **트랙 B가 a11y 그물을 열었지만 축은 *낭독 라벨의 모집단*이고 이 축은
...
- **AC-1의 일곱(형제가 실재하는 계약) · AC-2의 0(호출부 둘 이상의 필드 갈림) — 오늘 다시 돌리지 않았다.**
  어느 트랙도 형제 화면이 갈리는 사건을 만들지 않았고 `client.ts`·`admin-api.ts`의 시그니처를 늘리지
  않았다. ⚠️ **재개 조건 둘 다 사건형이고 종전 그대로다.**
- **신설 스윕이 재개 조건을 한 줄도 적지 않은 자리 — ⚠️ 재었고 오늘 0건이다(조건 미도래).**
  라운드 92가 세운 앱 내 스윕 둘(`keyboard-tap-guard.test.ts` · `admin-route-surface.test.ts`)과
  a11y 그물의 새 축이 **셋 다 재개 조건을 적었다**. ⚠️ **라운드 91에 이 줄이 선 이유(*조건을 안 적은
  것*과 *적을 것이 없던 것*이 문서에서 똑같이 읽힌다)는 그대로이고, 오늘은 그 자리가 서지 않았다는
  사실이 값이다.** ⚠️ **재개 조건(사건형 — 종전 그대로): 새 스윕이 재개 조건 0건으로 서는 라운드가
  다시 오는 날 — 그날 물을 것은 *왜 없는가*이고, 답이 *다 닫았다*가 아니면 그것이 이 줄의 첫 결함이다.**
- **조회 실패 창의 모집단 · 응답 필드 축(모바일) · `HouseholdMember.joinedAt` · 기록 탭 검색의 분류
  갈래 · 막대 포커스 가능화 — 다섯 다 판정 그대로이고 상태 변화 0.** ⚠️ **기록 탭 검색은 여덟 라운드
  이어 같은 답이다.** ⚠️ **재개 조건 다섯 다 종전 그대로다**(둘은 결정형 · 손은 저장소 안).
- **이월 다섯(U절 셋 · S-3 · `withdrawn_at`) — 판정 종전 그대로이고 접점만 갱신한다**(위 이월 목록).
  ⚠️⚠️ **낭독 스캐너 줄에는 오늘 *열었지만 그 축이 아니다*가 값으로 선다.** ⚠️ **재개 조건은 다섯 다
  종전 그대로다.**
- **성능 넷 — ⓐ·ⓒ는 재었고 미도래, ⓑ는 오늘도 값이 있으며, ⓓ는 접점이 다시 0건이다.**
  ⓐ **첫 페인트**(⚠️ 어느 트랙도 `useQuery` 선언을 늘리지 않았다 — 트랙 A가 더한 것은 여는 태그의
  속성 여덟이다) · ⓒ **번들**(⚠️ **새 런타임 의존성 0건**) · ⓑ **렌더 비용**(⚠️⚠️ **제품 62 ↔ 하네스
  8,765** — 위 N-4 문단) · ⓓ **api의 루프**(⚠️⚠️ **`apps/api/**`가 한 라운드 만에 다시 접점 0건이다**).
  ⚠️ **재개 조건 넷 다 종전 그대로다.**
- **N-4의 두 문턱 — 15라운드 이어 미발동이고 준비템 탭 비가상화는 이번에도 제안하지 않는다.**
  ⚠️⚠️ **오늘도 두 수가 나오고 둘이 서로 다른 사실이다**(제품 **62** ↔ 하네스 **8,765**).
  ⚠️ **어드민 카드가 세는 수는 여기 옮겨 적지 않는다**(O-3).
- **손이 밖인 다섯 — 넷은 다른 사람이 내릴 결정이고 하나는 배정이다.** 준비템 분류 필수 입력 ·
  서버의 중복 대기 초대 방지 · 감사 뷰의 대상 축 필터 · 타일 안 배지는 **카탈로그 정책 · 초대 정책 ·
  서버 계약 · 디자인 승인**이 각각 선행이고, C-3은 **사람·기기·날짜 배정**이다(이번 라운드로
  **스물여섯 라운드째 미확인**). ⚠️ **재개 조건(전부 결정형 · 손은 저장소 밖): 그 결정이 서는 날** —
  ⚠️ **넷과 하나를 한 줄에 적지 않는 이유가 그 칸이다**(넷은 결정을 기다리고 하나는 시간을 기다린다).
- **`${seller}에서 구매하기` 둘 — 제외 확인이지 제안이 아니다.** 준비템 판매처 1:1과 구매 확인
...
- **감사 뷰 CSV · 콘텐츠 리비전 편집 표면 · 어드민 손 미러 · `SYNC_STATUS_RETRY_ALL_LABEL` · 지출 입력
  ...
  상태 변화 0.**
- **제외 목록 준수 확인**: 준비템 목록 **가격 표시**(잠금 · ⚠️ **다섯 트랙 어디에도 가격·링크 수가
  들어가지 않았다** — 트랙 A가 바꾼 것은 **탭이 전달되는 경로**이고 링크의 표시·순서·문구·가격은
  바이트 불변이다) · 오프라인 로컬 아이 복구 · 외부 계정/키/자산 · **C-3 잠금 오버레이 낭독**
  (**스물여섯 라운드째**) · **P-2 법무 대조** · P-3 테스트 건수 자동화 · **표기 방언 통일** ·
  S-4 파기 `targetId` · 40주 초과 달력 · `onBudgetRelevantChange` · 4가구/`viewedHouseholdId` ·
  **지출→리포트 정확성 축** · **공유 왕복 축** · **준비템 완료율 동기부여** · **준비템 탭 비가상화** ·
  **홈 수치 정합** · **`getHome` 카탈로그 전량 읽기** — **전부 오늘도 제안하지 않는다.**

⚠️⚠️ **트립와이어 91 → 92 — 라운드 91이 남긴 래칫·하한을 다섯 트랙과 F가 머지된 뒤 다시 잰
대조표다.** ⚠️ **정찰의 수를 옮겨 적지 않는다** — 아래는 **오늘 HEAD에서 다시 잰 값**이고, 정찰과
갈린 자리는 그 칸이 갈렸다고 적는다.
⚠️⚠️ **시점은 하나다 — *A~F 머지 뒤 · 리뷰 전*.** 예외 0건이고, 재개 조건 대장의 자리 수도 접근성
표의 좌표 수도 짝 문서의 행 수도 전부 F가 자기 걸음을 끝낸 뒤의 값이다.

| 자리 | 라운드 91이 남긴 값 | 2026-08-31 HEAD 재실측(**A~F 머지 뒤** · 리뷰 전) | 움직였는가 |
| --- | --- | --- | --- |
| 사문 대장 항목 / 래칫 | 22 / 22 | **22 / 22** | 그대로다 — 어느 트랙도 `dead-export-ledger.*`를 열지 않았다(⚠️ **결정형 8이 여섯 라운드째 집히지 않고, 그 파일이 아무 트랙의 것도 아닌 두 번째 라운드다**) |
| 사문 대장의 사각 **여덟** | 22 · 0 · 226 · 20 · 55 · 105 · 141 · 0 | **22 · 0 · 226 · 20 · 55 · 105 · 141 · 0** | **그대로다 — 여덟 자리가 한 자리도 다르지 않다** |
| 사문 전수 / 면제 / 모듈 | 44 / 22 / 6 | **44 / 22 / 6** | 그대로다 — ⚠️ **정확히 절반이고 여유 0**이며 정찰의 예상(*"새 `export const` 0건일 것"*)이 실측으로 맞았다 |
| `CLOSED_BLIND_SPOTS` | 셋 | **셋** | 그대로다 |
| 앵커 대장(앵커 전수 · code-only · comment-tolerant/래칫 · comment-only/래칫 · unanchored) | 698 · 620 · 70/70 · 8/8 · 0 | **698 · 620 · 70/70 · 8/8 · 0** | **그대로다 — 일곱 번 재어 한 자리도 다르지 않다** |
| 앵커 대장의 면제 | 8 | **8** | 그대로다 |
| 앵커 대장의 사각 `helper-named-reader` | 165 | **166** | ⚠️⚠️ **움직였다 — 두 라운드 이어서다.** 트랙 A의 새 스윕(`keyboard-tap-guard.test.ts`)이 `readSweptSource`라는 헬퍼 이름으로 소스를 읽는다. ⚠️ **트랙 C의 새 스윕은 그 바늘 밖이라 한 라운드에 스윕 둘이 서도 이 수는 하나만 움직였다**(하한은 **164** 그대로) |
| 재개 조건 대장(자리 · 괄호 표기 · 손의 위치 · 결정형 · 손 누락 · 소스 표기/괄호 전수) | 333 · 111 · 29 · 30 · 1 · 11/15 | **383 · 127 · 35 · 36 · 1 · 11/15** | ⚠️⚠️ **문서 쪽 넷이 움직였고 소스 쪽 둘은 그대로다** — 늘린 것은 **이 절 자신**이다(AG절 다섯 · 결정형 열여섯 · 기각 스물넷). ⚠️ **트랙 D·E도 소스에 조건을 적었지만 전부 산문이라 표기 수(11/15)는 한 자리도 움직이지 않았고, 그 걸음은 사각 `unmarked-source-prose`에서만 보인다**(68 → 98) — **두 자리를 함께 보아야 그 라운드가 조건을 얼마나 적었는지가 나온다.** **손 누락은 1 그대로다**(라운드 86 Z-1 · `DECISIVE_HAND_EXEMPTIONS`가 면제하고 `DECISIVE_MISSING_HAND_TODAY = 1`이 그 값을 진다 — ⚠️ **F도 D도 그 줄을 건드리지 않았고, 양쪽이 함께 지켜야 그 자리가 산다**) |
| 재개 조건 대장의 사각 **여덟** | 184 · 2 · 319 · 15 · 4 · 21 · 11 · 56 | **208 · 3 · 377 · 17 · 4 · 23 · 11 · 98** | ⚠️⚠️ **움직였다** — 여덟 가운데 **마지막 하나가 두 번 움직였다**(라운드 91 F의 **56** → 정찰 시점 **68** → 오늘 **98**): 앞의 걸음은 **리뷰**가, 이번 걸음은 **트랙 D·E**가 소스에 적은 산문 조건이다 |
| ⚠️⚠️ 재개 조건 대장의 **경과 축**(그 줄 · 창 · 어디에도 없음) | **없던 축** | **4 · 83 · 300**(트랙 D 커밋 시점 **3 · 70 · 263** · 사각 셋 **9 · 83 · 89**) | ⚠️⚠️ **오늘 처음 세어졌다** — AF-2의 물음이 그대로 축이 됐다. ⚠️ **정찰의 창 61은 폭 넷 언저리의 값이었고 이 대장의 창(±5줄)으로는 70이다(오늘 이 절 뒤로는 83이다)** |
| 표기를 지닌 소스 파일 / 그 뿌리(걷는 파일) | 셋 · `apps`·`packages` 전수 파생(966) | **셋 · 전수 파생**(표기 **11** · 괄호 전수 **15** · 걷는 파일 **968**) | 표기 셋은 그대로이고 ⚠️ **걷는 파일이 둘 늘었다**(이번 라운드의 신설 스윕 둘) — ⚠️⚠️ **트랙 D가 라운드 91에 뿌리를 손 목록에서 전수 파생으로 바꾼 덕에 그 둘이 자동으로 걸렸다**(손 목록이었으면 다음 라운드까지 몰랐을 자리다) |
| 계약 그물 수 | 열다섯 | **열다섯** | ⚠️⚠️ **그대로다 — 그물 수가 늘지 않은 세 번째 라운드다.** 트랙 A·B·C의 앱 내 스윕 셋, 트랙 D의 새 축, 트랙 E의 짝 계약 축이 다섯 다 그 수에 들지 않고, **왜 들지 않는지를 각자의 머리말이 값으로 적는다** |
| 반쪽 프롭(짝 · role 단독 · live region 단독) | 일곱(37 · 1 · 6) | **일곱(37 · 1 · 6)** | 그대로다 — ⚠️ **a11y 그물을 연 트랙이 하나 있는데도 그대로다**(B의 축은 낭독 라벨의 모집단이다) |
| 어드민의 상태 문장 | 자리 47 / 파일 14 / 출구 45(`role="alert"` 34 · `role="status"` 11) / 면제 2 | **47 / 14 / 45(34 · 11) / 2** | 그대로다 |
| 어드민의 표 이름 / 무명 표 | 17 / 0(`app/**`) · 스윕 밖 하나 | **17 / 0**(`app/**`) · 스윕 밖 **하나**(표 전수 **18**) | 그대로다 |
| 어드민 `<section>` / 이름 | 34 / 0 | **34 / 0** | 그대로다 — 이름을 주지 않는 것이 판정이다 |
| 어드민 `<main>`(중첩) / `aria-current` | 2(중첩 0) / 1 | **2(중첩 0) / 1** | 그대로다 — ⚠️ **트랙 C가 그 파일의 사각 한 칸을 옮겼는데도 랜드마크 축은 바이트 불변이다** |
| ⚠️⚠️ 어드민 라우트 표면(`page.tsx` ↔ `NAV_ITEMS`) | 11 ↔ 11 · 어긋남 0 · **세는 계약 0건** | **11 ↔ 11 · 어긋남 0 · 세는 계약 하나**(트랙 C) | ⚠️⚠️ **값은 그대로이고 *누가 보는가*가 움직였다** — 리뷰 L-6이 결정형으로 세운 조건이 **한 라운드 만에** 집혔고, **결정형 표에서 다섯 라운드 이어지던 0건이 끊겼다** |
| 어드민 `<button` / `type=` / `disabled=`(줄) | 68 / 68 / 44 | **68 / 68 / 44** | 그대로다 — ⚠️ **`type=`은 68/68로 닫힌 관례이고, `disabled=` 절반은 여전히 어드민에 스윕이 0건이다** |
| 모바일 `useMutation`(선언 · 컨트롤 · 핸들러 · 안 막음) | 31 · 27 · 1 · 3 | **31 · 27 · 1 · 3** | 그대로다 — 그 스윕을 여는 트랙이 0건이다 |
| 모바일 Pressable / 역할 / 라벨 | 145 / 145 / 122 | **145 / 145 / 122** | 그대로다 |
| ⚠️⚠️ 모바일 스크롤 컨테이너(전수 · 판정 명시 · 키보드가 뜰 수 없는 화면 · 미가드) | **세는 자리 0건** | **17 · 11 · 6 · 0**(트랙 전 17 · 3 · 6 · **8**) | ⚠️⚠️ **오늘 처음 세어졌다** — 손 목록 셋뿐이던 자리가 전수 파생이 됐고, `"always"`는 **0건**이다 |
| ⚠️⚠️ 모바일 낭독 라벨(식 전수 · 목록 안 · 직접 · 파생 · 갈리지 않음) | **세는 자리 0건** | **162 · 35 · 34 · 1 · 0** | ⚠️⚠️ **오늘 처음 세어졌다** — 세 라운드 미배정이 끊겼다. ⚠️ **상수 문자열 라벨은 그 옆에 따로 서고 오늘 73이다**(⚠️ **정찰의 74를 트랙 B가 73으로 정정했다** — 두 수를 한 낱말로 적지 않는다) |
| ⚠️⚠️ 문턱·래칫 상수(전수 · 세계를 낱말로 적은 것 · 제품 · 소스 텍스트) | **세는 자리 0건**(정찰 30 · 넷 · 0 · 18) | **38 · 4 · 0 · 0**(재는 세계로는 하네스 **4** · 제품 **8** · 소스 텍스트 **26**) | ⚠️⚠️ **오늘 처음 F가 세었고 정찰의 30에서 늘었다** — 늘린 여덟은 전부 이번 라운드의 계약이고 전부 **소스 텍스트**를 잰다 |
| ⚠️⚠️ 자기 배제 관례(자리 / 이름) | 열하나 / 넷(AF-5) · 정찰 열둘 / 넷 | **열넷 / 넷** | ⚠️⚠️ **움직였다 — 부채가 아니라 이번 라운드의 트랙 둘이 각각 하나씩 더했다**(C의 `SELF_FILE` · E의 `SELF_PATH`). **정합을 세는 자리는 오늘도 0건이다** |
| 접근성 표 → 짝 문서 좌표(자리 · 인용 번호 · 서로 다른 번호 · 표면 · 역방향 · 어긋남) | 93 · 97 · 93 · 34 · 8 · 0 | **94 · 98 · 94 · 35 · 8 · 0** | ⚠️ **움직였다** — F가 A-33(`#108`)을 세우며 넷이 하나씩 늘었다(역방향과 어긋남은 그대로). ⚠️ **라운드 90 D가 이 축을 *하한*으로 세운 덕에 F의 걸음이 그대로 통과했다** |
| ⚠️⚠️ 짝 계약의 이동 의무(좌표 자리 · 절 이름 · 줄 번호 배열 · 의무가 빈 자리) | **없던 축**(트랙 전 1/9) | **9 · 8 · 1 · 0** | ⚠️⚠️ **오늘 처음 섰다** — 정찰이 센 *절 이름 여섯*은 `headingLine(…)` 꼴만 본 값이었고, 트랙 E가 자기 소스에서 전수로 파생해 **여덟**을 냈다 |
| 짝 문서(행 수 · 실기기 · 브라우저 · 서버 · 작업) | 167 · 137 · 20 · 9 · 1 | **168 · 138 · 20 · 9 · 1** | ⚠️ **움직였다** — `#168`(실기기 · 트랙 A) 하나뿐이고 **브라우저는 0건**이다(네 라운드 이어지던 배분이 끊겼다) |
| 접근성 표 | A-32까지(`#107`) · Batch 41 | **A-33까지(`#108`) · Batch 42** | ⚠️ **움직였다** |
| 접근성 표 C-3 경과 수(세 자리) | 스물다섯 · 스물다섯 · 스물다섯(갈림 0) | **스물여섯 · 스물여섯 · 스물여섯**(갈림 0) | ⚠️ **움직였고, 세 자리를 함께 올렸다** — 한 자리만 올리면 짝 계약이 빨개진다 |
| `MAX_DIVERGENCE` ↔ 문서가 옮겨 적은 그 상수 | **0** ↔ 문서 세 자리 **0** · 세는 계약 하나 | **0** ↔ 문서 세 자리 **0** · 세는 계약 하나 | 그대로다 — ⚠️ **경과 수를 올리는 걸음이 *"0을 넘지 않는가"* 를 한 글자도 바꾸지 않았다**(라운드 91 트랙 E의 ⓖ가 무는 그 축) |
| ⚠️⚠️ 접근성 짝 계약의 상수 인용 좌표 | `22 · 1223 · 1267` | **`22 · 1281 · 1333`** | ⚠️⚠️ **움직였고 F가 함께 옮겼다** — A-33 절과 C-3의 세 자리 갱신이 아래 두 좌표를 밀었다(`:22`는 A절보다 위라 움직이지 않는다). **값도 꼴도 바이트 불변이고 움직인 것은 좌표뿐이다** · ⚠️⚠️ **라운드 91에는 주석이 부탁한 걸음이었고 오늘은 계약이 값으로 지운 의무의 이행이다** |
| ⚠️⚠️ 공유 테스트 DB / 활성 카탈로그 ↔ 제품 카탈로그 | 722MB · 9,333(활성 8,706) ↔ 62 | **732MB · 9,402(활성 8,765)** ↔ **62**(`wooriai_dev` 24MB) | ⚠️⚠️ **셋 다 또 늘었고, 이번에 늘린 손은 *리뷰의 실행*이다**(트랙도 F도 아니다 · 세 시점의 실측이 됐다). **N-4의 판정은 미발동 그대로다** · ⚠️ **표식 갈림은 62 ↔ 9,340 · 겹침 0** |

⚠️ **이 표가 라운드 91의 트립와이어 관례에 더하는 것이 셋이다.** ⓐ ⚠️⚠️ **시점 예외가 0건인 두
번째 표다.** ⓑ **세는 자리가 0건이던 축 넷이 오늘 처음 수를 얻었다**(스크롤 컨테이너 · 낭독 라벨의
모집단 · 문턱이 재는 세계 · 짝 계약의 이동 의무) — 그중 하나는 **세 라운드가 배정을 기다린 축**
이었다. ⓒ ⚠️⚠️ **정찰의 수 넷이 트랙과 F의 재실측으로 정정됐다**: 절 이름 등호 **여섯 → 여덟**
(트랙 E) · 상수 라벨 **74 → 73**(트랙 B) · 창 바늘 **61 → 70**(트랙 D) · 문턱·래칫 상수
**30 → 38**(F). **일곱 라운드 이어 정찰의 전제가 낡았고, 정찰이 grep과 손 판정으로 낸 수는
하한이며 갈리면 그 수가 값이라는 관례가 일곱 라운드째 참이다.** ⚠️⚠️ **다만 오늘 한 칸이 반대
방향으로도 섰다** — **AF-5가 센 자기 배제 관례 열하나를 정찰이 열둘로 넓혔고, F가 다시 재니
열넷이다**: **정찰의 수가 낡는 것과 F의 수가 하한인 것은 같은 병의 두 얼굴이고, 처방도 같다.**

**이 라운드가 짝 문서에 남긴 것.** 확인의 표에 **`#168` 하나**가 서고 §0의 네 숫자와 합계·머리말의
재계산 줄·§1-1 머리말의 라운드 구간이 파싱으로 다시 세어졌으며, 접근성 표에는 **A-33 `#108` 하나**와
**C-3의 세 자리 갱신**, 그리고 **짝 계약의 좌표 한 줄**이 함께 옮겨졌다.
⚠️⚠️ **표면 배분이 라운드 88~91과 다르다 — 실기기 하나 · 브라우저 0건이고, 네 라운드 이어지던
*하나·하나*가 오늘 끊겼다. 그리고 그 이유가 §0의 수보다 값이 크다**: 다섯 트랙 가운데 **넷이 화면을
한 곳도 열지 않았다**(a11y 그물의 새 축 · 어드민 라우트 표면 · 재개 조건 대장의 경과 축 · 짝 계약
둘의 이동 의무 — 넷 다 **화면 0건 · 문구 0건 · 렌더 0건**이다). ⚠️ **라운드 84가 세운 그 구분**
(*"확인할 것이 없다"* 가 아니라 *"폰에 보이는 동작을 한 곳도 바꾸지 않았다"*)**의 일곱째 판이다.**
⚠️⚠️ **다만 라운드 91과 다른 점 하나를 함께 적는다 — 화면 트랙이 이번에는 하나뿐이고, 그 하나는
*어느 대장도 이름을 준 적 없는 자리*가 아니라 **계약이 있는데 그 계약의 모집단 밖이었던 자리**에서
나왔다.** 라운드 91의 하나는 계약이 0건인 자리였고 **오늘의 하나는 계약이 있는데 손 목록이라 못 본
자리다** — **표가 세는 수는 같은데 그 수가 어디서 왔는지가 또 달라졌다.**
⚠️ **A-33이 A-32와 다른 점 하나는 그 절의 머리말이 진다**: A-32는 *어느 칸도 지목한 적 없는 자리를
어떻게 찾았는가*를 물었고, **A-33은 *계약이 있는데도 왜 못 봤는가*를 묻는다.** ⚠️ **기기 조건은
A-32와 다르다 — 폰 한 대면 되고 느린 망이 필요 없다**(결함이 왕복의 창이 아니라 터치가 전달되는
경로다). ⚠️⚠️ **그리고 그 행이 처음 지는 물음 하나가 있다 — *부정 조건이 긍정 조건과 같은 크기다*.**
칩이 한 번에 눌리는가만큼이나 **빈 자리를 누르면 종전처럼 키보드가 내려가는가**가 중요하고,
`"always"`가 아니라 `"handled"`를 고른 이유가 정확히 그것이다.
⚠️ **트랙 B·C·D·E는 두 표 어디에도 행이 서지 않는다** — ⚠️⚠️ **다만 B와 C에 대해서는 이유를 한 칸
더 적었다**(접근성 표 A-33 절의 문단 둘): **B는 a11y 그물을 열었는데 화면 바이트가 0이었고**(고칠
자리가 0건이었다는 사실이 그 트랙의 값이다), **C는 어드민 계약을 세웠는데 어긋남이 0이라 어드민
소스가 0바이트였다** — ***"브라우저 화면이라 표의 조건 밖이다"* 와 *"고칠 것이 없었다"* 는 문서에서
똑같이 읽히므로 갈라 적는다.**
⚠️⚠️ **C-3(잠금 오버레이 TalkBack 투과)은 오늘로 스물여섯 라운드째 미확인**이고, ⚠️ **이번 라운드가
그 줄에 더하는 값은 경과 수가 아니라 *그 세 자리를 함께 올리는 걸음이 이제 계약이 지운 의무의
이행이다*라는 사실이다** — 확인은 여전히 배정을 기다린다.

### AG-1. **관례를 세는 계약이 있어도 그 계약의 모집단이 손 목록이면 관례에서 빠진 자리는 구조적으로 보이지 않는다** — 같은 라운드의 형제 계약이 이미 그 교훈을 배웠는데도 그랬다

- **사실.** 모바일 비테스트 `.tsx` **58**의 스크롤 컨테이너 여는 태그 **17** 가운데
  `keyboardShouldPersistTaps="handled"`를 명시한 것은 트랙 전 **셋**(`AppScreen` · `ScreenScaffold` ·
  기록 탭 `SectionList`)이었고, 명시하지 않은 **열넷** 중 **여덟이 키보드가 뜰 수 있는 화면의 중첩
  가로 칩 줄**이었다. ⚠️⚠️ **여덟 중 다섯은 *타이핑 중에 누르라고 만든 줄*이다** — 품목 자동완성 ·
  판매처 자동완성 · 최근 품목 · 최근 날짜 · 분류 칩.
- ⚠️⚠️ **왜 바깥의 셋이 안쪽의 여덟을 덮지 못하는가가 이 판정의 값 절반이다.** RN의 판단은
  `onStartShouldSetResponderCapture`(**capture 단계** — 바깥에서 안으로)에서 나므로, 바깥
  `AppScreen`이 `"handled"`로 통과시켜도 **가장 안쪽 스크롤러가 기본값이면 그 자리가 탭을
  가로챈다**. RN 소스의 주석이 그 동작을 문장으로 적는다 — *"첫 탭은 스크롤 뷰로 가서 키보드를
  내리고, **두 번째 탭이** 실제 자식에게 간다."* **사용자에게는 *눌렀는데 반응이 없다*로 보인다.**
- ⚠️⚠️ **해악의 이름도, 옳은 답도, 그 답을 고르지 않은 자리도 전부 이 저장소가 이미 적어 두었다.**
  `src/ui.tsx`(라운드 65 GAP-065 #6)가 *"눌렀는데 반응이 없다"* 라고 이름 붙이고 `"always"`가 아니라
  `"handled"`인 이유까지 적었으며, **그 근거로 인용한 자리(`app/expenses/new.tsx`의 `merchantFocused`
  주석)가 빠진 여덟 중 하나였다.**
- ⚠️⚠️ **그런데 계약이 손 목록 셋이라 여덟은 모집단에 들어온 적이 없다.** GAP-065 #6의 `scaffolds`는
  **손으로 적은 셋**이고, **같은 라운드의 형제 블록 #7은 정확히 그 병을 배워 손 목록 여덟을 전수
  스캔으로 바꿨다** — 그 파일이 스스로 그 이유를 적는다(*"이 계약이 그 줄을 한 번도 읽지 않았다"*).
  **한 라운드가 같은 교훈을 형제 하나에만 적용했고, 그 갈림이 스물일곱 라운드를 살아남았다.**
- **오늘의 값 — 여는 태그 여덟에 속성 하나씩과, 그 관례를 전수로 세우는 스윕 하나다.** ⓐ 화면
  변경은 **여는 태그의 속성 여덟**이고 렌더는 **픽셀 불변**이다(화면 한국어 0글자 · 새 낭독 0건 ·
  서버 0바이트 — 터치 전달 규칙이지 레이아웃 속성이 아니라 픽셀락 기준선이 흔들리지 않는다).
  ⓑ 스윕은 **17**을 전수로 파생하고 자리마다 판정 하나를 **소스에서** 낸다(판정 명시 **11** ·
  키보드가 뜰 수 있는 화면이 아닌 것 **6** · **미가드 0건**을 부정 단언으로 못 박는다).
  ⓒ ⚠️ **`"always"`는 오늘 0건이고 그 사실도 값으로 문다** — 빈 자리를 눌렀을 때 키보드가 내려가는
  종전 동작이 이 라운드의 **부정 조건**이다. ⓓ **낡은 주석 한 문단을 두 시점으로 정정했다**(AG-4).
- ⚠️⚠️ **그리고 이 트랙은 자기 전제를 다시 재어 정찰을 정정했다.** 정찰은 *"명시하지 않은 열넷 중
  여덟이 키보드가 뜰 수 있는 화면"* 이라고 적으며 **파일에 `<TextInput `이 있는가**로 판정했는데,
  그 바늘은 *그 스크롤러가 실제로 그 입력칸과 같은 화면에 동시에 서는가*를 묻지 않는다. **트랙이
  여덟을 하나씩 눈으로 다시 풀었고 여덟 다 참이었다** — 그리고 남은 여섯은 **파일에 `<TextInput `이
  0건**이라 키보드가 뜰 수 없다. **파일 단위 판정의 사각을 값으로 지고, 그 사각은 라운드 91 리뷰
  M-2가 연타 스윕에서 이름 붙인 것과 같은 얼굴이다.**
- **일반형.** ⚠️⚠️ **관례를 세는 계약이 있다는 사실은 그 관례가 세어지고 있다는 뜻이 아니다** —
  **모집단이 손 목록이면 그 계약은 *목록에 적힌 자리가 관례를 지키는가*만 묻고 *관례를 져야 하는
  자리가 이것뿐인가*는 묻지 못한다.** 그리고 그 병은 조용하다: 계약은 초록이고, 빠진 자리는 계약이
  없는 자리보다 **더** 안 보인다(계약이 있다는 사실 자체가 *그 자리는 세어졌다*로 읽힌다 — AB-4가
  이름 붙인 착시의 가장 나쁜 판이다). ⚠️⚠️ **오늘 그 병의 가장 짧은 증명이 나왔다: 같은 라운드가
  형제 계약 둘을 세웠고 하나는 손 목록을 버렸으며 하나는 버리지 않았는데, 버리지 않은 쪽의 모집단
  밖에서 스물일곱 라운드 뒤에 여덟 자리가 나왔다.** ⚠️ **AF-1이 세운 방법**(*저장소가 이미 고른
  답을 고르지 않은 자리를 센다*)**이 오늘 한 칸 더 나아간다** — 그 방법으로 찾을 자리는 **계약이
  없는 곳만이 아니라 계약이 있는데 모집단이 손인 곳**에도 있다.
  ⚠️ **다음 라운드가 먼저 세어 볼 만한 것**: 이 저장소에서 **관례를 세는 계약**이 몇이고, 그중
  **모집단을 손 목록으로 적은 것**이 몇인가(오늘 하나를 손 목록에서 전수로 바꿨고, ⚠️ **나머지가
  몇인지는 오늘 세지 않았다**).

### AG-2. **배정이 막는다는 진단이 옳았는지는 배정해 봐야 알고, 오늘 배정하니 고칠 것이 0건이었다** — 세 라운드를 미룬 이유는 고칠 것이 많아서가 아니었다

- **사실.** 라운드 89 AD-5가 *"조건이 도래할 수 없는 상태"* 와 *"도래할 수 있는데 배정이 없는
  상태"* 를 가르는 처방을 세웠고, 90 AE-5가 그 구별을 값으로 만들었으며, **91 AF-2가 배정되지 않은
  넷째를 이름으로 지목했다** — 행마다 갈리는 낭독 라벨의 모집단이고, 그것으로 **세 라운드
  연속**이었다. AF-2는 이렇게 적었다: *"세 번은 이 저장소가 그 조건을 **영원히 미도래처럼 읽고
  있다**는 뜻이다. 다음 라운드는 이 자리를 **먼저** 배정해야 한다."* **오늘 먼저 배정했다.**
- ⚠️⚠️ **배정하자마자 값 하나가 드러났다: 이 축은 오늘 값으로 통과했다.** 식으로 짓는 낭독 라벨
  **162** 가운데 목록 콜백 안이 **35**이고, **행 변수를 직접 쓰는 것 34 · 행에서 파생한 이름을 거쳐
  쓰는 것 하나 · 행마다 갈리지 않는 상수 0건**이다. **화면 바이트는 0**이다.
- ⚠️⚠️ **그 0이 이 판정의 값이다.** 세 라운드 동안 이 조건은 *미도래*와 구별되지 않는 낯으로
  문서에 서 있었고, **그 낯이 *고칠 것이 많아서 못 한다*로 읽혔다.** 오늘 재어 보니 고칠 것은
  0건이었다 — **막던 것은 규율도 크기도 아니고 배정이었다.** ⚠️ **AE-5·AF-2가 그렇게 적었지만,
  그 진단이 옳았다는 증명은 배정 자체이지 배정 전의 어떤 관측도 아니었다.**
- ⚠️ **판정은 둘이 아니라 셋이어야 했고, 그 셋째를 소스가 증명한다.** `app/family/index.tsx`의 대기
  초대 취소 라벨은 **행 변수 이름을 라벨 식에 쓰지 않고** 행에서 만든 두 이름을 거쳐 갈린다 —
  ⚠️⚠️ **직접/갈리지 않음 둘로만 갈랐으면 그 자리가 *갈리지 않는다*로 세어져 거짓 초록이 됐다.**
  **라운드 91 A가 25를 27로 정정한 것과 정확히 같은 모양이고, 이번에는 트랙이 그 갈래를 먼저
  세웠다.**
- **오늘의 값 — 화면 0바이트와, 손 핀을 대신하는 전수 파생 하나다.** ⓐ **고칠 자리가 0건이라 화면·
  문구·픽셀이 전부 바이트 불변**이다. ⓑ 스윕은 모집단을 `listComponentSources()`에서 파생하고
  자리마다 판정 하나를 소스에서 내며, **갈리지 않는 자리 0건**을 **부정 단언**으로 못 박는다(서는
  날 그 자리는 **왜 갈리지 않아도 되는지**를 40자 넘는 이유로 적어야 한다 — 빈 알리바이를 길이로
  막는다). ⓒ ⚠️ **규율을 지키던 것은 오늘까지 손 핀뿐이었고, 그 손 핀의 수는 바늘마다 다르다** —
  라운드 88~91이 적은 *"스물두 개 남짓"*, 정찰이 같은 낱말을 전수로 세어 낸 **일곱 파일 70**, 그리고
  F가 오늘 `toContain(… accessibilityLabel …)` 꼴을 전수로 다시 세어 낸 **쉰일곱 파일 152**다 —
  ⚠️⚠️ **세 수를 한 낱말로 적지 않는다.** **그런데 어느 바늘로 재도 *모집단을 전수에서 파생하는
  자리는 0건*이라는 사실은 같았고, 오늘 그 0건이 하나가 됐다.**
- **일반형.** ⚠️⚠️ ***배정이 막는다*는 진단은 그 자체로는 검증되지 않는다** — 배정하기 전에는
  *고칠 것이 많아서 미뤘다*와 구별되지 않고, 두 상태는 문서에서 똑같이 읽힌다. ⚠️ **그래서 그
  진단을 값으로 만드는 유일한 걸음은 배정이고, 배정의 결과가 *0건*일 때 그 0이 곧 진단의 증명이다.**
  ⚠️⚠️ **그리고 그 증명은 되돌릴 수 없다: 한 번 배정해 0건이 나오면 다음 라운드부터 그 조건은
  *미도래*로 읽힐 수 없다.** ⚠️ **AD-3이 세운 *순서를 적은 조건은 집힌다*와 AE-5의 *막는 것을 값으로
  적는다*가 오늘 한 칸 더 나아간다** — **막는 것을 적는 것만으로는 부족하고, 세 번째 라운드에는
  그 줄이 *먼저 배정하라*고 적어야 한다**(AF-2가 그렇게 적었고 오늘 그 문장이 이행됐다).
  ⚠️ **다음 라운드가 먼저 세어 볼 만한 것**: 이 문서의 재개 조건 가운데 **막는 것이 *배정*이라고
  적힌 것**이 몇이고, 그중 **몇 라운드째 배정을 기다리는지가 그 줄에 적힌 것**이 몇인가(오늘 하나를
  배정해 닫았고, ⚠️ **나머지가 몇인지는 트랙 D의 경과 축이 문서 전수로는 세지만 *막는 것이
  배정인가*로는 가르지 않는다**).

### AG-3. **조건을 세우는 손에 리뷰가 있고, 리뷰가 세운 조건은 F의 표가 닫힌 뒤에 오므로 구조적으로 다음 라운드의 정찰이 처음 센다** — 그리고 결정형도 결정이 작으면 집힌다

- **사실.** AF절의 결정형 재개 조건 전수표는 **열다섯**이었고, 오늘 다시 세니 **열여섯**이다.
  ⚠️⚠️ **열여섯째를 세운 것은 트랙도 F도 아니라 라운드 91 *리뷰*다** — L-6이 어드민 스윕의 사각에
  *"재개 조건(결정형 · 손은 저장소 안): 라우트 표면과 내비 표의 짝을 어느 계약이 지는지를 P3가
  정하는 날 — 오늘 그 축은 이 스윕이 아니라 라우트 표면 그물의 것이다"* 를 붙였다.
- ⚠️⚠️ **F의 표는 리뷰보다 먼저 닫힌다.** 라운드의 순서가 *트랙 → F → 리뷰*라, **리뷰가 세운 조건은
  그 라운드의 결정형 표에 들어올 수 없고 다음 라운드의 정찰이 처음 센다.** ⚠️ **이것은 결함이
  아니라 순서의 사실이고, 그래서 값으로 적는다** — AC-3이 세운 *사각이 조건을 값으로 적으면 다음
  표에 선다*의 **네 번째** 이행이며, **이번에는 그 손이 리뷰였다는 것이 새 칸이다.**
- ⚠️⚠️ **그리고 오늘 그 하나를 집는다 — 결정형 표에서 집는 것이 다섯 라운드 이어 0건이었고 그
  연속이 끊긴다.** AF절이 *"결정형 표는 다섯 라운드째 0건인데 사건형 배정은 라운드마다 늘고 있고,
  조건의 *형*이 그 조건이 언제 집히는지를 정한다"* 고 적었는데, **오늘 그 문장에 한 칸이 붙는다:
  결정형도 *결정이 작고 조건이 스스로 답을 가리키면* 집힌다.** 정할 것은 *어느 계약이 지는가*
  하나였고 조건이 이미 *"라우트 표면 그물의 것"* 이라고 적어 두었다.
- ⚠️ **집은 뒤에 그 결정이 조건의 문면과 한 칸 갈렸다는 사실도 함께 적는다.** 조건은 *"라우트 표면
  **그물**의 것"* 이라고 적었는데, 오늘 선 것은 저장소 그물이 아니라 **어드민 앱 안의 스윕**이다
  (라운드 89 B·90 B·91 A·91 B가 세운 계열 — *한 앱만 걷는 스윕은 그물 수에 들지 않는다*).
  ⚠️⚠️ **조건이 가리킨 답과 실제로 선 답이 한 칸 다르고, 그 갈림을 트랙이 사각 칸에 두 시점으로
  적었다** — **조건이 답을 가리켜도 그 답이 오늘의 관례와 같은지는 집는 걸음이 다시 물어야 한다.**
- **오늘의 값 — 어드민 소스 0바이트와, L-7의 전제를 처음 지는 계약 하나다.** ⓐ **`page.tsx` 11 ↔
  `NAV_ITEMS` href 11 · 어긋남 0**이라 **고칠 것이 0건**이고, 만든 것은 **세는 자** 하나다.
  ⓑ ⚠️⚠️ **그 계약이 라운드 91 리뷰 L-7의 전제를 처음 진다** — L-7의 사각은 `aria-current`의 **정확
  일치** 한계가 오늘 보이지 않는 이유를 *"설계가 아니라 실측"* 이라고 적으며 **라우트 열하나가 전부
  평면이라는 사실**에 기댔는데, **그 사실을 세는 자리가 0건이었다.** 오늘 그 전제(동적 세그먼트
  0건)를 계약이 값으로 진다. ⓒ 사각 한 칸이 **두 시점**으로 옮겨졌고, 그 칸의 재개 조건은 그날
  이후 **사건형**이 된다.
- **일반형.** ⚠️⚠️ **라운드의 순서가 조건의 수명을 정한다** — 트랙이 세운 조건은 그 라운드의 F가
  세고, F가 세운 조건은 다음 정찰이 세며, **리뷰가 세운 조건은 F의 표 밖에서 한 라운드를 기다린다.**
  ⚠️ **그 기다림은 부채가 아니라 순서이고, 적어 두지 않으면 *왜 이 조건이 지난 표에 없었는가*가
  결함으로 읽힌다.** ⚠️⚠️ **그리고 오늘 그 순서에 값이 하나 붙는다: 리뷰가 세운 조건은 다음 라운드에
  가장 먼저 집히기 좋은 자리다** — 갓 쓰인 조건이라 전제가 아직 참이고, 리뷰가 그 자리를 방금
  들여다본 손이라 조건이 답까지 가리키는 경우가 많다. **오늘 집힌 하나가 정확히 그 모양이었다.**
  ⚠️ **다음 라운드가 먼저 세어 볼 만한 것**: 이 저장소의 재개 조건 가운데 **리뷰가 세운 것**이
  몇이고, 그 조건들이 **정찰·트랙·F 가운데 누구에게 처음 세어졌는가**(오늘 하나를 그렇게 집었고,
  ⚠️ **리뷰가 세운 조건의 전수는 오늘 세지 않았다** — 표식이 남은 자리만 세도 라운드 91 리뷰 넷이다).

### AG-4. **조건도 주석도 자기가 몇 라운드째 서 있는지를 적지 않고, 그래서 *미도래*와 *오래 낡음*이 같은 낯으로 읽힌다** — 시간을 적지 않은 문장은 자기가 낡았다고 말하지 않는다

- **사실 ①(조건 쪽).** 판정 문서의 재개 조건 자리 **333** 가운데 **그 줄 자신이 경과 라운드 수를
  적은 것은 셋**이고, ±5줄 창까지 넓혀도 **70**이며 **나머지 263은 어디에도 없다**(트랙 D 커밋
  시점). ⚠️ **이 절이 조건을 더한 뒤의 오늘 값은 383 · 4 · 83 · 300이고, 두 시점을 한 낱말로 적지
  않는다.**
  ⚠️ AF-2가 지목한 그 병이 값으로 확인됐다 — *"그 수를 조건 자신은 적지 않는다 — 사람이 절을
  거슬러 읽어야 나온다."*
- **사실 ②(주석 쪽).** `app/expenses/new.tsx`의 `merchantFocused` 주석이 *"이 화면의 스크롤러
  (`src/ui.tsx` AppScreen)는 `keyboardShouldPersistTaps` **기본값(`"never"`)** 이라…"* 라고 적는데,
  **`src/ui.tsx`는 라운드 65부터 `"handled"`다.** ⚠️⚠️ **그리고 그 라운드의 계약 주석이 바로 이
  `merchantFocused` 주석을 *자기 근거*로 인용한다** — **인용한 손이 인용당한 자리를 고치지 않았고,
  그 갈림이 스물일곱 라운드를 살아남았다.**
- ⚠️⚠️ **둘은 같은 병의 두 얼굴이다.** 조건은 *언제부터 서 있는가*를 적지 않아 **미도래처럼**
  읽히고, 주석은 *언제 참이었는가*를 적지 않아 **아직 참인 것처럼** 읽힌다. **둘 다 자기가 낡았다고
  말하지 않고, 둘 다 사람이 다른 자리를 읽어야만 드러난다.**
- ⚠️⚠️ **그 낡음이 결함 하나를 덮고 있었다는 것이 이 판정의 값이다.** 그 주석의 결론(*칩을 blur에서
  접지 않는다*)은 **오늘도 옳지만 이유가 바뀌었다** — 첫 탭을 먹는 것은 바깥 스캐폴드가 아니라
  **그 칩 줄 자신의 중첩 `ScrollView`** 다. **문장이 낡아서 그 자리가 *이미 해결된 것*으로 읽혔고,
  그래서 여덟 자리가 스물일곱 라운드를 서 있었다.**
- **오늘의 값 — 대장의 새 칸 하나와, 주석 한 문단의 두 시점 정정이다.** ⓐ 재개 조건 대장이 자리마다
  ⓐ 형(사건형·결정형) ⓑ 손의 위치에 더해 **ⓒ 경과**를 센다(D 커밋 시점 그 줄 **3** · 창 **70** ·
  어디에도 없음 **263** ↔ 이 절 뒤 **4 · 83 · 300**). ⓑ ⚠️ **그 축은 반드시 하한이어야 한다** — F가 이 절을 쓰며 조건을 더하면 자리 수가
  커지고, 등호로 물면 **F가 계약을 맞추려고 문서를 고치게 된다**(AF-5가 값으로 적은 그 비용).
  **라운드 90 D가 고른 쪽을 골랐고, 그래서 오늘 이 절의 걸음이 그대로 통과했다.**
  ⓒ 주석은 **지우지 않고 두 시점으로** 적었다(*"종전 이 자리는 …이었다"* — 옛 문장을 죽이는 것이
  아니라 언제 무엇이 참이었는지를 남긴다 · AE-3이 세운 그 관례).
- ⚠️ **그리고 이 축은 자기가 못 보는 것을 값과 하한으로 든다** — ⓐ **산문으로 오래됨을 말하는 창
  9**(*"오래 서 있었다"* 처럼 수 없이 말한 자리라 기계가 셀 수 없다) · ⓑ **경과가 적혀 있다는
  것과 그 수가 오늘 참이라는 것은 다르다**(창 **83** 전부가 이 사각 안이고, 뒤쪽을 물으려면
  *조건의 도래*를 값으로 가르는 관례가 먼저 서야 한다 — 결정형 15) · ⓒ **짝 문서와 라운드 노트의
  경과 89는 모집단 밖이다.**
- **일반형.** ⚠️⚠️ **시간을 적지 않은 문장은 두 가지로 읽힐 수 있고, 그 둘은 정반대다** — 조건은
  *아직 때가 아니다*와 *오래 방치됐다*로, 주석은 *지금도 참이다*와 *한때 참이었다*로 갈린다.
  **어느 쪽인지는 그 문장 안에 없고, 사람이 다른 자리를 읽어야 나온다.** ⚠️ **그래서 값을 적을 때
  함께 적어야 하는 것은 *언제부터*와 *언제까지*이고**, 그것이 적혀 있으면 낡음이 **자기 자리에서**
  보인다. ⚠️⚠️ **오늘 그 처방이 한쪽에는 계약으로**(대장의 경과 칸) **다른 쪽에는 관례로**(두 시점
  주석) **섰는데, 둘 다 *앞쪽만* 센다** — **적혀 있는가는 세지만 그 수가 오늘 참인가는 아직 아무도
  세지 않는다.**
  ⚠️ **다음 라운드가 먼저 세어 볼 만한 것**: 이 저장소의 소스 주석 가운데 **다른 자리를 인용한
  것**이 몇이고, 그중 **인용당한 자리가 그 뒤에 바뀐 것**이 몇인가(오늘 하나를 그렇게 찾았고 그
  하나가 스물일곱 라운드를 서 있었다 — ⚠️ **그런 인용이 몇인지는 오늘 세지 않았다**).

### AG-5. **문서를 무는 계약이 좌표를 등호로 물 때 적어야 하는 것은 좌표가 아니라 *누가 언제 그것을 옮기는가*이고, 그 의무가 계약 안에 값으로 서면 F의 걸음은 기억이 아니라 이행이 된다**

- **사실.** AF-5가 처방을 적었다 — *"문서를 무는 계약이 좌표를 물 때 물어야 할 것은 그 좌표가
  맞는가가 아니라 **누가 언제 그것을 옮기는가**이고, 그 답은 계약 안에 적혀 있어야 한다."*
  ⚠️ **그때 그 답이 적혀 있던 자리는 하나**(줄 번호 배열 `[22, …]` 위의 주석)**였고, 정찰이 세어 보니
  절 이름을 등호로 무는 자리가 여섯 더 있었으며 그 여섯에는 0건이었다.**
- ⚠️⚠️ **오늘 트랙 E가 그 모집단을 손 목록이 아니라 *자기 소스에서* 파생했고, 그러자 여섯이
  여덟이 됐다** — 정찰의 바늘은 `headingLine("…")` 꼴만 보았고 `startsWith("### 0-1.")` 꼴 둘이 그
  밖에 있었다. **오늘 아홉 자리(절 이름 여덟 · 줄 번호 배열 하나) 다 의무가 적혀 있고 빈 자리
  0건이며, 그 의무를 계약이 *길이 · 누가 · 언제 · 옮긴다*로 문다**(빈 문장을 길이로 막는다).
- ⚠️⚠️ **그리고 그 계약이 오늘 F에게 실제로 한 줄을 지웠다.** F가 A-33 절과 C-3의 세 자리를 올리자
  접근성 표의 두 좌표가 밀렸고, `[22, 1223, 1267]`이 **`[22, 1281, 1333]`**이 됐다 —
  **값(0)도 꼴(인용+정정 · 단언 · 단언)도 한 바이트 바뀌지 않았고 움직인 것은 좌표뿐이다.**
  ⚠️ **`:22`는 A절보다 위라 움직이지 않는다.**
- ⚠️⚠️ **라운드 91 F의 첫 이행과 오늘의 두 번째 이행은 같은 한 줄을 옮겼는데, *잊었을 때 무엇이
  빨개지는지*가 달라졌다.** 라운드 91에는 그 의무가 **주석**이었고 잊으면 ⓖ의 등호가 빨개졌다 —
  **그 빨강은 *좌표가 틀렸다*고만 말하고 *네가 옮겨야 했다*고는 말하지 않는다.** 오늘부터는 ⓗ가
  그 의무의 **실재**를 세므로, 의무를 지우는 손이 있으면 **그 자리가 먼저** 빨개진다.
  ⚠️ **두 자는 서로 다른 것을 센다 — 앞은 좌표의 참, 뒤는 의무의 실재다.**
- ⚠️ **그리고 그 계약은 자기가 못 보는 것을 값과 하한으로 든다** — ⓐ **문서를 파싱해 파생한 좌표는
  이 바늘 밖이다**(라운드 90 D의 축 · 오늘 그 자리 수는 **94**이고 D가 **하한**을 골라 오늘도 초록
  이다 — **등호와 하한의 값이 같은 문서에서 나란히 보인다**) · ⓑ **문서가 소스의 좌표를 무는 반대
  방향은 이 바늘 밖이고 그 수는 훨씬 크다**(이 계약이 읽는 문서 셋만 세어도 서른셋이고, 그 자리들에는
  이동 의무가 한 자도 없으며 **옮기는 손도 반대다** — 소스가 밀리면 문서를 고쳐야 한다) ·
  ⓒ **의무가 적혀 있다는 것과 그 의무가 지켜진다는 것은 다르다** — 이 자는 앞쪽만 세고 **뒤쪽은
  라운드마다 F의 걸음이 답한다.** ⚠️⚠️ **오늘 그 뒤쪽의 답은 *지켜졌다*이고, 그 답을 낸 것은 이
  절을 쓰는 걸음 자체다.**
- **일반형.** ⚠️⚠️ **계약이 무는 값에는 두 층이 있다 — *그 값이 오늘 맞는가*와 *그 값을 내일 누가
  옮기는가*이고, 뒤쪽을 적지 않으면 앞쪽의 빨강이 *결함*으로 읽힌다.** ⚠️ **등호를 고르는 것은
  자리가 사라지거나 새로 생기는 것까지 잡는 대신 라운드마다 손 하나를 부르는 선택이고, 하한을
  고르는 것은 그 손을 부르지 않는 대신 사라짐을 못 보는 선택이다** — **어느 쪽이 옳다는 판정이
  아니라, 고른 쪽의 *비용*을 계약 안에 적어야 한다는 판정이다.** ⚠️⚠️ **그리고 오늘 그 비용을 적는
  일이 관례에서 계약으로 옮겨 갔다** — 라운드 91에는 F가 *기억해서* 적었고, 오늘부터는 계약이
  **자리마다 그 문장의 실재를 세므로** 다음 F는 잊을 수 없다. **관례를 계약으로 바꾸는 걸음의 값은
  *오늘 무엇이 고쳐지는가*가 아니라 *내일 무엇을 잊을 수 없게 되는가*에 있다.**
  ⚠️ **다음 라운드가 먼저 세어 볼 만한 것**: 이 저장소에서 **문서가 소스의 좌표를 무는 반대
  방향**(문서가 `<파일>:<줄>`을 적는 자리)이 몇이고, 그중 **누가 언제 그것을 옮기는지가 적힌 것**이
  몇인가(⚠️ **오늘 이 계약이 읽는 문서 셋만 세어도 서른셋이고 적힌 것은 0건이며, 저장소 전체로는
  그보다 훨씬 크다** — 그리고 그쪽은 **옮기는 손이 반대**라 처방도 같지 않을 수 있다).

## AH. 라운드 93에서 확정한 판정 (2026-08-31 · GAP-093 트랙 F)

라운드 92가 물은 것이 **저장소가 이미 고른 답을 세는 계약이 있는데도 빠진 자리가 보이지 않는
경우는 무엇인가**(답: 그 계약의 모집단이 손 목록일 때 · AG-1)였다면, 라운드 93은 그 답을 **저장소
전수로 돌린 라운드**다 — ⚠️⚠️ **AG-1이 *"나머지가 몇인지는 오늘 세지 않았다"* 로 닫은 그 나머지를
세니 관례를 세는 계약 **예순둘** 가운데 모집단이 손 목록인 것이 **서른둘**이었고, 그 서른둘 안에서
사람이 오늘 겪는 자리 **둘**이 나왔다.** 축은 라운드 81~92와 같이 **사용자 가치**였고(나가는 길 ·
사람이 지은 이름 · 낭독의 모집단 · 운영자 도구 · 저장소가 자기를 세는 자리), 다섯 판정 다 K~AG절과
같이 **결함 보고가 아니라 다음 결정의 입력**이며 2026-08-31 소스에서 확인됐다(라운드 93 트랙
A·B·C·D·E와 통합 커밋 머지 후).
⚠️⚠️ **이번 라운드가 연 것은 핵심 루프의 1단계 *직후*와 그 앞의 첫 문장이다** — 실패한 기록을
고쳐 저장한 사람이 도착하는 화면에 **나가는 길이 0개**였고, 온보딩 직후 사람이 만드는 첫 문장이
**"지훈를 추가했어요"** 였다. 가격·링크·픽셀은 바이트 불변이다.

⚠️⚠️ **이번 라운드의 가장 값진 관측: 손 목록 옆의 주석이 *모집단 전체를 두고* 단언하면, 그 자리는
계약이 세지 않는데도 *이미 해결된 것*으로 읽힌다**(AH-1). 오늘 실피해가 나온 두 자리가 둘 다
그 모양이었다 — `apps/mobile/src/screen-header-back.test.ts`의 손 목록 열하나 위에는
*"스택으로만 도달하는 화면 중 나가는 길이 없는 곳은 **이제 없다**"* 가 **스물두 라운드** 서 있었고,
`apps/admin/src/admin-load-error-copy.test.ts`의 손 목록 열 위에는 *"[다시 시도]가 서는 **열한
자리**"* 가 서 있었다(목록은 열이다). ⚠️ **뒤쪽은 두 수가 둘 다 참이었다는 것이 값이다** — *열*은
화면 수이고 *열하나*는 `styles.retryButton` **출현** 수다(대시보드가 배너를 둘 세운다). **두 수를
한 낱말로 적지 않는다.** ⚠️⚠️ **그리고 두 자리의 값이 서로 다르다**: 앞쪽은 **오늘 사람이 겪는
결함**이고 뒤쪽은 **오늘 0건인 미래의 결함**(열두 번째 화면이 서는 날)인데, **병은 하나다.**

⚠️⚠️ **두 번째 관측: 계약이 아예 0건인 자리에서는 저장소가 이미 세 번 고른 답조차 네 번째 자리에서
고르지 않는다**(AH-2). 사용자가 지은 이름 뒤에 **받침에서 갈려야 하는 조사**를 리터럴로 못 박은
자리가 넷이었고, 그 넷이 **아이 추가**와 **초대 수락 착지** — 사람이 앱에서 가장 먼저 만드는 두
문장이다. ⚠️ **저장소는 같은 물음에 이미 답을 골랐다**(`householdScopePhrase`의 `‘…’ 가구` ·
`pendingInviteTarget`의 `… 초대` · `recordsCountPhrase`의 `기록 N건` — **값의 꼬리를 고정 명사로
끝낸다**), 그리고 `src/home/baby-counter.ts`에는 **조사를 값에서 고르는 순수 함수 셋**이 이미 있었다.
**세는 계약만 0건이었다.**

⚠️⚠️ **세 번째 관측: 사각이 *"묻지 않은 수"* 라고 이름 붙인 크기를 열면 그 안은 한 갈래가 아니고,
열어 본 뒤에야 남는 수의 성격을 말할 수 있다**(AH-3). 라운드 92 B가 남긴 **124**를 트랙 C가 넷째
바늘로 열자 **열 자리**가 들어오고 **114**가 남았으며, 남는 114 가운데 *행이면서 밖*인 것은
**37이 아니라 일곱**으로 좁혀져 **이름과 꼴로** 적혔다. ⚠️ **라운드 92 리뷰 H-3이 *"그 조용한
갈래는 닫혔고"* 라고 적은 것이 오늘 보니 절반만 참이었다** — 닫힌 것은 렌더 프롭 쪽이고 `.map(`이
바로 그리는 행꼴 컴포넌트 쪽은 그대로 조용했다(실피해가 0건이 아니라 **열 자리**였다).

⚠️⚠️ **네 번째 관측: 파일 단위 트랙 배타는 *한 트랙이 다른 트랙의 파일에 만든 부채*를 막지 못하고,
그것을 닫는 손은 트랙이 아니라 통합이다**(AH-4). 트랙 B가 화면 문구 넷을 고치자 그 가운데 하나를
**트랙 C 소유 파일**(`src/a11y-contract.test.ts:1012-1014`)이 **옛 바이트로** 물고 있었다 — B는 그
파일을 열 수 없고(배타) C는 그 축이 아니다(*한 트랙이 한 그물에 축 둘을 얹지 않는다*). B가 그
사실을 사각 `fixed-particle-pinned-by-another-contract`(실측 **1**)와 **사건형 재개 조건**으로
남겼고, ⚠️⚠️ **통합 커밋이 같은 라운드 안에서 그 조건을 발동시켜 실측이 0이 됐다**(`357b021`).
**재개 조건이 다음 라운드가 아니라 그 라운드 안에서 발동한 첫 자리다.**

⚠️⚠️ **다섯 번째 관측: 좌표를 세는 자가 서자 그 자가 곧바로 F에게 여섯 줄을 지웠고, *이동 의무가
적힌 유일한 자리*가 그 의무를 열네 라운드 동안 지키지 않았다는 사실이 드러났다 — 그런데 같은 병이
F 자신의 트립와이어에도 있다**(AH-5). AG-5가 뒤쪽 물음(*의무가 지켜지는가*)을 라운드마다 F의
걸음에 맡겼는데, 트랙 E가 **반대 방향**(문서가 소스의 좌표를 무는 자리)을 처음 전수로 걸어
**좌표 26 · 이동 의무 1**을 냈고 **그 하나가 오늘 거짓이었다**(`kl:2584`가 무는 두 좌표 둘 다).
오늘 F가 **여섯을 두 시점으로 정정하며 그 의무를 처음 이행했고**, 같은 걸음에 의무가 적힌 자리를
**하나에서 여섯**으로 늘렸다(좌표는 **26 → 35**). ⚠️⚠️ **그런데 F가 라운드 92 트립와이어에 적은
수 둘**(`helper-named-reader` **166** · 문턱·래칫 상수 **38**)**은 그 수를 낸 *바늘*이 어디에도 값으로
없어서 오늘 같은 자를 다시 대지 못했다** — 오늘 넓은 바늘로는 각각 **207**과 **53**이다.
**좌표에는 세는 자가 섰는데 수에는 아직 아무도 서지 않았다.**

---

### AG-1~AG-5의 질문 다섯 — 라운드 93의 답 (출처: `docs/5차/round93-scout.md` §1 실측)

⚠️⚠️ **다섯 다 답했고 넷이 트랙을 발동시켰다.** ⚠️ **아래 수는 이 절이 새로 잰 것이 아니라
정찰이 2026-08-31 HEAD(`f206005`)에서 재어 그 노트 §1에 적은 값이고, 출처를 함께 적는다**(O-3의
반대 방향이 아니다 — *판정*은 여기 한 번만 쓰고 *수*는 낸 자리를 가리킨다). ⚠️⚠️ **그리고 이
답들은 계약이 아니라 손으로 잰 수라, 계약이 세는 수와 한 낱말로 적지 않는다.**

| # | 질문(AG-n이 남긴 자리) | 2026-08-31 답 | 낸 방법이 적힌 곳 | 발동 |
| --- | --- | --- | --- | --- |
| 1 | AG-1 — **관례를 세는 계약**이 몇이고 그중 **모집단이 손 목록**인 것이 몇인가 | 테스트 계약 **400** 중 관례를 세는 계약 **62**(트리를 걷는 것 40 · 경로를 배열로 적고 순회하는 것 32 · 겹침 10) · ⚠️⚠️ **손 목록 32**(자리 47) · 그중 **걷기가 한 줄도 없는 것 22** · 오늘 모집단 밖에 자리가 실재하는 것 **둘** | 정찰 §1 「답 1을 낸 방법」(모집단·바늘·사각 넷) | ⚠️⚠️ **트랙 A · 트랙 D** |
| 2 | AG-2 — 재개 조건 중 **막는 것이 *배정*이라고 적힌 것**이 몇이고 그중 **경과가 적힌 것**이 몇인가 | 표기형 재개 조건 **130**(전수 383) 중 *막는 것 = 배정*이 **8** · 그중 경과가 함께 적힌 것 **4** · ⚠️ 산문형 253 안의 같은 말은 바늘 밖 | 정찰 §1 「답 2를 낸 방법」(사각 셋) | ⚠️ 답했고 트랙이 되지 않았다(→ 기각 목록) |
| 3 | AG-3 — 재개 조건 중 **리뷰가 세운 것**이 몇이고 **누구에게 처음 세어졌는가** | 전수 **599**(판정 문서 383 + 소스 216) 중 리뷰가 세운 것 **27**(문서 5 · 소스 22) · ⚠️⚠️ **판정 문서는 F 378 · 리뷰 5이고 소스는 트랙 194 · 리뷰 22** — 문서를 쓰는 손과 소스를 쓰는 손이 다르다 | 정찰 §1 「답 3을 낸 방법」(`git blame` · 사각 셋) | ⚠️ 답했고 트랙이 되지 않았다(→ 기각 목록) |
| 4 | AG-4 — 소스 주석 중 **다른 자리를 인용한 것**이 몇이고 그중 **인용당한 자리가 그 뒤에 바뀐 것**이 몇인가 | 자리 **581**(인용 쌍 618 · 자기 인용 13 · 인용당한 고유 파일 258) · **뒤에 바뀐 쌍 319**(파일 136 · ⚠️ **상한이다**) · 손으로 표본을 풀어 **거짓 하나**(`merchantInputRef` 주석) | 정찰 §1 「답 4를 낸 방법」(사각 넷) | ⚠️ **트랙 B의 절반**(그 주석을 세 시점으로 정정했다) |
| 5 | AG-5 — **문서가 소스의 좌표를 무는 반대 방향**이 몇이고 그중 **이동 의무가 적힌 것**이 몇인가 | 좌표 전수 **929**(문서 38) — 라운드 노트 **902** · 살아 있는 문서 **27** · ⚠️⚠️ **이동 의무가 적힌 것은 하나이고 그 하나가 오늘 거짓이다** · 손으로 풀어 **오늘 거짓인 좌표 여섯**(자리 다섯) | 정찰 §1 「답 5를 낸 방법」(사각 넷) | ⚠️⚠️ **트랙 E** |

⚠️⚠️ **답 5는 트랙 E가 계약으로 다시 재며 정찰과 갈렸고, 그 갈림이 이 표의 값 하나다.**
정찰의 **27**은 *경로가 없는 이름만의 좌표*(`admin-api.ts:524`)를 하나 포함한 수이고, 트랙 E의
계약은 `/`가 있는 경로 꼴만 모집단으로 삼아 **26**을 냈다(`runtime-verification-required.md`에서
**6 → 5**). ⚠️ **어느 쪽이 틀린 것이 아니라 바늘이 다르다 — 두 수를 한 낱말로 적지 않는다.**
같은 이유로 정찰의 전수 **929/38**도 그 계약의 파서로는 **969/39**다(정찰의 수는 하한이었다).
⚠️ **그리고 오늘 F의 걸음 뒤에는 그 26이 **35**이다**(known-limitations 16 → **25** · 아래
트립와이어).

⚠️ **되짚기(라운드 92) — 이 다섯 질문에 답한 자리.** AG-1 → **AH-1** · AG-2 → 이 표의 2행(트랙이
되지 않았고 기각 목록에 재개 조건과 함께 있다) · AG-3 → 이 표의 3행(같다) · AG-4 → **AH-2**(트랙 B가
그 거짓 인용을 세 시점으로 정정했다) · AG-5 → **AH-5**. **판정은 그 판정이 사는 절 하나에만
적는다**(O-3·X-4).

---

### 정찰이 남긴 기록 의무 — 트랙이 정찰과 갈린 자리와, 밖에 둔 것의 크기

⚠️ **정찰의 마지막 줄이 *"각 트랙은 자기 축의 전제를 다시 재고, 정찰과 갈리면 갈렸다는 사실 자체를
값으로 적는다"* 를 요구했다.** 오늘 갈린 자리 넷과, 모집단 **밖**에 둔 것의 크기를 여기 값으로 남긴다.

- ⚠️⚠️ **살아 있는 문서의 좌표 — 정찰 27 ↔ 트랙 E 26.** 여섯째 자리는
  `docs/qa/runtime-verification-required.md:1012`가 무는 `admin-api.ts:524`이고, **경로가 없는
  이름만의 좌표**라 트랙 E의 바늘 밖이다(문서가 어느 파일인지 말하지 않았다). ⚠️ **같은 꼴이 오늘
  살아 있는 문서 넷에 **스물넷** 있다**(F의 걸음 뒤 실측 · 트랙 E 커밋 시점 **17**) — **그 스물넷은 사각
  ⓓ이지 결함이 아니다.**
- ⚠️⚠️ **라운드 노트의 죽은 좌표 — 오늘 셋이고, 셋째를 늘린 손이 정찰 자신이다.**
  `app/(tabs)/records.tsx`는 오늘 **1,774줄**인데 `round65-scout.md:195`와 `round67-scout.md:88`이
  `:1831`·`:1884`를 물고, **`round93-scout.md`가 그 사실을 인용하며 같은 죽은 좌표를 하나 더
  늘렸다.** ⚠️ **정찰을 쓰는 걸음이 곧 그 사각을 키우는 걸음이라는 사실이, 재개 조건 대장의
  `round-notes`(오늘 **410**)와 정확히 같은 모양으로 좌표 쪽에도 있다.**
- ⚠️ **모집단 밖의 크기 — 좌표 943 · 문서 35**(트랙 E 실측). 라운드 노트를 밖에 둔 판단 자체가
  손이고, 밖에 두었다는 사실과 그 크기를 값으로 적는 것이 그 계약의 마지막 `it`이다.
- ⚠️⚠️ **살아 있는데 모집단 밖인 문서 하나 — `docs/5차/design-restore-spec.md`(좌표 3).**
  라운드 노트가 아닌데 `COORDINATE_LIVE_DOCS` 넷에 들어 있지 않다. **그 문서를 넣을지는 소유가
  갈리는 결정이라 트랙 E가 재개 조건으로 남겼고, 이 절이 그 조건을 값으로 받는다.**
  ⚠️ **재개 조건(결정형 · 손은 저장소 안): `COORDINATE_LIVE_DOCS` 손 목록을 전수로 바꿀지 정하는 날 —
  그날 먼저 물을 것은 *`docs/5차/**` 가운데 라운드 노트가 아닌 문서를 무엇으로 가를 것인가*이고,
  그 첫 모집단은 오늘의 셋이다.**
- ⚠️ **트랙 B가 정찰의 처방 하나를 값으로 정정했다.** 정찰은 한글이 아닌 끝에서 *"종전 조사를 그대로
  둔다"* 를 권했는데, **종전 조사가 아이 화면에서는 `를`(받침 없는 형)이고 가구 화면에서는 `과`(받침
  있는 형)이라 그대로 두면 같은 이름이 두 화면에서 서로 다른 갈래로 떨어진다.** 저장소의 답은
  한 갈래(**받침 없는 형**)이고 그 근거는 `baby-counter.ts`가 이미 적어 둔 문장이다.
- ⚠️ **트랙 B의 모집단이 정찰과 갈렸다 — 33 ↔ 38.** 정찰은 *리터럴 조사가 붙은 자리*만 셌고 계약은
  **조사가 서는 자리**(리터럴 **29** + 값에서 고름 **9**)를 센다. ⚠️⚠️ **그러지 않으면 넷을 고치는
  순간 그 넷이 모집단에서 사라져 래칫이 *줄지 않는다*를 물 수 없다** — **두 수를 한 낱말로 적지 않는다.**

---

### 이월 — 판정 그대로이고 재실측 값만 갱신한다 (자리만 가리킨다 · O-3)

⚠️⚠️ **이월 다섯은 전부 보류 유지이고, 갱신 한 줄씩은 그 판정이 사는 절에 있다.**

- **이 스캐너가 쿼리로 분류한 자리의 낭독** — 재실측 상태 변화 0, A-20 `#85` 선행 → **U절 머리말**.
  ⚠️⚠️ **이번 라운드에도 이 줄에는 갈래가 있다**: 트랙 C가 **a11y 그물을 열었지만 그 여섯 화면의
  축이 아니었다**(C가 연 것은 *`.map(`이 그리는 행꼴 컴포넌트의 라벨*이다). ⚠️ ***"열지 않았다"* 와
  *"열었지만 그 축이 아니다"* 는 다른 문장이고**(라운드 87이 세운 구별), **접점 0건으로는 다섯 라운드
  연속 · *열었지만 그 축이 아니다*로는 두 라운드 연속이다.**
...
- **S-3(어드민 `disabled`)** — 판정 종전 그대로, 브라우저 확인 `#130` 선행. ⚠️ **`app/items/page.tsx` ·
  `app/links/page.tsx`가 *끊긴 채로* 세 라운드가 됐다**(89·90에 열렸고 91·92·93 제안이 다 그 밖이다 —
  이번 라운드 어드민 트랙이 연 파일은 **계약 하나**이고 화면은 0바이트다).
- **`withdrawn_at`** — ⚠️ **이번 라운드도 다시 재지 않았다**(라운드 88이 자기 정정으로 확정한 값을
  여기 옮겨 적지 않는다). **다섯 라운드 연속 재측정 대상이 아니다.**
- **AA-R ① 연속 실패 재판정** — deps는 오늘도 `[isError]` 하나 · 자동 재시도·폴러 **0건**.
  ⚠️ 이번 라운드의 어느 트랙도 그 훅도 그 훅의 호출부 화면도 열지 않았다.

---

⚠️⚠️ **AA-3의 이행 — 결정형 재개 조건 전수와 오늘의 처분이다. 라운드 92의 열여섯에서 하나가 집혀
열다섯이 됐고, 트랙 D가 둘을 세워 오늘 HEAD에서는 열일곱이다.**
⚠️⚠️ **검산 — 정찰이 센 열다섯과 이 표의 열일곱은 갈린 것이 아니라 *시점이 다르다*.** 정찰은 라운드
92 머지 시점(`f206005`)에서 셌고 그때 표는 **열다섯**이었다(#16 라우트 표면이 라운드 92 트랙 C에
집혀 내려갔다 — 그 사각 칸이 *"그날 이후의 재개 조건은 사건형으로 바뀐다"* 를 자기 안에 두 시점으로
적었다). **이 표는 A~F 머지 뒤라 트랙 D가 자기 계약에 적은 결정형 둘이 더 서 있다.**
⚠️⚠️ **AG-3이 *"조건을 세우는 손에 리뷰가 있고 F의 표가 닫힌 뒤에 오므로 다음 정찰이 처음 센다"* 고
적었는데, 오늘은 그 구조의 **한 칸 앞**이 보인다 — 트랙이 세운 조건은 *정찰*이 아니라 **그 라운드의
F가 처음 센다.** 라운드 92에는 트랙들의 새 조건이 전부 사건형이라 이 칸이 보이지 않았다.**

| # | 결정형 조건 | 사는 곳 | 손 | 오늘의 처분(2026-08-31 실측 · A~F 머지 뒤) |
| --- | --- | --- | --- | --- |
| 1 | 막대를 포커스 가능하게 | AA-2 | 안 | **집지 않는다 — 값 0.** 두 화면(`apps/admin/app/analytics/page.tsx` · `apps/admin/app/clicks/page.tsx`) 다 `trend.showTable` 표가 값을 텍스트로 준다(재실측 · 변화 0) |
| 2 | 기록 탭 검색의 분류 갈래 | Z절 | 안 | **집지 않는다 — 어긋남 0건 · 아홉 라운드 이어 같은 답.** `RECORDS_SEARCH_FIELDS_LABEL`이 여전히 하나이고 placeholder가 그 값에서 파생한다. ⚠️ 이번 라운드는 그 화면조차 접점 0건이다 |
| 3 | 준비템 분류 필수 입력 | Z-3 | **밖**(카탈로그 정책) | 집지 않는다. 변화 0 |
| 4 | 서버의 중복 대기 초대 방지 | AA-4 | **밖**(초대 정책) | 집지 않는다. 변화 0 |
| 5 | 감사 뷰의 대상 축 필터 | AA절 기각 | **밖**(서버 DTO 0건) | 집지 않는다 — ⚠️⚠️ **`apps/api/**`가 두 라운드 연속 접점 0건이다**(라운드 91이 다섯 라운드 만에 열었고 92·93이 다 그 밖이다) · 서버 소스와 DTO는 여전히 0바이트다 |
| 6 | 타일 안 배지 | AA-1 | **밖**(디자인 승인) | 집지 않는다. 변화 0 |
| 7 | C-3 잠금 오버레이 TalkBack 투과 | 접근성 표 C-3 | **밖**(사람·기기·날짜) | **F의 소유 밖이다** — 오늘로 **스물일곱 라운드째 미확인**(67~93). ⚠️ **오늘 이 줄에 붙는 값은 확인이 아니라 *세 자리를 함께 올리는 걸음이 두 라운드째 계약이 지운 의무의 이행이다*라는 사실이다** |
| 8 | JSX 사용을 참조로 세는 판정 | `packages/test-utils/src/dead-export-ledger.ts` | 안 | **집지 않는다 — 일곱 라운드 이어서다.** 막는 것은 **크기와 문턱의 겹침** 그대로다(`.tsx`의 `export function` 오늘도 **141** · `derived-exemptions` **44 중 22 · 여유 0**). ⚠️ 그 파일은 이번에도 아무 트랙의 것이 아니다 |
| 9 | 재조회 실패 창에서 옛 목록을 남길지 | AC절 기각 | 안 | **집지 않는다 — 화면 구조 결정이고 오늘도 접점 0건**(`src/offline/**` 무접촉) |
| 10 | 앵커 대장의 `helper-named-reader` | `packages/test-utils/src/comment-tolerant-anchor-ledger.ts` | 안 | **집지 않는다 — 래칫이다.** ⚠️⚠️ **다만 이번 라운드에 이 줄이 처음 움직였다**: 앵커 전수가 **698 → 699**(code-only 620 → **621**)로 **아홉 번 만에 처음** 갈렸고(트랙 B의 새 스윕이 세운 단언 하나), comment-tolerant **70/70** · comment-only **8/8** · unanchored **0** · 면제 **8**은 그대로다. ⚠️⚠️ **그리고 그 사각의 크기는 오늘 다시 재지 못했다** — 아래 기각 목록의 첫 줄이 그 이유를 값으로 든다 |
| 11 | 재개 조건 대장의 `prose-only` | `packages/test-utils/src/resume-condition-ledger.ts` | 안(**F**) | **집지 않는다.** 오늘 **230**(119 → 145 → 165 → 184 → 208 → **230**) — ⚠️ **늘린 것의 대부분이 이 절 자신이다.** 소급 적용은 한 라운드의 F가 질 수 없다 |
| 12 | 재개 조건 대장의 `round-notes` | `packages/test-utils/src/resume-condition-ledger.ts` | 안 | **집지 않는다 — 관례 0건.** 오늘 **410**(노트 **52**벌 · 377 → 410으로 늘었고 늘린 것이 `round93-scout.md` 한 벌이다). ⚠️ **라운드마다 한 벌씩 자라는 사각이라는 사실이 이 줄의 값이고, 오늘 그 사실이 좌표 쪽에서도 똑같이 보인다**(죽은 좌표 셋 중 하나를 정찰이 늘렸다) |
| 13 | 그물 파일이 자기를 이름으로 선언하는 표식 | `packages/test-utils/src/contract-net-ledger.test.ts` | 안 | **집지 않는다 — 관례를 새로 정하는 일이다.** 표식 **0건** · ⚠️⚠️ **자기 배제 관례의 수가 오늘 또 갈렸다**: 좁은 바늘(이름 넷)로는 **열넷**이고 넓은 바늘(`SELF`가 든 상수 전수)로는 **열여섯 · 이름 여섯**이다(`SELF_CODE` · `COORDINATE_SELF_PATH`가 그 갈림이고 뒤쪽은 **라운드 93 트랙 E**가 세웠다) — **AF-5의 열하나 · 정찰의 열둘 · 라운드 92 F의 열넷 · 오늘의 열여섯을 한 낱말로 적지 않는다.** **정합을 세는 자리는 오늘도 0건이다** |
| 14 | 공유 테스트 DB를 언제 무엇으로 비울지 | `apps/api/test/harness-catalog-cost.test.ts` | 안 | **집지 않는다 — 술어는 오늘도 참이다.** `wooriai_test`의 `item_templates` **9,439**(활성 **8,797** · DB **738MB**)이고 `wooriai_dev`는 **62**(활성 62 · **24MB**). ⚠️⚠️ **오늘 F가 다시 재니 정찰과 한 자리도 다르지 않다 — 라운드 92 이후 이 수를 움직인 손이 *리뷰의 실행* 하나뿐이고, 이번 라운드의 다섯 트랙도 F도 api 스위트를 돌리지 않았다**(네 시점의 실측이 됐고, 다섯째 시점은 값이 같다) |
| 15 | 조건의 *도래*를 값으로 가르는 관례 | `packages/test-utils/src/resume-condition-ledger.ts` | 안 | **집지 않는다 — 관례를 정하는 일이다.** 기다리는 자리는 오늘도 **둘**(대장의 `typed-but-unverified`와 경과 축의 사각 ⓑ · 창 **79** 전부가 그 사각 안이다) |
| 16 | **어드민 계약의 *다른* 손 목록 둘을 누가 전수 파생으로 옮기는가** | `apps/admin/src/admin-load-error-copy.test.ts`(라운드 93 트랙 D가 신설) | 안 | ⚠️⚠️ **오늘 처음 표에 선다 — 세운 손이 트랙이다.** 같은 파일의 `discardedSites`(**열**)와 `FALLBACK_PHRASE_OCCURRENCES`(**열둘**)가 여전히 손으로 적은 모집단이고, *한 트랙이 한 축을 진다*는 규율 때문에 이번 라운드가 열지 않았다. ⚠️ **AH-1이 이름 붙인 그 병이 *같은 파일 안에* 둘 더 남아 있다는 사실이 이 줄의 값이다** |
| 17 | **어드민에 컴포넌트 렌더 하네스를 세울지** | `apps/admin/src/admin-load-error-copy.test.ts`(라운드 93 트랙 D가 신설) | 안 | ⚠️⚠️ **오늘 처음 표에 선다.** 그 계약의 사각 `source-not-render`가 *"[다시 시도]가 **실제로 그려지는가**는 묻지 않는다 — 조건이 거짓인 가지 안에 있어도 초록이다"* 를 값으로 들고, 오늘 어드민에 렌더 하네스 표식은 **0건**이다. ⚠️ **집지 않는 이유는 크기다**(하네스 하나를 세우는 일이고 그 결정은 이 라운드의 축이 아니다) |

⚠️⚠️ **이 표가 라운드 92의 표와 다른 점 넷.** ⓐ **열여섯이 열다섯이 됐다가 열일곱이 됐고, 줄인 것은
*집힘*이며 늘린 손은 *트랙*이다**(#16은 라운드 92 트랙 C가 집었고, 오늘의 #16·#17은 라운드 93 트랙
D가 세웠다). ⓑ ⚠️⚠️ **이번 라운드는 결정형에서 집는 것이 0건이다** — 다섯 트랙 어느 것도 이 표의
조건을 소진하지 않았고, **집는 것이 0건인 라운드가 한 라운드 만에 돌아왔다**(라운드 92가 다섯 라운드
연속을 끊었으므로 오늘은 *한 라운드째*다 · **끊김과 끊긴 기간은 다른 두 칸이다**). ⓒ ⚠️⚠️ **#10이
아홉 라운드 만에 처음 움직였다** — 정찰이 *"후보 2의 스윕이 소스 리더를 세우면 그 사각이 오른다"*
고 **미리** 값으로 적어 두었고, 오늘 실제로 움직인 것은 그 사각이 아니라 **앵커 전수 자체**다
(698 → 699). **미리 적어 둔 덕에 이 오름을 *부채*로 읽지 않는다.** ⓓ ⚠️⚠️ **#14의 다섯째 시점은
값이 같고, 같다는 것이 이번 라운드의 사실이다** — 하네스를 돌리지 않은 라운드는 그 수를 키우지 않는다.

⚠️⚠️ **이번 라운드가 실측하고 기각한 스물일곱을 값으로 남긴다 — 전부 재개 조건과 함께**(V-2).
⚠️ **라운드 92의 스물넷을 그대로 이어 재었고, 새로 선 셋은 AG-2·AG-3의 축 둘과 ⚠️⚠️ *F 자신의
트립와이어에 바늘이 적히지 않은 수*다**(마지막 것이 이번 라운드가 처음 세는 자리다).
⚠️ **아래 마지막 두 줄은 그 스물일곱에 들지 않는다** — 라운드 62~92가 남긴 **상태 변화 0의 표식**과
**제외 목록 준수 확인**이고, 재론 대상이 아니라 스윕이 그것들을 다시 줍지 않게 하는 자리다.

- ⚠️⚠️ **F의 트립와이어에 *바늘이 적히지 않은 수* — 오늘 처음 세어 값으로 남기고 집지 않는다.**
  라운드 92 F가 이 자리에 적은 수 가운데 **둘**은 오늘 같은 자를 다시 댈 수 없었다:
  `helper-named-reader` **166**과 문턱·래칫 상수 **38**이다. 두 수 다 *어떻게 셌는가*(모집단·바늘)가
  그 자리에도 대장에도 없어서, 오늘 F가 넓은 바늘로 재니 각각 **207**(비테스트를 뺀 `.test.ts` 가운데
  `NAMED_SOURCE_READER` 꼴을 지닌 파일)과 **53**(`FLOOR`·`CAP`·`RATCHET`·`THRESHOLD`가 이름에 든
  상수 선언 전수)이 나온다. ⚠️⚠️ **어느 쪽이 옳다는 판정이 아니다 — 두 수가 서로 다른 자에서 나왔고
  그 자를 대조할 방법이 없다는 것이 이 줄의 값이다.** 대장이 지닌 하한(**164**)은 두 수 아래라
  오늘도 참이고 계약은 초록이다. ⚠️ **집지 않는 이유는 축이다** — 바늘을 값으로 적는 일은 트립와이어
  전체를 다시 쓰는 걸음이고, 이 라운드의 F는 좌표 여섯을 정정하는 걸음을 이미 졌다.
  ⚠️⚠️ **두 시점(라운드 93 리뷰 M-2) — 위 문장의 *둘*은 오늘 *하나*가 됐다.** 리뷰가 그 자리에
  물었다: **`helper-named-reader` 166의 바늘은 저장소 안에 이미 있었다.**
  `packages/test-utils/src/comment-tolerant-anchor-ledger.ts:532`의 `NAMED_SOURCE_READER`로 걷고
  `collectCommentToleranceAnchors().outside`에서 `reason === "helper-named-reader"` 인 자리를 세면
  **파일 하나가 한 자리**로 떨어지고(그 대장의 계약이 이미 같은 자를 대고 있다), **오늘 HEAD 실측은
  167**이다 — 라운드 92의 **166**에서 **정확히 하나** 올랐고 **그 하나가 트랙 B의 `readSweptSource`**
  라서, 트랙 B가 자기 머리말에 미리 적어 둔 예측(*"그 수가 오른다"*)이 **+1로 적중했다.**
  ⚠️ **그러니 이 줄이 *재현 불가*로 남기는 것은 문턱·래칫 상수 38 한 칸뿐이다** — 그쪽은 갈래 셋
  (하네스 4 · 제품 8 · 소스 텍스트 26)의 모집단이 어디에도 적혀 있지 않아 넓은 바늘의 53과 대조할
  길이 오늘도 없다. ⚠️⚠️ **그리고 이 정정이 위 문장을 무르게 하지 않는다** — *수를 낼 때 바늘을 함께
  적지 않으면 다음 라운드가 같은 자를 못 댄다*는 사실은 그대로 참이고, 오늘 달라진 것은 **그 사실을
  받치는 근거가 둘에서 하나로 줄었다**는 것과 **하나는 찾으면 있었다**는 것이다(⚠️ 뒤쪽이 더 값이
  크다: *바늘이 없다*와 *바늘로 가는 길이 안 적혀 있다*는 서로 다른 병이고, 처방도 다르다 —
  앞쪽은 자를 새로 세워야 하고 뒤쪽은 **가리키기만 하면 된다**).
  ⚠️ **재개 조건(결정형 · 손은 저장소 안): 트립와이어의 수마다 *그 수를 낸 바늘*을 어디에 적을지
  정하는 날 — 그날 먼저 물을 것은 *바늘을 F의 표에 적을 것인가 그 수를 내는 계약 안에 적을 것인가*
  이고(뒤쪽이면 계약이 늘고 앞쪽이면 표가 길어진다), 첫 모집단은 이 표의 수 전수다.**
- **AG-2의 축(막는 것이 *배정*인 조건 여덟) — 재었고 트랙이 되지 않는다.** 여덟 중 넷만 경과를 함께
  적는데, ⚠️ **그 넷을 세는 자는 이미 있다**(경과 축 · 라운드 92 D)**이라 새 축이 아니라 두 축의
  교집합이다.** ⚠️ **재개 조건(결정형 · 손은 저장소 안): 경과 축과 *막는 것* 축을 한 대장이 함께
  질지를 정하는 날 — 그날 먼저 물을 것은 *배정을 말한 산문 253을 어떻게 셀 것인가*다.**
- **AG-3의 축(재개 조건을 세운 손) — 재었고 트랙이 되지 않는다.** ⚠️⚠️ **막는 것은 규율도 배정도
  아니라 *바늘*이다**: 오늘의 답은 `git blame`으로 냈는데 계약이 blame을 돌리면 느리고 이력에 기대며
  rebase에 흔들린다. 표식(`리뷰 L-N`)으로 세면 또 다른 수(문서 19 · 소스 15)가 나오고 **그 둘 중
  어느 쪽이 *손*인지는 아직 아무도 정하지 않았다.** ⚠️ **재개 조건(결정형 · 손은 저장소 안): 조건의
  *손*을 무엇으로 셀지를 정하는 날 — 그날의 첫 모집단은 오늘의 27이다.**
- **문턱이 재는 세계(제품 ↔ 하네스 ↔ 소스 텍스트) — 재었고 집지 않는다.** ⚠️ **오늘의 수는 위 첫
  줄이 값으로 든다**(라운드 92의 38과 오늘의 53은 서로 다른 자다). ⚠️ **집지 않는 이유는 축과
  앱이다** — 가장 아픈 자리는 어드민이고 이번 라운드의 어드민 축은 **[다시 시도] 관례**다.
  ⚠️ **재개 조건(사건형 — 종전 그대로): 문턱이 재는 세계의 표기가 한 트랙의 축이 되는 라운드가
  서는 날.**
- **자기 배제 관례 — ⚠️⚠️ 재었고 좁은 바늘로 열넷 · 넓은 바늘로 열여섯이며, 이름은 넷이 아니라 여섯이다.**
  `LEDGER_SELF_FILES` **4** · `SWEEP_SELF_FILES` **2** · `SELF_PATH` **4** · `SELF_FILE` **4**(좁은
  바늘 **14**) 위에 `SELF_CODE` **1**(라운드 92 트랙 C)과 `COORDINATE_SELF_PATH` **1**(라운드 93
  트랙 E)이 있다. ⚠️⚠️ **라운드 92 F의 문장(*"트랙 C가 `SELF_FILE` 하나 · 트랙 E가
  `SELF_PATH` 하나"*)은 그 라운드에 대해 참이었고, 오늘 갈린 것은 *바늘*이다** — 라운드 92 트랙 C는
  `SELF_FILE`과 함께 `SELF_CODE`도 세웠고, 라운드 93 트랙 E가 세운 것은 `SELF_PATH`가 아니라
  **`COORDINATE_SELF_PATH`** 라 **좁은 바늘이 둘 다 못 본다**(그래서 좁은 바늘로는 열넷 그대로다).
  **이름이 넷이 아니라는 사실 자체가 이 줄이 기다리는 결정의 근거다.** ⚠️ **재개 조건(결정형 · 손은
  저장소 안 — 종전 그대로): 그물 파일이 자기를 이름으로 선언하는 표식을 세울지 정하는 날 — 그날 이
  열여섯의 이름도 함께 정해진다.**
- **공유 테스트 DB를 비우는 일 — 재었고 집지 않으며, 오늘의 값은 정찰과 같다.**
  `wooriai_test` **9,439**(활성 **8,797** · **738MB**) ↔ `wooriai_dev` **62**(활성 62 · **24MB**).
  ⚠️ **다섯째 시점인데 값이 움직이지 않았고, 그것이 *이 라운드가 하네스를 돌리지 않았다*는 사실의
  가장 짧은 증명이다.** ⚠️ **재개 조건(결정형 · 손은 저장소 안 — 종전 그대로): 하네스가 *어느 표를
  언제 비우는가*를 정하는 날 — 먼저 물을 것은 *비운 뒤에도 초록인가*이고 첫 모집단은 라운드 91
  트랙 C가 적어 둔 여섯 표다.**
- **`derived-exemptions`의 절반 문턱 — 재었고 여유가 0 그대로다.** 사문 전수 **44 중 면제 22**로
  **정확히 절반**이고 조건은 *"넘는 날"* 이라 아직 아니다(⚠️ **다섯 트랙 어디에도 새 `export const`가
  서지 않았다** — 트랙 B의 신설 모듈은 `export function` 셋뿐이고 그 계약이 부정 단언으로 문다).
  ⚠️ **재개 조건(사건형): 면제가 사문 전수의 절반을 넘는 날.**
- **`dead-export-ledger.ts`의 결정형 8 — 전제는 참이고 일곱 라운드째 집지 않는다.** `.tsx`의
  `export function` 오늘도 **141**이고 막는 것은 **크기와 문턱의 겹침**이다. ⚠️ **이번에도 그 파일은
  아무 트랙의 것이 아니다**(세 라운드 연속). ⚠️ **재개 조건(결정형 · 손은 저장소 안 — 종전 그대로):
  그 판정이 서는 날 — 그날 141이 모집단으로 들어오고 같은 걸음에 절반 문턱을 다시 재야 한다.**
- **앵커 대장 — ⚠️⚠️ 아홉 번 만에 전수가 처음 움직였고, 움직인 자리가 어디인지가 값이다.**
  앵커 전수 **699**(code-only **621** · comment-tolerant **70**/래칫 70 · comment-only **8**/래칫 8 ·
  unanchored **0**) · 면제 **8**. ⚠️ **늘어난 하나는 `code-only`이고 그것은 *코드에만 있는 단언*이라
  대장이 바라는 쪽이다** — 부채가 아니라 계약이 하나 늘었다는 뜻이다. ⚠️ **집지 않는 이유는
  래칫이다.** ⚠️ **재개 조건(결정형 · 손은 저장소 안 — 종전 그대로): 저장소가 소스 리더 이름을 한
  벌로 모으는 날, 또는 이 대장이 헬퍼 정의를 따라가 루트를 푸는 법을 배우는 날.**
- **재개 조건 대장의 `prose-only` · `round-notes` — 재었고 둘 다 집지 않는다.** 형 표기가 없는 산문
  조건은 **119 → 145 → 165 → 184 → 208 → 230**로 늘었고(⚠️ **이번에 늘린 것의 대부분이 이 절
  자신이다**), 라운드 노트는 **377 → 410**이며 늘린 것이 **`round93-scout.md` 한 벌**이다.
  ⚠️ **재개 조건은 둘 다 결정형 · 손은 저장소 안이고 종전 그대로다.**
- **재개 조건 대장의 `unmarked-source-prose` — ⚠️ 재었고 98에서 129가 됐으며, 이번에도 늘린
  손이 트랙이다.** 라운드 91 F의 **56** → 라운드 92 정찰 **68** → 라운드 92 F **98** → 오늘
  **129**. ⚠️ **하한(10) 설계 덕에 오늘도 초록이다.** ⚠️ **재개 조건(사건형 — 종전 그대로):
  이 수가 형 표기를 지닌 소스 조건 수를 넘는 날.**
- **어드민의 무명 표 하나 — 재었고 동결된 채로 둔다.** 오늘 재실측 **표 18 · 이름 17**이고 스윕 밖의
  하나는 `apps/admin/src/components/ProductLinkBulkReplace.tsx`다. ⚠️ **집지 않는 이유는 축이다.**
  ⚠️ **재개 조건(사건형 — 종전 그대로): 표 이름 스윕의 모집단이 `app/**` 밖으로 넓어지는 날.**
- **어드민 `<section>` 34에 이름을 주지 않는다 · `<nav>`·`<form>`의 이름 0건 — 재었고 그것이 판정이다.**
  오늘도 `<section>` **34 · 이름 0건** · `<nav>` **하나** · `<header>` **하나** · `<form>` **여섯**이다.
  ⚠️ **재개 조건(결정형 · 손은 저장소 안 — 종전 그대로): 한 화면의 카드가 너무 많아 랜드마크로
  건너뛰는 길이 필요해지는 날.**
- **어드민의 `<button type=` — 재었고 관례가 완전하다.** 어드민 비테스트 `.tsx` **18**의 `<button`
  여는 태그 **68**이 **전부 `type=`을 지닌다**. ⚠️ **재개 조건: 없다 — 스윕이 다시 줍지 않게 하는
  표식이다.**
- **어드민의 연타 가드 절반 — 재었고 이번에도 트랙 밖이다.** `<button` **68** · `disabled=`를 지닌
  줄 **44**(변화 0). ⚠️ **재개 조건(사건형 — 종전 그대로): 연타 가드가 어드민에서 한 트랙의 축이
  되는 라운드가 서는 날.**
- **`accessibilityState={{ disabled }}`의 열한 자리 — 재었고 결함이 아니다**(RN `Pressable`이
  `disabled`를 자동 병합한다). ⚠️ **재개 조건(사건형): 이 앱이 Pressable이 아닌 누르는 자리를
  `disabled`와 함께 세우는 날.**
- **`useMutation` 밖의 쓰기 경로 — 재었고 오늘도 핵심 루프 밖이다.** 선언 **31** · 컨트롤 **27** ·
  핸들러 **1** · 안 막음 **3**(변화 0). ⚠️ **재개 조건(사건형 — 종전 그대로): `useMutation` 밖의
  쓰기가 핵심 루프 안에 서는 날.**
- **연타 스윕의 사각 `control-verdict-is-file-scoped` — 재었고 전제가 참이며 미도래다.**
  ⚠️ **이 라운드의 어느 트랙도 그 파일을 열지 않았다.** ⚠️ **재개 조건(사건형 — 종전 그대로): 한
  파일 안에서 뮤테이션 둘 이상이 서로 다른 누르는 자리를 지고 그중 하나만 `disabled`를 받는 자리가
  처음 발견되는 날.**
- **반쪽 프롭 일곱 — 재었고 오늘도 일곱이다**(짝 **37** · role 단독 **1** · live region 단독 **6**).
  ⚠️⚠️ **트랙 C가 a11y 그물을 열었는데도 그대로다** — C의 축은 *행꼴 컴포넌트의 낭독 라벨*이고 이 축은
  라운드 90 A의 것이다(*한 트랙이 한 그물에 축 둘을 얹지 않는다* · 두 라운드 연속으로 같은 문장이 선다).
  ⚠️ **재개 조건(사건형 — 종전 그대로): 반쪽 자리가 늘거나 `silent`가 처음 서는 날.**
- **AC-1의 일곱 · AC-2의 0 — 오늘 다시 돌리지 않았다.** 어느 트랙도 형제 화면이 갈리는 사건을 만들지
  않았고 `client.ts`·`admin-api.ts`의 시그니처를 늘리지 않았다. ⚠️ **재개 조건 둘 다 사건형이고
  종전 그대로다.**
- **신설 스윕이 재개 조건을 한 줄도 적지 않은 자리 — 재었고 오늘도 0건이다(조건 미도래).**
  라운드 93이 세운 것 셋(`src/korean-particle-guard.test.ts` 신설 스윕 · `src/screen-header-back.test.ts`의
  전수 파생 개편 · `packages/test-utils/src/repo-self-description.test.ts`의 새 축)과 트랙 C·D의 축이
  **다섯 다 재개 조건을 적었다.** ⚠️ **재개 조건(사건형 — 종전 그대로): 새 스윕이 재개 조건 0건으로
  서는 라운드가 다시 오는 날.**
- **조회 실패 창의 모집단 · 응답 필드 축(모바일) · `HouseholdMember.joinedAt` · 기록 탭 검색의 분류
  갈래 · 막대 포커스 가능화 — 다섯 다 판정 그대로이고 상태 변화 0.** ⚠️ **기록 탭 검색은 아홉 라운드
  이어 같은 답이다.** ⚠️ **재개 조건 다섯 다 종전 그대로다**(둘은 결정형 · 손은 저장소 안).
- **이월 다섯(U절 셋 · S-3 · `withdrawn_at`) — 판정 종전 그대로이고 접점만 갱신한다**(위 이월 목록).
  ⚠️ **재개 조건은 다섯 다 종전 그대로다.**
- **성능 넷 — ⓐ·ⓒ는 재었고 미도래, ⓑ는 오늘도 값이 있으며, ⓓ는 접점이 두 라운드 연속 0건이다.**
  ⓐ **첫 페인트**(어느 트랙도 `useQuery` 선언을 늘리지 않았다 — 트랙 A가 더한 것은 여는 태그의 속성
  **하나**다) · ⓒ **번들**(⚠️ **새 런타임 의존성 0건** — 트랙 B의 신설 모듈은 import가 0줄이다) ·
  ⓑ **렌더 비용**(제품 **62** ↔ 하네스 **8,797**) · ⓓ **api의 루프**(⚠️⚠️ **`apps/api/**` 접점 0건
  두 라운드 연속**). ⚠️ **재개 조건 넷 다 종전 그대로다.**
- **N-4의 두 문턱 — 16라운드 이어 미발동이고 준비템 탭 비가상화는 이번에도 제안하지 않는다.**
  ⚠️⚠️ **오늘도 두 수가 나오고 둘이 서로 다른 사실이다**(제품 **62** ↔ 하네스 **8,797**).
  ⚠️ **어드민 카드가 세는 수는 여기 옮겨 적지 않는다**(O-3).
- **손이 밖인 다섯 — 넷은 다른 사람이 내릴 결정이고 하나는 배정이다.** 준비템 분류 필수 입력 ·
  서버의 중복 대기 초대 방지 · 감사 뷰의 대상 축 필터 · 타일 안 배지는 **카탈로그 정책 · 초대 정책 ·
  서버 계약 · 디자인 승인**이 각각 선행이고, C-3은 **사람·기기·날짜 배정**이다(이번 라운드로
  **스물일곱 라운드째 미확인**). ⚠️ **재개 조건(전부 결정형 · 손은 저장소 밖): 그 결정이 서는 날.**
- **`${seller}에서 구매하기` 둘 — 제외 확인이지 제안이 아니다.** 준비템 판매처 1:1과 구매 확인
  판매처 라벨은 **영구 기각** 축이다(라운드 62~92 그대로). ⚠️ **재개 조건: 없다.**
- **감사 뷰 CSV · 콘텐츠 리비전 편집 표면 · 어드민 손 미러 · `SYNC_STATUS_RETRY_ALL_LABEL` · 지출 입력
  두 화면의 조립 · CSV 왕복 다섯 열 손실 · `refund` 생성 불가 · `link_health`의 `errors` 카운터 ·
  서버 알림 층 · 홈의 손 폴 다섯 · 공유 카드 왕복 · 어드민 카탈로그 전량 조회 · 미출처 틴트 둘 ·
  첫돌 이후 마일스톤 고착 · api 하네스 동시 실행 구멍 · 서버 중복 아이 가드 부재 · 크래시 파이프라인
  부재 · `Share.share`의 catch 없는 `void` 둘 · 결제 수단 기본값 — 라운드 62~92가 남긴 그대로이고
  상태 변화 0.**
- **제외 목록 준수 확인**: 준비템 목록 **가격 표시**(잠금 · ⚠️ **다섯 트랙 어디에도 가격·링크 수가
  들어가지 않았다** — 트랙 A가 바꾼 것은 **나가는 길**이고 트랙 B가 바꾼 것은 **조사 한 글자씩 넷**이며
  링크의 표시·순서·문구·가격은 바이트 불변이다) · 오프라인 로컬 아이 복구 · 외부 계정/키/자산 ·
  **C-3 잠금 오버레이 낭독**(**스물일곱 라운드째**) · **P-2 법무 대조** · P-3 테스트 건수 자동화 ·
  **표기 방언 통일** · S-4 파기 `targetId` · 40주 초과 달력 · `onBudgetRelevantChange` ·
  4가구/`viewedHouseholdId` · **지출→리포트 정확성 축** · **공유 왕복 축** · **준비템 완료율 동기부여** ·
  **준비템 탭 비가상화** · **홈 수치 정합** · **`getHome` 카탈로그 전량 읽기** — **전부 오늘도
  제안하지 않는다.**

⚠️⚠️ **트립와이어 92 → 93 — 라운드 92가 남긴 래칫·하한을 다섯 트랙과 통합 커밋, 그리고 F가 머지된
뒤 다시 잰 대조표다.** ⚠️ **정찰의 수를 옮겨 적지 않는다** — 아래는 **오늘 HEAD에서 다시 잰 값**이고,
정찰과 갈린 자리는 그 칸이 갈렸다고 적는다.
⚠️⚠️ **시점은 하나다 — *A~F 머지 뒤 · 리뷰 전*.** 예외 0건이고, 재개 조건 대장의 자리 수도 접근성
표의 좌표 수도 짝 문서의 행 수도 전부 F가 자기 걸음을 끝낸 뒤의 값이다.
⚠️⚠️ **그리고 이번 표가 라운드 92의 표와 크게 다른 칸이 하나 있다 — *다시 잴 수 없었다*는 칸이
둘 섰다.** 그 둘은 값이 아니라 **바늘이 없어서** 비었고, 그 사실이 AH-5의 절반이다.
⚠️⚠️ **두 시점 — 라운드 93 리뷰(M-2)가 그 둘 가운데 하나를 채웠다.** `helper-named-reader`의
바늘은 **저장소 안에 있었다**(`comment-tolerant-anchor-ledger.ts:532`의 `NAMED_SOURCE_READER` +
`collectCommentToleranceAnchors().outside`의 `reason` 거르기) — 없었던 것은 *바늘*이 아니라
**그 칸에서 바늘로 가는 길**이었고, F는 그 자를 찾지 않은 채 넓은 바늘을 새로 댔다.
**그래서 오늘 그 칸은 167이고**(라운드 92의 166에서 **+1** · 트랙 B의 예측이 적중했다),
**비어 있는 칸은 문턱·래칫 상수 38 하나뿐이다.** ⚠️ **그 하나는 여전히 재현되지 않는다** —
*하네스 4 · 제품 8 · 소스 텍스트 26*이라는 갈래 셋의 모집단이 어디에도 적혀 있지 않아서, 오늘도
넓은 바늘의 53과 대조할 방법이 없다. ⚠️⚠️ **그리고 이 정정이 AH-5의 일반형을 바꾸지 않는다** —
*수는 바늘이 적혀 있지 않으면 아무도 풀 수 없다*는 문장은 그대로 참이고, 오늘 달라진 것은 그
문장을 받치는 **근거가 둘에서 하나로 줄었다**는 것뿐이다.

| 자리 | 라운드 92가 남긴 값 | 2026-08-31 HEAD 재실측(**A~F 머지 뒤** · 리뷰 전) | 움직였는가 |
| --- | --- | --- | --- |
| 사문 대장 항목 / 래칫 | 22 / 22 | **22 / 22** | 그대로다 — 어느 트랙도 `dead-export-ledger.*`를 열지 않았다(⚠️ **결정형 8이 일곱 라운드째 집히지 않고, 그 파일이 아무 트랙의 것도 아닌 세 번째 라운드다**) |
| 사문 대장의 사각 **여덟** | 22 · 0 · 226 · 20 · 55 · 105 · 141 · 0 | **22 · 0 · 226 · 20 · 55 · 105 · 141 · 0** | **그대로다 — 여덟 자리가 한 자리도 다르지 않다**(세 라운드 이어서다) |
| 사문 전수 / 면제 / 모듈 | 44 / 22 / 6 | **44 / 22 / 6** | 그대로다 — ⚠️ **정확히 절반이고 여유 0**이며 정찰의 예상(*"새 `export const` 0건일 것"*)이 실측으로 맞았다(트랙 B의 신설 모듈이 `export function` 셋뿐임을 그 계약이 부정 단언으로 문다) |
| `CLOSED_BLIND_SPOTS` | 셋 | **셋** | 그대로다 |
| ⚠️⚠️ 앵커 대장(앵커 전수 · code-only · comment-tolerant/래칫 · comment-only/래칫 · unanchored) | 698 · 620 · 70/70 · 8/8 · 0 | **699 · 621 · 70/70 · 8/8 · 0** | ⚠️⚠️ **움직였다 — 아홉 번 만에 처음이고, 움직인 것은 `code-only` 하나다**(트랙 B의 신설 스윕이 세운 단언). **대장이 바라는 쪽으로 늘었다** — 주석에 기대지 않는 단언이 하나 늘었다는 뜻이고 부채가 아니다 |
| 앵커 대장의 면제 | 8 | **8** | 그대로다 |
| ⚠️⚠️ 앵커 대장의 사각 `helper-named-reader` | 166(하한 164) | **167**(하한 **164**는 오늘도 참이다) | ⚠️⚠️ **움직였고, 라운드 93 리뷰가 이 칸을 채웠다 — 이 표에 *다시 잴 수 없다*고 적혀 있던 자리다.** ⚠️⚠️ **바늘이 저장소에 있었다**: `packages/test-utils/src/comment-tolerant-anchor-ledger.ts:532`의 `NAMED_SOURCE_READER`로 걷고 `collectCommentToleranceAnchors().outside`에서 `reason === "helper-named-reader"` 인 자리를 센다(**파일 하나가 한 자리** · `readSource` 자신은 뺀다 · 같은 자를 그 대장의 계약이 이미 대고 있다). ⚠️ **두 시점**: 라운드 92 F **166** → 오늘 **167**이고 **오른 하나가 트랙 B의 `readSweptSource`다** — 그 파일이 자기 머리말에 *"이 수가 오른다"* 고 **미리** 적어 둔 예측이 **+1로 적중했다**. ⚠️ **F가 넓은 바늘로 낸 207은 다른 자다**(비테스트를 뺀 `.test.ts` 가운데 `NAMED_SOURCE_READER` 꼴을 지닌 **파일** 전수 — 대장이 이미 모집단 안에 둔 것까지 센다). **두 수를 한 낱말로 적지 않는다**(AH-5) |
| 재개 조건 대장(자리 · 괄호 표기 · 손의 위치 · 결정형 · 손 누락 · 소스 표기/괄호 전수) | 383 · 127 · 35 · 36 · 1 · 11/15 | **433 · 146 · 44 · 45 · 1 · 11/15** | ⚠️⚠️ **문서 쪽 넷이 움직였고 소스 쪽 둘은 그대로다** — 늘린 것은 **이 절 자신**이다(AH절 다섯 · 결정형 열일곱 · 기각 스물일곱). ⚠️⚠️ **~~트랙 A~E도 소스에 조건을 적었지만 전부 산문이라~~ 이 설명은 거짓이었다 — 라운드 93 리뷰(M-3)가 정정한다**(옛 문장을 지우지 않고 남긴다 · AE-3). **트랙 A~E가 소스에 적은 조건 가운데 열일곱은 산문이 아니라 *표기형*이다**: `screen-header-back.test.ts` **5** · `korean-particle-guard.test.ts` **6** · `admin-load-error-copy.test.ts` **5** · `a11y-contract.test.ts` **1**. ⚠️⚠️ **그런데도 표기 수(11/15)가 한 자리도 안 움직인 진짜 이유는 *바늘*이다** — 좁은 바늘 `markedSourceNeedle()`(`packages/test-utils/src/resume-condition-ledger.ts:702-704`)이 `⚠️`와 `재개` 사이에 **아무것도 없을 것**을 요구하는데, 열일곱 가운데 열여섯은 `⚠️`가 아예 없는 데이터 필드(`reopenCondition:` 문자열)이고 하나(`admin-load-error-copy.test.ts:441`)는 `⚠️ **재개 조건(…)**` 꼴이라 **마크다운 굵게 두 글자가 사이에 끼어** 걸리지 않는다. ⚠️⚠️ **그리고 `sourceAxisFilesFrom`이 `marked.length === 0`인 파일을 모집단에서 통째로 빼므로, 그 열일곱은 사각 `unmarked-source-prose`(98 → **129**)에 *틀린 이름으로* 흡수됐다** — 그 사각의 이름은 *표기를 지니지 않은* 산문인데 이 열일곱은 **괄호로 형을 밝히고 있다.** **두 자리를 함께 보아야 그 라운드가 조건을 얼마나 적었는지가 나온다**는 문장 자체는 그대로 참이고, 오늘 갈린 것은 *왜 안 보이는가*다. ⚠️ **재개 조건(결정형 · 손은 저장소 안): `⚠️`와 `재개` 사이에 강조 표식(`**`)을 허용할지 정하는 날** — 그날 먼저 물을 것은 *바늘을 넓힐 것인가 소스의 표기 관례를 좁힐 것인가*이고(앞쪽이면 `markedSourceNeedle`이 `[\s*]*`를 물어 `admin-load-error-copy.test.ts:441` 한 자리가 들어오고, 뒤쪽이면 열일곱 자리의 손이 각자 `⚠️`를 옮겨 붙여야 한다), 첫 모집단은 이 열일곱이다. **손 누락은 1 그대로다** |
| 재개 조건 대장의 사각 **여덟** | 208 · 3 · 377 · 17 · 4 · 23 · 11 · 98 | **230 · 4 · 410 · 17 · 4 · 26 · 11 · 129** | ⚠️⚠️ **움직였다** — `round-notes`는 **정찰 노트 한 벌**이 늘렸고(377 → 410 · 노트 **52**벌), `prose-only`와 `unmarked-source-prose`는 **이 절과 다섯 트랙**이 늘렸다. ⚠️ **`folded-notation` 3 → 4와 `one-line-two-sites` 23 → 26도 이 절이 늘렸고, 나머지 넷은 한 자리도 다르지 않다** |
| 재개 조건 대장의 **경과 축**(그 줄 · 창 · 어디에도 없음) | 4 · 83 · 300 | **7 · 93 · 340**(읽지 못한 수사 **`['몇']`** · 래칫 **3 · 61**) | ⚠️⚠️ **움직였다 — 세 시점이다**: 라운드 92 F가 **4 · 83 · 300**을, 리뷰가 창을 **79**로 정정했고, 오늘 이 절이 그 위에 얹혀 **7 · 93 · 340**이다(그 줄 자신이 경과를 적은 자리가 넷에서 일곱이 됐다 — 늘린 셋이 이 절이다). ⚠️⚠️ **정찰이 *"AG절 본문의 83과 오늘의 79는 시점이 다르다"* 고 미리 값으로 적어 둔 그 칸이다** |
| 표기를 지닌 소스 파일 / 그 뿌리(걷는 파일) | 셋 · 전수 파생(968) | **셋 · 전수 파생**(표기 **11** · 괄호 전수 **15**) | 표기 셋은 그대로다 — ⚠️ **이번 라운드가 세운 신설 계약 셋은 형 표기를 괄호로 밝히지 않고 산문으로 적었다**(그래서 이 칸이 아니라 `unmarked-source-prose`에서 보인다) |
| 계약 그물 수 | 열다섯 | **열다섯** | ⚠️⚠️ **그대로다 — 그물 수가 늘지 않은 네 번째 라운드다.** 트랙 B의 앱 내 스윕, 트랙 A·C·D의 전수 파생 개편, 트랙 E의 새 축이 다섯 다 그 수에 들지 않고 **왜 들지 않는지를 각자의 머리말이 값으로 적는다** |
| 반쪽 프롭(짝 · role 단독 · live region 단독) | 일곱(37 · 1 · 6) | **일곱(37 · 1 · 6)** | 그대로다 — ⚠️ **a11y 그물을 연 트랙이 또 하나 있는데도 그대로다**(C의 축은 행꼴 컴포넌트의 라벨이다) |
| 어드민의 상태 문장 | 자리 47 / 파일 14 / 출구 45(`role="alert"` 34 · `role="status"` 11) / 면제 2 | **47 / 14 / 45(34 · 11) / 2** | 그대로다 |
| 어드민의 표 이름 / 무명 표 | 17 / 0(`app/**`) · 스윕 밖 하나(표 전수 18) | **17 / 0**(`app/**`) · 스윕 밖 **하나**(표 전수 **18**) | 그대로다 |
| 어드민 `<section>` / 이름 | 34 / 0 | **34 / 0** | 그대로다 — 이름을 주지 않는 것이 판정이다 |
| 어드민 `<main>`(중첩) / `aria-current` | 2(중첩 0) / 1 | **2(중첩 0) / 1** | 그대로다 — ⚠️ **트랙 D가 어드민 계약을 열었는데도 랜드마크 축은 바이트 불변이다**(어드민 소스 **0바이트**) |
| 어드민 라우트 표면(`page.tsx` ↔ `NAV_ITEMS`) | 11 ↔ 11 · 어긋남 0 · 세는 계약 하나 | **11 ↔ 11 · 어긋남 0 · 세는 계약 둘** | ⚠️⚠️ **값은 그대로이고 *같은 전수를 파생하는 자*가 하나 늘었다** — 트랙 D가 `admin-route-surface.test.ts`를 **복사하지 않고 같은 사실을 각자 파생**했다(사본을 만들면 두 자리가 갈린다) |
| ⚠️⚠️ 어드민 [다시 시도] 관례(모집단 · 판정) | **손 목록 열**(주석은 *"열한 자리"*) | **`page.tsx` 전수 11** · `retry` **10** · `message-only` **1** · `underived-banner` **0**(상한) · `no-load-failure` **0** | ⚠️⚠️ **오늘 처음 전수로 세어졌다** — 손 목록이 사라졌고 자리마다 판정 하나가 소스에서 나온다. ⚠️ **실피해는 오늘 0건이고 값은 *열두 번째 화면이 서는 날*에 있다** |
| 어드민 `<button` / `type=` / `disabled=`(줄) | 68 / 68 / 44 | **68 / 68 / 44** | 그대로다 |
| 모바일 `useMutation`(선언 · 컨트롤 · 핸들러 · 안 막음) | 31 · 27 · 1 · 3 | **31 · 27 · 1 · 3** | 그대로다 — 그 스윕을 여는 트랙이 0건이다 |
| 모바일 Pressable / 역할 / 라벨 | 145 / 145 / 122 | **145 / 145 / 122** | 그대로다 |
| 모바일 스크롤 컨테이너(전수 · 판정 명시 · 키보드가 뜰 수 없는 화면 · 미가드) | 17 · 11 · 6 · 0 | **17 · 11 · 6 · 0** | 그대로다 — ⚠️ **트랙 A가 모바일 화면을 열었는데도 그대로다**(더한 것은 `<ScreenHeader>`의 속성 하나이고 스크롤러가 아니다) |
| ⚠️⚠️ 모바일 낭독 라벨(식 전수 · 목록 안 · 직접 · 파생 · 갈리지 않음) | 162 · 38 · 36 · 2 · 0 | **162 · 48 · 44 · 4 · 0** | ⚠️⚠️ **움직였다 — 넷째 바늘이 열 자리를 데려왔고 그 열은 오늘의 38과 겹치지 않는다.** 목록 **밖**은 **124 → 114**로 줄었고, 그중 *행이면서 밖*인 것은 **일곱**으로 좁혀져 이름과 꼴로 적혔다. ⚠️ **상수 라벨은 그 옆에 따로 서고 오늘도 73이다**(같은 파일을 다른 자로 재면 `accessibilityLabel="` 출현이 **74**다 — **두 수를 한 낱말로 적지 않는다**) |
| ⚠️⚠️ 모바일 나가는 길(라우트 · 탭 밖 · `ScreenHeader` · 배선 · 판단 · 출구 없음) | **세는 자리 손 목록 열하나** | **36 · 31 · 18 · 12 · 6 · 0**(재수출 5 · `ScreenHeader` 미사용 8) | ⚠️⚠️ **오늘 처음 전수로 세어졌다** — 손 목록 열하나가 전수 파생이 됐고 배선이 **11 → 12**로 늘었다(늘린 하나가 `app/sync-status.tsx`다). *스택인데 나가는 길이 없다*는 **0을 상한으로** 물린다 |
| ⚠️⚠️ 모바일 조사(모집단 · 꼬리 고정 · 값에서 고름 · 갈리는데 고정) | **세는 자리 0건**(정찰 33 · 29 · 0 · 4) | **38 · 29 · 9 · 0**(걷는 파일 **정찰 하한 281 → HEAD 282**) | ⚠️⚠️ **오늘 처음 세어졌다** — 정찰의 33과 계약의 38은 **바늘이 다르다**(계약은 *값에서 고른 자리*도 같은 모집단에 둔다 · 그러지 않으면 넷을 고치는 순간 모집단이 줄어 래칫이 무너진다). ⚠️ **걷는 파일 281은 *정찰 하한*이지 HEAD의 값이 아니다**(라운드 93 리뷰 M-4 정정): `SCOUT_LOWER_BOUNDS.sweptFiles`가 그 이름대로 하한이고, **트랙 B가 `src/text/korean-particles.ts`를 신설해 HEAD 실측은 282**다(`apps/mobile/{app,src}/**`의 비테스트 `.ts`·`.tsx` 전수). **하한은 올리지 않는다** — 계약은 `toBeGreaterThanOrEqual`로 물고, 하한을 오늘의 값으로 올리면 파일 하나가 지워지는 정직한 걸음이 빨강을 맞는다 |
| 문턱·래칫 상수 | 38(하네스 4 · 제품 8 · 소스 텍스트 26) | **다시 재지 못했다**(넓은 바늘로는 **53**) | ⚠️⚠️ **이 칸도 비었다** — 라운드 92 F가 38을 낸 바늘이 적혀 있지 않다. ⚠️ **오늘 트랙 E 한 곳만 해도 `*_FLOOR`·`*_CAP` 넷을 세웠으므로 오른 것은 분명하고, 얼마인지는 말할 수 없다**(AH-5) |
| ⚠️⚠️ 자기 배제 관례(자리 / 이름) | 열넷 / 넷 | **좁은 바늘 열넷 / 넷 · 넓은 바늘 열여섯 / 여섯** | ⚠️⚠️ **좁은 바늘로는 그대로이고 넓은 바늘로는 둘 늘었다** — `SELF_CODE`(라운드 92 C가 `SELF_FILE`과 **함께** 세웠다)와 `COORDINATE_SELF_PATH`(라운드 93 E)가 그 갈림이다. ⚠️ **라운드 92 F의 문장은 그 라운드에 대해 참이었고 갈린 것은 바늘이다** — 좁은 바늘이 이름 넷만 보므로 **이 축은 이름이 늘 때마다 조용히 하한이 된다.** **정합을 세는 자리는 오늘도 0건이다** |
| 접근성 표 → 짝 문서 좌표(자리 · 인용 번호 · 서로 다른 번호 · 표면 · 역방향 · 어긋남) | 94 · 98 · 94 · 35 · 8 · 0 | **96 · 100 · 96 · 37 · 8 · 0** | ⚠️ **움직였다** — F가 A-34(`#109`·`#110`)를 세우며 넷이 둘씩 늘었다(역방향과 어긋남은 그대로). ⚠️ **라운드 90 D가 이 축을 *하한*으로 세운 덕에 F의 걸음이 그대로 통과했다** |
| 짝 계약의 이동 의무(좌표 자리 · 절 이름 · 줄 번호 배열 · 의무가 빈 자리) | 9 · 8 · 1 · 0 | **9 · 8 · 1 · 0** | 그대로다 — ⚠️⚠️ **그리고 오늘 그 의무가 두 번째로 이행됐다**: `[22, 1281, 1333]`이 **`[22, 1343, 1404]`** 가 됐고 F가 같은 걸음에 옮겼다(값도 꼴도 바이트 불변 · `:22`는 A절보다 위라 움직이지 않는다) |
| ⚠️⚠️ 문서→소스 좌표(살아 있는 문서 넷 · 실재 · 범위 안 · 이동 의무) | **세는 자리 0건** | **35 · 35 · 35 · 6**(모집단 밖 **943** · 문서 35 · 죽은 좌표 **3**) | ⚠️⚠️ **오늘 처음 세어졌고, 그 자가 서자마자 F에게 여섯 줄을 지웠다.** 트랙 E 커밋 시점 **26 · 26 · 26 · 1**이었고, **F가 좌표 여섯을 두 시점으로 정정하며 자리와 의무가 둘 다 늘었다** — ⚠️ **정찰의 27과 계약의 26은 바늘이 다르다**(경로 없는 이름만의 좌표) |
| 짝 문서(행 수 · 실기기 · 브라우저 · 서버 · 작업) | 168 · 138 · 20 · 9 · 1 | **170 · 140 · 20 · 9 · 1** | ⚠️ **움직였다** — `#169`(트랙 A) · `#170`(트랙 B) **둘 다 실기기**이고 **브라우저는 0건**이다(두 라운드 연속 · `실기기`가 둘인 것은 라운드 87 이후 처음이다) |
| 접근성 표 | A-33까지(`#108`) · Batch 42 | **A-34까지(`#110`) · Batch 43** | ⚠️ **움직였다** — 행 **둘**(`#109` 트랙 B · `#110` 트랙 C)과 문단 **셋** |
| 접근성 표 C-3 경과 수(세 자리) | 스물여섯 · 스물여섯 · 스물여섯(갈림 0) | **스물일곱 · 스물일곱 · 스물일곱**(갈림 0) | ⚠️ **움직였고, 세 자리를 함께 올렸다** — 한 자리만 올리면 짝 계약이 빨개진다 |
| `MAX_DIVERGENCE` ↔ 문서가 옮겨 적은 그 상수 | **0** ↔ 문서 세 자리 **0** · 세는 계약 하나 | **0** ↔ 문서 세 자리 **0** · 세는 계약 하나 | 그대로다 — ⚠️ **경과 수를 올리는 걸음이 *"0을 넘지 않는가"* 를 한 글자도 바꾸지 않았다**(세 라운드 연속) |
| 접근성 짝 계약의 상수 인용 좌표 | `22 · 1281 · 1333` | **`22 · 1343 · 1404`** | ⚠️⚠️ **움직였고 F가 함께 옮겼다** — A-34 절과 C-3의 세 자리, 머리말의 라운드 93 문단이 아래 두 좌표를 밀었다. **값도 꼴도 바이트 불변이고 움직인 것은 좌표뿐이다**(두 번째 이행) |
| 공유 테스트 DB / 활성 카탈로그 ↔ 제품 카탈로그 | 732MB · 9,402(활성 8,765) ↔ 62 | **738MB · 9,439(활성 8,797)** ↔ **62**(`wooriai_dev` 24MB) | ⚠️⚠️ **정찰 시점에서 한 자리도 움직이지 않았다** — 늘린 손이 라운드 92 *리뷰의 실행*이었고 **이번 라운드의 트랙도 F도 api 스위트를 돌리지 않았다.** **N-4의 판정은 미발동 그대로다** |

⚠️ **이 표가 라운드 92의 트립와이어 관례에 더하는 것이 셋이다.** ⓐ ⚠️⚠️ **시점 예외가 0건인 세
번째 표다.** ⓑ **세는 자리가 0건이던 축 셋이 오늘 처음 수를 얻었다**(나가는 길의 전수 · 조사의
모집단 · 문서→소스 좌표) — 그중 둘이 **사람이 오늘 겪는 자리**를 냈다. ⓒ ⚠️⚠️ **그리고 처음으로
*다시 잴 수 없는 칸*이 둘 섰다** — 라운드 92 F가 적은 수 둘의 바늘이 어디에도 없어서다.
**이 표의 값은 수의 대조인데, 대조할 자가 없는 수는 표에 있어도 대조되지 않는다**(AH-5).
⚠️⚠️ **두 시점(라운드 93 리뷰 M-2): 그 둘이 하나가 됐다.** `helper-named-reader`의 자는
`comment-tolerant-anchor-ledger.ts:532`에 있었고 리뷰가 그것을 대어 **167**을 채웠다(라운드 92의
166 → **+1** · 오른 하나가 트랙 B의 `readSweptSource`). **남은 빈 칸은 문턱·래칫 상수 38 하나**이고,
그 하나는 갈래 셋(하네스 4 · 제품 8 · 소스 텍스트 26)의 모집단이 적혀 있지 않아 오늘도 재현되지
않는다. ⚠️ **ⓒ의 문장을 지우지 않는 이유**: *둘이 섰다*는 것은 이 표가 세워진 시점의 사실이고,
**하나가 채워졌다는 것은 그 뒤에 일어난 다른 사실이다** — 앞 문장을 지우면 *왜 이 라운드에 그
칸을 찾아 나섰는가*가 함께 사라진다.
⚠️ **한 칸은 반대 방향으로도 섰다**: 자기 배제 관례를 F가 *넓은 바늘*로 다시 재니 라운드 92 F가
적은 이름 넷이 여섯이었다 — **정찰의 수가 낡는 것과 F의 수가 하한인 것은 같은 병의 두 얼굴이고,
오늘 그 병이 F의 표 안에서 한 번 더 보였다.**

**이 라운드가 짝 문서에 남긴 것.** 확인의 표에 **`#169`·`#170` 둘**이 서고 §0의 네 숫자와 합계·
머리말의 재계산 줄·§1-1 머리말의 라운드 구간이 파싱으로 다시 세어졌으며, 접근성 표에는
**A-34 `#109`·`#110` 둘**과 **C-3의 세 자리 갱신**, 그리고 **짝 계약의 좌표 한 줄**이 함께 옮겨졌다.
⚠️ **`#47`의 인용도 두 시점으로 고쳤다** — 그 행이 라운드 63부터 아이 추가 토스트를 *"…**를**
추가했어요."* 로 조사를 고정해 인용하고 있었고, 트랙 B가 그 조사를 값에서 고르게 만든 오늘 그
인용은 거짓이 된다. ⚠️⚠️ **소스가 고쳐지고 문서가 뒤에 남는 그 모양이 이번 라운드에 두 번 났다**
(짝 문서의 `#47`과 이 문서의 좌표 여섯) — **한쪽은 F가 같은 라운드에 잡았고 다른 쪽은 열네 라운드가
걸렸다. 그 차이를 만든 것은 규율이 아니라 *세는 자*다**(AH-5).
⚠️⚠️ **표면 배분이 라운드 92와 다르다 — 실기기 **둘** · 브라우저 0건이고, `실기기`가 둘인 것은
라운드 87 이후 처음이다. 그리고 그 이유가 §0의 수보다 값이 크다**: 다섯 트랙 가운데 **제품 소스를
한 바이트라도 고친 것이 둘**이고(A의 속성 한 줄 · B의 조사 넷) **나머지 셋은 화면을 한 곳도 열지
않았다**(행꼴 컴포넌트의 낭독 라벨 · 어드민 [다시 시도] 관례 · 문서→소스 좌표 — 셋 다 **화면 0건 ·
문구 0건 · 렌더 0건**이다). ⚠️ **라운드 84가 세운 그 구분**(*"확인할 것이 없다"* 가 아니라 *"폰에
보이는 동작을 한 곳도 바꾸지 않았다"*)**의 여덟째 판이다.**
⚠️⚠️ **A-34가 A-33과 다른 점 하나는 그 절의 머리말이 진다**: A-33은 *계약이 있는데도 왜 못 봤는가*를
물었고, **A-34는 그 물음을 저장소 전수로 돌린 뒤에 남는 것 — *계약이 아예 0건인 자리*를 묻는다.**
⚠️ **기기 조건은 A-33과 같지만 재료가 다르다** — 이름 셋(받침 있는 · 없는 · 비한글)을 미리 정해
두고 밟아야 갈림이 보인다.
⚠️⚠️ **C-3은 오늘로 스물일곱 라운드째 미확인**이고, ⚠️ **이번 라운드가 그 줄에 더하는 값은 경과
수가 아니라 *그 세 자리를 함께 올리는 걸음의 의무를 이제 양방향으로 센다*는 사실이다** — 확인은
여전히 배정을 기다린다.

### AH-1. **손 목록 옆의 주석이 *모집단 전체*를 두고 단언하면, 계약이 세지 않는 자리가 *이미 해결된 것*으로 읽힌다** — 오늘 실피해가 나온 두 자리가 둘 다 그 모양이었다

- **사실.** AG-1이 *"관례를 세는 계약이 몇이고 그중 모집단이 손 목록인 것이 몇인지는 오늘 세지
  않았다"* 로 닫았고, 오늘 저장소 전수로 세니 테스트 계약 **400** 가운데 관례를 세는 계약 **62**
  (트리를 걷는 것 40 · 경로 배열을 순회하는 것 32 · 겹침 10)이고 **모집단이 손 목록인 것이 32**
  (자리 47 · **걷기가 한 줄도 없는 것 22**)다. ⚠️⚠️ **그 서른둘 안에서 사람이 오늘 겪는 자리 둘이
  나왔다.**
- ⚠️⚠️ **둘 다 목록 옆 주석이 목록보다 큰 단언을 하고 있었다.**
  - `apps/mobile/src/screen-header-back.test.ts`의 `backWiredScreens`(손 **열하나**) 위에는
    *"스택으로만 도달하는 화면 중 나가는 길이 없는 곳은 **이제 없다**"* 가 있었다 — **모집단 전체를
    두고 한 단언인데, 계약이 실제로 센 것은 *목록에 적힌 열하나가 관례를 지키는가*이지 *관례를 져야
    하는 자리가 이것뿐인가*가 아니다.** 라운드 55가 둘을 더한 뒤로 **스물두 라운드** 자라지 않았다.
  - `apps/admin/src/admin-load-error-copy.test.ts`의 `RETRY_BUTTON_SCREENS`(손 **열**) 위에는
    *"조회 실패 배너 아래 [다시 시도]가 서는 **열한 자리**"* 가 있었다. ⚠️ **두 수가 둘 다 참이었다** —
    *열*은 화면 수이고 *열하나*는 `styles.retryButton` **출현** 수다(대시보드가 배너를 둘 세운다).
    **두 수를 한 낱말로 적지 않는다.**
- ⚠️⚠️ **그런데 두 자리의 값이 서로 다르고, 그 다름이 이 판정의 절반이다.**
  앞쪽은 **오늘 사람이 겪는 결함**이었다 — `apps/mobile/app/sync-status.tsx`는 목록이 빌 때만 서는
  [닫기] 말고는 화면 안의 나가는 길이 **0개**였고(전역 `headerShown: false`라 OS 헤더가 없다),
  ⚠️⚠️ **앱은 실패 행을 고쳐 저장한 사람을 스스로 그 화면으로 보낸다**(`POST_SAVE_SYNC_STATUS_DESTINATION` ·
  핵심 루프 1단계 직후 · **그때 그 목록은 비어 있지 않다**). 뒤쪽은 **오늘 0건**이다 —
  `users-lookup`이 [다시 시도] 없이 문장만 받는 것은 검색형 화면이라는 **판정**이고 같은 파일의 다른
  `it`이 이미 그 자리를 이름으로 적고 있었다. **그 자리의 값은 *열두 번째 화면이 서는 날*에 있다.**
- **오늘의 전수와 트랙 뒤.** 나가는 길: 라우트 **36** · 탭 밖 **31** · 한 줄 재수출 **5** ·
  `ScreenHeader`를 쓰는 것 **18** · 배선 **11 → 12** · *뒤로가기가 없는 것이 판단*이 **6**(온보딩
  다섯 + 딥링크 착지 하나 · 이유가 40자 하한의 사람 문장으로 붙는다) · *스택인데 나가는 길이 없다*
  **0건**(상한). [다시 시도]: `page.tsx` 전수 **11** · `retry` **10** · `message-only` **1** ·
  *배너는 있는데 판정에서 파생하지 않는다* **0건**(상한) · **어드민 소스 0바이트**.
- **일반형.** ⚠️⚠️ **손 목록은 그 자체로는 거짓말을 하지 않는다 — 거짓말을 하는 것은 그 옆에서
  *모집단 전체*를 두고 단언하는 문장이다.** 목록이 모집단이면 그 계약이 답할 수 있는 물음은
  *목록의 자리가 관례를 지키는가* 하나뿐인데, 주석은 흔히 *관례를 져야 하는 자리가 이것뿐인가*까지
  말한다. 그 두 문장은 문서에서 똑같이 읽히고, 뒤쪽이 적혀 있으면 **다음 라운드가 그 자리를 다시
  열지 않는다.** ⚠️ **그래서 모집단을 전수로 바꾸는 걸음은 옛 문장을 지우는 걸음이 아니라 *그 문장이
  참이 되게 만드는* 걸음이어야 한다**(오늘 트랙 A가 `onBack` 한 줄로 그렇게 했고, 옛 문장은 두
  시점으로 남겼다).
- ⚠️ **다음 라운드가 먼저 세어 볼 만한 것**: 손 목록이 모집단인 계약 **서른둘** 가운데 **그 목록 옆
  주석이 *모집단 전체*를 두고 단언한 것**이 몇인가(오늘 둘을 이름으로 집었고, ⚠️ **나머지 서른의
  주석은 읽지 않았다** — 그리고 그 서른의 *완전성*도 아무도 세지 않았다).
- ⚠️ **되짚기(라운드 92)** — 이 질문에 답한 자리는 **AH-1**이다(판정은 여기 옮겨 적지 않는다 ·
  O-3·X-4).

### AH-2. **저장소가 이미 세 번 고른 답을 네 번째 자리에서 고르지 않았는데, 그것을 세는 계약이 0건이라 아무도 몰랐다** — 그리고 그 넷이 사람이 앱에서 가장 먼저 만드는 문장이었다

- **사실.** 사용자가 직접 친 이름을 문장에 넣고 그 뒤에 **받침에서 갈려야 하는 조사**를 리터럴로
  붙인 자리가 넷이었다: `app/settings/children.tsx`의 아이 추가 **토스트와 낭독 둘**
  (`` `${nickname.trim()}를 추가했어요.` ``)과 `app/family/accept/[token].tsx`의 참여 착지 **둘**
  (`` `${household.name}과 함께해요.` ``). ⚠️⚠️ **받침 있는 별명이면 *"지훈를 추가했어요"* 가 나고,
  받침 없는 가구 이름이면 *"김가네과 함께해요"* 가 난다.**
- ⚠️⚠️ **저장소는 이 물음에 이미 답을 골랐고, 두 갈래로 골랐다.**
  ⓐ **값의 꼬리를 고정 명사로 끝낸다** — `householdScopePhrase`의 `‘…’ 가구` ·
  `pendingInviteTarget`의 `… 초대` · `recordsCountPhrase`의 `기록 N건`(그래서 그 뒤의
  `${…}를`·`${…}로`·`${…}은`이 전부 옳다). ⓑ **조사를 값에서 고른다** — `src/home/baby-counter.ts`에
  `hasFinalConsonant`·`objectParticle`·`withParticle` **셋**이 이미 있었다. ⚠️⚠️ **네 자리는 어느
  쪽도 고르지 않았고, 그 사실을 세는 계약이 0건이었다.**
- ⚠️ **트랙이 정찰의 처방 하나를 값으로 정정했다.** 정찰은 한글이 아닌 끝(라틴·숫자·이모지)에서
  *"종전 조사를 그대로 둔다"* 를 권했는데, **종전 조사가 아이 화면에서는 `를`(받침 없는 형)이고 가구
  화면에서는 `과`(받침 있는 형)이라 그대로 두면 같은 이름이 두 화면에서 서로 다른 갈래로 떨어진다.**
  저장소의 답은 한 갈래(**받침 없는 형**)이고 그 근거는 `baby-counter.ts`가 이미 적어 둔 문장이다.
  ⚠️ **그 답은 옳음이 아니라 안전이다** — *"둘째2"* 는 소리로 받침이 없고 *"둘째3"* 은 있는데,
  소스는 소리를 모른다.
- **오늘의 전수.** 걷는 파일 **정찰 하한 281 → 트랙 B의 `src/text/korean-particles.ts` 신설로
  HEAD 282**(⚠️ 라운드 93 리뷰 M-4 정정 — **281은 `SCOUT_LOWER_BOUNDS.sweptFiles`의 *하한*이고
  HEAD의 실측이 아니다**. 계약이 그것을 `toBeGreaterThanOrEqual`로 무니 282는 초록이고,
  **하한은 올리지 않는다**) · 조사가 서는 자리 **38**(리터럴 **29** · 값에서 고름 **9**) ·
  `fixed-tail` **29** · `chooses-from-value` **9**(트랙 전 다섯 · 전부 `baby-counter.ts`) ·
  ⚠️⚠️ `varies-but-written-fixed` **0**(상한). ⚠️ **정찰의 33과 이 38은 바늘이 다르다** — 계약은
  *값에서 고른 자리*도 같은 모집단에 두는데, 그러지 않으면 넷을 고치는 순간 그 넷이 모집단에서
  사라져 모집단이 33에서 29로 줄고 **래칫이 *줄지 않는다*를 물 수 없다.**
- ⚠️⚠️ **AG-4의 답이 여기서 한 칸을 더 얻었다.** 정찰이 손으로 푼 거짓 인용
  (`app/expenses/new.tsx:576-579`의 `merchantInputRef` 주석이 *"`itemNameInputRef`와 같은 관례 —
  포커스를 주는 데는 쓰지 않는다"* 고 적는데 그 참조는 정확히 포커스를 주는 데 **두 번** 쓰인다)을
  트랙 B가 **세 시점**으로 정정했다. ⚠️⚠️ **그 문장은 라운드 92 *리뷰*가 어제 적은 것이라 *낡음*이
  아니라 *태어날 때부터 거짓*이었고, AG-4의 처방(두 시점)으로는 잡히지 않는 갈래다** — 두 시점은
  *언제 참이었다가 언제 거짓이 됐는가*를 적는 꼴인데, 참이었던 시점이 없다.
- **일반형.** ⚠️⚠️ **계약이 있는데 모집단이 손인 자리**(AH-1)**와 계약이 아예 0건인 자리는 찾는
  방법이 다르다.** 앞쪽은 *계약의 모집단*을 세면 나오고, 뒤쪽은 **저장소가 같은 물음에 이미 여러 번
  고른 답을 고르지 않은 자리**를 세야 나온다(AF-1의 방법). ⚠️ **그리고 뒤쪽에는 신호가 하나 더
  있다 — *답이 이미 두 갈래로 있다*는 것.** 한 갈래만 있으면 그것이 관례인지 우연인지 알 수 없는데,
  두 갈래가 서로 다른 층(문구의 꼬리 · 순수 함수)에 있으면 **그 물음을 저장소가 실제로 고민했다는
  증거**가 된다.
- ⚠️ **다음 라운드가 먼저 세어 볼 만한 것**: 이 저장소의 계약 가운데 **화면 문구를 *바이트로* 인용한
  것**이 몇이고, 그중 **인용당한 문구가 그 뒤에 바뀐 것**이 몇인가(오늘 하나를 그렇게 찾았고 ─
  아래 AH-4 ─ 그런 인용이 몇인지는 오늘 세지 않았다. ⚠️ **AG-4의 답 4가 센 것은 *주석*의 인용이고
  이 질문이 묻는 것은 *계약이 단언으로 무는* 인용이라, 두 수를 한 낱말로 적지 않는다**).
- ⚠️ **되짚기(라운드 92)** — AG-4의 질문에 답한 자리는 이 절과 AH절 머리말의 답 표 4행이다.

### AH-3. **사각이 *"묻지 않은 수"* 라고 크기만 적어 둔 자리를 열면 그 안은 한 갈래가 아니고, 열어 본 뒤에야 남는 수의 성격을 말할 수 있다** — 오늘 열 자리가 들어오고 일곱이 남았다

- **사실.** 라운드 92 B(+리뷰 H-3)가 행 라벨 스윕의 사각 ⓐ에 이렇게 적어 두었다: *"남는 124의
  성격은 하나가 아니라 둘이다 … **행이면서도 이 바늘 밖인 자리가 남아 있다** … 그 둘을 가르는 일은
  이 축이 아직 하지 않았고, 그래서 124는 **묻지 않은 수**이지 **물을 것이 없는 수**가 아니다."*
  ⚠️⚠️ **오늘 트랙 C가 그 갈래를 열었고, 실재했다.**
- ⚠️⚠️ **왜 안 보였는가 — 바늘 셋이 전부 *렌더 함수에서 한 걸음*까지만 따라갔기 때문이다.**
  이 저장소에서 행을 그리는 가장 흔한 꼴은 `{rows.map((row) => <PreparationItemCard …/>)}` 처럼
  **`.map(` 안에서 컴포넌트를 바로 그리는 것**이고, 그 컴포넌트의 **본문**은 목록 콜백 밖(대개 **다른
  파일**)에 산다. 넷째 바늘은 **JSX 사용처 → 컴포넌트 정의**(같은 파일 → import 한 줄 → 재수출 한
  줄)로 잇고 **행을 실어 준 프롭 이름 → 파라미터**를 *한 걸음*만 따라간다. ⚠️ **판정 셋은 한 글자도
  바뀌지 않았다 — 넷째 바늘은 모집단을 넓히는 것이지 판정을 바꾸는 것이 아니다.**
- **오늘의 값(세 시점).** 목록 안 자리 **35 → 38 → 48**(직접 34 → 36 → **44** · 한 걸음 파생
  1 → 2 → **4**) · 넷째 바늘이 데려온 자리 **10**(그 자리가 사는 행꼴 컴포넌트 **8** · 오늘의 38과
  **겹치지 않는다**) · 목록 안 자리를 지닌 파일 **18 → 21** · *행마다 갈리지 않음* **0건**(상한).
  ⚠️⚠️ **두 시점(라운드 93 리뷰 M-9) — 그 *열*의 열거가 아홉뿐이었다.** 빠진 하나는 **설정 목록
  행**(`src/design-system/components/ApplicationPrimitives.tsx`의 `ListRow` · 스윕 자리 `at 8279` ·
  `app/settings/index.tsx:342`의 `supportRows.map(`이 그린다)이고, **direct 여덟 가운데 여덟째**다.
  ⚠️ **수는 그때도 옳았다**(열 · 여덟 · direct 여덟 · 파생 둘) — 틀린 것은 **이름의 열거**이고,
  같은 열거가 이 절과 계약 두 자리와 짝 문서 `#110`에 **세 벌**로 적혀 있어 셋이 함께 아홉이었다.
  ⚠️⚠️ **그리고 이것이 AH-3 자신의 일반형이 말하는 병의 한 겹 더 안쪽이다** — *크기만 적으면 성격을
  잃는다*고 적어 둔 절이, **성격(이름)을 적으면서 하나를 빠뜨렸다.** 크기는 자가 세지만 **열거는
  손이 적고, 손이 적은 것은 세는 자가 없으면 조용히 하나 모자란다.**
  ⚠️ **재개 조건(결정형 · 손은 저장소 안): 열거를 손이 아니라 자가 내게 할지 정하는 날** — 그날
  먼저 물을 것은 *열거를 계약이 파생해 문서로 내보낼 것인가, 문서의 열거 길이를 계약이 셀 것인가*
  이고(앞쪽이면 생성 단계가 하나 늘고 뒤쪽이면 문서가 계약의 모집단이 된다), 첫 모집단은 이
  열 자리와 그것을 적은 세 벌이다.
  ⚠️ **핵심 루프에 걸리는 셋이 그 열 안에 있다** — 빠른 품목 타일(1단계) · 준비템 카드(3단계) ·
  구매 링크 비교 행(4단계).
- ⚠️⚠️ **남는 수의 성격이 오늘 처음 말해졌다.** 목록 **밖**은 **124 → 114**이고, 그 114 가운데
  대부분(**107**)은 정말 행이 아닌 자리(화면 머리말·요약 카드·시트·탭바처럼 한 화면에 한 번 서는
  컨트롤)이며 **일곱**이 여전히 *행이면서 밖*이다 — 여섯은 이름이 행꼴이 아니고(공용 버튼 셋의
  라벨 프롭 등) 하나는 라벨이 격자 자체를 읽는다. ⚠️⚠️ **114는 여전히 *묻지 않은 수*이지만, 묻지
  못한 부분은 이제 37이 아니라 일곱이고 그 일곱은 0건이 아니라 이름과 꼴로 적혀 있다.**
- ⚠️ **라운드 92 리뷰 H-3이 *"그 조용한 갈래는 닫혔고"* 라고 적은 것이 오늘 보니 절반만 참이었다.**
  닫힌 것은 **렌더 프롭 쪽**의 조용한 갈래이고, `.map(`이 바로 그리는 쪽은 그대로 조용했다 —
  그 실피해가 **0건이 아니라 열 자리**였음을 오늘 넷째 바늘이 값으로 보였다.
- **일반형.** ⚠️⚠️ **사각을 *크기*로만 적으면 다음 라운드가 그 크기를 *성격*으로 읽는다.**
  124는 *묻지 않은 수*였는데 문서에서는 *물을 것이 없는 수*처럼 읽혔고, 그 둘을 가른 것은 새 관측이
  아니라 **같은 사각을 한 번 더 열어 본 걸음**이다. ⚠️ **그래서 사각의 이상적인 꼴은 *크기 + 갈래 +
  각 갈래의 크기*이고, 갈래를 아직 모르면 *모른다는 사실*을 값으로 적어야 한다**(라운드 92 B가
  정확히 그렇게 적었고, 그래서 오늘 열렸다). **사각이 자기 갈래를 적으면 그 사각은 다음 라운드의
  후보가 된다.**
- ⚠️ **다음 라운드가 먼저 세어 볼 만한 것**: 이 저장소의 사각 가운데 **크기만 적힌 것**이 몇이고
  그중 **갈래(그 안이 몇 종류인가)까지 적힌 것**이 몇인가(오늘 하나를 그렇게 열었고, ⚠️ **저장소의
  사각 전수는 오늘 세지 않았다** — 대장 다섯의 사각만 해도 여덟·여덟·셋·다섯·넷이다).
- ⚠️ **되짚기(라운드 92)** — 이 질문에 답한 자리는 **AH-3**이다.

### AH-4. **파일 단위 트랙 배타는 *한 트랙이 다른 트랙의 파일에 만든 부채*를 막지 못하고, 그것을 닫는 손은 트랙이 아니라 통합이다** — 그리고 오늘 재개 조건이 처음으로 *같은 라운드 안에서* 발동했다

- **사실.** 트랙 B가 화면 문구 넷의 조사를 값에서 고르게 고치자, 그 넷 가운데 하나를
  **트랙 C 소유 파일**이 **옛 바이트로** 물고 있었다 —
  `apps/mobile/src/a11y-contract.test.ts:1012-1014`가 `children.tsx`의 낭독 한 줄을 `를`까지 포함해
  통째로 인용한다. ⚠️ **트랙 배타 규칙상 B는 그 파일을 열 수 없고**(*다섯 트랙 어디에도 같은 파일이
  두 번 서지 않는다*), **C는 그 축이 아니다**(*한 트랙이 한 그물에 축 둘을 얹지 않는다*).
- ⚠️⚠️ **그래서 B는 고치는 대신 *값과 조건으로* 남겼다** — 사각
  `fixed-particle-pinned-by-another-contract`(실측 **1** · 하한 0)와 **사건형 재개 조건**
  (*"트랙 C 또는 그 뒤의 손이 그 세 줄을 오늘의 바이트로 옮겨 적는 날 — 그날 이 사각의 실측은
  1에서 0으로 내려간다"*)이다. ⚠️⚠️ **그리고 통합 커밋(`357b021`)이 같은 라운드 안에서 그 조건을
  발동시켰다**: 핀을 오늘의 바이트로 옮기고 두 시점 주석을 달았으며, 그 `it`을 **두 방향**
  (핀이 오늘의 바이트를 들고 있는가 · 화면과 계약 어느 쪽에도 옛 바이트가 코드로 없는가)으로 다시
  세워 실측을 **0**으로 내렸다.
- ⚠️⚠️ **이 저장소에서 재개 조건이 *다음 라운드*가 아니라 *그 라운드 안에서* 발동한 첫 자리다.**
  종전의 이행은 전부 라운드를 건너뛰었다(AC-3의 *사각이 조건을 값으로 적으면 다음 표에 선다* ·
  AF-5 → 라운드 92 F의 첫 이행 · AG-5 → 오늘의 이행). **오늘 걸린 시간은 커밋 둘 사이다.**
- ⚠️ **그 걸음이 가능했던 이유가 값이다** — B가 남긴 것이 *"누가 언젠가 고쳐야 한다"* 가 아니라
  **ⓐ 자리(파일과 줄) · ⓑ 옛 바이트와 오늘의 바이트 · ⓒ 발동했을 때 이 수가 얼마가 되는가**
  셋이었다. **그 셋이 있으면 조건을 발동시키는 데 그 축을 다시 조사할 필요가 없다.**
- ⚠️ **부채의 방향도 값으로 적는다.** 이 부채는 *화면이 계약을 앞질러 간* 모양이고, 그 반대
  (계약이 화면을 앞질러 가는 것)는 이 저장소에서 **금지**다(*화면이 계약을 맞추는 것이 아니라 계약이
  화면을 지킨다*). ⚠️⚠️ **그래서 이 갈래의 부채는 앞으로도 계속 생긴다** — 화면 문구를 고치는 트랙이
  설 때마다 그 문구를 바이트로 무는 계약이 남의 파일에 있을 수 있다.
- **일반형.** ⚠️⚠️ **파일 단위 상호 배타는 *쓰기 충돌*을 막지 부채를 막지 않는다.** 한 트랙이 자기
  파일만 고쳐도 **남의 파일이 그 바이트를 물고 있으면** 그 순간 부채가 선다. 배타 규칙 아래에서 그
  부채를 닫을 수 있는 손은 **통합**뿐이고, **통합이 그것을 닫으려면 트랙이 자리·바이트·발동값 셋을
  남겨야 한다.** ⚠️ **그리고 그 걸음은 트랙의 커밋이 아니라 통합의 커밋에 실려야 한다** — 트랙의
  커밋에 실으면 배타가 깨지고, 안 실으면 라운드가 부채를 지고 닫힌다.
- ⚠️ **다음 라운드가 먼저 세어 볼 만한 것**: 한 라운드의 트랙이 **다른 트랙 소유 파일에 부채를 만든**
  자리가 몇이고, 그중 **그 라운드 안에서 닫힌 것**이 몇인가(오늘 하나를 그렇게 닫았고, ⚠️ **라운드
  62~92에 그런 자리가 몇이었는지는 오늘 세지 않았다** — 그리고 그 수를 세는 자리는 오늘 0건이다).
- ⚠️ **되짚기(라운드 92)** — 이 질문에 답한 자리는 **AH-4**이다.

### AH-5. **문서가 무는 것에는 두 층이 있다 — *좌표*와 *수*이고, 오늘 좌표에는 세는 자가 섰지만 수에는 아직 아무도 서지 않았다**

- **사실.** AG-5가 *"의무가 적혀 있다는 것과 그 의무가 지켜진다는 것은 다르다 — 이 자는 앞쪽만 세고
  **뒤쪽은 라운드마다 F의 걸음이 답한다**"* 로 닫았다. 오늘 트랙 E가 **반대 방향**을 처음 전수로
  걸었다: 살아 있는 문서 **넷**(이 문서 · 짝 문서 · 접근성 표 · round4 감사)에서 `<소스경로>:<줄>`
  꼴 좌표 **26**을 파생해 ⓐ 파일이 실재하는가(**26**) ⓑ 줄이 파일 길이 안인가(**26**)
  ⓒ 이동 의무가 곁에 적혀 있는가(**1**)를 값으로 물었고, ⚠️⚠️ **그 하나가 오늘 거짓이었다.**
- ⚠️⚠️ **거짓인 좌표 여섯을 오늘 F가 두 시점으로 정정했다**(자리 다섯 · 옛 좌표는 지우지 않는다).
  - `kl:245` — `apps/mobile/app/items/[itemTemplateId].tsx` **997 → 1069**(그 줄에 `caption={hasSession`이
    없다 · **72줄** 밀렸다).
  - `kl:254` — `apps/mobile/app/(tabs)/index.tsx` **2105 → 2142**(`action={<NotificationBell`이 그 줄에
    없다 · **37줄** 밀렸다).
  - `kl:413`·`kl:414` — ⚠️⚠️ **좌표 갱신이 아니라 문장 자체를 다시 썼다.**
    `app/settings/privacy.tsx:150`와 `app/import/[importJobId].tsx:131`이 무는 심볼
    `loadFailedText`가 **두 파일 다 오늘 0건**이다 — 줄이 밀린 것이 아니라 **무는 대상이 사라졌다**
    (라운드 74 트랙 D가 셋을 공용 `useLoadErrorCopy`로 배선했다). ⚠️⚠️ **그리고 계약과 문서가 정확히
    반대를 말하고 있었다**: `apps/mobile/src/settings/destructive-flow-messages.test.ts`가
    `not.toContain("const loadFailedText =")` 로 **그 심볼의 *없음*을 계약으로 물고 있는데** 문서는
    그것이 *있다*고 적었다. **계약은 라운드 74부터 옳았고 문서만 열아홉 라운드 낡아 있었다.**
  - `kl:2584` — ⚠️⚠️ **이 저장소에서 *이동 의무*가 곁에 적힌 유일한 자리이고, 그 의무가 지켜지지
    않았다.** `monthlyUsed`가 서는 자리는 **1482가 아니라 1519**이고(**37줄** 밀렸다), 함께 적힌
    `:1260`은 **훅 호출이 아니라 대기 행 주석의 한 줄**이다(그 줄에 `use…(` 꼴이 0건이다).
    ⚠️ **두 번째 좌표는 다시 숫자로 적지 않았다** — 그 자리는 *훅 호출이 `monthlyUsed`보다 위에 있다*
    는 사실만 필요하고, 숫자로 적는 순간 다음 라운드에 또 거짓이 된다.
- ⚠️⚠️ **AG-5의 뒤쪽 물음에 라운드 93이 내는 답은 *지켜지지 않았다*이다** — 그 문장은 **라운드 79**에
  쓰였고 오늘까지 **열네 라운드** 서 있었다. **그리고 오늘 F가 그 의무를 처음 이행하며, 같은 걸음에
  의무가 적힌 자리를 하나에서 여섯으로 늘렸다**(좌표는 **26 → 35** · known-limitations **16 → 25**).
- ⚠️⚠️ **그런데 같은 병이 F 자신의 트립와이어에 있다 — 그리고 그쪽에는 세는 자가 없다.**
  라운드 92 F가 이 절의 트립와이어에 적은 수 가운데 **둘**은 오늘 같은 자를 다시 댈 수 없었다:
  `helper-named-reader` **166**과 문턱·래칫 상수 **38**이다. **두 수 다 *어떻게 셌는가*(모집단·바늘)가
  그 자리에도 대장에도 적혀 있지 않아서**, 오늘 F가 넓은 바늘로 재니 각각 **207**과 **53**이 나온다.
  ⚠️ **어느 쪽이 옳다는 판정이 아니다 — 두 수가 서로 다른 자에서 나왔고 그 자를 대조할 방법이 없다는
  것이 값이다.** ⚠️⚠️ **좌표는 파일과 줄이 있어 기계가 풀 수 있지만, 수는 바늘이 적혀 있지 않으면
  아무도 풀 수 없다 — 그래서 수 쪽이 더 조용하다.**
- ⚠️⚠️ **두 시점 — 라운드 93 리뷰(M-2)가 그 둘 가운데 하나를 풀었고, 그 결과가 AH-5를 반으로
  줄이면서 동시에 날카롭게 한다.** 위 문단은 **A~F 머지 뒤 · 리뷰 전** 시점의 사실이고, 리뷰가
  그 자리에 물어 본 답은 이렇다:
  - `helper-named-reader` **166** — ⚠️⚠️ **바늘은 저장소 안에 있었다.**
    `packages/test-utils/src/comment-tolerant-anchor-ledger.ts:532`의 `NAMED_SOURCE_READER`(이름이
    다른 소스 리더의 정의를 무는 정규식)로 걷고, `collectCommentToleranceAnchors().outside`에서
    `reason === "helper-named-reader"` 인 자리를 세면 된다 — **파일 하나가 한 자리**이고
    (`readSource` 자신은 뺀다), 그 대장의 계약이 이미 같은 자를 대고 있었다. **오늘 HEAD 실측
    167**이고, 라운드 92의 **166**에서 **정확히 하나** 올랐으며 **오른 하나가 트랙 B의
    `readSweptSource`**다 — 트랙 B가 자기 머리말에 미리 적어 둔 예측이 **+1로 적중했다.**
    ⚠️ **F가 낸 207은 틀린 수가 아니라 다른 자의 수다**(모집단이 *파일 전수*라 대장이 이미 안에
    둔 것까지 센다). **두 수를 한 낱말로 적지 않는다.**
  - 문턱·래칫 상수 **38** — ⚠️⚠️ **이쪽은 오늘도 재현되지 않는다.** 그 수는 갈래 셋(*하네스 4 ·
    제품 8 · 소스 텍스트 26*)으로 적혀 있는데 **갈래마다의 모집단이 어디에도 없어서**, 넓은
    바늘의 **53**과 대조할 길이 없다. **이 절이 *재현 불가*로 남기는 칸은 이제 이 하나뿐이다.**
  - ⚠️⚠️ **그래서 AH-5의 일반형은 그대로 서고, 근거가 하나로 줄었다.** *수는 바늘이 적혀 있지
    않으면 아무도 풀 수 없다* 는 문장은 38 한 칸으로도 참이다. ⚠️⚠️ **다만 오늘 한 갈래가
    갈라졌다 — *바늘이 없다*와 *바늘로 가는 길이 안 적혀 있다*는 다른 병이다.** 166은 뒤쪽이었고
    (자는 있었고 그 자리에서 자를 가리키지 않았다), 38은 앞쪽이다. **처방도 다르다**: 뒤쪽은
    표의 칸에 `<파일>:<줄>`을 한 번 적으면 닫히고, 앞쪽은 자를 새로 세워야 닫힌다.
  - ⚠️ **재개 조건(결정형 · 손은 저장소 안): 문턱·래칫 상수 38의 갈래 셋에 모집단을 붙이는 날** —
    그날 먼저 물을 것은 *하네스·제품·소스 텍스트를 가르는 자가 경로인가 파일 종류인가*이고
    (앞쪽이면 디렉터리 목록이 값이 되고 뒤쪽이면 `.test.ts` 여부가 값이 된다), 첫 모집단은
    넓은 바늘이 낸 **53**이다.
- ⚠️ **하한을 고른 비용도 그 계약이 값으로 든다** — 자리 수도 의무 수도 하한이라 **문서가 좌표를
  *지우면* 이 계약은 그것을 보지 못한다**. 등호를 고르면 F가 절을 늘릴 때마다 빨개지고 빨간 계약은
  아무것도 지키지 못하므로, 이 라운드는 **사라지는 좌표를 못 보는 쪽**을 값으로 택했다(AG-5의 일반형).
- **일반형.** ⚠️⚠️ **판정 문서가 무는 것에는 두 층이 있다 — *좌표*(어느 파일 어느 줄)와 *수*(오늘
  얼마인가)이고, 둘은 낡는 방식이 다르다.** 좌표는 **기계가 풀 수 있어** 세는 자를 세우면 그날로
  거짓이 드러나고(오늘 여섯이 그렇게 드러났다), 수는 **그 수를 낸 바늘이 함께 적혀 있어야만** 다시
  잴 수 있다. ⚠️⚠️ **AC-5·AD-4가 세운 규율(*수를 낼 때 그 수를 어떻게 냈는지를 함께 적는다*)은
  정찰과 트랙에는 적용됐는데 F의 트립와이어에는 적용되지 않았고, 오늘 그 자리에서 처음 물렸다.**
  **인용이 실측을 대신한다는 O-3의 병이, 인용할 실측 자체가 재현되지 않는 꼴로 한 겹 더 깊어진 것이다.**
- ⚠️ **다음 라운드가 먼저 세어 볼 만한 것**: 이 저장소의 판정 문서가 **재실측 수치**를 적은 자리가
  몇이고, 그중 **그 수를 낸 바늘(모집단·자)이 곁에 적힌 것**이 몇인가(오늘 트립와이어의 서른 남짓한
  줄 가운데 **둘**이 재현되지 않는다는 것을 확인했고, ⚠️ **나머지가 재현되는지는 오늘 세지 않았다** —
  세려면 그 줄마다 자를 한 번씩 다시 대야 하고, 그것이 이 질문의 크기다).
- ⚠️ **되짚기(라운드 92)** — AG-5의 질문에 답한 자리는 **AH-5**이다.

## AI. 라운드 94에서 확정한 판정 (2026-09-01 · GAP-094 트랙 F)

라운드 93이 물은 것이 **모집단이 손 목록인 계약이 몇이고 그 안에서 사람이 오늘 겪는 자리가
무엇인가**(답: 관례를 세는 계약 예순둘 중 서른둘 · 그 안에서 둘 · AH-1)였다면, 라운드 94는 그
답을 **자와 바늘의 층으로 내린 라운드**다 — ⚠️⚠️ **다섯 트랙 가운데 넷이 고친 것이 판정도 화면도
아니라 *자기 계약이 수를 내는 방법*이었다**(사각의 자 · 손 목록의 모집단 · 소스 바늘의 갈래 ·
판정 문서의 수 곁에 선 바늘). 축은 라운드 81~93과 같이 **사용자 가치**였고(사람이 지은 이름 ·
나가는 길의 자 · 운영자 도구의 모집단 · 저장소가 자기를 세는 자리), 다섯 판정 다 K~AH절과 같이
**결함 보고가 아니라 다음 결정의 입력**이며 2026-09-01 소스에서 확인됐다(라운드 94 트랙
A·B·C·D·E 머지 후 · 리뷰 전).
⚠️⚠️ **이번 라운드가 연 화면은 하나다** — 핵심 루프의 **첫 문장**(더보기 탭의 가구 카드가
*"지훈네"* 였다)과 **2단계의 칩 라벨**(*"지난달 실지출(300,000원)로"* — 보이는 줄과 낭독이 서로
다른 조사를 썼다)이고, **둘 다 라운드 93이 자리·바이트·발동값을 적어 두고 축이 갈려 남긴 자리**다.
가격·링크·픽셀은 바이트 불변이고, **통합이 닫아야 할 부채는 0건이다**(AH-4의 처방을 배정 단계에서
쓴 첫 라운드다).

⚠️⚠️ **이번 라운드의 가장 값진 관측: *두 수를 한 낱말로 적지 않는다*는 이 저장소의 규율이 오늘
세 번 물렸고, 세 번 다 물린 자리가 정찰의 표였다**(AI-1). 트랙 D가 정찰의 *"필드형 56"* 을 열자
그 안에 **필드가 아닌 자리 열둘**이 함께 있어 **44 + 12**로 갈라졌고, 트랙 C가 정찰의 *"손 열"* 을
전수로 옮기자 오늘의 걷기는 **열여섯**이었으며(여섯 자리가 목록 밖에서 조용했다), 트랙 E가 정찰의
*"좌표꼴 1"* 을 파서로 다시 재니 **2**였다(줄 범위꼴 하나에서 갈렸다). ⚠️ **세 번 다 어느 쪽이
틀린 것이 아니라 바늘이 다르고, 세 번 다 그 사실을 트랙이 값으로 적었다.**

⚠️⚠️ **두 번째 관측: 리뷰가 세운 결정형이 *다음 정찰의 표*가 아니라 *그 다음 라운드*에서 소진됐고,
그 속도가 이 표의 성질을 바꾼다**(AI-2). 라운드 93 리뷰가 세운 둘(#18 사각의 자를 함수로 · #19
`⚠️`와 `재개` 사이의 강조 표식)이 오늘 트랙 B·D에게 **한 라운드 만에** 집혔고, 라운드 93 트랙 D가
세운 #16이 **두 라운드 만에** 트랙 C에게 집혔다. ⚠️⚠️ **결정형 전수표에서 집는 것이 0건이던
라운드가 하나 걸러 왔는데, 오늘은 스물셋 가운데 셋이 한 번에 내려갔다** — 그리고 셋 다 **손이
저장소 안**이고 **세운 손이 트랙 또는 리뷰**였다.

⚠️⚠️ **세 번째 관측: 자를 둘 수 없는 사각에서 `() => 0`은 자가 아니라 상수이고, 그 자리가 지어야
하는 것은 크기가 아니라 *불가의 증거*다**(AI-3). 트랙 B가 사각 여섯의 자를 함수로 옮기다 **둘은
파생이 불가능하다**는 것을 만났다(하드웨어 뒤로가기·엣지 스와이프 · 소스가 아니라 런타임). 그
둘에 `measure`를 두지 않고 **증인**을 두자 곧바로 값이 하나 나왔다 — ⚠️ **같은 바늘이 스윕 밖의
`AppLockOverlay`에서 자리를 낸다**. **즉 `app/**`의 0은 *가로채기가 없다*가 아니라 *이 자가 볼 수
있는 곳에 없다*였다.**

⚠️⚠️ **네 번째 관측: 배정이 부채를 없애려면 배정표가 아니라 *소스*가 세어야 한다**(AI-4). 정찰이
AH-4의 처방대로 *"문구를 고치는 트랙이 그 문구를 무는 계약 전부를 가진다"* 를 배정 단계에서 썼고
계약 **넷**을 트랙 A에 줬는데, 트랙 A가 오늘 소스를 다시 걸으니 **다섯**이었고 핀은 **아홉**이었다
(`settings/more-menu.test.ts` 셋 · `home/budget-edit.test.ts` 둘이 배정표에 없었다).
⚠️ **정찰의 넷은 틀린 수가 아니라 하한이었고, 배정이 완전했는지를 말한 것은 배정표가 아니라
그 트랙이 세운 핀 이관 대장이다.** **통합이 닫을 부채는 오늘 0건이다.**

⚠️⚠️ **다섯 번째 관측: 좌표에 이어 *수*에도 세는 자가 섰고, 그 자가 서자마자 F의 숙제가 수로
바뀌었다**(AI-5). AH-5가 *"좌표에는 세는 자가 섰는데 수에는 아직 아무도 서지 않았다"* 로 닫았고,
오늘 트랙 E가 최신 판정 절의 재실측 표를 전수로 파생해 **행 37 · 수가 있는 행 37 · 바늘이 곁에
적힌 행 13 · 좌표꼴 2**를 냈다. ⚠️⚠️ **그래서 이 절의 트립와이어는 이 저장소에서 처음으로 *행마다
그 수를 낸 바늘을 함께 적는 표*다** — 오늘 F가 붙인 바늘이 몇인지는 아래 표가 지고, 그 수를 다시
내는 것은 다음 라운드의 그 자다.

---

### AH-1~AH-5의 질문 다섯 — 라운드 94의 답 (출처: `docs/5차/round94-scout.md` §1 실측)

⚠️⚠️ **다섯 다 답했고 넷이 트랙을 발동시켰다.** ⚠️ **아래 수는 이 절이 새로 잰 것이 아니라 정찰이
2026-09-01 HEAD(`1a5d18b`)에서 재어 그 노트 §1에 적은 값이고, 낸 방법이 적힌 곳을 함께 가리킨다**
(O-3의 반대 방향이 아니다 — *판정*은 여기 한 번만 쓰고 *수*는 낸 자리를 가리킨다).
⚠️⚠️ **그리고 이 답들은 계약이 아니라 손으로 잰 수라, 계약이 세는 수와 한 낱말로 적지 않는다.**

| # | 질문(AH-n이 남긴 자리) | 2026-09-01 답 | 낸 방법이 적힌 곳 | 발동 |
| --- | --- | --- | --- | --- |
| 1 | AH-1 — 손 목록이 모집단인 계약 가운데 **그 목록 옆 주석이 *모집단 전체*를 두고 단언한 것**이 몇인가 | 손 목록 **자리 61 · 파일 33** · 목록 옆에 주석이 있는 자리 **27** · 아예 없는 자리 **34** · ⚠️⚠️ **모집단 전체를 두고 단언한 것 17** · 그중 **그 단언을 세는 자가 있는 것 6 · 없는 것 11** · 손으로 푼 열하나에서 **수가 틀린 자리 하나**(`a11y-contract.test.ts:1343`의 *"스크롤러 셋"* — 오늘 여덟 · **실피해 0건**) · ⚠️⚠️ **라운드 93의 *"손 목록 32"* 는 오늘 재현되지 않는다**(세 가지로 읽어 20 · 33 · 47) | 정찰 §1 「답 1을 낸 방법」(모집단 401 · 바늘 셋 · 사각 넷) | ⚠️ 답했고 트랙이 되지 않았다(→ 기각 목록) · ⚠️⚠️ **다만 그 병의 *주석 대 자* 갈래는 트랙 B·C가 각자 자기 파일에서 닫았다** |
| 2 | AH-2 — 계약이 **화면 문구를 *바이트로* 인용한 것**이 몇이고, 그중 **인용당한 문구가 그 뒤에 바뀐 것**이 몇인가 | 자리 **391**(긍정 300 · 부정 91) · 계약 파일 **74** · 물린 제품 파일 **138** · 쌍 **326** · ⚠️⚠️ **뒤쪽 물음의 답은 구조적으로 0이다**(인용당한 바이트가 바뀌면 그날 그 계약이 빨개진다 — AG-4의 주석 인용과 **정반대의 성질**) · ⚠️⚠️ **그래서 값은 다른 수에 있다: 두 계약 이상이 같은 제품 파일을 바이트로 무는 자리 73** | 정찰 §1 「답 2를 낸 방법」(바늘 `expect(<소스텍스트>)…toContain("…한글…")` · 사각 넷) | ⚠️⚠️ **트랙 A의 배정 근거** — 그리고 그 배정이 **하한이었다**는 것이 AI-4의 값이다 |
| 3 | AH-3 — 이 저장소의 **사각** 가운데 **크기만 적힌 것**이 몇이고 그중 **갈래까지 적힌 것**이 몇인가 | 사각 전수 **66자리 · 파일 11** · 크기가 적힌 것 **64** · 없는 것 **2** · ⚠️⚠️ **갈래까지 적힌 것 11 · 크기만 적힌 것 53** · ⚠️ AH-3이 든 *"대장 다섯의 여덟·여덟·셋·다섯·넷"* 과 오늘의 66은 **바늘이 다르다** | 정찰 §1 「답 3을 낸 방법」(`BLIND_SPOT` 상수 + `id`/서술/재개 조건 · 사각 셋) | ⚠️ 답했고 **트랙 B·C의 근거**가 됐다(둘 다 사각의 자·모집단을 고친다) |
| 4 | AH-4 — 한 라운드의 트랙이 **다른 트랙 소유 파일에 부채를 만든** 자리가 몇이고 그중 **그 라운드 안에서 닫힌 것**이 몇인가 | ⚠️⚠️ **그 꼴로 기록된 자리는 라운드 62~93 통틀어 1이고 그 하나가 라운드 93이다**(1/1 · 실측 0) · ⚠️ 앞 서른한 라운드의 0건은 *없었다*가 아니라 *세는 자가 없었다*이다 · ⚠️⚠️ **대신 다른 갈래 둘이 실재한다**: **부채를 만들기 전에 멈춘 기록 5** · **반대 방향(의존)의 기록 1**(자리 다섯 · 라운드 79) | 정찰 §1 「답 4를 낸 방법」(바늘 셋 · 사각 셋) | ⚠️⚠️ **발동 → 트랙 배정 규율 자체를 바꿨다**(문구를 고치는 트랙이 그 문구를 무는 계약 전부를 가진다) |
| 5 | AH-5 — 판정 문서가 **재실측 수치**를 적은 자리가 몇이고 그중 **그 수를 낸 바늘(모집단·자)이 곁에 적힌 것**이 몇인가 | AH절 재실측 표 행 **37** · ⚠️⚠️ **바늘이 곁에 적힌 행 13 · 수만 있는 행 24** · 좌표꼴로 가리킨 행 **1**(정찰의 손 판정) ↔ **2**(트랙 E의 파서 · 줄 범위꼴 하나에서 갈렸다) · ⚠️ 재현 불가로 남은 칸은 오늘도 **하나**(문턱·래칫 상수 38 · 넓은 바늘 53) · ⚠️ `helper-named-reader`는 리뷰 M-2의 자로 **167**이고 넓은 바늘로는 **189**다(**세 수를 한 낱말로 적지 않는다**) | 정찰 §1 「답 5를 낸 방법」(모집단 = AH절 재실측 표 · 바늘 넷 · 사각 넷) | ⚠️⚠️ **발동 → 트랙 E**(그리고 그 자가 낸 24가 **오늘 F의 숙제**가 됐다) |

⚠️ **되짚기(라운드 93) — 이 다섯 질문에 답한 자리.** AH-1 → 이 표의 1행(트랙이 되지 않았고 기각
목록에 재개 조건과 함께 있다) · AH-2 → **AI-4**(배정이 하한이었다) · AH-3 → 이 표의 3행(트랙 B·C의
근거가 됐고 그 자체가 판정이 되지는 않았다) · AH-4 → **AI-4** · AH-5 → **AI-5**.
**판정은 그 판정이 사는 절 하나에만 적는다**(O-3·X-4).

---

### 정찰이 남긴 기록 의무 — 트랙이 정찰과 갈린 자리 다섯

⚠️ **정찰의 마지막 줄이 *"각 트랙은 자기 축의 전제를 다시 재고, 정찰과 갈리면 갈렸다는 사실 자체를
값으로 적는다"* 를 요구했다.** 오늘 갈린 자리는 **다섯**이고, ⚠️⚠️ **다섯 다 트랙이 자기 소스에
먼저 적었으며 이 절은 그것을 옮겨 적을 뿐이다.**

- ⚠️⚠️ **정찰 *"필드형 56"* ↔ 트랙 D **44 + 12**.** 정찰의 56은 *`⚠️`가 없는 자리 전부*였고, 필드
  이름(`reopenCondition:`·`resumeCondition:`)을 실제로 요구하면 **44**다. 남는 **열둘**은 주석의
  인용 · 단언이 문자열로 문 자리 · `statement:` 같은 다른 필드이고, 트랙 D는 그 열둘을 지우지 않고
  **이름과 값과 사각으로**(`three-needle-residual`) 세웠다. ⚠️ **그래서 정찰이 내다본 *"어느 바늘에도
  안 걸리는 자리 0건"* 은 오늘 성립하지 않고, 성립하지 않는다는 사실이 값이 됐다.**
- ⚠️⚠️ **정찰 *"`discardedSites` 열"* ↔ 트랙 C **걷기 열여섯**.** 손 목록이 열이었고 오늘 한 벌
  호출 자리를 전수로 걸으니 **16**이다 — **라운드 73 뒤에 들어온 여섯**(감사 로그 둘 · 카테고리 ·
  준비템 분류 · 사용자 조회 · MFA 관문)이 목록 밖에서 조용했다. ⚠️⚠️ **같은 트랙의 다른 목록은
  반대였다**: `FALLBACK_PHRASE_OCCURRENCES`의 손 값 **열둘**은 오늘의 전수(**화면 12** · 출현 13)와
  **일치했다**. **두 대조의 결과가 서로 다르다는 것이 이 소진의 값이다.**
- ⚠️⚠️ **정찰 *"좌표꼴 1"* ↔ 트랙 E **2**.** 둘째는 재개 조건 대장 행이 무는 줄 **범위**꼴이고,
  정찰의 손 판정은 그것을 좌표로 세지 않았다. **하한은 1에 둔다** — 두 수를 한 낱말로 적지 않는다.
- ⚠️⚠️ **정찰 *"계약 넷"* ↔ 트랙 A **계약 다섯 · 핀 아홉**.** 배정표에 없던 둘이
  `apps/mobile/src/settings/more-menu.test.ts`(핀 셋)와 `apps/mobile/src/home/budget-edit.test.ts`
  (핀 둘)이고, 아홉째 핀은 스윕 자신의 `literalParticle` 등호라 **대장 밖에** 둔다(대장이 자기를
  무는 순환을 만들지 않는다). **정찰의 넷은 하한이었다.**
- ⚠️⚠️ **정찰이 스스로 적은 재현 불가 하나 — AH-1의 *"손 목록 32"*.** 정찰이 그 문장을 세 가지로
  좁혀·넓혀 읽어 **20 · 33 · 47**을 냈고 **어느 것도 32가 아니다.** ⚠️ **같은 걸음에서 *걷는 계약
  40*은 정확히 재현됐다** — 바늘이 `readdirSync` **낱말 하나**였기 때문이다. ⚠️⚠️ **이것이 AH-5의
  일반형이 F의 표 밖에서도 참임을 보이는 첫 자리다: 바늘이 낱말이면 재현되고 문장이면 재현되지
  않는다.** **옛 수를 지우지 않고 두 시점으로 남긴다**(AE-3).

⚠️ **밖에 둔 것의 크기도 값으로 남긴다** — 트랙 E가 모집단 밖에 둔 **앞선 절들의 재실측 표 넷 ·
행 92**(⚠️ **두 시점**: 그것은 트랙 E 커밋 시점의 수이고, 이 절이 서면서 AH절 표가 함께 밖으로
내려가 HEAD에서는 **표 다섯 · 행 129**다 — 넣으면 이 계약의 수가 라운드마다 통째로 흔들린다),
트랙 D가 모집단 밖에 둔 **잔여 열둘**,
트랙 B가 자를 두지 않은 **사각 둘**이 그것이다.
...
- **이 스캐너가 쿼리로 분류한 자리의 낭독** — 재실측 상태 변화 0, A-20 `#85` 선행 → **U절 머리말**.
  ⚠️ **이번 라운드는 그 여섯 화면에 접점이 0건이다** — 트랙 A가 a11y 그물을 열었지만 든 것은
  **핀 두 줄**이다(*"열지 않았다"* 와 *"열었지만 그 축이 아니다"* 는 다른 문장이고, 오늘은 뒤쪽이
  세 라운드 연속이다).
- **`monthly_wrapup`의 달 이동 구멍** → **U-3.** ⚠️ **`src/notifications/**`가 이번에도 접점 0건이고
  이것으로 일곱 라운드째다.** 판정 종전 그대로.
- **S-3(어드민 `disabled`)** — 판정 종전 그대로, 브라우저 확인 `#130` 선행. ⚠️ **`app/items/page.tsx` ·
  `app/links/page.tsx`가 *끊긴 채로* 네 라운드가 됐다**(이번 라운드 어드민 트랙이 연 파일은 **계약
  하나**이고 화면은 0바이트다).
...
⚠️⚠️ **AA-3의 이행 — 결정형 재개 조건 전수와 오늘의 처분이다. 그리고 이번 라운드가 이 표에 더하는
것이 하나 있다: 두 표를 하나로 합쳤다.**
⚠️ **라운드 93 F가 세운 전수표는 **열일곱**이었고, 그 표가 닫힌 뒤에 결정형 **여섯**이 더 섰다
(넷은 F 자신이 AH절 본문·기각 목록에 적었고 **둘은 리뷰가 세웠다**). 정찰이 그 여섯을 둘째 표로
따로 세웠고, **오늘 F가 스물셋을 한 표로 합친다** — *같은 부류의 조건이 두 표에 나뉘어 서면 다음
라운드가 그 둘을 다른 부류로 읽는다*는 것이 합치는 이유다.
⚠️⚠️ **검산 — 17 + 6 = 23이고, 오늘 집힌 것은 셋(#16 · #18 · #19)이다. 표에서 지우지 않고 두
시점으로 내린다**(AE-3 · 그러므로 표의 행 수는 **스물셋 그대로**이고 *남은 결정형*은 **스물**이다).
⚠️ **두 수를 한 낱말로 적지 않는다 — 행 스물셋과 남은 조건 스물은 다른 수다.**

| # | 결정형 조건 | 사는 곳 | 손 | 오늘의 처분(2026-09-01 실측 · A~F 머지 뒤) |
| --- | --- | --- | --- | --- |
| 1 | 막대를 포커스 가능하게 | AA-2 | 안 | **집지 않는다 — 값 0.** 두 화면(`apps/admin/app/analytics/page.tsx` · `apps/admin/app/clicks/page.tsx`) 다 `trend.showTable` 표가 값을 텍스트로 준다(재실측 · 변화 0 · **어드민 소스 0바이트**) |
| 2 | 기록 탭 검색의 분류 갈래 | Z절 | 안 | **집지 않는다 — 어긋남 0건 · 열 라운드 이어 같은 답.** `RECORDS_SEARCH_FIELDS_LABEL`이 여전히 하나이고 placeholder가 그 값에서 파생한다. ⚠️⚠️ **오늘 값 하나를 더 적는다 — 이 조건이 발동하는 날 조사도 함께 갈린다**(라벨 뒤가 `${…}에서만`이라 오늘은 옳고, 라벨 끝이 `분류`가 되면 조사 스윕의 `fixed-tail` 한 자리가 함께 움직인다) |
| 3 | 준비템 분류 필수 입력 | Z-3 | **밖**(카탈로그 정책) | 집지 않는다. 변화 0 |
| 4 | 서버의 중복 대기 초대 방지 | AA-4 | **밖**(초대 정책) | 집지 않는다. 변화 0 |
| 5 | 감사 뷰의 대상 축 필터 | AA절 기각 | **밖**(서버 DTO 0건) | 집지 않는다 — ⚠️⚠️ **`apps/api/**`가 세 라운드 연속 접점 0건이다**(91이 다섯 라운드 만에 열었고 92·93·94가 다 그 밖이다) · 서버 소스와 DTO는 여전히 0바이트다 |
| 6 | 타일 안 배지 | AA-1 | **밖**(디자인 승인) | 집지 않는다. 변화 0 |
| 7 | C-3 잠금 오버레이 TalkBack 투과 | 접근성 표 C-3 | **밖**(사람·기기·날짜) | **F의 소유 밖이다** — 오늘로 **스물여덟 라운드째 미확인**(67~94). ⚠️ **오늘 이 줄에 붙는 값은 확인이 아니라 *그 경과 수 곁에 이제 바늘이 적힌다*는 사실이다**(AI-5) |
...
| 9 | 재조회 실패 창에서 옛 목록을 남길지 | AC절 기각 | 안 | **집지 않는다 — 화면 구조 결정이고 오늘도 접점 0건**(`src/offline/**` 무접촉) |
| 10 | 앵커 대장의 `helper-named-reader` | `packages/test-utils/src/comment-tolerant-anchor-ledger.ts` | 안 | **집지 않는다 — 래칫이다.** 오늘 **167**(하한 164)이고 라운드 93에서 한 자리도 움직이지 않았다 — ⚠️ **트랙 A가 스윕을 열었지만 소스 리더를 새로 세우지 않았다**(그 파일의 머리말이 그 예측을 미리 값으로 적어 두었고, 오늘은 **0으로 적중했다**). ⚠️ 넓은 바늘의 **189**는 다른 자다 |
| 11 | 재개 조건 대장의 `prose-only` | `packages/test-utils/src/resume-condition-ledger.ts` | 안(**F**) | **집지 않는다.** 오늘 **246**(119 → 145 → 165 → 184 → 208 → 230 → **246**)이고 ⚠️ **늘린 것의 대부분이 이 절 자신이다**(일곱째 시점). 소급 적용은 한 라운드의 F가 질 수 없다 |
| 12 | 재개 조건 대장의 `round-notes` | `packages/test-utils/src/resume-condition-ledger.ts` | 안 | **집지 않는다 — 관례 0건.** 오늘 **440**(377 → 410 → **440**)이고 노트는 **53벌**이며 늘린 것이 `round94-scout.md` 한 벌이다. ⚠️ **라운드마다 한 벌씩 자라는 사각이라는 사실이 이 줄의 값이고, 정찰이 그 사실을 자기 노트 안에 미리 적었다** |
| 13 | 그물 파일이 자기를 이름으로 선언하는 표식 | `packages/test-utils/src/contract-net-ledger.test.ts` | 안 | **집지 않는다 — 관례를 새로 정하는 일이다.** 표식 **0건** · ⚠️⚠️ **자기 배제 관례의 수가 오늘 또 갈렸다**: 좁은 바늘(이름 넷)로는 **열다섯**이고 넓은 바늘로는 **열일곱 · 이름 여섯**이다(늘린 하나가 트랙 C의 `apps/admin/src/admin-load-error-copy.test.ts:120` `SELF_PATH`다) · **정합을 세는 자리 0건** |
| 14 | 공유 테스트 DB를 언제 무엇으로 비울지 | `apps/api/test/harness-catalog-cost.test.ts` | 안 | **집지 않는다 — 술어는 오늘도 참이다.** ⚠️⚠️ **여섯째 시점이고 셋 다 움직였다**(정찰 실측): `wooriai_test`의 `item_templates` **9,439 → 9,476**(활성 **8,797 → 8,829** · **738MB → 744MB**)이고 `wooriai_dev`는 **62**(활성 62 · 24MB) 그대로다. ⚠️⚠️ **그 +37을 만든 손은 라운드 93 *리뷰*의 실행이고, 누가 올렸는지를 세는 자가 0건이라는 사실이 이 줄이 기다리는 결정의 근거다** — 이 라운드의 다섯 트랙도 F도 api 스위트를 돌리지 않았다 |
...
| 16 | **어드민 계약의 *다른* 손 목록 둘을 누가 전수 파생으로 옮기는가** | `apps/admin/src/admin-load-error-copy.test.ts:537` | 안 | ⚠️⚠️ **집혔다 → 트랙 C(GAP-094 #3) · 두 시점으로 내린다.** ① 세워진 날 = 라운드 93 트랙 D. ② 소진된 날 = 오늘. `discardedSites` 손 **10** → 한 벌 호출 자리 **걷기 16**(갈렸다 · 판정 셋: 사유를 통째로 버린다 **12** · 문구를 남긴다 **4** · 리터럴이 아닌 것 **0** 상한) · `FALLBACK_PHRASE_OCCURRENCES` 손 **12** → 폴백 어미를 쓰는 **화면 걷기 12**(일치 · 출현 13) · `handListNeedle`이 **이름 바늘에서 꼴 바늘로**(경로꼴 문자열 셋 이상을 지닌 배열 상수 · 이 파일 전체) |
| 17 | **어드민에 컴포넌트 렌더 하네스를 세울지** | `apps/admin/src/admin-load-error-copy.test.ts` | 안 | **집지 않는다 — 크기다.** 사각 `source-not-render`가 오늘도 **0**을 들고 어드민에 렌더 하네스 표식은 **0건**이다. ⚠️ 트랙 C가 같은 파일을 열었지만 **축은 손 목록이지 하네스가 아니다** |
| 18 | **사각의 자를 `measure: () => number` 꼴로 바꿀지** | `apps/mobile/src/screen-header-back.test.ts:234` | 안 | ⚠️⚠️ **집혔다 → 트랙 B(GAP-094 #2) · 두 시점으로 내린다.** ① 세운 손 = 라운드 93 **리뷰**(L-3). ② 소진 = 오늘. 사각 **여섯**의 자가 상수 **6 · 파생 0**에서 **상수 0 · 파생 4 · 자 없음 2**가 됐다(`noScreenHeader` **8** · `noBackByDesign` **6** · `reExports` **5** · 비 `.tsx` **0**). ⚠️ **재개 조건(사건형)을 그 자리에 새로 세웠다** — 자를 두지 않은 둘 가운데 하나가 소스에서 셀 수 있게 되는 날 |
| 19 | **`⚠️`와 `재개` 사이에 강조 표식(`**`)을 허용할지** | `packages/test-utils/src/resume-condition-ledger.ts:759` | 안 | ⚠️⚠️ **집혔다 → 트랙 D(GAP-094 #4) · 두 시점으로 내린다.** ① 세운 손 = 라운드 93 **리뷰**(M-3). ② 소진 = 오늘. **결정은 *넓히지도 좁히지도 않고 셋으로 갈라 각자 센다*이다.** ⚠️⚠️ **그 갈래의 수는 두 시점으로 적는다 — 이 표의 머리(A~F 머지 뒤)와 아래 수가 어긋나 있던 자리다(라운드 94 리뷰 M-1이 정정)**: ① **라운드 시작(정찰 트리 `69a7ab8`)** — 표식형 **18** · 강조 낀 표식형 **14** · 필드형 **44** · 잔여 **12**(합 **88** = 저장소 전수 · 트랙 D의 대장이 정찰과 대조하려고 이 시점에 서 있다) · ② **HEAD(A~F 머지 뒤 · 이 표의 머리와 같은 시점)** — **25 · 18 · 50 · 17**(합 **110**). ⚠️ 그 사이도 값이다: 같은 라운드의 앞 트랙이 먼저 올려 **C 뒤 92 · B 뒤 93**이었고, D가 시작한 자리는 88이 아니라 **93**이다. ⚠️ **`markedSourceNeedle()`의 바이트가 불변임을 등호로 증명한다** — 넓히지 않았다는 사실이 값이고, 25는 바늘이 넓어진 수가 아니라 **저장소가 그만큼 더 적은 수**다 |
| 20 | **열 자리의 *열거*를 손이 아니라 자가 내게 할지** | AH-3 | 안 | **집지 않는다 — 축이다.** 그 열거가 사는 세 벌 가운데 둘이 `apps/mobile/src/a11y-contract.test.ts` 안이고, 이번 라운드에 트랙 A가 그 파일에서 든 것은 **핀 두 줄**이다(*한 트랙이 한 그물에 축 둘을 얹지 않는다*) |
| 21 | **문턱·래칫 상수 38의 갈래 셋에 모집단을 붙일지** | AH-5 | 안 | **집지 않는다 — 재현 불가로 남은 유일한 칸 그대로다.** 넓은 바늘은 오늘도 **53**이고 갈래 셋(하네스 4 · 제품 8 · 소스 텍스트 26)의 모집단은 어디에도 없다. ⚠️⚠️ **다만 오늘 그 칸에 *이름으로 집는 자*가 섰다** — 트랙 E가 `RETEST_UNREPRODUCIBLE_CELL`로 그 행이 표에서 사라지는 것을 막는다(붙이는 것은 다음 라운드다) |
| 22 | **트립와이어의 수마다 *그 수를 낸 바늘*을 어디에 적을지** | AH-5 기각 목록 | 안(**F**) | ⚠️⚠️ **절반이 집혔다 → 트랙 E(세는 자) + 오늘의 F(붙이는 손).** 트랙 E는 *어디에 적을지*를 정하지 않고 **오늘 몇이 적혀 있는지를 세는 자**를 세웠고(37 · 13 · 2), F는 이 절의 표에 **행마다 바늘을 함께 적는 꼴**을 처음 썼다. ⚠️ **결정 자체는 아직 열려 있다** — *바늘을 F의 표에 적을 것인가 그 수를 내는 계약 안에 적을 것인가*이고, 오늘 F가 고른 것은 앞쪽이며 그 비용(표가 길어진다)이 이 절의 길이로 보인다 |
| 23 | **`COORDINATE_LIVE_DOCS` 손 목록을 전수로 바꿀지** | `packages/test-utils/src/repo-self-description.test.ts` | 안 | **집지 않는다 — 소유가 갈리는 결정이다.** 오늘도 밖에 `docs/5차/design-restore-spec.md` **3건**이 있고, 그 사실을 그 주석이 **스스로 값으로 적는다**(AH-1이 처방한 꼴의 실물). ⚠️ 트랙 E가 같은 파일에 축을 하나 더 얹었지만 **좌표 축의 손 목록은 한 바이트도 건드리지 않았다** |

⚠️⚠️ **이 표가 라운드 93의 표와 다른 점 넷.** ⓐ **두 표가 한 표가 됐다** — 열일곱과 여섯이
스물셋으로 서고, 합친 손은 **F**다. ⓑ ⚠️⚠️ **집힌 것이 셋이다**(#16 · #18 · #19) — **집는 것이
0건이던 라운드 뒤에 셋이 한 번에 내려간 첫 자리**이고, 셋 다 **손이 저장소 안**이다. ⓒ ⚠️⚠️ **셋
가운데 둘을 세운 손이 *리뷰*다** — AG-3이 *"조건을 세우는 손에 리뷰가 있고 F의 표가 닫힌 뒤에
오므로 다음 정찰이 처음 센다"* 고 적었는데, **오늘은 그 조건들이 다음 정찰이 세자마자 그 라운드의
트랙에게 집혔다**(세운 라운드와 집힌 라운드 사이가 **한 라운드**다). ⓓ ⚠️⚠️ **#10이 처음으로
*예측대로 움직이지 않았다*** — 트랙 A가 스윕을 열었으므로 오를 수 있었는데 소스 리더를 새로 세우지
않아 **0으로 적중했다**. **미리 적어 둔 예측은 오를 때만이 아니라 안 오를 때도 값을 낸다.**

⚠️⚠️ **이번 라운드가 실측하고 기각한 스물여덟을 값으로 남긴다 — 전부 재개 조건과 함께**(V-2).
⚠️⚠️ **검산: 라운드 93의 스물일곱 − 오늘 트랙이 되어 내려간 둘 + 오늘 새로 선 셋 = 스물여덟.**
내려간 둘은 **#18·#19를 낳은 두 줄**이고(리뷰가 세운 조건이 그 다음 라운드의 트랙이 됐다), 새로
서는 셋은 **AH-1의 축**(손 목록 옆 단언 열일곱) · **사각의 자를 저장소 전체로 옮길지**(트랙 B가
자기 파일에서만 닫았다) · ⚠️⚠️ **바늘이 적혔다는 것과 그 바늘로 재면 같은 수가 나온다는 것의
갈림**(트랙 E가 앞쪽만 세운 자리)이다. ⚠️ **아래 목록은 같은 부류를 한 줄로 묶어 적으므로 줄 수와
항목 수가 다르다 — 두 수를 한 낱말로 적지 않는다.**

- ⚠️⚠️ **AH-1의 축(손 목록 옆의 단언 열일곱) — 재었고 트랙이 되지 않는다.** 막는 것은 **바늘**이다 —
  *주석이 모집단 전체를 두고 단언하는가*는 사람의 문장 읽기라 계약이 세울 수 없고, 오늘의 17은 손이
  낸 수다. ⚠️⚠️ **다만 그 병의 한 겹 안쪽(*주석 대 자*)은 오늘 두 트랙이 각자 닫았다**: 트랙 C가
  `handListNeedle`을 이름에서 꼴로 넓혔고(주석은 *"이 절에 손 목록이 다시 서지 않는다"* 인데 자는
  이름 하나였다), 트랙 B가 사각 ⓒ의 증인을 **이름 하나에서 부류 전수 여덟**으로 넓혔다.
  ⚠️ **재개 조건(결정형 · 손은 저장소 안): 손 목록 옆 주석의 *단언 꼴*을 관례로 정하는 날 — 그날
  먼저 물을 것은 *단언을 금지할 것인가 단언에 자를 붙이게 할 것인가*이고, 첫 모집단은 오늘의
  열일곱과 그중 자가 없는 열하나다.**
- ⚠️⚠️ **사각의 자를 `measure: () => number`로 옮기는 일을 저장소 전체로 넓힐지 — 오늘 처음 세어
  값으로 남기고 집지 않는다.** 트랙 B가 자기 파일 여섯을 옮겼는데, AH-3의 답이 값으로 든 사각
  **66자리 · 파일 11** 가운데 **크기가 적힌 것이 64**이고 그 대부분이 여전히 상수다.
  ⚠️ **집지 않는 이유는 크기와 소유다** — 열한 파일이 여섯 트랙의 것이고, 한 트랙이 남의 파일의
  자를 옮기면 AH-4가 이름 붙인 그 부채가 그대로 난다. ⚠️ **재개 조건(결정형 · 손은 저장소 안):
  사각의 자 꼴을 저장소 관례로 정하는 날 — 그날 먼저 물을 것은 *파생이 불가능한 사각에 무엇을
  둘 것인가*이고(트랙 B의 답은 `measure`를 두지 않고 **증인**을 두는 것이다), 첫 모집단은 오늘의
  66과 그중 크기가 적힌 64다.**
- ⚠️⚠️ **바늘이 적혔다 ↔ 그 바늘로 재면 같은 수가 나온다 — 오늘 처음 세어 값으로 남기고 집지
  않는다.** 트랙 E의 자는 **앞쪽만** 센다. 오늘 `helper-named-reader` 행은 바늘이 적힌 열셋 안인데도
  넓은 바늘로 재면 **189**이고 그 칸이 적은 수는 **167**이다. ⚠️ **집지 않는 이유는 크기다** — 뒤쪽을
  물려면 표의 행마다 그 자를 **실제로 대어 보아야** 하고(그 본보기는 같은 파일의 `근거:` 인용 축이다 —
  명령을 진짜로 돌린다), 그것은 라운드 하나의 트랙이 아니라 축 하나다. ⚠️ **재개 조건(결정형 ·
  손은 저장소 안): 표의 수를 *다시 대어 보는* 자가 서는 날 — 그날 먼저 물을 것은 *그 자를 계약이
  돌릴 것인가 정찰의 걸음이 돌릴 것인가*이고, 첫 모집단은 오늘 바늘이 적힌 행이다.**
- **AG-2의 축(막는 것이 *배정*인 조건 여덟) · AG-3의 축(재개 조건을 세운 손) — 재었고 판정 종전
  그대로다.** ⚠️⚠️ **다만 AG-3의 축에 오늘 값 하나가 붙는다**: 이번 라운드가 집은 셋 가운데 **둘을
  세운 손이 리뷰**라, *손을 무엇으로 셀 것인가*라는 그 질문이 **표의 처분 칸에서 실제로 갈렸다**
  (표식으로 세면 리뷰 둘 · `git blame`으로 세면 그 커밋을 만든 손이다). ⚠️ **재개 조건 둘 다
  종전 그대로(결정형 · 손은 저장소 안).**
- **문턱이 재는 세계(제품 ↔ 하네스 ↔ 소스 텍스트) — 재었고 집지 않는다.** ⚠️ 오늘의 수는 아래
  트립와이어의 *문턱·래칫 상수* 칸이 값으로 든다(38 ↔ 넓은 바늘 53). ⚠️ **재개 조건(사건형 —
  종전 그대로): 문턱이 재는 세계의 표기가 한 트랙의 축이 되는 라운드가 서는 날.**
- **자기 배제 관례 — ⚠️ 재었고 좁은 바늘로 열다섯 · 넓은 바늘로 열일곱이며 이름은 여섯이다.**
  ⚠️ **오늘 늘린 하나가 트랙 C**(`apps/admin/src/admin-load-error-copy.test.ts:120`의 `SELF_PATH`)
  이고, **좁은 바늘도 이번에는 함께 움직였다**(라운드 93에는 넓은 쪽만 움직였다). **정합을 세는
  자리는 오늘도 0건이다.** ⚠️ **재개 조건(결정형 · 손은 저장소 안 — 종전 그대로).**
- **공유 테스트 DB를 비우는 일 — 재었고 집지 않으며, 오늘의 값은 정찰의 것이다.**
  `wooriai_test` **9,476**(활성 **8,829** · **744MB**) ↔ `wooriai_dev` **62**(활성 62 · 24MB).
  ⚠️⚠️ **여섯째 시점에 셋 다 움직였고, 움직인 손을 세는 자가 0건이다.** ⚠️ **재개 조건(결정형 ·
  손은 저장소 안 — 종전 그대로).**
- **`derived-exemptions`의 절반 문턱 — 재었고 여유가 0 그대로다.** 사문 전수 **44 중 면제 22**로
  정확히 절반이고 조건은 *"넘는 날"* 이라 아직 아니다(⚠️ **다섯 트랙 어디에도 새 `export const`가
  서지 않았다** — 트랙 A가 더한 것은 `export function` 하나다). ⚠️ **재개 조건(사건형): 면제가
  사문 전수의 절반을 넘는 날.**
- **`dead-export-ledger.ts`의 결정형 8 — 전제는 참이고 여덟 라운드째 집지 않는다.** `.tsx`의
  `export function` 오늘도 **141**이다. ⚠️ **그 파일이 아무 트랙의 것도 아닌 네 번째 라운드다.**
  ⚠️ **재개 조건(결정형 · 손은 저장소 안 — 종전 그대로).**
- **앵커 대장 — 재었고 한 라운드 만에 다시 조용하다.** 앵커 전수 **699**(code-only **621** ·
  comment-tolerant **70**/래칫 70 · comment-only **8**/래칫 8 · unanchored **0**) · 면제 **8** ·
  사각 `helper-named-reader` **167**. ⚠️ **집지 않는 이유는 래칫이다.** ⚠️ **재개 조건(결정형 ·
  손은 저장소 안 — 종전 그대로).**
- **재개 조건 대장의 `prose-only` · `round-notes` — 재었고 둘 다 집지 않는다.** ⚠️ **둘 다 이 절
  자신과 정찰 노트가 키운다**(아래 트립와이어가 값으로 든다). ⚠️ **재개 조건 둘 다 결정형 · 손은
  저장소 안이고 종전 그대로다.**
- **재개 조건 대장의 `unmarked-source-prose` — ⚠️⚠️ 재었고 오늘 처음 *줄었다*.** 129에서
  **61**이 됐고, 줄어든 자리는 사라진 것이 아니라 **필드형이라는 제 이름을 얻었다**
  (트랙 D). ⚠️ **사각이 줄어드는 것이 늘 좋은 일은 아니다 — 이름이 틀렸던 것이지 크기가 틀린 것이
  아니었다는 사실을 함께 적는다.**
- **낭독 라벨 스윕의 남는 일곱 · 반쪽 프롭 일곱 · `accessibilityState={{ disabled }}` 열한 자리 ·
  `useMutation` 밖의 쓰기 35 · AC-1의 일곱 · AC-2의 0 · 어드민의 무명 표 하나 · `<section>` 34 ·
  어드민 연타 가드 절반** — 재었고 판정 종전 그대로이며 재개 조건 전부 사건형이다.
- **`a11y-contract.test.ts`의 사각 셋이 문자열 배열이라 AH-3의 바늘 밖이다** — 라운드 93이 처음
  적은 그대로다. ⚠️ **재개 조건(결정형 · 손은 저장소 안 — 종전 그대로).**
- **AH절 본문과 갈리는 수를 값으로 남긴다**(재론이 아니라 *어느 시점의 수인지*를 가르기 위해서다):
  조사 모집단 **38 → 39**, 조사 스윕의 사각 **일곱 → 여덟**, 걷는 파일 **281(하한) → 282**,
  `unmarked-source-prose` **129 → 61**, `quoted-source-conditions` **4 → 55**,
  자기 배제 좁은 바늘 **14 → 15**, 재개 조건 문서 축 **433 → 435**(리뷰와 상태 커밋이
  움직였다 · **이 절이 서면 또 오른다**). ⚠️ **일곱 다 라운드 93 *리뷰* 또는 라운드 94의 트랙이
  움직였고, AH절 본문에는 옛 수가 남아 있다.**

---

⚠️⚠️ **트립와이어 대조표 — 그리고 이 표가 라운드 93의 표와 다른 점 하나는 *꼴*이다.**
⚠️ **시점은 종전과 같다: A~F 머지 뒤 · 리뷰 전(2026-09-01).** ⚠️⚠️ **다른 것은 마지막 칸이다** —
AH-5가 *"수는 바늘이 적혀 있지 않으면 아무도 풀 수 없다"* 를 세웠고 라운드 94 트랙 E가 그 사실을
자로 만들었으므로(행 **37** 가운데 바늘이 곁에 적힌 행 **13**), **오늘부터 이 표는 행마다 수 곁에
*그 수를 낸 바늘*을 적는다.** ⚠️ **한 라운드에 전부는 못 한다 — 그러니 *몇을 붙였는지*를 값으로
적고, 그 수를 다시 내는 것은 트랙 E의 자다**(오늘 붙인 뒤의 수는 **39 + 명시적 무바늘 1**이고, 라운드
93의 표에서는 13이었다). ⚠️⚠️ **그 *+1*을 한 낱말로 적지 않는다(라운드 94 리뷰 M-4)** — F 커밋
시점의 이 문장은 *"오늘 붙인 뒤의 수는 40"* 이었는데, 그 40에는 **잴 자가 없다고 스스로 적은 행
하나**(*문턱·래칫 상수* · `**바늘**: 없다`)가 함께 세어져 있었다. 트랙 E의 자가 쓰던 낱말 바늘 둘이
*바늘*·*모집단* 이라는 낱말만 보고 **부정문을 긍정으로 셌기** 때문이고, 오늘 그 자가 다섯째 판정으로
갈라 센다. **바늘이 아직 안 붙은 행 0 · 바늘이 붙은 행 39 · 붙일 자가 없다고 적은 행 1.**
⚠️⚠️ **하한을 고른 비용도 함께 적는다 — *행이 사라지거나 바늘이 지워지는
것*을 그 자는 보지 못한다**(AG-5·AH-5의 일반형 · 오르는 쪽 래칫이다).

| 자리 | 라운드 93이 남긴 값 | 2026-09-01 HEAD 재실측(**A~F 머지 뒤** · 리뷰 전) | 움직였는가 · **그 수를 낸 바늘** |
| --- | --- | --- | --- |
| 사문 대장 항목 / 래칫 | 22 / 22 | **22 / 22** | 그대로다 — 어느 트랙도 그 대장을 열지 않았다(**네 라운드 연속**). **바늘**: `packages/test-utils/src/dead-export-ledger.ts`의 대장 항목과 래칫 상수를 그 짝 계약이 등호로 문다 |
| 사문 대장의 사각 **여덟** | 22 · 0 · 226 · 20 · 55 · 105 · 141 · 0 | **22 · 0 · 226 · 20 · 55 · 105 · 141 · 0** | **그대로다 — 여덟 자리가 한 자리도 다르지 않다**(네 라운드 이어서다). **바늘**: 그 대장의 사각마다 선 `measure`가 오늘 다시 센다 · ⚠️ **한 칸에 여덟 수가 들어 이 행은 한 행으로 센다**(트랙 E의 사각 ⓒ) |
| 사문 전수 / 면제 / 모듈 | 44 / 22 / 6 | **44 / 22 / 6** | 그대로다 — ⚠️ **정확히 절반이고 여유 0**이다. 트랙 A가 `apps/mobile/src/text/korean-particles.ts`에 `export function` 하나를 더했는데 **쓰이는 export라 사문이 아니고**, 다섯 트랙 어디에도 새 `export const`가 서지 않았다. **바늘**: 그 대장이 `apps`·`packages`를 **걷어** 참조 0건인 export를 파생한다 |
| `CLOSED_BLIND_SPOTS` | 셋 | **셋** | 그대로다. **바늘**: `packages/test-utils/src/dead-export-ledger.ts`의 그 상수 길이(등호) |
| ⚠️⚠️ 앵커 대장(앵커 전수 · code-only · comment-tolerant/래칫 · comment-only/래칫 · unanchored) | 698 → 699 · 621 · 70/70 · 8/8 · 0 | **699 · 621 · 70/70 · 8/8 · 0** | 그대로다 — 아홉 번 만에 움직인 뒤 **한 라운드 쉬었다**. ⚠️ **트랙 A가 스윕을 열었는데도 소스 리더를 새로 세우지 않아 이 수가 오르지 않았다.** **바늘**: `packages/test-utils/src/comment-tolerant-anchor-ledger.ts`의 `collectCommentToleranceAnchors()`로 걷고 `countVerdicts()`로 가른다(오늘 F가 그 자를 직접 대어 다시 냈다) |
| 앵커 대장의 면제 | 8 | **8** | 그대로다. **바늘**: 같은 파일의 인용 면제 목록 길이 |
| ⚠️⚠️ 앵커 대장의 사각 `helper-named-reader` | 167(하한 164) | **167**(하한 **164**는 오늘도 참이다) | 그대로다 — 라운드 93 리뷰가 채운 칸이 **처음으로 조용하다**. **바늘**: `packages/test-utils/src/comment-tolerant-anchor-ledger.ts:532`의 `NAMED_SOURCE_READER`로 걷고 `collectCommentToleranceAnchors().outside`에서 `reason === "helper-named-reader"` 인 자리를 센다(**파일 하나가 한 자리** · `readSource` 자신은 뺀다). ⚠️ **넓은 바늘의 189는 다른 자다 — 두 수를 한 낱말로 적지 않는다**(트랙 E의 사각 ⓐ가 이 행을 **이름으로** 집는다) |
| 재개 조건 대장(자리 · 괄호 표기 · 손의 위치 · 결정형 · 손 누락) | 433 · 146 · 44 · 45 · 1 | **467 · 156 · 52 · 53 · 1** | ⚠️⚠️ **움직였고, 이 절 자신이 대부분을 늘렸다.** ⚠️ **AH절의 433과 오늘의 수는 갈린 것이 아니라 시점이 다르다** — 그 사이에 라운드 93 **리뷰**·상태 커밋이 이 문서에 조건을 더했고(그때 **435**), 그 위에 이 절이 얹혔다. **손 누락은 1 그대로다.** **바늘**: `packages/test-utils/src/resume-condition-ledger.ts`의 `collectDocumentSites()`로 이 문서를 걷고 `tallyNeedles().paren` · `decisiveSites()` · `decisiveSitesMissingHand()`로 가른다 |
| 재개 조건 대장의 사각 **여덟** | 230 · 4 · 410 · 17 · 4 · 26 · 11 · 129 | **246 · 8 · 440 · 18 · 55 · 28 · 11 · 61** | ⚠️⚠️ **움직였고 둘은 트랙 D가, 나머지는 이 절과 정찰 노트가 움직였다** — `quoted-source-conditions` **4 → 55**(넓은 바늘이 인용까지 세게 됐다)와 `unmarked-source-prose` **129 → 61**(필드형이 제 이름을 얻고 빠져나갔다)이 D의 걸음이다. **바늘**: 그 대장의 사각마다 선 `measure(baseDir)`가 오늘 다시 센다 · ⚠️ **한 칸에 여덟 수가 들어 한 행으로 센다** |
| 재개 조건 대장의 **경과 축**(그 줄 · 창 · 어디에도 없음) | 7 · 93 · 340 | **7 · 102 · 365**(래칫 **3 · 61**) | ⚠️ **움직였다 — 창과 *어디에도 없음*이 늘었고 늘린 것이 이 절 자신이다**(그 줄 자신이 경과를 적은 자리는 **일곱** 그대로이고, 래칫 **3 · 61**은 오늘도 아래에 있다). **바늘**: 같은 대장의 `documentElapsedSites()` + `tallyElapsed()`(창 반지름 **5줄**) |
| 표기를 지닌 소스 파일 / 그 뿌리(걷는 파일) | 셋 · 전수 파생(표기 **11** · 괄호 **15**) | **모집단 11 · 표식형을 지닌 파일 3 · 걷는 파일 970** | ⚠️⚠️ **움직였다 — 그리고 움직인 것은 저장소가 아니라 *모집단 판정*이다.** 트랙 D가 모집단을 *표식형 하나*에서 **세 갈래의 합**으로 바꿔 파일이 **3 → 11**이 됐고, **표식형을 지닌 파일 3은 그대로**라 옛 등호가 그대로 문다. **바늘**: 걷기는 `packages/test-utils/src/resume-condition-ledger.ts:712`(`readSourceAxisEntries` · 걷는 파일 970을 낸 자)이고 모집단 판정은 `:1260`(`sourceAxisFilesFrom` · *세 갈래의 합이 0이 아닌 파일*을 남겨 11을 낸 자)이며, 그 셋의 갈래가 `:759`(표식형) · `:775`(강조 낀 표식형) · `:796`(필드형)이다. ⚠️ **두 시점(라운드 94 리뷰 L-5)**: F 커밋 시점 이 칸은 `:759` 하나만 들어 **이 행의 수를 낸 자리가 아니라 갈래의 정의**를 가리켰다 — 970도 11도 그 줄에서 나오지 않는다(리뷰가 든 `:1265`는 같은 함수의 본문 줄이라 선언 줄 `:1260`을 적는다) |
| 계약 그물 수 | 열다섯 | **열다섯** | ⚠️⚠️ **그대로다 — 그물 수가 늘지 않은 다섯 번째 라운드다.** 트랙 B·C의 개편, D의 세 갈래, E의 새 축이 넷 다 그 수에 들지 않고 **왜 들지 않는지를 각자의 머리말이 값으로 적는다**. **바늘**: `packages/test-utils/src/contract-net-ledger.test.ts`의 그물 목록 길이 + 1을 두 대장이 각자 지니고 갈리지 않는지를 문다 |
| 반쪽 프롭(짝 · role 단독 · live region 단독) | 일곱(37 · 1 · 6) | **일곱(37 · 1 · 6)** | 그대로다 — ⚠️ **a11y 그물을 연 트랙이 있는데도 그대로다**(A가 그 파일에서 든 것은 **핀 두 줄**이다). **바늘**: `apps/mobile/src/a11y-contract.test.ts`가 `app`·`src`의 비테스트 `.tsx`를 **걷어** 짝 프롭을 파생한다 |
| 어드민의 상태 문장 | 자리 47 / 파일 14 / 출구 45(`role="alert"` 34 · `role="status"` 11) / 면제 2 | **47 / 14 / 45(34 · 11) / 2** | 그대로다 — **어드민 소스 0바이트**다. **바늘**: `apps/admin/src/admin-status-announce.test.ts`가 어드민 `app`·`src/components`를 걷어 여는 태그를 센다 |
| 어드민의 표 이름 / 무명 표 | 17 / 0(`app/**`) · 스윕 밖 하나(표 전수 18) | **17 / 0**(`app/**`) · 스윕 밖 **하나**(표 전수 **18**) | 그대로다. **바늘**: `apps/admin/src/admin-table-name.test.ts`의 전수 걷기 |
| 어드민 `<section>` / 이름 | 34 / 0 | **34 / 0** | 그대로다 — 이름을 주지 않는 것이 판정이다. **바늘**: `apps/admin/src/admin-landmark-current.test.ts`의 전수 걷기 |
| 어드민 `<main>`(중첩) / `aria-current` | 2(중첩 0) / 1 | **2(중첩 0) / 1** | 그대로다 — ⚠️ **트랙 C가 어드민 계약을 열었는데도 랜드마크 축은 바이트 불변이다**(어드민 소스 **0바이트**). **바늘**: 같은 파일의 랜드마크 스윕 |
| 어드민 라우트 표면(`page.tsx` ↔ `NAV_ITEMS`) | 11 ↔ 11 · 어긋남 0 · 세는 계약 둘 | **11 ↔ 11 · 어긋남 0 · 세는 계약 둘** | 그대로다 — **같은 전수를 파생하는 자가 둘**이고 사본이 아니라 각자 파생한다. **바늘**: `apps/admin/src/admin-route-surface.test.ts`와 `apps/admin/src/admin-load-error-copy.test.ts`가 각각 `app/**`을 걸어 `page.tsx`를 센다 |
| ⚠️⚠️ 어드민 [다시 시도] 관례(모집단 · 판정) | `page.tsx` 전수 11 · `retry` 10 · `message-only` 1 · `underived-banner` 0 · `no-load-failure` 0 | **11 · 10 · 1 · 0 · 0** | 그대로다 — 라운드 93 트랙 D가 세운 전수가 오늘도 같은 수를 낸다. **바늘**: `apps/admin/src/admin-load-error-copy.test.ts`가 `app/**`을 걸어 `page.tsx`를 파생하고 자리마다 판정 하나를 소스에서 낸다 |
| ⚠️⚠️ 어드민의 남은 손 목록 둘(폐기 자리 · 폴백 어미) | **손 목록 열 · 손 목록 열둘**(결정형 #16이 이름으로 지목했다) | **걷기 16**(사유를 통째로 버린다 **12** · 문구를 남긴다 **4** · 리터럴 아님 **0** 상한) · **화면 12**(출현 **13**) | ⚠️⚠️ **오늘 처음 전수로 세어졌다 — 그리고 두 목록의 결과가 서로 다르다.** 앞쪽은 손 **10**과 오늘의 **16**이 **갈렸고**(라운드 73 뒤에 들어온 여섯이 목록 밖에서 조용했다), 뒤쪽은 손 **12**와 오늘의 **12**가 **일치했다**. ⚠️ **실피해는 오늘 0건이고 값은 *열일곱 번째 폐기 자리가 서는 날*에 있다.** **바늘**: 같은 파일의 한 벌 호출 자리 걷기(`apps/admin/src/admin-load-error-copy.test.ts:537`)와 폴백 어미 화면 걷기(`:216`) |
| 어드민 `<button` / `type=` / `disabled=`(줄) | 68 / 68 / 44 | **68 / 68 / 44** | 그대로다. **바늘**: 어드민 `app/**`을 걷는 연타 가드 스윕 |
| 모바일 `useMutation`(선언 · 컨트롤 · 핸들러 · 안 막음) | 31 · 27 · 1 · 3 | **31 · 27 · 1 · 3** | 그대로다 — 그 스윕을 여는 트랙이 0건이다. **바늘**: `apps/mobile/src/mutation-press-guard.test.ts`의 전수 걷기 |
| 모바일 Pressable / 역할 / 라벨 | 145 / 145 / 122 | **145 / 145 / 122** | 그대로다. **바늘**: `apps/mobile/src/a11y-contract.test.ts`의 여는 태그 스윕 |
| 모바일 스크롤 컨테이너(전수 · 판정 명시 · 키보드가 뜰 수 없는 화면 · 미가드) | 17 · 11 · 6 · 0 | **17 · 11 · 6 · 0** | 그대로다 — ⚠️ **트랙 A가 모바일 화면을 열었는데도 그대로다**(바뀐 것은 문구를 만드는 식이지 스크롤러가 아니다). **바늘**: `apps/mobile/src/keyboard-tap-guard.test.ts`가 비테스트 `.tsx`를 걸어 스크롤 컨테이너 여는 태그를 파생한다 |
| ⚠️ 모바일 낭독 라벨(식 전수 · 목록 안 · 직접 · 파생 · 갈리지 않음 · 상수) | 162 · 48 · 44 · 4 · 0 · 73 | **하한 162 · 48 · 44 · 4 · 0 · 73이 오늘도 초록이다** | 그대로다(하한). ⚠️⚠️ **이 칸은 *오른 것*을 보지 못한다 — 이 축이 하한이라 오늘의 값은 그 자가 말하지 않는다**(트랙 E의 사각 ⓐ가 이름 붙인 병의 다른 얼굴이다: *바늘은 적혀 있는데 이 자는 바닥만 센다*). **바늘**: `apps/mobile/src/a11y-contract.test.ts:6889`의 래칫 표와 그 옆의 `app`·`src` 전수 걷기 |
| ⚠️⚠️ 모바일 나가는 길(라우트 · 탭 밖 · `ScreenHeader` · 배선 · 판단 · 출구 없음) | 36 · 31 · 18 · 12 · 6 · 0(재수출 5 · `ScreenHeader` 미사용 8) | **36 · 31 · 18 · 12 · 6 · 0**(재수출 **5** · 미사용 **8**) | 판정 값은 그대로이고 ⚠️⚠️ **자의 꼴이 바뀌었다** — 사각 **여섯**의 자가 상수 **6 · 파생 0**에서 **상수 0 · 파생 4 · 자 없음 2**가 됐다(결정형 #18 소진). ⚠️ **자를 두지 않은 둘은 크기 대신 *불가의 증거*를 진다.** **바늘**: `apps/mobile/src/screen-header-back.test.ts:234`의 두 시점 기록과 그 파일의 `app/**` 전수 걷기 |
| ⚠️⚠️ 모바일 조사(모집단 · 꼬리 고정 · 값에서 고름 · 갈리는데 고정) | 38 · 29 · 9 · 0 | **39 · 30 · 9 · 0**(걷는 파일 **282**) | ⚠️ **움직였다 — 모집단이 하나 늘었고 그 하나가 라운드 93 *리뷰*가 더한 계사 바늘의 자리다.** ⚠️⚠️ **그리고 이 행 옆에 오늘 새 모집단이 하나 섰다**(아래 행). **바늘**: `apps/mobile/src/korean-particle-guard.test.ts`가 `apps/mobile/{app,src}/**`의 비테스트 `.ts`·`.tsx`를 걸어 주석을 마스킹한 뒤 보간 뒤 조사를 파생한다 |
| ⚠️⚠️ 모바일 접미사(모집단 · 값에서 고름 · 갈리는데 고정) · 핀 이관 | **세는 자리 0건**(사각 `suffix-not-particle` 실측 2) | **3 · 3 · 0**(상한) · 사각 **2 → 0** · **계약 5 · 핀 9** | ⚠️⚠️ **오늘 처음 세어졌다** — 라운드 93이 *"조사가 아니라 접미사라 이 축 밖"* 으로 남긴 사각이 **판정으로 승격**됐고 실측이 **2에서 0**이 됐다. ⚠️⚠️ **핀 이관 대장이 정찰의 배정(계약 넷)을 넘었다 — 오늘 계약 다섯 · 핀 아홉**이다. **바늘**: `apps/mobile/src/korean-particle-guard.test.ts:679`의 핀 대장(자리 · 옛 바이트 · 오늘의 바이트 셋)과 같은 파일의 접미사 모집단 파생 |
| 문턱·래칫 상수 | 38(하네스 4 · 제품 8 · 소스 텍스트 26) | **다시 재지 못했다**(넓은 바늘로는 **53**) | ⚠️⚠️ **이 칸은 오늘도 비어 있다 — 이 표에서 *재현 불가*로 남은 유일한 자리다.** 갈래 셋의 모집단이 어디에도 적혀 있지 않아 넓은 바늘의 53과 대조할 길이 없다(결정형 21). ⚠️ **오늘 이 칸에 처음 붙는 것은 바늘이 아니라 *이름으로 집는 자*다** — 트랙 E가 이 행 이름이 표에서 사라지는 것을 막는다. **바늘**: 없다(넓은 바늘은 `FLOOR`·`CAP`·`RATCHET`·`THRESHOLD`가 이름에 든 상수 선언 전수 · **모집단이 적히면 그것은 새 판정이다**) |
| ⚠️ 자기 배제 관례(자리 / 이름) | 좁은 바늘 열넷 / 넷 · 넓은 바늘 열여섯 / 여섯 | **좁은 바늘 열다섯 / 넷 · 넓은 바늘 열일곱 / 여섯** | ⚠️ **둘 다 하나씩 늘었고 늘린 손이 트랙 C다**(`apps/admin/src/admin-load-error-copy.test.ts:120`의 `SELF_PATH`). ⚠️ **라운드 93에는 넓은 쪽만 움직였는데 오늘은 좁은 쪽도 함께 움직였다 — 새 이름이 아니라 *이미 세던 이름*이 하나 늘었기 때문이다.** **정합을 세는 자리는 오늘도 0건이다.** **바늘**: 이름에 `SELF`가 든 상수 선언 전수(넓은 바늘) ↔ 이름 넷만 보는 좁은 바늘 |
| 접근성 표 → 짝 문서 좌표(자리 · 인용 번호 · 서로 다른 번호 · 표면 · 역방향 · 어긋남) | 96 · 100 · 96 · 37 · 8 · 0 | **97 · 101 · 97 · 38 · 8 · 0** | ⚠️ **움직였다** — F가 A-35(`#111`)를 세우며 앞의 넷이 하나씩 늘었다(역방향과 어긋남은 그대로). ⚠️ **라운드 90 D가 이 축을 *하한*으로 세운 덕에 F의 걸음이 그대로 통과했다.** **바늘**: `packages/test-utils/src/accessibility-checklist-shape.test.ts:336`의 표면 바늘과 그 위의 좌표 바늘 |
| 짝 계약의 이동 의무(좌표 자리 · 절 이름 · 줄 번호 배열 · 의무가 빈 자리) | 9 · 8 · 1 · 0 | **9 · 8 · 1 · 0** | 그대로다 — ⚠️⚠️ **그리고 오늘 그 의무가 세 번째로 이행됐다**: `[22, 1343, 1404]`가 **`[22, 1378, 1446]`** 이 됐고 F가 같은 걸음에 옮겼다(값도 꼴도 바이트 불변 · `:22`는 A절보다 위라 네 라운드째 움직이지 않는다). **바늘**: 그 짝 계약의 절 이름·줄 번호 배열 바늘과 그 자리 위 40자 하한의 의무 문장 |
| ⚠️ 문서→소스 좌표(살아 있는 문서 넷 · 실재 · 범위 안 · 이동 의무) | 35 · 35 · 35 · 6 | **54 · 54 · 54 · 6**(모집단 밖 **961** · 문서 **36** · 죽은 좌표 **3**) | ⚠️⚠️ **움직였고 늘린 손이 이 절이다** — 오늘 F가 트립와이어의 바늘을 `<파일>:<줄>` 꼴로 적기 시작하면서 좌표가 함께 늘었다. ⚠️ **해석되지 않은 경로 꼴 좌표는 0건이다**(적기 전에 다 풀어 보았다). **바늘**: `packages/test-utils/src/repo-self-description.test.ts:563`의 좌표 정규식으로 살아 있는 문서 넷을 걷고, 파일 실재·줄 범위·의무 창(±2줄)을 각각 판정한다 |
| 짝 문서(행 수 · 실기기 · 브라우저 · 서버 · 작업) | 170 · 140 · 20 · 9 · 1 | **171 · 141 · 20 · 9 · 1** | ⚠️ **움직였다** — `#171`(트랙 A) **하나**이고 **브라우저는 0건**이다(세 라운드 이어 브라우저가 0건이다). **바늘**: `packages/test-utils/src/runtime-checklist-shape.test.ts`가 그 문서의 표를 파싱해 §0의 네 수·합계·머리말·§1-1 구간을 전부 파생시킨다 |
| 접근성 표 | A-34까지(`#110`) · Batch 43 | **A-35까지(`#111`) · Batch 44** | ⚠️ **움직였다** — 행 **하나**(`#111` 트랙 A)와 문단 **넷**. **바늘**: 같은 문서의 절 제목 꼴과 행 번호 |
| 접근성 표 C-3 경과 수(세 자리) | 스물일곱 · 스물일곱 · 스물일곱(갈림 0) | **스물여덟 · 스물여덟 · 스물여덟**(갈림 0) | ⚠️ **움직였고, 세 자리를 함께 올렸다** — 한 자리만 올리면 짝 계약이 빨개진다. **바늘**: `packages/test-utils/src/accessibility-checklist-shape.test.ts`의 *"<한글 수사> 라운드 연속 (67·68·…)"* 바늘이 C절 행 · 수동 증거 절 · 짝 문서 §0-1 셋을 각각 문다 |
| `MAX_DIVERGENCE` ↔ 문서가 옮겨 적은 그 상수 | **0** ↔ 문서 세 자리 **0** · 세는 계약 하나 | **0** ↔ 문서 세 자리 **0** · 세는 계약 하나 | 그대로다 — ⚠️ **경과 수를 올리는 걸음이 *"0을 넘지 않는가"* 를 한 글자도 바꾸지 않았다**(네 라운드 연속). **바늘**: 같은 짝 계약의 *"끝 라운드 차이가 …을 넘지"* 산문 바늘(오늘 자리 셋) |
| 접근성 짝 계약의 상수 인용 좌표 | `22 · 1343 · 1404` | **`22 · 1378 · 1446`** | ⚠️⚠️ **움직였고 F가 함께 옮겼다** — A-35 절과 C-3의 세 자리, 배치 줄이 아래 두 좌표를 **35줄·42줄** 밀었다. **값도 꼴도 바이트 불변이고 움직인 것은 좌표뿐이다**(세 번째 이행). **바늘**: 그 계약이 자기 소스에서 등호로 무는 줄 번호 배열 하나 |
| 공유 테스트 DB / 활성 카탈로그 ↔ 제품 카탈로그 | 738MB · 9,439(활성 8,797) ↔ 62 | **744MB · 9,476**(활성 **8,829**) ↔ **62**(24MB) | ⚠️⚠️ **움직였고, 움직인 손을 세는 자가 0건이다** — 이 라운드의 트랙도 F도 api 스위트를 돌리지 않았으므로 그 +37은 라운드 93 **리뷰**의 실행이다. ⚠️ **이 수는 F가 다시 재지 않았다 — 정찰이 2026-09-01에 잰 값이다**(바늘: `wooriai_test`·`wooriai_dev`에 직접 질의). **N-4의 판정은 미발동 그대로다** |
| ⚠️⚠️ 판정 문서의 재실측 표(행 · 수가 있는 행 · 바늘이 곁에 적힌 행 · 좌표꼴) | **세는 자리 0건** | **37 · 37 · 13 · 2**(수만 있는 행 **24** · 모집단 밖의 옛 표 넷 **행 92**) | ⚠️⚠️ **오늘 처음 세어졌고, 그 자가 서자마자 F에게 숙제를 냈다** — 이 수는 **AH절의 표**를 잰 값이고, 오늘 F가 이 표에 바늘을 붙였으므로 다음 라운드에 그 자가 내는 수는 **이 표의 수**다. ⚠️ **정찰의 좌표꼴 1과 이 계약의 2는 바늘이 다르다**(줄 범위꼴) — **두 수를 한 낱말로 적지 않는다.** **바늘**: `packages/test-utils/src/repo-self-description.test.ts`가 *마지막 판정 절*을 꼴로 따라가 머리가 `자리`인 표의 행을 전수 파생하고, 행마다 ⓐ 값 칸의 수 ⓑ 바늘의 꼴 넷 ⓒ 좌표꼴을 판정한다(셋 다 **하한**) |

⚠️ **이 표가 라운드 93의 트립와이어 관례에 더하는 것이 셋이다.** ⓐ ⚠️⚠️ **시점 예외가 0건인 네
번째 표다.** ⓑ ⚠️⚠️ **마지막 칸에 *바늘*이 들어왔다 — 오늘 이 표의 행은 40이고, 그 가운데 **39**에
바늘이 곁에 섰다. 나머지 하나는 바늘이 빠진 것이 아니라 *잴 자가 없다*고 스스로 적은 행이다**
(*문턱·래칫 상수* · `**바늘**: 없다` · **행 40 = 바늘 39 + 명시적 무바늘 1 + 수만 있는 행 0**).
⚠️ **두 시점**: F 커밋 시점의 이 문장은 *"그 40에 전부 바늘이 곁에 섰다"* 였고, 라운드 94 리뷰(M-4)가
**세는 자의 부정문 오독**을 고치며 39와 1로 갈랐다 — **세 수를 한 낱말로 적지 않는다.**
⚠️ 라운드 93의 표에서는 행 **37** 가운데 **13**이었고 **수만 있는 행이 24**였다(오늘 수만 있는
행은 **0**이다). **그중 기계가 따라갈 수 있는 좌표꼴은 9다**(라운드 93의 표에서는
**2**). ⚠️ **행이 37에서 40이 된 것은 자리가 셋 늘었기 때문이다** — 어드민의 남은 손 목록 둘 ·
모바일 접미사와 핀 이관 · 판정 문서의 재실측 표 자신이고, **셋 다 오늘 처음 세어진 축**이다.
ⓒ ⚠️⚠️ **그런데 재현 불가로 남은 칸은 오늘도 하나다**(문턱·래칫 상수 38) — **바늘을 붙이는 것과
그 바늘로 재면 같은 수가 나오는 것은 다른 일이고, 이 표는 앞쪽만 지킨다**(트랙 E의 사각 ⓐ).
⚠️ **그리고 40 · 39는 *이 표의 오늘 값*이지 트랙 E의 자가 낸 수가 아니다** — 그 자가 이 표를 재는
것은 다음 라운드이고, F 커밋 시점에 그 자가 낸 **37 · 37 · 13 · 2**는 **AH절의 표**를 잰 값이다.
⚠️ **다만 라운드 94 리뷰가 그 자를 고치며 이 표를 한 번 재 두었다 — 행 40 · 수가 있는 행 40 ·
바늘 39 · 명시적 무바늘 1 · 좌표꼴 9 · 모집단 밖의 옛 표 다섯(행 129)** 이고, 그 자의 하한(37 ·
37 · 13 · 1 · 80)은 **한 자리도 올리지 않았다**(하한은 오늘의 값이 아니라 바닥이다).
**두 시점을 한 낱말로 적지 않는다.**

**이 라운드가 짝 문서에 남긴 것.** 확인의 표에 **`#171` 하나**가 서고 §0의 네 숫자와 합계·머리말의
재계산 줄·§1-1 머리말의 라운드 구간이 파싱으로 다시 세어졌으며, 접근성 표에는 **A-35 `#111`
하나**와 **C-3의 세 자리 갱신**, 그리고 **짝 계약의 좌표 한 줄**이 함께 옮겨졌다.
⚠️⚠️ **표면 배분은 실기기 하나 · 브라우저 0건이고, 그 이유가 §0의 수보다 값이 크다**: 다섯 트랙
가운데 **제품 소스를 한 바이트라도 고친 것이 하나**이고(A의 세 바이트와 조사 한 글자) **나머지 넷은
자기 계약의 자와 바늘을 고쳤다**(사각의 자 · 손 목록의 모집단 · 소스 바늘의 갈래 · 판정 문서의
수) — 넷 다 **화면 0건 · 문구 0건 · 렌더 0건**이다. ⚠️ **라운드 84가 세운 그 구분**(*"확인할 것이
없다"* 가 아니라 *"폰에 보이는 동작을 한 곳도 바꾸지 않았다"*)**의 아홉째 판이다.**
⚠️⚠️ **A-35가 A-34와 다른 점 하나는 그 절의 머리말이 진다**: A-34는 *계약이 아예 0건인 자리*를
물었고, **A-35는 그 계약이 선 뒤에 그 계약이 스스로 사각으로 남긴 자리를 묻는다.**
⚠️⚠️ **C-3은 오늘로 스물여덟 라운드째 미확인**이고, ⚠️ **이번 라운드가 그 줄에 더하는 값은 경과
수가 아니라 *그 수 곁에 이제 바늘이 적힌다*는 사실이다** — 확인은 여전히 배정을 기다린다.

### AI-1. **두 수를 한 낱말로 적지 않는다는 규율이 한 라운드에 세 번 물렸고, 세 번 다 물린 자리가 *정찰의 표*였다** — 그리고 세 번 다 갈린 것은 저장소가 아니라 바늘이었다

- **사실 ①(트랙 D · 56 = 44 + 12).** 정찰이 재개 조건 대장의 소스 축을 열며 *"`⚠️`가 없는 필드형
  **56**"* 을 값으로 냈다. 트랙 D가 필드 이름(`reopenCondition:`·`resumeCondition:`)을 **실제로
  요구하는** 바늘을 대자 **44**였고, 남는 **열둘**은 주석의 인용 · 단언이 문자열로 문 자리 ·
  `statement:` 같은 다른 필드였다. ⚠️⚠️ **정찰의 56은 *필드형의 수*가 아니라 *`⚠️`가 없는 자리의
  수*였고, 두 수가 한 낱말로 적혀 있었다.**
- **사실 ②(트랙 C · 손 10 ↔ 걷기 16).** 결정형 #16이 이름으로 지목한 `discardedSites`가 **열**이고
  정찰이 그것을 그대로 옮겨 적었다. 오늘 한 벌 호출 자리를 전수로 걸으니 **16**이고, 갈린 **여섯**은
  라운드 73 뒤에 들어온 화면들이다(감사 로그 둘 · 카테고리 · 준비템 분류 · 사용자 조회 · MFA 관문).
  ⚠️⚠️ **같은 트랙의 두 번째 목록은 반대였다** — `FALLBACK_PHRASE_OCCURRENCES`의 **열둘**은 오늘의
  전수와 **일치했다**(출현으로 세면 13이라 그쪽도 두 수다). ⚠️ **두 목록이 같은 파일 · 같은 꼴 ·
  같은 라운드에 적혔는데 결과가 갈렸다는 것이 이 자리의 값이다.**
- **사실 ③(트랙 E · 좌표꼴 1 ↔ 2).** 정찰이 손으로 세어 *"좌표로 가리킨 것은 하나뿐"* 이라고
  적었고, 트랙 E가 라운드 93 E의 파서를 **그대로 불러** 재니 **2**였다. 둘째는 줄 **범위**꼴
  (`…ledger.ts:702-704`)이고 정찰의 손 판정이 그것을 좌표로 세지 않았다. **하한은 1에 둔다.**
- ⚠️⚠️ **세 자리의 공통점 하나가 처방을 정한다 — 셋 다 *정찰이 문장으로 적은 바늘*이었다.**
  ①은 *"`⚠️`가 없는 것"*, ②는 *"손으로 적은 목록"*, ③은 *"좌표로 가리킨 것"* 이고, 셋 다 사람이
  읽으면 하나로 읽히는데 기계가 대면 둘로 갈린다. ⚠️ **반대로 오늘 정확히 재현된 수도 있다** —
  정찰의 *걷는 계약 40*은 바늘이 `readdirSync` **낱말 하나**였고 한 자리도 다르지 않았다.
- **일반형.** ⚠️⚠️ ***두 수를 한 낱말로 적지 않는다*는 규율은 지금까지 **판정 문서**에 적용돼
  왔는데, 오늘 그 규율이 필요한 자리가 **정찰의 표**임이 드러났다.** 정찰은 트랙의 **입력**이고,
  입력의 수가 두 수를 한 낱말로 적으면 트랙은 그것을 *하나의 사실*로 받아 자기 계약에 못 박는다.
  ⚠️ **오늘 그 일이 일어나지 않은 이유는 규율이 아니라 배정이다** — 정찰이 마지막 줄에서
  *"갈리면 갈렸다는 사실 자체를 값으로 적으라"* 고 **미리** 요구했고, 세 트랙이 각자 그렇게 했다.
- ⚠️ **다음 라운드가 먼저 세어 볼 만한 것**: 정찰 노트가 낸 수 가운데 **바늘이 낱말 하나인 것**이
  몇이고 **문장인 것**이 몇인가(오늘 표본 넷 가운데 낱말 하나였던 것이 하나이고 그 하나만
  재현됐다 — ⚠️ **나머지 노트의 수는 오늘 세지 않았다**).
- ⚠️ **되짚기(라운드 93)** — AH-2의 질문(계약이 문구를 바이트로 문 자리)에 답한 자리는 **AI-4**이고,
  이 절이 답한 것은 그 답의 **성질**이다.

### AI-2. **리뷰가 세운 결정형이 다음 정찰의 표에 서자마자 그 라운드의 트랙에게 집혔다 — 조건이 서고 소진되기까지 한 라운드다**

- **사실.** 라운드 93 **리뷰**가 세운 결정형 둘(#18 *사각의 자를 `measure: () => number`로 바꿀지* ·
  #19 *`⚠️`와 `재개` 사이에 강조 표식을 허용할지*)이 라운드 94 정찰의 둘째 표에 처음 섰고,
  **같은 라운드의 트랙 B·D가 둘 다 집었다.** 라운드 93 트랙 D가 세운 #16은 **두 라운드** 만에
  트랙 C에게 집혔다.
- ⚠️⚠️ **AG-3이 적어 둔 구조가 오늘 한 칸 더 접혔다.** AG-3은 *"조건을 세우는 손에 리뷰가 있고
  F의 표가 닫힌 뒤에 오므로 **다음 정찰**이 처음 센다"* 였고, AH절은 그 앞 칸(*트랙이 세운 조건은
  그 라운드의 F가 처음 센다*)을 찾았다. **오늘 보이는 것은 그 뒤 칸이다 — 정찰이 처음 세는 그
  라운드에 트랙이 곧바로 집는다.**
- **오늘의 전수.** 결정형 전수표 **스물셋**(열일곱 + 여섯) · 집힌 것 **셋** · 남은 것 **스물** ·
  집힌 셋의 손은 **전부 저장소 안** · 세운 손은 **리뷰 둘 · 트랙 하나**.
  ⚠️ **집는 것이 0건이던 라운드**(93)**와 셋이 내려간 라운드**(94)**가 잇달아 왔다.**
- ⚠️⚠️ **그런데 이 속도에는 값이 하나 더 있다 — 집히는 조건과 집히지 않는 조건이 갈리는 자리가
  *손*이 아니라 *크기*다.** 오늘 집힌 셋은 다 **한 파일 안에서 닫히는** 조건이었다(사각의 자 하나 ·
  손 목록 둘 · 바늘 하나). 집히지 않은 스물 가운데 손이 저장소 안인 것은 **열다섯**이고 밖인 것은
  **다섯**이다(#3·#4·#5·#6·#7). ⚠️ **그 열다섯 가운데 집히지 않은 이유가 *관례를 정하는 일*인 것이
  넷**(#13·#15·#21·#22)**이고 *저장소 전체에 걸치는 일*인 것이 넷**(#8·#10·#11·#12)이며,
  **나머지 일곱은 크기나 접점이 이유다**(#1·#2·#9·#14·#17·#20·#23).
  ⚠️ **AA-3이 *손이 안인가 밖인가*로 가른 축 위에, 오늘 *한 파일인가 저장소인가*라는 축이 하나 더
  선다.**
- **일반형.** ⚠️⚠️ **결정형 재개 조건은 *언젠가*가 아니라 *다음 라운드*에 집힐 수 있고, 집히는
  속도를 정하는 것은 그 조건의 손이 아니라 그 조건이 닫히는 범위다.** 한 파일 안에서 닫히는
  조건은 정찰이 세는 그 라운드에 트랙이 되고, 저장소에 걸치는 조건은 손이 안에 있어도 라운드를
  넘긴다(#8이 여덟 라운드째인 이유가 그것이다).
- ⚠️ **다음 라운드가 먼저 세어 볼 만한 것**: 결정형 전수표의 스물셋을 **닫히는 범위**로 갈라
  *한 파일*이 몇이고 *한 앱*이 몇이며 *저장소 전체*가 몇인가 — ⚠️⚠️ **오늘은 그 갈래를 세지
  않았다.** 오늘 손이 센 축은 *손이 안인가 밖인가* 하나뿐이고 그 답은 **열여덟 대 다섯**이다
  (집힌 셋을 포함한 전수 스물셋 기준). **범위의 갈래를 세는 자는 0건이고, 세려면 조건마다
  *닫히는 파일 목록*을 먼저 적어야 한다.**
- ⚠️ **되짚기(라운드 93)** — AH-4의 질문(트랙이 남의 파일에 만든 부채)에 답한 자리는 **AI-4**다.

### AI-3. **자를 둘 수 없는 사각에서 `() => 0`은 자가 아니라 상수이고, 그 자리가 지어야 하는 것은 크기가 아니라 *불가의 증거*다** — 그리고 그 증거가 서자마자 0이 무엇의 0인지가 드러났다

- **사실.** 트랙 B가 결정형 #18을 집어 `apps/mobile/src/screen-header-back.test.ts`의 사각 여섯을
  **상수에서 파생**으로 옮겼는데, **둘은 파생이 불가능했다** — `hardware-back-and-swipe-are-not-seen`
  (안드로이드 하드웨어 뒤로가기·iOS 엣지 스와이프)과 `source-not-runtime`(이 계약이 화면을 렌더하지
  않는다). ⚠️ **재는 대상이 *소스에 없는 것*이라 `app/` 트리를 아무리 걸어도 수가 나오지 않는다.**
- ⚠️⚠️ **정찰이 미리 물어 둔 것이 그 자리였다** — *"`() => 0`은 다시 상수다"*. 트랙 B의 답은
  **`measure`를 두지 않는 것**이고, 대신 그 둘은 **증거 넷**(*세려 한 것이 무엇인가* · *왜 소스에
  없는가* · *0의 뜻이 무엇인가* · *증인 걷기*)을 진다.
- ⚠️⚠️ **그리고 증인을 파생으로 내자 곧바로 값이 하나 나왔다.** *하드웨어 뒤로가기를 가로채는
  자리가 `app/**`에 0건*이라는 증인을 걸으니, **같은 바늘이 스윕 밖의 `AppLockOverlay`에서 자리를
  낸다.** ⚠️⚠️ **즉 그 0은 *가로채기가 없다*가 아니라 *이 자가 볼 수 있는 곳에 없다*였다** —
  **0은 없음이 아니라 셀 수 없음의 값이다.**
- ⚠️ **곁딸린 발견 하나도 값으로 남긴다** — 같은 트랙이 주석 마스킹 함수가 **자기 파일에서**
  정규식의 홀수 따옴표에 갇히는 것을 만났고, 줄 단위 대체로 고치며 **오차의 방향**(거짓 빨강 =
  안전)을 함께 적었다. **자를 고치는 걸음이 자기 자를 재는 걸음이 된 자리다.**
- **오늘의 전수.** 그 파일의 사각 **여섯** · 파생 자 **넷**(`noScreenHeader` **8** ·
  `noBackByDesign` **6** · `reExports` **5** · 비 `.tsx` **0**) · 자 없음 **둘** · 유령 방지 부정
  단언(**함수 본문에 `return <숫자>` 꼴 0건**). ⚠️ **저장소 전체로 보면 사각 66자리 가운데 크기가
  적힌 것이 64이고, 그 대부분이 아직 상수다**(기각 목록에 재개 조건과 함께 있다).
- **일반형.** ⚠️⚠️ **사각에 크기를 적으라는 규율은 *크기를 적을 수 있는 사각*을 전제하는데, 셀 수
  없는 사각이 실재한다.** 그런 자리에 억지로 수를 두면 **상수를 함수로 포장한 것**이라 병이 그대로
  남고, 두지 않으면 그 사각이 *크기가 없는 것*으로 조용히 읽힌다. ⚠️ **답은 크기 대신 증거이고,
  증거가 갖춰야 하는 것은 *0의 뜻이 무엇인가*다** — AH-3이 *"크기만 적으면 다음 라운드가 그 크기를
  성격으로 읽는다"* 고 적은 병의 **반대쪽 얼굴**이다(이쪽은 크기가 없어서 성격이 안 읽힌다).
- ⚠️ **다음 라운드가 먼저 세어 볼 만한 것**: 이 저장소의 사각 가운데 **크기가 0인 것**이 몇이고,
  그 0이 *실제로 0*인 것과 *셀 수 없어서 0*인 것이 각각 몇인가(오늘 AH-3의 66 안에서 크기가 없는
  것이 **둘**이고 크기가 0으로 적힌 것은 세지 않았다 — ⚠️ **그 둘을 가르는 자는 오늘 0건이다**).
- ⚠️ **되짚기(라운드 93)** — AH-3의 질문(사각의 크기와 갈래)에 답한 자리는 위 「질문 다섯」 표의
  3행이고, 그 답이 이 절과 AI-1을 함께 낳았다.

### AI-4. **부채를 배정 단계에서 없애려면 배정표가 아니라 *소스*가 세어야 한다 — 정찰이 준 계약 넷은 하한이었고 오늘의 실측은 다섯·아홉이다**

- **사실.** AH-4가 *"파일 단위 트랙 배타는 한 트랙이 다른 트랙의 파일에 만든 부채를 막지 못한다"* 를
  세웠고, 라운드 94 정찰이 그 처방을 **배정 단계**에서 썼다 — *제품 문구를 고치는 트랙은 그 문구를
  바이트로 무는 계약 파일 전부를 함께 소유한다*. 정찰이 트랙 A에 준 계약은 **넷**이었다
  (`design-restore-p2d` · `a11y-contract` · `korean-particle-guard` · `budget-edit.test`).
- ⚠️⚠️ **트랙 A가 오늘 소스를 다시 걸으니 다섯이었다.** 배정표에 없던 것이
  `apps/mobile/src/settings/more-menu.test.ts`(핀 **셋**)이고, 칩 라벨을 무는
  `apps/mobile/src/home/budget-edit.test.ts`의 핀 **둘**까지 세면 **계약 넷 · 핀 여덟**이며, 여기에
  **스윕 자신의 핀 하나**(`literalParticle` 등호)를 더해 **계약 다섯 · 핀 아홉**이다.
  ⚠️ **아홉째는 대장 밖에 둔다 — 대장이 자기를 무는 순환을 만들지 않는다**(두 수를 한 낱말로 적지
  않는다).
- ⚠️⚠️ **값은 수가 아니라 *누가 그 수를 냈는가*에 있다.** 배정표는 정찰이 손으로 쓴 문서이고, 오늘
  그 완전성을 판정한 것은 **트랙이 세운 핀 이관 대장**이다(자리 · 옛 바이트 · 오늘의 바이트 셋을
  지고, 옛 바이트가 **코드 어디에도 없음**을 부정 단언으로 문다). **AH-4의 처방은 배정 규율을
  바꾸는 것으로 끝나지 않고, 그 배정이 완전했는지를 세는 자를 함께 세워야 닫힌다.**
- **오늘의 결과.** ⚠️⚠️ **통합이 닫을 부채는 0건이다** — 라운드 93은 같은 일을 **통합 커밋**이
  뒤늦게 했고(`357b021`), 오늘은 **트랙의 커밋 하나**가 아홉 핀을 동시에 옮겼다.
  ⚠️ **그리고 그 걸음이 발견을 하나 더 낳았다**: 새 순수 함수의 본문이 **자기 스윕의 바늘에
  자기가 걸려** 빨개졌고, 본문을 조각으로 쪼개 피하면서 *스윕이 자기 저장소를 무는 자리*를 값으로
  적었다.
- **함께 닫힌 것.** 조사 모집단 **39**(꼬리 고정 **30** · 값에서 고름 **9** · *갈리는데 고정* **0**
  상한) 옆에 **접미사 모집단 셋**이 서고 갈래가 뒤집혔다(*값에서 고른다* **0 → 3** ·
  *갈리는데 고정으로 적었다* **3 → 0**). 사각 `suffix-not-particle`은 실측 **2 → 0**으로 발동했고,
  그 자리에는 **실사용 갈래**(이미 `네`로 끝나는 이름)가 새 조건으로 섰다.
- **일반형.** ⚠️⚠️ **배정은 문서이고 부채는 소스다 — 문서로 부채를 막으려면 그 문서가 옳았는지를
  소스가 판정해야 한다.** ⚠️ **오늘 정찰의 넷이 틀린 수가 아니라 하한이었다는 것이 그 증명이다**:
  손으로 센 배정은 늘 하한이고, 하한인 줄 알고 쓰면 트랙이 그 위를 채우며 부채가 0이 된다.
  **AH-2의 답이 값으로 든 *두 계약 이상이 같은 제품 파일을 바이트로 무는 자리 73*이 이 병의 표면
  크기이고, 오늘 그 표면 하나를 여는 데 계약 다섯이 필요했다.**
- ⚠️ **다음 라운드가 먼저 세어 볼 만한 것**: 제품 파일 하나를 고칠 때 **함께 열어야 하는 계약이
  평균 몇 개**인가 — 오늘 표본은 하나이고(제품 두 파일 ↔ 계약 다섯 · 핀 아홉), AH-2의 73은 *파일*의
  수이지 *한 자리를 여는 비용*이 아니다(⚠️ **두 수를 한 낱말로 적지 않는다** · 그 비용을 세는
  자는 오늘 0건이다).
- ⚠️ **되짚기(라운드 93)** — AH-2·AH-4의 질문에 답한 자리가 이 절이다.

### AI-5. **좌표에 이어 *수*에도 세는 자가 섰고, 그 자가 선 첫날 F의 표가 꼴을 바꿨다** — 그러나 *바늘이 적혔다*와 *그 바늘로 재면 같은 수가 나온다*는 여전히 다른 일이다

- **사실.** AH-5가 *"문서가 무는 것에는 두 층이 있다 — 좌표와 수이고, 좌표에는 세는 자가 섰지만
  수에는 아직 아무도 서지 않았다"* 로 닫았다. 오늘 트랙 E가 **최신 판정 절의 재실측 표를 전수로
  파생**해 행마다 셋을 판정했다: ⓐ **수가 있는가**(37) ⓑ **그 수를 낸 바늘이 곁에 적혔는가**(13)
  ⓒ **그 바늘이 `<경로>:<줄>` 꼴이어서 기계가 따라갈 수 있는가**(2). **수만 있는 행은 24였다.**
- ⚠️⚠️ **절 이름을 손으로 적지 않은 것이 이 자의 값 절반이다** — *판정 절의 꼴*을 물고 그 **마지막**을
  따라가므로, 오늘 이 AI절이 서자 그 자는 **자동으로** 이 표를 본다. ⚠️ **표를 못 찾으면 0이 아니라
  빨개진다**(유령 방지).
- ⚠️⚠️ **그 자가 선 첫날 F의 표가 꼴을 바꿨다** — 위 트립와이어의 마지막 칸에 **행마다 그 수를 낸
  바늘**이 들어왔고, 오늘 바늘이 곁에 적힌 행은 **40**이다(라운드 93의 표에서는 13).
  ⚠️ **한 라운드에 전부는 못 했고, 못 했다는 사실이 아니라 *몇을 붙였는지*가 값이다** — 그 수를
  다시 내는 것은 다음 라운드의 그 자다.
- ⚠️⚠️ **그러나 이 자는 앞쪽만 센다.** 오늘 `helper-named-reader` 행은 바늘이 적힌 열셋 안인데도
  **넓은 바늘로 재면 189**가 나오고 그 칸이 적은 수는 **167**이다. ⚠️ **바늘이 적혔다는 것과 그
  바늘로 재면 같은 수가 나온다는 것은 다른 일이고, 뒤쪽을 무는 자는 오늘도 0건이다**(기각 목록에
  재개 조건과 함께 있다).
- ⚠️ **재현 불가로 남은 칸은 오늘도 하나다** — 문턱·래칫 상수 **38**(하네스 4 · 제품 8 · 소스 텍스트
  26)의 갈래 셋에 모집단이 어디에도 없어 넓은 바늘의 **53**과 대조할 길이 없다. ⚠️⚠️ **오늘 그 칸에
  처음 붙은 것은 바늘이 아니라 *이름으로 집는 자*다** — 트랙 E가 그 행 이름이 표에서 사라지는 것을
  막는다(**사각이 조용히 없어지는 것**을 막는 자이지 사각을 닫는 자가 아니다).
- ⚠️ **하한을 고른 비용도 값으로 적는다** — 행 수도 바늘 수도 좌표꼴 수도 **하한**이라, **행이
  사라지거나 바늘이 지워지는 것을 이 자는 보지 못한다.** 등호를 고르면 F가 바늘을 붙이는 정직한
  걸음마다 빨개지고 **빨간 계약은 아무것도 지키지 못한다**(AF-5·AG-5·AH-5가 값으로 적은 그 비용).
- ⚠️⚠️ **그리고 같은 걸음에 짝 계약의 이동 의무가 세 번째로 이행됐다** — `[22, 1343, 1404]`가
  **`[22, 1378, 1446]`** 이 됐고, **값도 꼴도 바이트 불변이며 움직인 것은 좌표뿐이다.** 라운드 91
  F의 첫 이행 · 라운드 93 F의 둘째 이행에 이어 **세 라운드 연속으로 F가 그 등호를 스스로 옮겼다.**
- **일반형.** ⚠️⚠️ **세는 자가 서면 그 자가 세는 것을 만드는 손이 곧바로 그 자를 향해 움직인다.**
  라운드 93 E가 좌표를 세자 그 라운드의 F가 여섯 줄을 고쳤고, 라운드 94 E가 수 곁의 바늘을 세자
  이 라운드의 F가 표의 꼴을 바꿨다. ⚠️ **그러니 이 저장소에서 판정 문서가 낡는 것을 막는 것은
  규율이 아니라 *그 규율을 수로 내는 자*이고, 그 자는 늘 F가 아니라 트랙이 세운다**(옮기는 손과
  세는 손이 반대라는 AG-5의 판정이 세 번째로 확인됐다).
- ⚠️ **다음 라운드가 먼저 세어 볼 만한 것**: 이 표의 **수만 있는 행이 24에서 얼마로 줄었고**, 그중
  **좌표꼴로 가리킨 행이 몇으로 늘었는가**(오늘 F가 붙인 뒤의 수는 위 표가 값으로 들지만, ⚠️ **그
  수가 *다음 라운드의 자*로도 같은지는 오늘 세지 않았다** — 자를 대는 것은 트랙 E의 계약이고 F의
  손이 아니다). ⚠️⚠️ **두 시점 — 리뷰가 그 자를 대 보았다(라운드 94 리뷰 M-4)**: 그 자의 낱말
  바늘이 *"바늘: 없다"* 를 긍정으로 세고 있어 먼저 고쳤고, 고친 자로 재니 **행 40 · 바늘 39 ·
  명시적 무바늘 1 · 수만 있는 행 0 · 좌표꼴 9**다. **F의 수와 자의 수가 이번에는 갈리지 않았다 —
  갈린 것은 *전부*라는 낱말 하나였다.**
- ⚠️ **되짚기(라운드 93)** — AH-5의 질문에 답한 자리가 이 절이다.

---

## AJ. 라운드 95에서 확정한 판정 (2026-09-01 · GAP-095 트랙 F)

라운드 94가 물은 것이 **자와 바늘의 층**(다섯 트랙 가운데 넷이 *자기 계약이 수를 내는 방법*을
고쳤다 · AI절)이었다면, 라운드 95는 그 층에서 한 칸 더 내려가 **정찰이 낸 수 자체가 무엇이었는가**를
답한 라운드다 — ⚠️⚠️ **다섯 트랙이 다섯 다 자기 축의 전제를 다시 재었고, 다섯 다 정찰의 수보다
컸다**(대장의 사각 **열하나 → 열다섯** · 정리 걸음 **넷 → 열여섯** · 낭독 모집단 **쉰일곱 → 쉰여덟** ·
그 바이트를 무는 계약 **셋 → 넷** · 라벨로만 상태를 말하는 자리 **0 → 둘**). 축은 라운드 81~94와
같이 **사용자 가치**였고(사람이 듣는 문장 · 운영자 도구의 0 · 저장소가 자기를 세는 자리), 다섯 판정
다 K~AI절과 같이 **결함 보고가 아니라 다음 결정의 입력**이며 2026-09-01 소스에서 확인됐다(라운드 95
트랙 A·B·C·D·E 머지 후 · 리뷰 전).
⚠️⚠️ **이번 라운드가 연 화면은 하나이고 그 하나가 세 자리다** — 선택 상태를 **라벨과 상태 프롭으로
두 번** 말하던 자리 셋(빠른 품목 타일 · 달 점프 시트 · 날짜 픽커)이고 셋 다 핵심 루프 **1·2단계**
위에 있다. 가격·링크·픽셀은 바이트 불변이고, **통합이 닫아야 할 부채는 0건이다**(AI-4의 처방을
배정 단계에서 쓴 두 번째 라운드다).

⚠️⚠️ **이번 라운드의 가장 값진 관측: 정찰의 전수가 다섯 자리에서 다 하한이었고, 다섯 다 *바늘이
문장이라서* 갈렸다**(AJ-1). AI-1이 라운드 94에 *"바늘이 낱말 하나면 재현되고 문장이면 재현되지
않는다"* 를 세웠는데, 오늘 그 일반형이 **정찰의 표가 아니라 트랙의 걸음에서** 다섯 번 확인됐다.
⚠️ **다섯 다 어느 쪽이 틀린 것이 아니라 바늘이 다르고, 다섯 다 그 사실을 트랙이 값으로 적었다.**

⚠️⚠️ **두 번째 관측: 값은 라운드 사이에만 낡는 것이 아니라 *한 라운드 안에서도* 낡는다**(AJ-2).
트랙 C가 재개 조건 대장의 사각 열다섯을 걷는 동안 **셋이 그 자리에서 움직였고**(같은 라운드의 앞
트랙 커밋이 소스에 재개 조건을 더했다), 그래서 C는 적힌 값을 *갱신*하는 대신 `valueToday`를
**자에서 파생**시키고 옛 상수를 `recorded`로 강등했다. ⚠️⚠️ **그 선택이 오늘 값을 하나 더 냈다** —
C 커밋 시점에 **여덟**이던 갈린 자리가 F가 이 절을 쓰는 오늘 **열하나**다.
⚠️ **정정(라운드 95 리뷰 M-3 — 위 문장은 보존한다).** 마지막 문장의 시점·귀속이 재실측과 갈린다:
**여덟은 C 커밋 시점의 수가 아니라 *정찰 스냅숏*(`96231d1`)을 오늘의 자로 잰 수**이고, 갈린
자리는 정찰 노트·E·D의 커밋으로 **C 커밋 시점에 이미 열하나**였으며, 그 뒤 A와 F(이 절)를
지나도 **열하나 그대로**다 — A·F가 움직인 것은 갈린 자리의 **수가 아니라 크기**다(AJ-2의 정정
표가 시점·모집단을 칸으로 진다).

⚠️⚠️ **세 번째 관측: 규율이 자보다 넓으면 위반은 자 밖에서 산다 — 그리고 그 자리가 화면이었다**
(AJ-3). `apps/mobile/src/a11y-contract.test.ts:2296`이 *"선택 여부를 문구로 다시 말하지 않는다 —
그건 상태가 진다"* 를 부류 전체를 두고 적어 두었는데, 그 문장을 무는 자의 모집단은 **온보딩 카드
한 자리**였다. 오늘 그 자를 `accessibilityState` 자리 **전수 58**로 넓히자 위반 **셋**이 곧바로
드러났고, 셋 다 사람이 매일 듣는 자리였다. **자가 규율보다 좁으면 그 규율은 문서이지 계약이 아니다.**

⚠️⚠️ **네 번째 관측: 관례는 파일을 건너간다 — 0의 뜻이 한 파일에서 세 파일이 됐다**(AJ-4 · ⚠️ 그 낱말이 사는 파일을 저장소 전수로 세면 **다섯**이다 — 두 수를 한 낱말로 적지 않는다). 라운드 94
트랙 B가 모바일 한 파일에 세운 `UncountableEvidence`(*세려 한 것* · *왜 소스에 없는가* · **`zeroMeans`** ·
*증인*)를 라운드 95 트랙 B가 **복사하지 않고 인용해** 어드민 두 파일에 `CountedZero`로 이식했다.
⚠️ **그리고 이식하는 걸음이 값을 하나 냈다** — 같은 이름 `source-not-runtime`이 세 자리에 있는데
**성질이 반대**(어드민 둘은 걷어서 낸 0 · 모바일 하나는 셀 수 없어 0)라는 사실이
`SAME_NAME_OTHER_KIND`로 소스에 섰다.

⚠️⚠️ **다섯 번째 관측: 자가 하나면 그 자의 오차 방향이 곧 계약의 오차 방향이 된다**(AJ-5). 트랙 D가
공유 테스트 DB를 불리는 손을 세면서 *정리가 있었는가*를 **자 셋**(같은 파일 · 생성보다 뒤 · 같은
바인딩)으로 나란히 재었고, 답이 **7 · 9 · 11**로 갈렸다. 자 하나만 두었다면 그 하나가 곧 진실이
됐을 텐데, 셋을 나란히 두자 *어느 자로 봐도 정리가 없는 바닥*(**7**)과 *가장 좁은 자가 보는 천장*
(**11**)이 함께 값이 됐다.

---

### AI-1~AI-5의 질문 다섯 — 라운드 95의 답 (출처: `docs/5차/round95-scout.md` §1 실측)

⚠️⚠️ **다섯 다 답했고 넷이 발동했다.** ⚠️ **아래 수는 이 절이 새로 잰 것이 아니라 정찰이
2026-09-01 HEAD(`96231d1`)에서 재어 그 노트 §1에 적은 값이고, 낸 방법이 적힌 곳을 함께 가리킨다**
(O-3의 반대 방향이 아니다 — *판정*은 여기 한 번만 쓰고 *수*는 낸 자리를 가리킨다).
⚠️⚠️ **그리고 이 답들은 계약이 아니라 손으로 잰 수라, 계약이 세는 수와 한 낱말로 적지 않는다** —
아래 트립와이어의 같은 이름 칸들이 그 대조다.

| # | 질문(AI-n이 남긴 자리) | 2026-09-01 답 | 낸 방법이 적힌 곳 | 발동 |
| --- | --- | --- | --- | --- |
| 1 | AI-1 — 정찰 노트가 낸 수 가운데 **바늘이 낱말 하나인 것**이 몇이고 **문장인 것**이 몇인가 | 라운드 94 노트가 명시한 바늘 **아홉** · ⚠️⚠️ **낱말 하나 둘 · 문장 일곱** · ⚠️⚠️ **재현 실험 여섯 자리: 낱말·정규식이던 셋이 셋 다 정확히 재현**(테스트 파일 전수 **401** · 걷는 계약 **43** · 넓은 바늘 파일 **189**)**되고 문장이던 셋이 셋 다 갈렸다**(손 목록 61/33 → **56/26** · 사각 전수 66/11 → **62/14** · 자기 배제 15 → **17**) | 정찰 §1 「답 1을 낸 방법」(모집단 아홉 · 바늘 둘 · 사각 넷) | ⚠️⚠️ **발동 → 트랙 E**(여섯째 판정) |
| 2 | AI-2 — 결정형 전수표의 스물셋을 **닫히는 범위**로 갈라 *한 파일* 몇 · *한 앱* 몇 · *저장소 전체* 몇인가 | ⚠️⚠️ **한 파일 8 · 한 앱 8 · 저장소 전체 7**(검산 23) · ⚠️⚠️ **집힌 셋이 전부 한 파일** · ⚠️ **여덟·아홉 라운드째 안 집히는 다섯이 전부 저장소 전체** · ⚠️ **#22 하나는 결정 자체가 범위를 정한다** | 정찰 §1 「답 2를 낸 방법」(모집단 스물셋 · 바늘 = 쓰기가 닿는 파일 목록 · 사각 셋) | ⚠️⚠️ **발동 → 배정 규율의 근거이자 아래 통합표의 새 칸**(*닫히는 범위*) |
| 3 | AI-3 — 저장소의 사각 가운데 **크기가 0인 것**이 몇이고 그 0이 *실제로 0*인 것과 *셀 수 없어 0*인 것이 각각 몇인가 | 크기가 **0으로 적힌** 사각 자리 **15 · 파일 5** · ⚠️⚠️ **열다섯 다 `measure`를 지녀 걷어서 낸 0** · ⚠️⚠️ **셀 수 없어 0인 자리는 크기를 아예 두지 않는 둘**(그 안의 증인 0이 셋) · ⚠️⚠️ **그 갈래를 적는 낱말 `zeroMeans`가 저장소에 한 파일 여섯 번뿐** | 정찰 §1 「답 3을 낸 방법」(모집단 62자리·파일 14 · 바늘 = 최상위 `today`/`value`가 리터럴 0 · 사각 넷) | ⚠️⚠️ **발동 → 트랙 B** |
| 4 | AI-4 — 제품 파일 하나를 고칠 때 **함께 열어야 하는 계약이 평균 몇 개**인가 | ⚠️⚠️ **평균 3.33 · 중앙값 1 · 최대 29**(물린 제품 파일 **181** · 계약 **187** · 쌍 **602**) · ⚠️⚠️ **절반(92)이 계약 하나만 물고 꼬리 열둘이 열하나 이상을 문다** · ⚠️⚠️ **가장 비싼 자리가 정확히 핵심 루프다**(`app/expenses/new.tsx` **29** · `app/(tabs)/index.tsx` **29**) | 정찰 §1 「답 4를 낸 방법」(모집단 제품 507·계약 408 · 바늘 = 경로 문자열 + 한국어 리터럴 · 사각 넷) | ⚠️⚠️ **발동 → 트랙 A의 배정 근거**(그리고 그 배정이 **또 하한이었다** — AJ-1) |
| 5 | AI-5 — 재실측 표의 **수만 있는 행이 24에서 얼마로 줄었고** 그중 **좌표꼴로 가리킨 행이 몇으로 늘었는가** | 행 **40** · 수가 있는 행 **40** · 바늘이 곁에 적힌 행 **39** · 명시적 무바늘 **1** · ⚠️⚠️ **수만 있는 행 24 → 0** · ⚠️⚠️ **좌표꼴 2 → 9** · ⚠️ **붐비는 칸 2 → 18**(한 칸에 수가 여덟 넘게 든 행에 바늘이 하나다) | 정찰 §1 「답 5를 낸 방법」(모집단 = AI절 재실측 표 40행 · 바늘 넷 + 다섯째 · 사각 넷) | ⚠️ 답했고 트랙이 되지 않는다 — ⚠️⚠️ **다만 그 답의 *뒤쪽*이 트랙 E의 여섯째 판정을 낳았다** |

⚠️ **되짚기(라운드 94) — 이 다섯 질문에 답한 자리.** AI-1 → **AJ-1**(그 일반형이 트랙의 걸음에서
다섯 번 확인됐다) · AI-2 → 이 표의 2행과 **아래 통합표의 *닫히는 범위* 칸**(질문이 표의 꼴이 됐다) ·
AI-3 → **AJ-4** · AI-4 → 이 표의 4행과 **AJ-1**(배정이 또 하한이었다) · AI-5 → 이 표의 5행과
**트립와이어의 갈래 칸**(낱말 ↔ 문장)이고, 그 답의 **다음 칸**이 **AJ-5**다.
**판정은 그 판정이 사는 절 하나에만 적는다**(O-3·X-4).

---

### 정찰이 남긴 기록 의무 — 트랙이 정찰과 갈린 자리 일곱

⚠️ **정찰의 마지막 줄이 *"각 트랙은 자기 축의 전제를 다시 재고, 정찰과 갈리면 갈렸다는 사실 자체를
값으로 적는다"* 를 요구했다.** 오늘 갈린 자리는 **일곱**이고, ⚠️⚠️ **여섯 다 트랙이 자기 소스에
먼저 적었으며 이 절은 그것을 옮겨 적을 뿐이다.**

- ⚠️⚠️ **정찰 *"사각 열하나"* ↔ 트랙 C **열다섯**.** 정찰은 `LEDGER_BLIND_SPOTS` 여덟과
  `ELAPSED_BLIND_SPOTS` 셋만 세었고 `sourceNeedleBlindSpots()`의 **넷**을 세지 않았다.
  ⚠️ **그래서 *갈린 자리*의 수도 함께 갈린다** — 정찰의 **일곱**은 *열하나 안의 갈림*이고 C의
  **여덟**은 *열다섯 안의 갈림*이며, **오늘 F가 다시 대니 열하나**다(그 셋은 C 커밋 뒤에 선 트랙 A의
  커밋과 이 절 자신이 움직였다). **세 수를 한 낱말로 적지 않는다.**
  ⚠️⚠️ **그리고 갈린 몫의 일부는 정확히 정찰 자신의 노트다** — `round-notes` **+28** · 경과 창
  **+7**이 `round95-scout.md` 한 벌이 만든 수이고, C가 그 사실을 *"정찰은 자기가 쓰고 있던 문서를
  세지 못했다"* 로 소스에 적었다.
- ⚠️⚠️ **정찰 *"정리 걸음 넷"* ↔ 트랙 D **열여섯**.** 정찰이 이름으로 든 넷은 `onboarding` 둘과
  `admin-affiliate-click-breakdown` 둘뿐이었고, 오늘 같은 뿌리를 걸으니 `items-commerce` **일곱** ·
  `link-health` **둘** · `data-retention-purge` **둘** · `product-link-price-honesty` **하나**가
  더 있었다(파일 **여섯**). ⚠️⚠️ **그래서 정찰의 *"정리 없이 누적을 만드는 자리 열셋"* 은 오늘
  어느 자로도 나오지 않는다**(**7 · 9 · 11**). ⚠️⚠️ **그 열셋을 상한으로 옮겨 적었다면 이 계약은
  첫날부터 거짓 초록이었다** — 새 자리가 둘 붙어도 상한 아래에 남는다.
- ⚠️⚠️ **정찰 *"모집단 열일곱을 하한으로"* ↔ 트랙 D **하한 12**.** D가 하한을 오늘의 값(17)에
  두지 않은 이유를 값으로 적었다: 결정 #14가 내려져 픽스처를 한 자리로 모으면 생성 자리는
  **정직하게 줄고**, 등호에 가까운 하한은 **그 옳은 손에게 빨강을 준다**(라운드 89 D가
  `dead-export-ledger`의 하한을 셋에 둔 채 값이 넷 → 여섯으로 오르는 것을 지켜본 그 판단이 판례다).
  ⚠️ **하한의 일은 오늘의 수를 지키는 것이 아니라 *걷기가 깨졌을 때 판정보다 먼저 빨개지는 것*이고,
  12는 한 파일의 최대 몫(`items-commerce` **넷**)의 세 배보다 크다.**
- ⚠️⚠️ **정찰 *"모집단 57"* ↔ 트랙 A **56 → 58**.** 정찰의 57에는 **주석 안의 한 자리**가 들어
  있었고(`app/(tabs)/reports.tsx`의 JSDoc이 그 프롭을 인용한다), 낭독되는 것은 주석이 아니므로
  트랙 전 값은 **56**이다. 오늘은 **58**이다 — 비선택 가지 둘에 상태 프롭이 섰다.
  ⚠️⚠️ **그리고 정찰의 *"다른 넷 갈래는 0건"* 도 갈렸다** — 오늘 전수로 세니 **로딩 갈래가 둘**이고
  (`src/ui/Skeleton.tsx`의 실루엣 둘) 그 둘은 **상태 프롭이 아예 없어 한 번만 읽힌다.**
  정찰의 사각 ⓑ가 미리 물은 *"라벨로만 상태를 말하는 자리의 크기"* 의 답이 **0이 아니라 2**다.
- ⚠️⚠️ **정찰 *"계약 셋"* ↔ 트랙 A **계약 넷 · 핀 여섯**.** 배정표에 없던 넷째 계약이
  `apps/mobile/src/keyboard-tap-guard.test.ts`이고, 그 자리는 문구를 **바이트가 아니라 수로**
  물었다(한국어 리터럴 **54** → **53**). ⚠️ **바늘이 달라도 부채는 같으므로 핀 대장이 함께 진다.**
  ⚠️⚠️ **AI-4의 일반형이 두 번째로 확인됐다 — 손으로 센 배정은 늘 하한이다.**
  ⚠️ **그리고 트랙 A는 정찰의 처방 하나를 *따르지 않고 값으로 반박했다*** — 정찰은 비선택 가지에
  `disabled`를 함께 세우라고 적었는데, 그 칸의 라벨이 이미 *못 고르는 이유*를 문장으로 말하고 있어
  상태로 또 말하면 **이 트랙이 걷어 낸 바로 그 이중 낭독이 다시 선다.** 그 판단이 제품 주석과
  계약의 부정 단언 둘로 소스에 섰다.
- ⚠️⚠️ **정찰 *"좌표꼴 아홉은 낱말 쪽에 가깝다"* ↔ 트랙 E **교집합 0**.** 정찰 후보 5가 그렇게
  적었는데 오늘 실측하면 **두 집합이 한 행도 겹치지 않는다** — 좌표를 적은 행은 예외 없이 그 곁에
  토큰을 더 적거나 조건을 이어 붙였기 때문이다. ⚠️⚠️ **좌표꼴 아홉과 낱말 아홉은 *다른 아홉*이고,
  트랙 E가 그 기각을 단언으로 못 박았다.**
- ⚠️⚠️ **정찰의 배정 *"`apps/api/test/harness-catalog-cost.test.ts`"* ↔ 트랙 D가 세운 파일은
  `packages/test-utils/src/harness-catalog-cost.test.ts`다.** 정찰은 기존 api 계약에 축을 얹으라고
  적었는데, 트랙은 **같은 이름의 새 파일을 다른 뿌리에** 세우고 `apps/api/test/**`를 **읽기만**
  했다(쓰는 함수를 들여오지 않았음을 부정 단언으로 문다). ⚠️⚠️ **그 하나가 이 라운드의 사실을
  바꿨다** — 정찰이 *"`apps/api/**`의 네 라운드 연속 접점 0건을 이 후보가 깬다"* 고 적었는데
  **깨지지 않았고 다섯 라운드가 됐다.** ⚠️ **그리고 그 걸음이 값을 하나 냈다**: 두 뿌리에 같은
  이름의 계약이 서 있고, 그 계약이 *걷어서 만난 짝이 자기 소스와 같지 않은지*를 스스로 문다
  (**이름이 같다고 같은 파일이 아니다**).
- ⚠️ **곁딸린 갈림 하나 — 트랙 B가 이름 겹침을 값으로 세웠다.** 어드민 `source-not-runtime` 둘과
  모바일 `source-not-runtime` 하나가 **같은 이름인데 성질이 반대**라는 사실이 `SAME_NAME_OTHER_KIND`로
  섰고, 그 자리의 `zeroMeans`가 *"셀 수 없어서 0"* 이라는 낱말로 반대 갈래를 문장으로 가리킨다.
  **AI-3이 예고한 오독이 오늘 계약으로 막혔다.**

⚠️ **밖에 둔 것의 크기도 값으로 남긴다** — 트랙 B가 밖에 둔 **어드민의 0 다섯**
(`admin-load-error-copy.test.ts` · 라운드 94 트랙 C의 파일이다), 트랙 C가 밖에 둔 **자가 없는 대장의
사각 일곱**(`comment-tolerant-anchor-ledger.ts`는 `measure`가 0건이라 *갈림*이라는 질문이 서지 않는다),
트랙 D가 밖에 둔 **`itemTemplate` 밖의 표 다섯**과 **시드의 고정 62**, 트랙 E가 밖에 둔 **앞선 절들의
옛 표**가 그것이다.
...
- **이 스캐너가 쿼리로 분류한 자리의 낭독** — 재실측 상태 변화 0, A-20 `#85` 선행 → **U절 머리말**.
  ⚠️ **이번 라운드는 그 여섯 화면에 접점이 0건이다** — 트랙 A가 a11y 그물을 열었지만 든 것은
  **상태 이중 낭독 하나**다(*"열지 않았다"* 와 *"열었지만 그 축이 아니다"* 는 다른 문장이고, 오늘은
  뒤쪽이 **네 라운드 연속**이다).
- **`monthly_wrapup`의 달 이동 구멍** → **U-3.** ⚠️ **`src/notifications/**`가 이번에도 접점 0건이고
  이것으로 **여덟 라운드째**다.** 판정 종전 그대로.
- **S-3(어드민 `disabled`)** — 판정 종전 그대로, 브라우저 확인 `#130` 선행. ⚠️ **`app/items/page.tsx` ·
  `app/links/page.tsx`가 *끊긴 채로* **다섯 라운드**가 됐다**(이번 라운드 어드민 트랙이 연 파일은
  **계약 둘**이고 화면은 0바이트다).
...
⚠️⚠️ **AA-3의 이행 — 결정형 재개 조건 전수와 오늘의 처분이다. 그리고 이번 라운드가 이 표에 더하는
것이 둘이다: 표가 스물셋에서 스물여섯이 됐고, 칸이 하나 늘었다.**
⚠️ **라운드 94 F가 두 표를 하나로 합쳐 **스물셋**을 세웠고, 그 표가 닫힌 뒤에 선 결정형 **셋**을
F 자신이 기각 목록에 적었다(정찰이 그 셋을 둘째 표로 따로 세웠다). **오늘 F가 그 셋을 같은 표로
올려 스물여섯으로 세운다** — *같은 부류의 조건이 두 표에 나뉘어 서면 다음 라운드가 그 둘을 다른
부류로 읽는다*는 라운드 94의 이유가 이번에도 그대로다.
⚠️⚠️ **검산 — 23 + 3 = 26이고, 오늘 집힌 것은 0건이다. 이미 집힌 셋(#16 · #18 · #19)은 지우지 않고
두 시점으로 남는다**(AE-3 · 그러므로 표의 행 수는 **스물여섯**이고 *남은 결정형*은 **스물셋**이다).
⚠️ **두 수를 한 낱말로 적지 않는다 — 행 스물여섯과 남은 조건 스물셋은 다른 수다.**
⚠️⚠️ **그리고 이번 라운드가 이 표에 세우는 칸이 *닫히는 범위*다**(AI-2의 답이 표의 꼴이 됐다).
**검산: 한 파일 7 · 한 앱 8 · 저장소 전체 10 · *결정이 범위를 정한다* 1 = 26**(정찰의 8·8·7에서
새로 오른 셋이 전부 *저장소 전체*로 붙어 7이 10이 됐다). ⚠️ **정찰의 *한 파일 8*은 #22를 *앞쪽을
고르면 한 파일*로 세어 넣은 수다** — 오늘 이 표는 그 하나를 따로 세워 **7 + 1**로 적는다(두 수를 한
낱말로 적지 않는다). ⚠️⚠️ **AI-2가 예고한 상관이 오늘도 반증되지 않았다** — 이번 라운드에 집힌 것은 **0건**인데,
다섯 트랙이 연 축은 다섯 다 **표 밖의 새 축**이었고 넷이 *한 파일*에서 닫혔다.

| # | 결정형 조건 | 사는 곳 | 닫히는 범위 | 손 | 오늘의 처분(2026-09-01 실측 · A~F 머지 뒤) |
| --- | --- | --- | --- | --- | --- |
| 1 | 막대를 포커스 가능하게 | AA-2 | 한 앱 | 안 | **집지 않는다 — 값 0.** 두 화면 다 `trend.showTable` 표가 값을 텍스트로 준다(재실측 · 변화 0 · **어드민 소스 0바이트**) |
| 2 | 기록 탭 검색의 분류 갈래 | Z절 | 한 앱 | 안 | **집지 않는다 — 어긋남 0건 · 열두 라운드 이어 같은 답.** `RECORDS_SEARCH_FIELDS_LABEL`이 여전히 하나이고 placeholder가 그 값에서 파생한다 |
| 3 | 준비템 분류 필수 입력 | Z-3 | 저장소 전체 | **밖**(카탈로그 정책) | 집지 않는다. 변화 0 |
| 4 | 서버의 중복 대기 초대 방지 | AA-4 | 한 앱 | **밖**(초대 정책) | 집지 않는다. 변화 0 |
| 5 | 감사 뷰의 대상 축 필터 | AA절 기각 | 저장소 전체 | **밖**(서버 DTO 0건) | 집지 않는다 — ⚠️⚠️ **`apps/api/**`가 다섯 라운드 연속 접점 0건이다.** ⚠️ **정찰은 이 라운드가 그 0을 깬다고 적었는데 깨지지 않았다** — 트랙 D가 계약을 `apps/api/test/`가 아니라 `packages/test-utils/src/`에 세우고 api 뿌리를 **읽기만** 했다(그 트랙의 부정 단언이 쓰는 함수 0건을 문다) |
| 6 | 타일 안 배지 | AA-1 | 한 앱 | **밖**(디자인 승인) | 집지 않는다. 변화 0 |
| 7 | C-3 잠금 오버레이 TalkBack 투과 | 접근성 표 C-3 | 한 파일(짝 문서) | **밖**(사람·기기·날짜) | **F의 소유 밖이다** — 오늘로 **스물아홉 라운드째 미확인**(67~95). ⚠️ **오늘 이 줄에 붙는 값은 확인이 아니라 *그 수 곁의 바늘이 이제 갈래까지 진다*는 사실이다**(트랙 E의 여섯째 판정으로 이 경과 수는 *낱말 하나* 쪽에 선다) |
| 8 | JSX 사용을 참조로 세는 판정 | `packages/test-utils/src/dead-export-ledger.ts` | 저장소 전체 | 안 | **집지 않는다 — 아홉 라운드 이어서다.** 막는 것은 **크기와 문턱의 겹침** 그대로다(`.tsx`의 `export function` 오늘도 **141** · `derived-exemptions` **44 중 22 · 여유 0**). ⚠️ 그 파일은 이번에도 아무 트랙의 것이 아니다(**다섯 라운드 연속**) |
| 9 | 재조회 실패 창에서 옛 목록을 남길지 | AC절 기각 | 한 앱 | 안 | **집지 않는다 — 화면 구조 결정이고 오늘도 접점 0건**(`src/offline/**` 무접촉) |
| 10 | 앵커 대장의 `helper-named-reader` | `packages/test-utils/src/comment-tolerant-anchor-ledger.ts` | 저장소 전체 | 안 | **집지 않는다 — 래칫이다.** 오늘 **167**(하한 164)이고 앵커 여덟 자리가 **두 라운드 연속** 한 자리도 움직이지 않았다. ⚠️ 넓은 바늘의 **189**는 다른 자다 |
| 11 | 재개 조건 대장의 `prose-only` | `packages/test-utils/src/resume-condition-ledger.ts` | 저장소 전체 | 안(**F**) | **집지 않는다.** 오늘 **261**(119 → 145 → 165 → 184 → 208 → 230 → 246 → **261**)이고 하한 60 위 초록이다. ⚠️⚠️ **오늘 이 줄이 달라진 것은 값이 아니라 *그 값이 어디서 오는가*다** — 트랙 C가 `valueToday`를 상수에서 **자에서 파생**으로 바꿔, 이 절을 쓰는 F의 걸음이 그 수를 **실시간으로** 올린다 |
| 12 | 재개 조건 대장의 `round-notes` | `packages/test-utils/src/resume-condition-ledger.ts` | 저장소 전체 | 안 | **집지 않는다 — 관례 0건.** 오늘 **468**(319 → 377 → 410 → 440 → **468**)이고 노트는 **54벌**이며 늘린 것이 `round95-scout.md` 한 벌이다. ⚠️⚠️ **정찰이 자기 노트를 세지 못했다는 사실이 오늘 값으로 섰다** — 정찰의 440과 오늘의 자가 갈린 몫 **28**이 그 노트 한 벌이다 |
| 13 | 그물 파일이 자기를 이름으로 선언하는 표식 | `packages/test-utils/src/contract-net-ledger.test.ts` | 저장소 전체 | 안 | **집지 않는다 — 관례를 새로 정하는 일이다.** 표식 **0건** · ⚠️ **자기 배제 관례는 오늘 이름 여섯의 선언 자리 17**(`LEDGER_SELF_FILES` 4 · `SELF_PATH` 5 · `SELF_FILE` 4 · `SWEEP_SELF_FILES` 2 · `SELF_CODE` 1 · `COORDINATE_SELF_PATH` 1)**이고 넓은 바늘로는 18 · 이름 일곱**이다 · **정합을 세는 자리 0건** |
| 14 | 공유 테스트 DB를 언제 무엇으로 비울지 | `packages/test-utils/src/harness-catalog-cost.test.ts` | 한 앱 | 안 | ⚠️⚠️ **집지 않는다 — 그러나 오늘 그 술어가 처음 움직였다.** 이 줄이 여섯 라운드 동안 든 근거는 *"올린 손을 세는 자가 0건"* 이었고, 트랙 D가 그 자를 세워 **0 → 1**이 됐다(그 수가 소스에 `DECISION_14_PREDICATE`로 서 있다). ⚠️ **결정 자체는 그대로 열려 있다** — *어느 표를 언제 무엇으로 비울지*는 오늘도 아무도 정하지 않았다. 일곱째 시점의 환경 값은 정찰의 것이다(`wooriai_test` **9,545** · 활성 **8,888** · **753MB**) |
| 15 | 조건의 *도래*를 값으로 가르는 관례 | `packages/test-utils/src/resume-condition-ledger.ts` | 한 파일 | 안 | **집지 않는다 — 관례를 정하는 일이다.** 기다리는 자리는 오늘도 **둘** |
| 16 | **어드민 계약의 *다른* 손 목록 둘을 누가 전수 파생으로 옮기는가** | `apps/admin/src/admin-load-error-copy.test.ts:537` | 한 파일 | 안 | **집혔다(라운드 94 트랙 C) — 두 시점으로 내려가 있다.** 오늘 재확인: 그 절에 손 목록 **0건** · 걷기 **16**(12 · 4 · 0) · 폴백 어미 화면 **12**(출현 13) · **두 라운드째 조용하다** |
| 17 | **어드민에 컴포넌트 렌더 하네스를 세울지** | `apps/admin/src/admin-load-error-copy.test.ts` | 한 앱 | 안 | **집지 않는다 — 크기다.** 사각 `source-not-render`가 오늘도 **0**을 들고 어드민에 렌더 하네스 표식 **0건**이다. ⚠️ 트랙 B가 같은 앱의 **다른 두 파일**을 열었지만 축은 *0의 뜻*이지 하네스가 아니다 |
| 18 | **사각의 자를 `measure: () => number` 꼴로 바꿀지** | `apps/mobile/src/screen-header-back.test.ts:234` | 한 파일 | 안 | **집혔다(라운드 94 트랙 B) — 두 시점으로 내려가 있다.** 오늘 재확인: 그 파일의 사각 **여섯** · 파생 자 **넷** · 자 없음 **둘**. ⚠️⚠️ **그리고 오늘 그 꼴이 *다른 앱으로 건너갔다*** — 트랙 B가 어드민 두 파일에 `CountedZero`를 세우며 이 파일의 `uncountable`을 **인용**했다(복사가 아니다) |
| 19 | **`⚠️`와 `재개` 사이의 강조 표식** | `packages/test-utils/src/resume-condition-ledger.ts:759` | 한 파일 | 안 | **집혔다(라운드 94 트랙 D) — 두 시점으로 내려가 있다.** 오늘 재확인: 표식형 **11** · 강조 낀 표식형 **7** · 필드형 **42** · 잔여 **17**(합 **77**) · `unmarked-source-prose` **73**. ⚠️ **`markedSourceNeedle()`의 바이트는 오늘도 불변이다** |
| 20 | **열 자리의 *열거*를 자가 내게 할지** | AH-3 | 한 앱 | 안 | **집지 않는다 — 축이다.** 그 열거가 사는 세 벌 가운데 둘이 `apps/mobile/src/a11y-contract.test.ts` 안이고, 이번 라운드에 트랙 A가 그 파일에서 연 축은 **하나**(상태 이중 낭독)다 — *한 트랙이 한 그물에 축 둘을 얹지 않는다* |
| 21 | **문턱·래칫 상수 38의 갈래 셋에 모집단을** | AH-5 | 한 파일 | 안 | **집지 않는다 — 재현 불가로 남은 유일한 칸 그대로다.** ⚠️ 정찰의 넓은 바늘로는 **76**(이름에 `FLOOR`/`CEILING`/`THRESHOLD`/`RATCHET`)이고 줄머리 상수만 보면 **84**다 — F의 **38**·**53**과 **네 수를 한 낱말로 적지 않는다.** 트랙 E의 `RETEST_UNREPRODUCIBLE_CELL`이 그 행이 사라지는 것만 막는다 |
| 22 | **트립와이어의 수마다 바늘을 어디에 적을지** | AH-5 기각 목록 | ⚠️ **결정이 범위를 정한다** | 안(**F**) | **절반이 집혔다(라운드 94 E + F) — 나머지 절반은 열려 있다.** ⚠️⚠️ **오늘 그 비용이 한 겹 더 보인다**: 앞쪽(표에 적기)을 고른 덕에 *수만 있는 행*은 **0**인데, **붐비는 칸이 2 → 18**이고 그 열여덟 행은 바늘이 하나라 *어느 수에 어느 바늘인지*가 없다. ⚠️ **오늘 F는 그 열여덟에 바늘을 *여럿* 적기 시작하지 않았다** — 고르지 않은 길도 값이라, 아래 트립와이어 머리말이 그 이유를 적는다 |
| 23 | **`COORDINATE_LIVE_DOCS` 손 목록을 전수로** | `packages/test-utils/src/repo-self-description.test.ts` | 한 파일 | 안 | **집지 않는다 — 소유가 갈리는 결정이다.** 오늘도 밖에 `docs/5차/design-restore-spec.md`가 있고, 그 사실을 그 계약의 주석이 스스로 값으로 적는다. ⚠️ 트랙 E가 같은 파일에 판정을 하나 더 얹었지만 **좌표 축의 손 목록은 한 바이트도 건드리지 않았다** |
| 24 | **손 목록 옆 주석의 *단언 꼴*을 관례로 정할지** | AI절 기각 목록 | 저장소 전체 | 안 | ⚠️⚠️ **오늘 처음 이 표에 선다**(세운 손 = 라운드 94 F). **집지 않는다 — 관례이고 바늘이 사람의 문장 읽기다.** 첫 모집단은 열일곱과 그중 자가 없는 열하나 그대로다. ⚠️⚠️ **그런데 그 병의 한 겹 안쪽이 이번 라운드에 *화면에* 나타났고 트랙 A가 그것을 닫았다** — 규율은 부류 전체를 두고 말하는데 자의 모집단이 한 자리였다(AJ-3) |
| 25 | **사각의 자 꼴을 저장소 관례로 정할지** | AI절 기각 목록 | 저장소 전체 | 안 | ⚠️⚠️ **오늘 처음 이 표에 선다**(세운 손 = 라운드 94 F). **집지 않는다 — 크기와 소유다.** ⚠️⚠️ **다만 오늘 그 조건 옆에 답이 하나 붙었다** — 자를 두는 것보다 먼저 *0의 뜻*을 적어야 한다는 AI-3의 답을 트랙 B가 어드민 두 파일에서 이행했고, `zeroMeans`가 **한 파일에서 세 파일**이 됐다 |
| 26 | **표의 수를 *다시 대어 보는* 자가 설지** | AI절 기각 목록 | 저장소 전체 | 안 | ⚠️⚠️ **오늘 처음 이 표에 선다**(세운 손 = 라운드 94 F). **집지 않는다 — 크기다.** ⚠️⚠️ **다만 오늘 그 앞쪽이 한 겹 깊어졌다** — 트랙 E가 바늘의 *꼴*을 낱말 **9**와 문장 **30**으로 갈랐고(⚠️ 리뷰 M-2의 시점 표기: 그 9/30은 **트랙 E 커밋 시점 · AI절 재실측 표 39행**의 수다 — E의 자는 *마지막 판정 절의 표*를 따라가므로 F가 이 절의 표를 세운 뒤로는 같은 자가 **낱말 23 · 문장 20**을 낸다 · 두 표의 수를 한 낱말로 적지 않는다), 그 갈래는 **재현이 아니라 재현 가능성의 대리 지표**다. 뒤쪽(실제로 다시 대어 보는 것)은 오늘도 **0건**이고, 정찰이 손으로 대어 본 두 자리는 **둘 다 갈렸다** |

⚠️⚠️ **이 표가 라운드 94의 표와 다른 점 넷.** ⓐ **행이 스물셋에서 스물여섯이 됐다** — 기각 목록에
있던 셋이 표로 올라왔고, 올린 손은 **F**다. ⓑ ⚠️⚠️ **칸이 하나 늘었다**(*닫히는 범위*) — AI-2의
질문이 표의 꼴이 된 자리이고, **7 · 8 · 10 + 1**로 검산된다(마지막 하나는 *결정이 범위를 정한다* · #22). ⓒ ⚠️⚠️ **집힌 것이 0건이다** — 셋이
한 번에 내려간 라운드 바로 뒤에 0이 왔고, **그것이 AI-2의 일반형과 어긋나지 않는다**: 이번 라운드의
다섯 트랙은 표 안의 조건을 집은 것이 아니라 **표 밖의 새 축**을 열었다(넷이 한 파일에서 닫혔다).
ⓓ ⚠️⚠️ **#14가 처음으로 *술어* 쪽에서 움직였다** — 결정은 그대로인데 그 결정이 기다리던
*"올린 손을 세는 자"* 가 **0에서 1**이 됐다. **결정형 조건은 집히지 않아도 술어가 움직일 수 있고,
그 움직임을 적지 않으면 다음 라운드가 그 줄을 여섯 라운드째 같은 줄로 읽는다.**

⚠️⚠️ **이번 라운드가 실측하고 기각한 서른하나를 값으로 남긴다 — 전부 재개 조건과 함께**(V-2).
⚠️⚠️ **검산: 라운드 94의 스물여덟 − 오늘 통합표로 올라간 셋 + 오늘 새로 선 여섯 = 서른하나**
(= 스물여덟 + 셋이기도 하다 — **두 셈법이 같은 수에 닿는다**).
⚠️ **정정(라운드 95 리뷰 L-2 — 위 문장은 보존한다):** 위 "두 셈법"은 검산이 아니다 — 둘째 식은
첫째 식의 이항(−3 + 6 = +3)이라 **같은 입력을 두 번 쓴 같은 식**이고, 서로를 검사하지 못한다.
독립 경로는 아래 목록의 항목을 **직접 세는 것**뿐인데 오늘 그 자는 0건이다(목록은 같은 부류를 한
줄로 묶어 줄 수 ≠ 항목 수 — 바로 아래 문장이 그 이유를 진다). *검산*이라는 낱말을 거둔다.
⚠️ **아래 목록은 같은 부류를 한 줄로 묶어 적으므로 줄 수와 항목 수가 다르다 — 두 수를 한 낱말로
적지 않는다.**

- ⚠️⚠️ **`useQuery`를 쓰는 화면 열여덟 가운데 `resolveScreenPhase`를 쓰는 것은 다섯이다 — 오늘 처음
  세어 값으로 남기고 집지 않는다.** ⚠️ **남는 열셋이 틀렸다는 뜻이 아니다** — `records.tsx`는 자기
  사다리를 네 갈래(로딩 · 오류 · 필터 0건 · 빈 달)로 갖고 있고 실피해 **0건**이다(TanStack v5에서
  `isError`와 `isLoading`은 동시에 참이 될 수 없다). ⚠️ **집지 않는 이유는 규율이 아직 관례가 아니기
  때문이다** — 다섯 화면의 주석은 *"에러 → 로딩 → 정상 순서는 `resolveScreenPhase`가 정한다"* 인데
  그 문장이 부류 전체를 두고 말하지는 않는다. ⚠️ **재개 조건(결정형 · 손은 저장소 안): 화면의 상태
  사다리를 한 곳에서 정하는 것이 관례가 되는 날 — 그날 먼저 물을 것은 *열셋을 옮길 것인가 다섯의
  주석을 좁힐 것인가*이고, 첫 모집단은 오늘의 열여덟과 그중 다섯이다.**
- ⚠️⚠️ **`apps/mobile/src/a11y-contract.test.ts`에 NUL 바이트가 한 자리 있다 — 오늘 처음 값으로
  남기고 집지 않는다.** 구분자를 이스케이프가 아니라 **실제 바이트**로 적은 자리이고, ripgrep과
  `readFileSync`는 읽지만 **GNU grep은 이 파일을 이진으로 보아 내용을 감춘다.** ⚠️ **실피해는 오늘
  0건이다**(계약은 전부 `readFileSync`로 읽는다). ⚠️ **재개 조건(사건형): 셸 `grep`에 기대는
  스윕·게이트가 서는 날 — 그날 저장소에서 가장 큰 낭독 계약이 통째로 빠진다.**
- ⚠️⚠️ **라벨로만 상태를 말하는 자리 둘 — 트랙 A가 값으로 남기고 집지 않는다.**
  `src/ui/Skeleton.tsx`의 실루엣 둘이 `accessibilityLabel="불러오는 중"` 을 지니는데 상태 프롭이
  없다. **두 번 읽히지 않으므로 이 축의 병이 아니고**(정찰의 0이 아니라 2라는 사실만 값이다),
  라벨을 걷으면 실루엣이 이름을 잃으므로 걷을 수도 없다. ⚠️ **재개 조건(결정형 · 손은 저장소 안):
  그 둘에 `accessibilityState`를 세워 라벨을 걷을지 정하는 날 — 그 트랙은 그 파일과 그 바이트를
  무는 계약을 함께 소유해야 하고, 이번 라운드는 그 파일을 **0바이트** 고쳤다.**
- ⚠️⚠️ **정찰 노트가 자기 바늘의 갈래를 값으로 적는 관례 — 트랙 E가 사각으로 남기고 집지 않는다.**
  같은 갈래(낱말 ↔ 문장)가 노트에도 사는데(오늘 아홉 가운데 둘/일곱) 그것을 세는 자는 **0건**이다.
  ⚠️ **재개 조건(결정형 · 손은 저장소 안): 정찰 노트가 자기 바늘의 갈래를 값으로 적는 관례가 서는 날.**
- ⚠️⚠️ **기록 축을 다른 대장으로 넓힐지 — 트랙 C가 사각으로 남기고 집지 않는다.**
  `comment-tolerant-anchor-ledger.ts`의 사각 **일곱**에는 `measure`가 **0건**이라 *적힌 값과 오늘의
  자가 갈리는가*라는 질문 자체가 서지 않는다. ⚠️ **집지 않는 이유는 범위다** — 저장소의 다른 대장으로
  넓히는 것은 *저장소 전체*이고(AI-2), 이 라운드의 배정 규율은 *한 파일 또는 한 앱*이다.
  ⚠️ **재개 조건(결정형 · 손은 저장소 안): 사각의 기록 축을 저장소 관례로 정하는 날 — 그날 첫
  모집단은 자가 없는 그 일곱이다.**
- ⚠️⚠️ **어드민의 남은 0 다섯에 *0의 뜻*을 붙일지 — 트랙 B가 값으로 남기고 집지 않는다.**
  `apps/admin/src/admin-load-error-copy.test.ts`의 0 **다섯**은 오늘도 갈래가 없고, 그 파일은
  **라운드 94 트랙 C의 것**이라 이번 라운드가 열지 않았다(0바이트).
  ⚠️ **재개 조건(결정형 · 손은 저장소 안): 그 파일을 여는 트랙이 서는 날** — 그날 첫 모집단은
  오늘의 다섯이고, 본보기는 같은 앱의 두 파일에 이미 서 있다.
- ⚠️⚠️ **AH-1의 축(손 목록 옆의 단언 열일곱) · 사각의 자 꼴을 저장소 전체로 · 표의 수를 다시 대어
  보는 자 — 셋 다 오늘 이 목록을 떠나 위 통합표의 #24·#25·#26이 됐다.** ⚠️ **떠난 것이지 닫힌 것이
  아니다** — 셋 다 오늘도 집히지 않았고, 자리를 옮긴 이유는 *같은 부류가 두 표에 나뉘어 서지 않게*다.
- **AG-2의 축(막는 것이 *배정*인 조건 여덟) · AG-3의 축(재개 조건을 세운 손) — 재었고 판정 종전
  그대로다.** ⚠️ **다만 AG-3의 축에 오늘 값 하나가 붙는다**: 이번 라운드가 세운 결정형은 **여섯**이고
  **여섯 다 손이 트랙**이다(리뷰가 세운 것은 오늘 0건이다 — 리뷰가 아직 오지 않았다).
- **문턱이 재는 세계(제품 ↔ 하네스 ↔ 소스 텍스트) — 재었고 집지 않는다.** ⚠️ 오늘의 수는 위 #21이
  값으로 든다(38 ↔ 53 ↔ 76 ↔ 84). ⚠️ **재개 조건(사건형 — 종전 그대로).**
- **자기 배제 관례 — ⚠️ 재었고 이름 여섯의 선언 자리 **17** · 넓은 바늘 **18** · 이름 **일곱**이다.**
  ⚠️ **이번 라운드가 늘린 자리는 0건이다**(트랙 D의 새 계약은 자기 배제를 `SIBLING_CONTRACT` 라는
  다른 이름으로 지어 이 바늘 밖에 있다 — **그 사실 자체가 이 조건이 기다리는 정합의 실물이다**).
  ⚠️ **재개 조건(결정형 · 손은 저장소 안 — 종전 그대로).**
- **공유 테스트 DB를 비우는 일 — 재었고 집지 않으며, 오늘의 값은 정찰의 것이다.**
  `wooriai_test` **9,545**(활성 **8,888** · **753MB**) ↔ `wooriai_dev` **62**(활성 62 · 24MB).
  ⚠️⚠️ **일곱째 시점이고 오름이 빨라졌다**(다섯째→여섯째 **+37** · 여섯째→일곱째 **+69**).
  ⚠️ **재개 조건(결정형 · 손은 저장소 안 — 종전 그대로).**
- **`derived-exemptions`의 절반 문턱 — 재었고 여유가 0 그대로다.** 사문 전수 **44 중 면제 22**이고
  조건은 *"넘는 날"* 이라 아직 아니다(⚠️ **다섯 트랙 어디에도 새 `export const`가 서지 않았다**).
  ⚠️ **재개 조건(사건형): 면제가 사문 전수의 절반을 넘는 날.**
- **`dead-export-ledger.ts`의 결정형 8 — 전제는 참이고 아홉 라운드째 집지 않는다.** `.tsx`의
  `export function` 오늘도 **141**이다. ⚠️ **그 파일이 아무 트랙의 것도 아닌 다섯 번째 라운드다.**
  ⚠️ **재개 조건(결정형 · 손은 저장소 안 — 종전 그대로).**
- **앵커 대장 — 재었고 두 라운드 연속 조용하다.** 앵커 전수 **699**(code-only **621** ·
  comment-tolerant **70**/래칫 70 · comment-only **8**/래칫 8 · unanchored **0**) · 면제 **8** ·
  사각 `helper-named-reader` **167**. ⚠️ **재개 조건(결정형 · 손은 저장소 안 — 종전 그대로).**
- **재개 조건 대장의 `prose-only` · `round-notes` — 재었고 둘 다 집지 않는다.** ⚠️⚠️ **오늘 달라진
  것은 값이 아니라 그 값이 오는 길이다** — 트랙 C 뒤로 둘 다 **파생**이라 이 절을 쓰는 걸음이 그 수를
  실시간으로 올린다. ⚠️ **재개 조건 둘 다 결정형 · 손은 저장소 안이고 종전 그대로다.**
- **낭독 라벨 스윕의 남는 일곱 · 반쪽 프롭 일곱 · `accessibilityState={{ disabled }}` 열일곱 자리 ·
  `useMutation` 밖의 쓰기 35 · AC-1의 일곱 · AC-2의 0 · 어드민의 무명 표 하나 · `<section>` 34 ·
  어드민 연타 가드 절반** — 재었고 판정 종전 그대로이며 재개 조건 전부 사건형이다. ⚠️ **`disabled`
  자리의 수를 오늘 다시 재어 **17**로 적는다**(정찰의 18에는 주석 한 자리가 들어 있었다 — **두 수를
  한 낱말로 적지 않는다**).
- **`a11y-contract.test.ts`의 사각 셋이 문자열 배열이라 AH-3의 바늘 밖이다** — 라운드 93이 처음
  적은 그대로다. ⚠️ **재개 조건(결정형 · 손은 저장소 안 — 종전 그대로).**
- **AI절 본문과 갈리는 수를 값으로 남긴다**(재론이 아니라 *어느 시점의 수인지*를 가르기 위해서다):
  재개 조건 대장의 사각 여덟 가운데 일곱이 움직였고(`prose-only` 246 → **261** ·
  `round-notes` 440 → **468** · `unmarked-source-prose` 61 → **73**), 경과 축의 창
  102 → **116**, 소스 축 모집단 파일 11 → **12**, 걷는 파일 970 → **972**, 노트 53 → **54벌**,
  `accessibilityState` 자리 57(정찰) → **58**. ⚠️ **여덟 다 이번 라운드의 트랙 또는 이 절이
  움직였고, AI절 본문에는 옛 수가 남아 있다.**

---

⚠️⚠️ **트립와이어 대조표 — 그리고 이 표가 라운드 94의 표와 다른 점 하나는 *마지막 칸의 꼴*이다.**
⚠️ **시점은 종전과 같다: A~F 머지 뒤 · 리뷰 전(2026-09-01).** ⚠️⚠️ **다른 것은 바늘의 *모양*이다** —
라운드 94 F가 마흔 행 가운데 서른아홉에 바늘을 붙였고, 라운드 95 트랙 E가 그 서른아홉을
**낱말 하나/정규식 9 ↔ 문장 30**으로 갈랐다. AI-1의 일반형(*낱말이면 재현되고 문장이면 재현되지
않는다*)이 참이라면 **바늘을 붙이는 것만으로는 부족하고 그 바늘이 낱말이어야 한다**는 뜻이라,
**오늘부터 이 표는 *낱말 하나로 적을 수 있는 행은 낱말 하나로* 적는다.**
⚠️ **전부는 못 한다 — 그러니 *몇이 낱말이 됐는지*를 값으로 적고, 그 수를 다시 내는 것은 트랙 E의
자다**(오늘 이 표의 값은 아래 마지막 행이 든다).
⚠️⚠️ **고르지 않은 길도 값이다 — 붐비는 칸 열여덟에 바늘을 *여럿* 적기 시작하지 않았다**(결정형
22의 나머지 절반). 그 길을 고르면 한 행의 마지막 칸이 표를 넘치고, 그러면 *어느 수에 어느 바늘인지*는
풀리지만 **표가 읽히지 않는다.** 오늘 F가 고른 것은 **바늘의 꼴을 바꾸는 쪽**이다 — 붐비는 칸은
그대로 두고 **낱말 바늘로 적을 수 있는 행부터 낱말로** 바꿨다. ⚠️⚠️ **그 선택의 비용을 값으로
적는다**: 붐비는 칸은 줄기는커녕 **18에서 20이 됐고**(이번 라운드가 새로 세운 축 셋이 한 칸에 수를
여럿 실었다), 그 행의 바늘 하나는 오늘도 여덟 수를 다 가리키지 못한다. **결정형 22의 나머지 절반은
그대로 열려 있다.**
⚠️⚠️ **하한을 고른 비용도 함께 적는다 — *행이 사라지거나 바늘이 지워지는 것*을 그 자는 보지
못한다**(AG-5·AH-5·AI-5의 일반형 · 오르는 쪽 래칫이다).

| 자리 | 라운드 94가 남긴 값 | 2026-09-01 HEAD 재실측(**A~F 머지 뒤** · 리뷰 전) | 움직였는가 · **그 수를 낸 바늘** |
| --- | --- | --- | --- |
| 사문 대장 항목 / 래칫 | 22 / 22 | **22 / 22** | 그대로다 — 어느 트랙도 그 대장을 열지 않았다(**다섯 라운드 연속**). **바늘**: `DEAD_EXPORT_RATCHET` |
| 사문 대장의 사각 **여덟** | 22 · 0 · 226 · 20 · 55 · 105 · 141 · 0 | **22 · 0 · 226 · 20 · 55 · 105 · 141 · 0** | **그대로다 — 여덟 자리가 한 자리도 다르지 않다**(다섯 라운드 이어서다). **바늘**: `dead-export-ledger.ts`의 사각마다 선 `measure` |
| 사문 전수 / 면제 / 모듈 | 44 / 22 / 6 | **44 / 22 / 6** | 그대로다 — ⚠️ **정확히 절반이고 여유 0**이다. 다섯 트랙 어디에도 새 `export const`가 서지 않았다. **바늘**: `contractOnlyExemptions` |
| `CLOSED_BLIND_SPOTS` | 셋 | **셋** | 그대로다. **바늘**: `CLOSED_BLIND_SPOTS` |
| ⚠️⚠️ 앵커 대장(앵커 전수 · code-only · comment-tolerant/래칫 · comment-only/래칫 · unanchored) | 699 · 621 · 70/70 · 8/8 · 0 | **699 · 621 · 70/70 · 8/8 · 0** | 그대로다 — **두 라운드 연속 조용하다**. **바늘**: `collectCommentToleranceAnchors` |
| 앵커 대장의 면제 | 8 | **8** | 그대로다. **바늘**: `QUOTATION_EXEMPTIONS` |
| ⚠️⚠️ 앵커 대장의 사각 `helper-named-reader` | 167(하한 164) | **167**(하한 **164**는 오늘도 참이다) | 그대로다. ⚠️ 넓은 바늘의 189는 다른 자다 — 두 수를 한 낱말로 적지 않는다. **바늘**: `NAMED_SOURCE_READER`로 걷고 `reason === "helper-named-reader"`인 자리를 센다 |
| 재개 조건 대장(자리 · 괄호 표기 · 손의 위치 · 결정형 · 손 누락) | 467 · 156 · 52 · 53 · 1 | **500 · 170 · 62 · 63 · 1** | ⚠️⚠️ **움직였고, 이 절 자신이 전부를 늘렸다** — 다섯 트랙은 이 문서를 **0바이트** 고쳤다. 손 누락은 **1** 그대로다. **바늘**: `collectDocumentSites` |
| 재개 조건 대장의 사각 **여덟** | 246 · 8 · 440 · 18 · 55 · 28 · 11 · 61 | **261 · 8 · 468 · 18 · 58 · 28 · 11 · 73** | ⚠️⚠️ **움직였고, 오늘부터 이 여덟은 상수가 아니라 파생이다**(트랙 C). **바늘**: `resume-condition-ledger.ts`의 사각마다 선 `measure(baseDir)` |
| 재개 조건 대장의 **경과 축**(그 줄 · 창 · 어디에도 없음) | 7 · 102 · 365 | **7 · 116 · 384**(래칫 **3 · 61**) | ⚠️ **움직였다 — 그 줄 자신이 경과를 적은 자리는 일곱 그대로다.** **바늘**: `documentElapsedSites` |
| 표기를 지닌 소스 파일 / 그 뿌리(걷는 파일) | 모집단 11 · 표식형 3 · 걷는 파일 970 | **모집단 12 · 표식형 3 · 걷는 파일 972** | ⚠️ **움직였다 — 트랙 D의 새 계약 파일이 모집단에 들어왔다**(표식형을 지닌 파일 **3**은 그대로라 옛 등호가 그대로 문다). **바늘**: `sourceAxisFilesFrom` |
| 계약 그물 수 | 열다섯 | **열다섯** | ⚠️⚠️ **그대로다 — 그물 수가 늘지 않은 여섯 번째 라운드다.** 트랙 D가 파일을 새로 세웠는데도 그물이 늘지 않았고 **왜 들지 않는지를 그 파일의 머리말이 값으로 적는다**. **바늘**: `CONTRACT_NET_COUNT_WITH_THIS_ONE` |
| 반쪽 프롭(짝 · role 단독 · live region 단독) | 일곱(37 · 1 · 6) | **일곱(37 · 1 · 6)** | 그대로다 — ⚠️ **트랙 A가 상태 프롭 둘을 새로 세웠는데도 그대로다**(그 둘은 짝이 필요한 프롭이 아니다). **바늘**: `apps/mobile/src/a11y-contract.test.ts`의 짝 프롭 파생 |
| 어드민의 상태 문장 | 자리 47 / 파일 14 / 출구 45(`role="alert"` 34 · `role="status"` 11) / 면제 2 | **47 / 14 / 45(34 · 11) / 2** | 그대로다 — **어드민 소스 0바이트**다. **바늘**: `admin-status-announce.test.ts`가 어드민 `app`을 걷는다 |
| 어드민의 표 이름 / 무명 표 | 17 / 0(`app/**`) · 스윕 밖 하나(표 전수 18) | **17 / 0** · 스윕 밖 **하나**(표 전수 **18**) | 그대로다. **바늘**: `admin-table-name.test.ts`의 전수 걷기 |
| 어드민 `<section>` / 이름 | 34 / 0 | **34 / 0** | 그대로다 — ⚠️ **트랙 B가 이 파일을 열었는데도 랜드마크 축은 바이트 불변이다**(연 축은 *0의 뜻*이다). **바늘**: `admin-landmark-current.test.ts`의 전수 걷기 |
| 어드민 `<main>`(중첩) / `aria-current` | 2(중첩 0) / 1 | **2(중첩 0) / 1** | 그대로다. **바늘**: 같은 파일의 랜드마크 스윕 |
| 어드민 라우트 표면(`page.tsx` ↔ `NAV_ITEMS`) | 11 ↔ 11 · 어긋남 0 · 세는 계약 둘 | **11 ↔ 11 · 어긋남 0 · 세는 계약 둘** | 그대로다 — 같은 전수를 파생하는 자가 둘이고 사본이 아니라 각자 파생한다. **바늘**: `admin-route-surface.test.ts`가 `app/**`을 걸어 `page.tsx`를 센다 |
| ⚠️⚠️ 어드민 [다시 시도] 관례(모집단 · 판정) | 11 · 10 · 1 · 0 · 0 | **11 · 10 · 1 · 0 · 0** | 그대로다 — 라운드 93 트랙 D가 세운 전수가 오늘도 같은 수를 낸다. **바늘**: `admin-load-error-copy.test.ts` |
| ⚠️⚠️ 어드민의 남은 손 목록 둘(폐기 자리 · 폴백 어미) | 걷기 16(12 · 4 · 0) · 화면 12(출현 13) | **걷기 16**(12 · 4 · 0) · **화면 12**(출현 **13**) | 그대로다 — **두 라운드째 조용하다**(결정형 16이 집힌 뒤 첫 침묵). **바늘**: 같은 파일의 한 벌 호출 자리 걷기 |
| 어드민 `<button` / `type=` / `disabled=`(줄) | 68 / 68 / 44 | **68 / 68 / 44** | 그대로다. **바늘**: 어드민 `app/**`을 걷는 연타 가드 스윕이 `<button`을 센다 |
| ⚠️⚠️ 어드민 사각의 0과 *0의 뜻*(0인 자리 · 뜻이 적힌 자리 · 모집단 자 · 갈래 없음) | **세는 자리 0건** | **7 · 7 · 7 · 0**(상한) | ⚠️⚠️ **오늘 처음 세어졌다 — `zeroMeans`가 한 파일에서 세 파일이 됐다**(모바일 6 · 어드민 16 + 14). **바늘**: `apps/admin/src/admin-route-surface.test.ts:452`의 `UNEXPLAINED_ZERO_CEILING` |
| 모바일 `useMutation`(선언 · 컨트롤 · 핸들러 · 안 막음) | 31 · 27 · 1 · 3 | **31 · 27 · 1 · 3** | 그대로다 — 그 스윕을 여는 트랙이 0건이다. **바늘**: `mutation-press-guard.test.ts` |
| 모바일 Pressable / 역할 / 라벨 | 145 / 145 / 122 | **145 / 145 / 122** | 그대로다 — ⚠️ **트랙 A가 라벨 셋을 고쳤는데도 수는 그대로다**(걷은 것은 낱말이지 태그가 아니다). **바늘**: `a11y-contract.test.ts`의 여는 태그 스윕 |
| 모바일 스크롤 컨테이너(전수 · 판정 명시 · 키보드가 뜰 수 없는 화면 · 미가드) | 17 · 11 · 6 · 0 | **17 · 11 · 6 · 0** | 그대로다 — ⚠️ **트랙 A가 그 파일의 리터럴 수를 54에서 53으로 갱신했는데도 스크롤러는 그대로다**. **바늘**: `keyboard-tap-guard.test.ts` |
| ⚠️ 모바일 낭독 라벨(식 전수 · 목록 안 · 직접 · 파생 · 갈리지 않음 · 상수) | 하한 162 · 48 · 44 · 4 · 0 · 73 | **그 하한 여섯이 오늘도 초록이다** | 그대로다(하한). ⚠️⚠️ **이 칸은 *오른 것*도 *내린 것*도 보지 못한다** — 트랙 A가 라벨에서 낱말을 걷었는데 이 자는 바닥만 센다. **바늘**: 같은 파일의 래칫 표 |
| ⚠️⚠️ 모바일 나가는 길(라우트 · 탭 밖 · `ScreenHeader` · 배선 · 판단 · 출구 없음) | 36 · 31 · 18 · 12 · 6 · 0(재수출 5 · 미사용 8) | **36 · 31 · 18 · 12 · 6 · 0**(재수출 **5** · 미사용 **8**) | 그대로다 — 사각 여섯의 자도 꼴 그대로다(파생 넷 · 자 없음 둘). **바늘**: `screen-header-back.test.ts`의 전수 걷기 |
| ⚠️ 모바일 조사(모집단 · 꼬리 고정 · 값에서 고름 · 갈리는데 고정) | 39 · 30 · 9 · 0(걷는 파일 282) | **39 · 30 · 9 · 0**(걷는 파일 **282**) | 그대로다 — 라운드 94가 세운 값이 한 자리도 움직이지 않았다. **바늘**: `korean-particle-guard.test.ts` |
| ⚠️ 모바일 접미사(모집단 · 값에서 고름 · 갈리는데 고정) · 핀 이관 | 3 · 3 · 0 · 계약 5 · 핀 9 | **3 · 3 · 0** · 계약 **5** · 핀 **9** | 그대로다. **바늘**: 같은 파일의 접미사 모집단 파생 |
| ⚠️⚠️ 모바일 상태 이중 낭독(자리 · state-only · also-in-label · no-label) | **세는 자리 0건** | **58 · 49 · 0**(상한) **· 9** | ⚠️⚠️ **오늘 처음 세어졌다 — 규율의 모집단이 한 자리에서 전수가 됐다**(걷는 파일 282 · 갈래 다섯 19·17·10·9·3). **바늘**: `apps/mobile/src/a11y-contract.test.ts:7859`의 `STATE_ECHO_RATCHET` |
| 문턱·래칫 상수 | 38(하네스 4 · 제품 8 · 소스 텍스트 26) | **다시 재지 못했다**(넓은 바늘로는 **53** · 정찰의 넓은 바늘로는 **76** · 줄머리 상수만 보면 **84**) | ⚠️⚠️ **이 칸은 오늘도 비어 있다 — 이 표에서 *재현 불가*로 남은 유일한 자리다.** 갈래 셋의 모집단이 어디에도 적혀 있지 않다(결정형 21). **바늘**: 없다 |
| ⚠️ 자기 배제 관례(자리 / 이름) | 좁은 바늘 열다섯 / 넷 · 넓은 바늘 열일곱 / 여섯 | **정찰의 바늘(이름 여섯) 열일곱 · 넓은 바늘(이름에 `SELF`) 열여덟 / 일곱** | ⚠️ **이번 라운드가 늘린 자리는 0건이고, 갈린 것은 저장소가 아니라 바늘이다.** ⚠️⚠️ **그런데 트랙 D의 새 계약은 자기 배제를 다른 이름으로 지어 이 바늘 밖에 있다** — *정합을 세는 자리 0건*이라는 이 조건의 술어가 그 자리에서 실물이 됐다. **바늘**: 이름에 `SELF`가 든 상수 선언 전수 |
| 접근성 표 → 짝 문서 좌표(자리 · 인용 번호 · 서로 다른 번호 · 표면 · 역방향 · 어긋남) | 97 · 101 · 97 · 38 · 8 · 0 | **98 · 102 · 98 · 39 · 8 · 0** | ⚠️ **움직였다** — F가 A-36(`#112`)을 세우며 앞의 넷이 하나씩 늘었다(역방향과 어긋남은 그대로). **바늘**: `accessibility-checklist-shape.test.ts`의 표면 바늘 |
| 짝 계약의 이동 의무(좌표 자리 · 절 이름 · 줄 번호 배열 · 의무가 빈 자리) | 9 · 8 · 1 · 0 | **9 · 8 · 1 · 0** | 그대로다 — ⚠️⚠️ **그리고 오늘 그 의무가 네 번째로 이행됐다**(값도 꼴도 바이트 불변 · 움직인 것은 좌표뿐이다). **바늘**: 그 짝 계약이 등호로 무는 줄 번호 배열 하나 |
| ⚠️ 문서→소스 좌표(살아 있는 문서 넷 · 실재 · 범위 안 · 이동 의무) | 54 · 54 · 54 · 6 | **65 · 65 · 65 · 6**(미해석 **0** · 문서→문서 **4** · 이름만 **29**) | ⚠️⚠️ **움직였고 늘린 손이 이 절이다** — 오늘 F가 낱말 바늘 옆에 좌표를 여섯 자리 더 적었다. ⚠️ **해석되지 않은 경로 꼴 좌표는 0건이다**(적기 전에 다 풀어 보았다). **바늘**: `packages/test-utils/src/repo-self-description.test.ts:563`의 좌표 정규식으로 걷고 줄이 파일 길이 안인지를 `isLineWithinFile`이 판정한다 |
| 짝 문서(행 수 · 실기기 · 브라우저 · 서버 · 작업) | 171 · 141 · 20 · 9 · 1 | **172 · 142 · 20 · 9 · 1** | ⚠️ **움직였다** — `#172`(트랙 A) **하나**이고 **브라우저는 0건**이다(네 라운드 이어 브라우저가 0건이다). **바늘**: `runtime-checklist-shape.test.ts`가 그 문서의 표를 파싱해 §0의 네 수를 파생시킨다 |
| 접근성 표 | A-35까지(`#111`) · Batch 44 | **A-36까지(`#112`) · Batch 45** | ⚠️ **움직였다** — 행 **하나**(`#112` 트랙 A)와 문단 **여섯**. ⚠️ 정찰은 배치 번호를 **46**으로 적었는데 실측은 **45**다(직전이 44다 — 두 수를 한 낱말로 적지 않는다). **바늘**: 같은 문서의 절 제목 꼴 |
| 접근성 표 C-3 경과 수(세 자리) | 스물여덟 · 스물여덟 · 스물여덟(갈림 0) | **스물아홉 · 스물아홉 · 스물아홉**(갈림 0) | ⚠️ **움직였고, 세 자리를 나란히 올렸다** — 한 자리만 올리면 짝 계약이 빨개진다. **바늘**: `ROUND_RUN_PATTERN` |
| `MAX_DIVERGENCE` ↔ 문서가 옮겨 적은 그 상수 | **0** ↔ 문서 세 자리 **0** · 세는 계약 하나 | **0** ↔ 문서 세 자리 **0** · 세는 계약 하나 | 그대로다 — ⚠️ **경과 수를 올리는 걸음이 *"0을 넘지 않는가"* 를 한 글자도 바꾸지 않았다**(다섯 라운드 연속). **바늘**: `CONSTANT_QUOTE_PATTERN` |
| 접근성 짝 계약의 상수 인용 좌표 | `22 · 1378 · 1446` | **`22 · 1430 · 1506`** | ⚠️⚠️ **움직였고 F가 나란히 옮겼다** — A-36 절과 C-3의 세 자리, 머리말 문단이 아래 두 좌표를 **52줄·60줄** 밀었다. **바늘**: 그 계약이 자기 소스에서 등호로 무는 줄 번호 배열 하나 |
| 공유 테스트 DB / 활성 카탈로그 ↔ 제품 카탈로그 | 744MB · 9,476(활성 8,829) ↔ 62 | **753MB · 9,545**(활성 **8,888**) ↔ **62**(24MB) | ⚠️⚠️ **움직였고, 이 수는 F가 다시 재지 않았다 — 정찰이 2026-09-01에 잰 일곱째 시점 값이다.** ⚠️ **+69**이고 직전은 +37이다. **N-4의 판정은 미발동 그대로다**. **바늘**: `wooriai_test`·`wooriai_dev`에 직접 질의 |
| ⚠️⚠️ 공유 DB를 불리는 손(생성 · 파일 · 정리 · 파일 · 정리 없음 셋) | **세는 자리 0건** | **17 · 10 · 16 · 6 · 7/9/11** | ⚠️⚠️ **오늘 처음 세어졌다 — #14가 여섯 라운드 기다린 그 자리다.** ⚠️ 정찰의 *정리 걸음 넷*과 *정리 없는 자리 열셋*은 오늘 어느 자로도 나오지 않는다. **바늘**: `packages/test-utils/src/harness-catalog-cost.test.ts:249`의 `POPULATION_FLOOR` |
| ⚠️⚠️ 재개 조건 대장 사각의 기록 축(전수 · 자 · 적힌 값 · 갈린 자리) | **세는 자리 0건** | **15 · 15 · 15 · 11**(트랙 C 커밋 시점 **8**) | ⚠️⚠️ **오늘 처음 세어졌고, 한 라운드 안에서 여덟이 열하나가 됐다** — 값이 라운드 사이에만 낡는 것이 아니라는 실물이다. **바늘**: `packages/test-utils/src/resume-condition-ledger.ts:2802`의 `allBlindSpots` |
| ⚠️⚠️ 판정 문서의 재실측 표(행 · 수가 있는 행 · 바늘이 곁에 적힌 행 · 좌표꼴 · 낱말 · 문장) | 37 · 37 · 13 · 2 | **40 · 40 · 39 · 9 · 9 · 30**(AI절 표를 잰 값) | ⚠️⚠️ **움직였고 갈래 둘이 새로 섰다 — 트랙 E의 여섯째 판정이다.** ⚠️ **좌표꼴 아홉과 낱말 아홉은 *다른 아홉*이고 교집합은 0이다**(정찰 후보 5의 예측이 기각됐다). **바늘**: `packages/test-utils/src/repo-self-description.test.ts:1438`의 `retestNeedleShape` |

⚠️ **이 표가 라운드 94의 트립와이어 관례에 더하는 것이 셋이다.** ⓐ ⚠️⚠️ **시점 예외가 0건인 다섯
번째 표다.** ⓑ ⚠️⚠️ **마지막 칸의 바늘이 *꼴*을 지기 시작했다** — 오늘 F가 낱말 하나로 적을 수
있는 행을 낱말로 바꿨고, 그 수가 몇인지는 **다음 라운드의 트랙 E가 낸다**(이 표를 재는 것은 그
자이고 F의 손이 아니다 · AG-5의 판정이 네 번째로 확인된다). ⓒ ⚠️⚠️ **재현 불가로 남은 칸은 오늘도
하나다**(문턱·래칫 상수 38) — **바늘을 낱말로 바꾸는 것과 그 바늘로 재면 같은 수가 나오는 것은
다른 일이고, 이 표는 여전히 앞쪽만 지킨다.**
⚠️ **그리고 이 표의 행 수도 갈래 수도 *이 표의 오늘 값*이지 트랙 E의 자가 낸 수가 아니다** — 그
자가 이 표를 재는 것은 다음 라운드이고, 트랙 E 커밋 시점에 그 자가 낸 **40 · 40 · 39 · 9 · 9 · 30**은
**AI절의 표**를 잰 값이다. **두 시점을 한 낱말로 적지 않는다.**
⚠️⚠️ **그래도 오늘 F가 이 표를 자기 손으로 한 번 대 보았고, 그 수를 값으로 남긴다** — 행 **44** ·
수가 있는 행 **44** · 바늘이 곁에 적힌 행 **43** · 명시적 무바늘 **1** · 수만 있는 행 **0** ·
**낱말/정규식 23 · 문장 20** · 좌표꼴 **6** · 붐비는 칸 **20** · 좌표꼴과 낱말의 교집합 **0**.
⚠️⚠️ **그리고 그 수 가운데 하나가 *내려갔다* — 좌표꼴이 9에서 6이 됐다.** 낱말 바늘로 바꾼 행은
토큰이 하나뿐이라 좌표를 곁들일 수 없고(좌표를 적는 순간 토큰이 둘이 되어 문장으로 떨어진다),
**그래서 낱말을 늘리는 걸음은 좌표를 줄이는 걸음이기도 하다.** ⚠️ **고른 길의 비용을 그 자리에
적는다**: 기계가 바로 따라갈 수 있는 좌표는 셋 줄었고, 대신 검색창에 그대로 넣을 수 있는 낱말이
아홉에서 스물셋이 됐다. **어느 쪽이 옳은지는 이 절이 정하지 않는다 — 다음 라운드의 그 자가 낸
수와 정찰의 재현 실험이 답한다.**

**이 라운드가 짝 문서에 남긴 것.** 확인의 표에 **`#172` 하나**가 서고 §0의 네 숫자와 합계·머리말의
재계산 줄·§1-1 머리말의 라운드 구간이 파싱으로 다시 세어졌으며, 접근성 표에는 **A-36 `#112`
하나**와 **C-3의 세 자리 갱신**, 그리고 **짝 계약의 좌표 한 줄**이 나란히 옮겨졌다.
⚠️⚠️ **표면 배분은 실기기 하나 · 브라우저 0건이고, 그 이유가 §0의 수보다 값이 크다**: 다섯 트랙
가운데 **제품 소스를 한 바이트라도 고친 것이 하나**이고(A의 라벨 한 조각 · 순수 함수의 갈래 둘 ·
상태 프롭 둘) **나머지 넷은 자기 계약의 자와 바늘을 고쳤다**(어드민 사각의 0의 뜻 · 대장 사각의
기록 축 · 공유 DB를 불리는 손 · 판정 문서 바늘의 갈래) — 넷 다 **화면 0건 · 문구 0건 · 렌더 0건**이다.
⚠️ **라운드 84가 세운 그 구분**(*"확인할 것이 없다"* 가 아니라 *"폰에 보이는 동작을 한 곳도 바꾸지
않았다"*)**의 열째 판이다.**
⚠️⚠️ **A-36이 A-35와 다른 점 하나는 그 절의 머리말이 진다**: A-35는 *문구가 값에서 나오는가*를
물었고, **A-36은 맞는 문구가 몇 번 들리는가를 묻는다.** ⚠️⚠️ **그리고 A-36의 한 행이 여는 화면이
셋이다** — 행 수와 밟을 자리 수를 한 낱말로 적지 않는다.
⚠️⚠️ **C-3은 오늘로 스물아홉 라운드째 미확인**이고, ⚠️ **이번 라운드가 그 줄에 더하는 값은 경과
수가 아니라 *그 수 곁의 바늘이 이제 갈래까지 진다*는 사실이다** — 확인은 여전히 배정을 기다린다.

### AJ-1. **정찰의 전수가 다섯 자리에서 다 하한이었고, 다섯 다 *바늘이 문장이라서* 갈렸다** — AI-1의 일반형이 이번에는 정찰의 표가 아니라 트랙의 걸음에서 확인됐다

- **사실 ①(트랙 C · 사각 열하나 ↔ 열다섯).** 정찰이 재개 조건 대장의 사각을 *"여덟 + 셋 = 열하나"*
  로 적었는데, 그 문장은 `LEDGER_BLIND_SPOTS`와 `ELAPSED_BLIND_SPOTS`를 **이름으로** 든 것이었고
  같은 파일의 `sourceNeedleBlindSpots()`가 지는 **넷**이 그 이름 밖에 있었다. 전수는 **열다섯**이다.
  ⚠️⚠️ **그래서 *갈린 자리*의 수도 함께 갈린다** — 정찰의 **일곱**은 열하나 안의 갈림이고 C의
  **여덟**은 열다섯 안의 갈림이며, 오늘 F가 다시 대니 **열하나**이다.
- **사실 ②(트랙 D · 정리 걸음 넷 ↔ 열여섯).** 정찰이 `itemTemplate.deleteMany`를 지닌 자리를
  **넷**으로 적었는데 그 넷은 두 파일뿐이었고, 같은 뿌리를 걸으니 **열여섯 · 파일 여섯**이다.
  ⚠️⚠️ **그 하나가 정찰의 결론을 통째로 바꿨다** — *"정리 없이 누적을 만드는 자리 열셋"* 이
  오늘은 자에 따라 **7 · 9 · 11**이고, **어느 자로 봐도 열셋에 못 미친다.**
- **사실 ③(트랙 A · 모집단 57 ↔ 56 ↔ 58).** 정찰의 57에는 **주석 안의 한 자리**가 들어 있었다.
  낭독되는 것은 주석이 아니므로 트랙 전은 **56**이고, 비선택 가지 둘에 프롭이 서서 오늘은 **58**이다.
- **사실 ④(트랙 A · 계약 셋 ↔ 넷 · 핀 셋 ↔ 여섯).** 배정표에 없던 넷째 계약이 문구를 **수로**
  물고 있었다(한국어 리터럴 54 → 53). ⚠️ **바늘이 달라도 부채는 같다.**
- **사실 ⑤(트랙 A · 넷 갈래 0건 ↔ 로딩 둘).** 정찰이 *"비활성·체크·펼침/접힘·로딩 낱말이 낭독
  라벨에 든 자리는 0건"* 이라고 적었는데 오늘 전수로 세니 **로딩이 둘**이다.
- ⚠️⚠️ **다섯 자리의 공통점 하나가 처방을 정한다 — 다섯 다 *정찰이 문장으로 적은 바늘*이었다.**
  ①은 *"대장의 사각"*, ②는 *"정리를 함께 지닌 자리"*, ③은 *"`accessibilityState` 자리"*,
  ④는 *"그 바이트를 무는 계약"*, ⑤는 *"낭독 라벨에 든 낱말"* 이고, 다섯 다 사람이 읽으면 하나로
  읽히는데 기계가 대면 둘로 갈린다. ⚠️ **반대로 정찰이 낱말 하나로 적은 수는 오늘도 정확했다** —
  `itemTemplate.create` **17 · 파일 10**이 한 자리도 다르지 않았다.
- **오늘의 전수.** 이 절이 든 갈린 자리 **다섯**(⚠️ 그 밖에 트랙 E의 기각 하나와 트랙 D의 소유
  파일 하나를 더해 위 「기록 의무」의 항목은 **일곱**이고, 트랙 B의 이름 겹침은 *곁딸린 하나*로 따로
  적었다 — **두 수를 한 낱말로 적지 않는다**) · 일곱 다 **트랙이 자기 소스에 먼저 적었다** ·
  **통합이 닫을 부채 0건**.
- **일반형.** ⚠️⚠️ **손으로 센 전수는 늘 하한이고, 그 하한이 얼마나 낮은지는 *바늘의 꼴*이 정한다.**
  낱말 하나로 적힌 바늘은 하한이 아니라 값이고, 문장으로 적힌 바늘은 *어느 조건까지 이었는가*만큼
  낮다. ⚠️ **그러므로 배정표의 수를 계약에 그대로 옮겨 적는 것은 언제나 위험하다** — 트랙 D가
  정찰의 *열셋*을 상한으로 옮겨 적었다면 그 계약은 **첫날부터 거짓 초록**이었다.
- ⚠️ **다음 라운드가 먼저 세어 볼 만한 것**: 정찰 노트가 **트랙에 배정하며 적은 수**(모집단 · 계약 ·
  자리) 가운데 **트랙이 다시 재어 갈린 것**이 몇이고 **그대로였던 것**이 몇인가 — ⚠️⚠️ **오늘 그
  비율을 세지 않았다**(다섯이 갈렸다는 것만 안다 · 갈리지 않은 배정 수를 세는 자는 **0건**이다).
- ⚠️ **되짚기(라운드 94)** — AI-1의 질문(정찰 노트의 바늘이 낱말인가 문장인가)에 답한 자리는 위
  「질문 다섯」 표의 1행이고, **이 절이 답한 것은 그 답의 *적용 범위*다**(정찰의 표만이 아니라
  배정표에서도 참이다). AI-4의 질문에 답한 자리도 이 절이다 — 배정이 **두 번째로** 하한이었다.

### AJ-2. **값은 라운드 사이에만 낡는 것이 아니라 한 라운드 안에서도 낡는다 — 그래서 대장은 갱신이 아니라 파생을 골랐다**

- **사실.** 트랙 C가 재개 조건 대장의 사각 **열다섯**을 걸어 *적힌 값 ↔ 오늘의 자*를 맞대는데,
  **걷는 동안 셋이 그 자리에서 움직였다** — 같은 라운드의 앞 트랙 커밋이 소스에 재개 조건을 더했기
  때문이다. ⚠️⚠️ **그 관찰이 곧 설계가 됐다**: 적힌 값을 오늘 값으로 *갱신*하면 그 값은 **다음
  라운드가 아니라 같은 라운드 안에** 다시 낡는다.
- ⚠️⚠️ **그래서 C의 답은 갱신이 아니라 파생이다.** `valueToday`가 상수에서 **자를 부르는 게터**가
  됐고(라운드 94 트랙 B의 `measure` 관례를 인용했다), 옛 상수는 지워지지 않고 `recorded`
  (*시점* · *값* · *까닭*)로 내려갔다. ⚠️ **손이 적은 값이 남아 있지 않다는 것을 그 계약이 부정
  단언으로 문다.**
- ⚠️⚠️ **오늘 그 선택이 값을 하나 더 냈다.** C 커밋 시점에 **여덟**이던 갈린 자리가, 그 뒤에 선
  트랙 A의 커밋과 F의 이 절을 지나 오늘 **열하나**이다. ⚠️ **등호를 골랐다면 이 걸음마다
  빨개졌을 것이고, 빨간 계약은 아무것도 지키지 못한다**(AF-5·AG-5·AH-5·AI-5가 값으로 적은 그 비용).
  ⚠️ **하한만 골랐다면 조용했을 것이다** — 그것이 라운드 92~94에 실제로 일어난 일이고, 이 축이
  처음 열린 이유다.
- ⚠️⚠️ **정정(라운드 95 리뷰 M-3 — 위 항목의 옛 문장은 보존한다).** 위 "C 커밋 시점 여덟 →
  A·F를 지나 열하나"는 **갈림의 시점과 원인 귀속이 재실측과 갈린다.** 라운드 95의 스냅숏마다
  같은 자(오늘의 `blindSpotReadings`)로 다시 재면 갈린 자리는 아래처럼 움직였고, **두 모집단의
  수를 한 낱말로 적지 않는다** — *일곱*과 *여덟*은 서로 다른 모집단의 수다.

  | 시점 | 갈린 자리(적힌 값 ↔ 오늘의 자) — **열다섯 모집단**(대장 전수) | 같은 자 — **열하나 모집단**(정찰이 대어 본 자리만) | 무엇이 움직였나 |
  | --- | --- | --- | --- |
  | 정찰 스냅숏(`96231d1`) | **8** | 정찰 자신의 기록 **7** | — (여기가 위 문장의 "여덟"이 실제로 산 시점이고, *일곱*은 이 칸의 모집단 안의 수다) |
  | 정찰 노트 · E · D 커밋 뒤 | **11** | **9** | 정찰 노트가 `docs/5차`와 경과 창을, E·D가 소스 자리를 올렸다(`quoted-source-conditions` · `unmarked-source-prose` · `three-needle-residual`이 새로 갈렸다) |
  | **C 커밋 시점** | **11** | **9** | **이미 열하나였다** — C가 파생을 고른 바로 그 자리에서 잰 수다 |
  | A 커밋 뒤 · F(이 절) 뒤 | **11** | **9** | ⚠️⚠️ **A·F가 움직인 것은 갈린 자리의 *수*가 아니라 *크기*다**(A: `unmarked-source-prose` 63 → 73 · `three-needle-residual` 10 → 17 / F: `elapsed-truth` 102 → 116 · `markdown-source-notation` 261 → 275 등) |
- ⚠️ **그리고 C가 고른 방향에는 값이 하나 더 붙는다** — **차이가 음수인 자리가 0건**임을 부정
  단언으로 못 박았다. **오르는 쪽은 안전하고 내려가는 쪽은 이름이 틀렸던 날이다**(라운드 94 D의
  `unmarked-source-prose` 129 → 61이 그 실물이고, 그 줄어듦은 옳았다).
- ⚠️⚠️ **그런데 같은 파일이 *반대 방향의 하한*을 하나 세웠고, 오늘 F의 걸음이 그것을 바닥까지
  끌어내렸다.** 트랙 C는 등호를 피해 파생을 골랐으면서, 그 옆에 **정찰이 손으로 잰 수와 오늘의 자가
  같은 자리가 다섯 이상**이라는 하한을 함께 세웠다(열한 자리 가운데). ⚠️ **그 수는 F가 판정 문서를
  쓸 때마다 *내려가는* 쪽이다** — 이 절이 서기 전에는 **일곱**이었고, 이 절이 서자 **다섯**이다
  (여유 **0**). ⚠️⚠️ **이 절을 쓰면서 그 열하나 가운데 넷이 정찰의 수를 떠났고, F가 둘은 되돌렸으며
  둘은 되돌릴 수 없었다**: 되돌린 둘은 `folded-notation`(표기 하나를 한 줄로 펴 **9 → 8**)과
  `sibling-documents`(짝 문서에서 대장을 *이름*이 아니라 **파일 이름**으로 불러 **20 → 18**)이고,
  되돌릴 수 없는 둘은 `prose-only` **246 → 261**과 `elapsed-truth` **102 → 116**이다 —
  **이 절이 기각 목록마다 조건과 경과를 적었기 때문에 오른 수**라, 되돌리려면 V-2의 규율을 어겨야 한다.
  ⚠️⚠️ **즉 C가 등호에서 피한 그 병이 같은 파일의 다른 자리에서 하한의 얼굴로 다시 섰다** —
  **빨개지는 방향이 반대일 뿐 원인은 같다**(F의 정직한 걸음이 계약을 깬다).
  ⚠️ **재개 조건(사건형): 다음 라운드의 F가 이 문서에 절을 더하는 날** — 그날 그 하한이 먼저
  빨개진다. ⚠️ **이 라운드의 F는 그 파일을 고치지 않았다**(트랙 C의 소유다 · 문서 트랙은 소스를
  0바이트 고친다). **고칠 손은 그 파일을 여는 트랙이고, 고칠 방향은 *정찰과 같은 자리의 수*를
  하한으로 두지 않는 것이다** — 그 수는 값이지 래칫이 아니다.
  ⚠️⚠️ **닫혔다(라운드 95 리뷰 M-1 — 위 문장이 말한 그 방향 그대로).** 리뷰가 그 역방향 하한을
  걷고 자리마다 **오늘 ≥ 정찰**의 방향 단언으로 바꿨으며, *몇이 같은가*는 시점·값·까닭을 붙인
  recorded로 내려갔다(옛 단언은 그 자리 주석이 보존한다). 함께 서 있던 완전 등호의 부등식 판
  (468 − 440 ≤ 정찰 노트의 자리 수 · 여유 0)도 같은 걸음에 recorded로 내려갔다 — `docs/5차`에
  새 파일이 서는 순간 좌변만 자라 거짓이 되는 꼴이었다. **다음 F는 이 문서에 절을 더할 때 그
  파일을 되돌아볼 필요가 없다.**
- **오늘의 전수.** 사각 **열다섯** · 자를 지닌 자리 **열다섯** · 적힌 값을 지닌 자리 **열다섯** ·
  갈린 자리 **열하나**(C 커밋 시점 여덟 · 정찰 일곱) · `valueToday`가 게터인 자리 **열다섯** ·
  손이 적은 값 **0건** · 본보기(사문 대장)의 등호 **여섯 · 갈림 0**.
  ⚠️ **정정(라운드 95 리뷰 M-3 — 위 괄호의 옛 표기는 보존한다):** 괄호 안 *여덟*은 C 커밋
  시점이 아니라 **정찰 스냅숏**의 수이고(C 커밋 시점도 이미 **열하나**), *일곱*은 열다섯이 아니라
  **열하나 모집단** 안의 수다 — 시점·모집단은 위 정정 표의 칸이 진다.
- **일반형.** ⚠️⚠️ **문서가 낡는 속도의 하한은 라운드가 아니라 커밋이다.** 한 라운드 안에서 여섯
  손이 같은 저장소를 고치므로, *오늘 잰 값*은 그 라운드가 끝나기 전에 이미 옛값이 된다. ⚠️ **그래서
  적힌 값을 지키는 길은 셋뿐이다**: 등호(빨개진다) · 하한(조용하다) · **파생**(값이 스스로 따라온다).
  **앞의 둘은 값을 지키고 뒤의 하나는 값을 버린다 — 그리고 대장이 지켜야 하는 것은 값이 아니라
  *언제 무엇이 참이었는가*라, 옛 값은 `recorded`로 남고 오늘 값은 자에서 온다.**
- ⚠️ **다음 라운드가 먼저 세어 볼 만한 것**: 이 저장소의 사각 가운데 **`valueToday` 꼴로 값을 적은
  것**이 몇이고 그중 **파생인 것**이 몇인가 — ⚠️⚠️ **오늘 그 비율은 한 대장 안에서만 안다**
  (열다섯 중 열다섯) · **저장소 전체에서 세는 자는 0건이다**(AI-3의 66과 그중 크기가 적힌 64가
  첫 모집단이다).
- ⚠️ **되짚기(라운드 94)** — AI-2의 질문(결정형이 닫히는 범위)에 답한 자리는 위 통합표의 새 칸이고,
  **이 절이 답한 것은 그 표의 *값이 언제 낡는가*다.**

### AJ-3. **규율이 자보다 넓으면 위반은 자 밖에서 산다 — 그리고 이번에는 그 자리가 사람이 매일 듣는 화면이었다**

- **사실.** `apps/mobile/src/a11y-contract.test.ts:2296`이 오래전부터 *"선택 여부를 문구로 다시
  말하지 않는다 — 그건 상태가 진다(같은 사실을 두 번 읽지 않는다)"* 를 적고 그 문장을 부정 단언으로
  물고 있었다. ⚠️⚠️ **그런데 그 자가 보는 곳은 온보딩 단계 카드 *한 자리*였고**, 같은 규율을 어기는
  자리가 저장소에 **셋** 살아 있었다 — 빠른 품목 타일(1단계) · 달 점프 시트(2단계) · 날짜 픽커(1단계).
- ⚠️⚠️ **셋 다 라벨과 상태 프롭이 같은 사실을 함께 말하고 있었다.** RN의 `accessibilityState.selected`가
  안드로이드 `setSelected(true)` · iOS `UIAccessibilityTraitSelected`로 내려가 **OS가 스스로 상태를
  읽는데**, 라벨이 `선택됨`을 한 번 더 이어 붙였다. ⚠️ **그 사실은 이 저장소가 이미 알고 적어 둔
  것이다** — `app/(tabs)/records.tsx`의 *"TalkBack은 '리스트, 탭, 선택됨'처럼 읽는다"* 라는 주석 한 줄.
- ⚠️⚠️ **고친 바이트는 다섯이고 새 한국어 낱말은 0건이다.** 라벨 템플릿의 한 조각 · 순수 함수의
  갈래 하나씩 둘 · 비선택 가지의 상태 프롭 둘. ⚠️ **그리고 걷는 걸음이 소리를 *더했다*** — 그 갈래가
  `else if`라, 선택됐는데 못 고르는 칸에서 종전에는 조용하던 **이유가 대신 들어온다**.
- ⚠️⚠️ **트랙이 정찰의 처방 하나를 값으로 반박했다** — 정찰은 비선택 가지에 `disabled`도 함께
  세우라고 적었는데, 그 칸의 라벨이 이미 못 고르는 이유를 **문장으로** 말하고 있어 상태로 또 말하면
  **걷어 낸 그 이중 낭독이 다시 선다.** 그 판단이 제품 주석과 계약의 부정 단언 둘로 소스에 섰다.
- **오늘의 전수.** `accessibilityState` 자리 **58**(걷는 파일 **282** · selected 19 · disabled 17 ·
  expanded 10 · checked 9 · busy 3) · 판정 셋 *상태로만 말한다* **49** / *라벨에도 적었다* **0**
  (상한 · 트랙 전 셋) / *여는 태그에 라벨이 없다* **9** · 어느 판정에도 안 떨어지는 자리 **0건** ·
  라벨로만 상태를 말하는 자리 **2** · 핀 이관 **계약 넷 · 핀 여섯**.
- **일반형.** ⚠️⚠️ **저장소가 문장으로 적어 둔 규율과 그 문장을 무는 자의 모집단은 자동으로 같지
  않고, 갈리면 그 틈에서 위반이 조용히 산다.** AH-3이 *"자가 주석보다 좁다"* 로, AI-3이
  *"0은 셀 수 없음의 값이다"* 로 이름 붙인 병의 세 번째 얼굴이고, ⚠️ **이번 것이 앞의 둘과 다른 점은
  틈에 있던 것이 *수*가 아니라 *사람이 듣는 소리*였다는 것이다.**
- ⚠️ **다음 라운드가 먼저 세어 볼 만한 것**: 이 저장소의 계약 가운데 **부류 전체를 두고 말하는
  규율 문장**을 지닌 것이 몇이고, 그중 **그 문장을 무는 자의 모집단이 한 자리인 것**이 몇인가 —
  ⚠️⚠️ **오늘 그 수를 세지 않았다**(AH-1이 센 *손 목록 옆 단언 열일곱*은 손 목록이 있는 자리만
  보므로 이 물음의 부분집합이다 · **두 수를 한 낱말로 적지 않는다**).
- ⚠️ **되짚기(라운드 94)** — AI-3의 질문(사각의 0이 무엇의 0인가)에 답한 자리는 **AJ-4**이고,
  이 절이 답한 것은 그 병의 *반대쪽*이다: 저기서는 자가 **크기만** 적었고, 여기서는 자가 **한
  자리만** 보았다.

### AJ-4. **관례는 파일을 건너간다 — 라운드 94가 한 파일에 세운 *0의 뜻*이 오늘 다른 앱의 두 파일에 섰고, 이식하는 걸음이 값을 하나 냈다**

- **사실.** AI-3이 *"0은 없음이 아니라 셀 수 없음의 값이다"* 를 세우고 *"그 둘을 가르는 자는 오늘
  0건"* 으로 닫았다. 정찰이 오늘 세니 **크기가 0으로 적힌 사각 열다섯 가운데 그 갈래를 지닌 것은
  0**이고, 갈래를 적는 낱말 `zeroMeans`는 **한 파일에 여섯 번**뿐이었다.
- ⚠️⚠️ **트랙 B가 그 꼴을 어드민 두 파일에 이식했다 — 복사가 아니라 인용이다.** 라운드 94 트랙 B가
  모바일에 세운 `UncountableEvidence`(*세려 한 것* · *왜 소스에 없는가* · **`zeroMeans`** · *증인*)를
  읽어, 그 **반대 갈래**를 지는 `CountedZero`(*0의 뜻* · *모집단 자* · *0이 아닌 자리와 배타*)를
  각자 파생시켰다. 어드민의 0 **일곱**이 오늘 전부 그 칸을 진다.
- ⚠️⚠️ **그리고 이식하는 걸음이 값을 하나 냈다 — 이름이 겹치는데 성질이 반대인 자리 셋.**
  `source-not-runtime`이 어드민 둘(`admin-landmark-current` · `admin-route-surface`)과 모바일
  하나(`screen-header-back`)에 있는데, **어드민 둘은 걷어서 낸 0이고 모바일 하나는 셀 수 없어 0**이다.
  그 사실이 `SAME_NAME_OTHER_KIND`로 서고 그 자리의 `zeroMeans`가 *"셀 수 없어서 0"* 이라는 낱말로
  반대 갈래를 문장으로 가리킨다. ⚠️⚠️ **AH-3이 예고한 오독(*이름을 성격으로 읽는다*)이 오늘 계약으로
  막혔다.**
- ⚠️ **유령 방지도 함께 섰다** — 0이라고 적힌 자리의 자가 **오늘 실제로 불려 0을 낸다**는 것을
  값으로 보이고, 상수를 감싼 함수가 0건임을 부정 단언으로 문다. 같은 걸음에 상수 셋이 파생으로
  바뀌었다(내비에 짝이 없는 라우트 전수 · 재걷기 · 순수 함수화).
- **오늘의 전수.** 어드민 두 파일의 `today: 0` **일곱** · 그 일곱 다 `zeroMeans` 문장(80자 하한)과
  **모집단 자**를 진다 · *갈래가 없다* **0**(상한 · `UNEXPLAINED_ZERO_CEILING`) ·
  `zeroMeans`가 사는 파일 **한 파일 → 다섯 파일**(모바일 6 · 어드민 16 + 14 · 대장 2 + 1) ·
  ⚠️ **이 트랙 밖에 남은 0은 여덟이다**(`admin-load-error-copy` 다섯 · `screen-header-back` 하나 · `dead-export-ledger` 둘 — 정찰의 열다섯에서 이 트랙의 일곱을 뺀 수다).
- **일반형.** ⚠️⚠️ **관례는 그것을 세운 파일에서 끝나지 않고, 다음 라운드가 *인용해* 다른 앱으로
  건너간다 — 그리고 건너가는 걸음이 원본 혼자서는 못 내던 값을 낸다.** 한 파일 안에서는 `zeroMeans`가
  *이 0은 셀 수 없다*만 말하면 됐는데, 두 갈래가 같은 저장소에 나란히 서자 **이름이 같고 성질이
  반대인 자리**가 보였다. ⚠️ **그러므로 관례를 저장소 전체로 한 번에 넓히는 것과, 한 앱씩 인용해
  옮기는 것은 결과가 다르다** — 뒤쪽만이 *옮기다 발견하는 값*을 낸다.
- ⚠️ **다음 라운드가 먼저 세어 볼 만한 것**: 이 저장소의 사각 이름 가운데 **두 파일 이상에서 쓰이는
  것**이 몇이고, 그중 **성질이 갈리는 것**이 몇인가 — ⚠️⚠️ **오늘 한 이름(`source-not-runtime`)에서만
  세었다**(셋 · 갈림 하나). **이름의 전수를 세는 자는 0건이다.**
- ⚠️ **되짚기(라운드 94)** — AI-3의 질문에 답한 자리가 이 절이다.

### AJ-5. **자가 하나면 그 자의 오차 방향이 곧 계약의 오차 방향이 된다 — 그래서 자 셋을 나란히 두면 바닥과 천장이 함께 값이 된다**

- **사실.** 트랙 D가 공유 테스트 DB를 불리는 손을 세면서 *이 생성 자리에 정리가 있었는가*를 자
  **셋**으로 나란히 재었다: **같은 파일**에 정리가 있는가 · **생성보다 뒤**에 있는가 · **같은
  바인딩**을 지우는가. 답이 **7 · 9 · 11**로 갈렸다.
- ⚠️⚠️ **자 하나만 두었다면 그 하나가 곧 진실이 됐을 것이다.** 넓은 자(같은 파일)는 *같은 파일에
  `deleteMany`가 있기만 하면 정리로 세므로* 빚을 **작게** 보고, 좁은 자(같은 바인딩)는 *이름으로
  이어지지 않으면 정리가 아니라고 보므로* 빚을 **크게** 본다. **셋을 나란히 두자 어느 자로 봐도
  정리가 없는 바닥(7)과 가장 좁은 자가 보는 천장(11)이 함께 값이 됐고, 결정 #14가 지워야 할 자리는
  그 사이에 있다.**
- ⚠️⚠️ **그리고 그 걸음이 이 표에서 여섯 라운드 기다린 술어 하나를 처음 움직였다** — #14가 든
  근거는 *"올린 손을 세는 자가 0건"* 이었고 오늘 **0 → 1**이 됐다(`DECISION_14_PREDICATE`).
  ⚠️ **결정 자체는 그대로 열려 있다** — *어느 표를 언제 무엇으로 비울지*는 오늘도 아무도 정하지
  않았고, 이 트랙은 **자만 세웠다.**
- ⚠️⚠️ **정찰과 갈린 자리가 이 트랙에 셋이다** — 정리 걸음 **넷 ↔ 열여섯** · 정리 없는 자리
  **열셋 ↔ 7/9/11** · **소유 파일**(정찰은 `apps/api/test/harness-catalog-cost.test.ts`를 주었는데
  트랙은 `packages/test-utils/src/harness-catalog-cost.test.ts`를 새로 세우고 api 뿌리를 **읽기만**
  했다). ⚠️⚠️ **그 셋째가 이 라운드의 사실 하나를 바꿨다 — `apps/api/**`의 접점 0건이 깨지지 않고
  다섯 라운드가 됐다.**
- ⚠️ **이름이 같다고 같은 파일이 아니다** — 두 뿌리에 같은 이름의 계약이 서 있고, 그 사실을 그
  계약이 스스로 부정 단언으로 문다(걷어서 만난 짝이 자기 소스와 같지 않은지).
- **오늘의 전수.** 생성 자리 **17 · 파일 10** · 정리 걸음 **16 · 파일 6** · 정리 없는 자리
  **7 / 9 / 11** · 상한은 **오늘 파생값과 등호로 되묻고** 모집단 하한은 **12**(오늘 17보다 낮다) ·
  롤백 관례 **0** · api 소스 **0바이트** · DB 접속 **0건**.
- **일반형.** ⚠️⚠️ **하나뿐인 자는 자기 오차 방향을 감춘다 — 그 자가 곧 정의가 되기 때문이다.**
  같은 사실을 넓이가 다른 자 여럿으로 재면 답이 **구간**으로 나오고, 그 구간의 두 끝이 각각
  *다툴 수 없는 바닥*과 *가장 엄한 천장*이 된다. ⚠️ **그리고 그 구간은 결정을 기다리는 자리에
  특히 값이 크다** — 결정 #14가 내려지는 날 지워야 할 자리 수를 오늘 **7에서 11 사이**로 알고
  들어가는 것과, **열셋이라는 한 수**를 들고 들어가는 것은 다르다.
- ⚠️ **다음 라운드가 먼저 세어 볼 만한 것**: 이 저장소의 계약 가운데 **같은 사실을 넓이가 다른 자
  둘 이상으로 재는 것**이 몇인가 — ⚠️⚠️ **오늘 이름으로 아는 것은 셋이다**(공유 DB의 자 셋 ·
  자기 배제의 좁은/넓은 바늘 · `helper-named-reader`의 167 ↔ 189) **그리고 그 셋을 세는 자는
  0건이다.** ⚠️ **그날 먼저 물을 것은 *두 수를 한 낱말로 적지 않는다*는 규율이 이미 그 축인가이다.**
- ⚠️ **되짚기(라운드 94)** — AI-5의 질문(수만 있는 행이 얼마로 줄었는가)에 답한 자리는 위
  「질문 다섯」 표의 5행과 트립와이어의 갈래 칸이고, **이 절이 답한 것은 그 질문의 다음 칸이다**:
  바늘이 하나뿐이면 그 바늘의 오차 방향이 곧 그 행의 오차 방향이 된다.
