...
- ⚠️ **갱신 (2026-08-30 · 라운드 82) — census 재실측: 트랙 C **직전**의 모양은 **열여섯**이었고, 항목
  ...
  *"for 안 await"* 의 전수 스윕이 아니다.
  ⚠️ **정정 (2026-08-30 · 라운드 83) — 이 절과 W-3이 인용한 `$transaction` 자리 수 "스물일곱"은 오늘
  실측 스물아홉이다.** `apps/api/src` 전수가 **29자리**이고(`prisma/`·`scripts/`에는 0건이라 모집단은
  완전하다) ⚠️ **`transaction-bounds.test.ts`는 수를 세지 않으므로 초록이다** — 그 계약이 무는 것은
  자리 수가 아니라 *각 자리가 명시 상한을 갖거나 이유와 함께 대장에 있는가*(양방향)다. **판정은 그대로
  참이고 낡은 것은 인용된 수치다** — 이 절이 산문형 전수에 대해 말한 것이 **값을 세는 계약이 있는
  축에서도** 참이라는 사실을 일반형으로 적은 것이 **X-4**다. ⚠️ 라운드 83의 어느 트랙도 `for` 안
...
- ⚠️ **그리고 그 판단을 저장소 전체에 물었다** — `apps/api/test/transaction-bounds.test.ts`가
  ...
  ⚠️ **정정 (2026-08-30 · 라운드 83) — 위 "스물일곱 자리"는 오늘 실측 스물아홉이다.**
  `apps/api/src`의 `$transaction`은 오늘 **29자리**이고(저장소의 다른 `$transaction`은
  `prisma/`·`scripts/`에 **0건**이라 이 모집단은 여전히 완전하다), ⚠️ **`transaction-bounds.test.ts`는
  초록이다 — 그 계약은 수를 세지 않기 때문이다**(자리 목록을 자기가 만들고, 각 자리가 *명시 상한* 또는
  *이유가 적힌 대장* 둘 중 하나에 속하는지를 양방향으로만 문다). **판정은 그대로 참이고 낡은 것은
  여기 옮겨 적은 수치다** — 그 성질을 일반형으로 적은 것이 **X-4**다.
...
같은 사실을 **소스는 다섯**으로, **W절은 하나**로 적고 있었고, W-3이 인용한 `$transaction` **스물일곱**은
오늘 실측 **스물아홉**이다. **둘 다 계약은 초록이다** — 계약이 무는 것이 그 수가 아니기 때문이다.
라운드 74 O-3이 이름 붙인 병의 세 번째 재발이고, 이번에는 **계약이 있는 축에서** 났다.
...
- **사실 ⓑ — 스물일곱 대 스물아홉.** W-3이 상한 대장을 세우며 적은 *"`apps/api/src`의 `$transaction`
  **스물일곱 자리**"* 는 오늘 실측 **스물아홉**이다. ⚠️ **그런데 `transaction-bounds.test.ts`는
  초록이다 — 그 계약은 수를 세지 않기 때문이다.** 자리 목록을 **자기가 만들고**(파일 전수 스캔),
  각 자리가 *명시 상한* 또는 *이유가 적힌 대장* 둘 중 하나에 속하는지를 **양방향**으로만 문다.
  ⚠️ **그것이 옳은 설계다** — 자리 수를 고정하면 새 트랜잭션이 이유 없이 대장을 늘리는 것과
  구분되지 않는다. **낡은 것은 계약이 아니라 문서가 옮겨 적은 사본이다.**
- ⚠️ **판정은 둘 다 그대로 참이다.** W절의 기각(*"준비템 상세의 판매처가 언제나 한 줄인 것은 관측이지
  결함이 아니다"*)도, W-3의 판정(*"문턱을 시드 값으로 세우면 그 문턱은 운영이 넘는다"*)도 오늘
  참이고, **낡은 것은 그 판정이 인용한 수치**다 — 라운드 82 리뷰 M-4가 `link-marker.ts:97` 주석에
  대해 내린 그 결론과 **같은 모양**이다.
- **오늘의 값.** 정정을 **한 줄씩** 남기고 판정은 다시 쓰지 않았다 — W절 세 번째 기각 문단의
  *"하나"* → **다섯**(그 문단이 적어 둔 재개 조건의 도래와 함께), V-3·W-3의 **스물일곱** → **스물아홉**
  (`transaction-bounds.test.ts`가 수를 세지 않아 초록이라는 사실을 같은 자리에).
...
- **일반형.** **값을 세는 계약이 있는 축에서도, 문서에 옮겨 적힌 그 값은 계약 밖의 사본이다.**
  ...
  계약이 초록인 채로 낡는다. ⚠️ **다음 라운드가 먼저 세어 볼 만한 것**: 이 문서가 소스의 수를
  인용하는 자리 중 **그 수를 세는 계약이 실제로 있는 것이 몇인가**(오늘 둘을 정정했고, 둘 다
  "계약은 있는데 그 계약이 수를 세지는 않는" 자리였다).
...
- **사실.** W-3은 자기 판정의 남은 절반을 **정확히** 적었다: *"남은 공백을 값으로 적는다: 카탈로그의
  ...
  라운드 동안 그 자리는 0건 그대로였고, 같은 절의 다른 수는 바로 그 한 라운드에 낡았다**(X-4 ⓑ).
  **문장은 정확했고, 정확한 문장은 아무것도 세지 않는다.**
