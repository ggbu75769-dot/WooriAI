...
| 26 | **표의 수를 *다시 대어 보는* 자가 설지** | AI절 기각 목록 | 저장소 전체 | 안 | ⚠️⚠️ **오늘 처음 이 표에 선다**(세운 손 = 라운드 94 F). **집지 않는다 — 크기다.** ⚠️⚠️ **다만 오늘 그 앞쪽이 한 겹 깊어졌다** — 트랙 E가 바늘의 *꼴*을 낱말 **9**와 문장 **30**으로 갈랐고, 그 갈래는 **재현이 아니라 재현 가능성의 대리 지표**다. 뒤쪽(실제로 다시 대어 보는 것)은 오늘도 **0건**이고, 정찰이 손으로 대어 본 두 자리는 **둘 다 갈렸다** |
