...
### P-4. **관례는 본보기 하나로 남는다** — 손 미러 여덟(오늘 아홉) 중 대조가 붙은 것은 하나였다
...
- **오늘의 값**(`apps/admin/src/admin-canonical-mirrors.test.ts`, 신설 — `apps/admin` **440**):
  정본 파일을 **소스 텍스트로 파싱해** 미러 **아홉**의 **리터럴과 순서**를 대조한다(정본 넷:
  `packages/domain/src/enums.ts` · `packages/contracts/src/analytics.ts` · `apps/api/prisma/schema.prisma` ·
  `apps/api/src/worker/jobs/link-health.job.ts` — 전부 **읽기만**). ⓑ **라벨 표는 키 집합만** 묻고
  ...
  **열**과 정확히 일치할 것(부정 단언: 라벨 없는 이름 0건 · 유령 라벨 0건 — 그 폴백이 깨지면 운영자의
  분석 표에 영문 이름이 그대로 뜬다). ⓓ **전수 대장** — `admin-api.ts`의 상수 표를 긁어 미러 목록과
  대조하므로 **새 미러는 대조 없이 생기지 못한다.** ⚠️ **값 0건**(아홉이 전부 정본과 같다 — 이 트랙은
  **묶기만** 했다) · 렌더·정렬·응답 처리·6+4 분리 규칙 0건 · `package.json`·`next.config.js` 무접촉
