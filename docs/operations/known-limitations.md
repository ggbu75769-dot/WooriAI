...
자리에만 서 있었고, 그 한 자리는 같은 파일 세 줄 위에 있었다**(AE-2). 모바일은 라운드 79~89가
*저장 실패 문장은 낭독되는 노드 안에 선다*를 대장·판정 칸·출구 셋으로 세웠는데, 어드민에서 사람에게
**조작의 결과**를 말하는 자리 **마흔일곱** 가운데 소리로 닿는 것은 **하나**였다. ⚠️ **그리고 그
하나의 세 줄 위(실패 배너)와 열여섯 줄 아래(성공 배너)가 둘 다 0건이었다** — *옳은 형식이 같은
...
**A~E가 전부 머지된 뒤** 대장을 실행해 다시 낸 값이다(라운드 89 F가 정찰의 수를 그대로 옮겼다면 그
절이 전부 틀렸을 것이라는 기록이 이 표의 관례다). ⚠️⚠️ **오늘 다시 재니 열여섯 자리 중 아홉이
움직였고, 움직이지 않은 일곱 가운데 하나는 이 저장소에서 처음 있는 일이다 — 계약 그물 수가 늘지
않은 첫 라운드다.**

| 자리 | 라운드 89가 남긴 값 | 2026-08-31 HEAD 재실측(A~E 머지 뒤) | 움직였는가 |
| --- | --- | --- | --- |
...
| 앵커 대장의 면제 | 8 | **8** | 그대로다 |
| 재개 조건 대장(자리 · 괄호 표기 · 손의 위치 · 결정형 · 손 누락 · 소스 표기/괄호 전수) | 252 · 80 · 18 · 19 · 1 · 5/7 | **252 · 80 · 18 · 19 · 1 · 5/8** | ⚠️ **마지막 하나만 움직였다** — 인용을 포함한 넓은 바늘이 여덟이 됐다(트랙 E의 새 계약이 그 관례를 인용한다) |
...
| 접근성 표 → 짝 문서 좌표 | **무는 자리 하나**(C-3의 `#26`) | 자리 **91** · 인용 번호 **95** · 서로 다른 번호 **91** · 표면까지 적은 참조 **32** · 역방향 **8** · 어긋남 **0** | ⚠️⚠️ **움직였다** — ⚠️ **정찰의 *서로 다른 번호 87*·*역방향 7*을 트랙 D가 91·8로 정정했다**(자리마다 첫 번호만 세면 87이고, 줄바꿈을 넘는 역방향 하나를 더 찾았다) |
...
### AE-2. **한 앱에서 열 라운드에 걸쳐 대장이 된 규율이 형제 앱에서는 마흔일곱 중 한 자리에 서 있었고, 그 한 자리는 같은 파일 세 줄 위에 있었다** — 옆 자리를 보면 보이는 것을 아무도 보지 않았다
...
- **사실.** 모바일은 라운드 79~89에 걸쳐 *저장 실패 문장은 낭독되는 노드 안에 선다*를 **대장 ·
  ...
  ⚠️ **그 세 줄 위의 실패 배너와 열여섯 줄 아래의 성공 배너는 둘 다 0건**이었다.
...
- **일반형.** **한 층에서 대장으로 세운 규율은 옆 층으로 건너가지 않는다**(AB-1) — 그리고 **그 층이
  ...
  **모집단을 세는 스윕이 앱 하나만 걷기 때문에 옆 앱의 0건은 어느 계약도 빨갛게 만들지 않는다.**
  ...
  이고**, 그 문장이 없으면 **옳은 형식이 같은 저장소의 세 줄 위에 있어도 열 라운드가 지나간다.**
