...
(S-4) · R-1 리뷰 M-3(`indexOf` 끝점) → **74자리의 일반형**(S-5). 고친 자리는 고쳐졌고 **같은 병이
...
- **오늘의 값.** 표에 **세 줄**이 편입되고(`child-form.ts`의 리터럴 하나가
  ...
  온보딩 모듈의 갈래가 **넷**이 된다: **전용 둘 → 오프라인 → 표 → 전용 폴백.** ⚠️ 표를 오프라인
  **뒤**에 둔 것이 그 갈래의 자리다(오프라인으로 판정된 실패에는 서버 코드가 애초에 없다) — 그래서
  `CONSENT_REQUIRED`·403·오프라인·모르는 실패의 출력이 **바이트 불변**이고, 달라지는 것은 **표가 아는
  코드**뿐이다. 그리고 **두 번째 여정 스윕**이 섰다(`CHILD_PROFILE_JOURNEY_SERVER_FILES` — 서버 파일
  둘 · 코드 **10** 전수 · **이유가 적힌 제외 넷**). ⚠️ **기존 아웃박스 스윕과 합치지 않았다** —
  단위가 다르고(아이 저장에는 큐가 없다), R-1이 얻은 규율(*제외 사유는 그 스윕의 단위로만 적는다*)이
  그 분리의 근거다. `apps/mobile` **4,635 → 4,648**(신규 **13**).
...
- ⚠️ **대가를 값으로 적는다 — 실패 틱의 `lastSummary`가 `{}`가 된다.** 스케줄러가 실패 시 빈 요약을
  기록하므로 그 틱의 `publishedCount`는 대시보드에서 사라진다. **파기 잡이 이미 치른 그 대가**이고,
  라운드 44 M-3이 그것을 다루는 방법을 이미 배웠다(요약 대신 `lastStatus`·`consecutiveFailures`를
  읽는다). ⚠️ **테스트 쪽 잔여도 함께 정리됐다** — 영구 실패 픽스처를 **10년 미래로 격리**해 다른
  케이스의 due 집합을 흔들지 않게 했고, 손으로 지우던 잔여 행 **둘**이 정리됐다.
...
- **오늘의 값.** 생성 카드가 `canEdit` 뒤로 가고 그 자리에 **라운드 77이 만든 캡션 상수 그대로**가 서며
  ...
  이 트랙의 경계였다). `apps/admin` **510 → 521**(트랙 C 여덟 · 트랙 D 셋 합산).
...
- **오늘의 값.** 세 분류가 **함수 표 하나에서 파생**된다(호출부 세기와 시그니처 세기를 섞지 않는다) —
  ...
  `admin-api.test.ts` **27 → 31**. ⚠️ **제품 소스 0건**(`src/lib/admin-api.ts` 무접촉 — 이 트랙은
  **세는 방법**만 고쳤다).
...
- **오늘의 값.** **열두 자리**에 시작·끝 두 인덱스의 실재 확인이 섰고(형식은 M-3이 세운 그것 —
  ...
  **107 → 112**(신규 **5**). ⚠️ **제품 소스 0건.**
- ⚠️ **정찰의 어림값과 스윕의 값이 달랐고, 스윕 쪽이 옳다.** 스윕이 센 값은 **자리 104 · 가드 23 ·
  미가드 81 / 파일 54**다(정찰: 74 / 41). 차이의 이유는 셋이고 셋 다 스윕이 **더 넓게** 잡기 때문이다 —
  ⓐ `lastIndexOf`도 센다(못 찾으면 똑같이 -1이다) · ⓑ 끝점을 `slice(` 안에서 곧바로 부르지 않고 **이름
  붙은 인덱스**로 빼 둔 자리도 센다 · ⓒ 가드를 **자리별**로 보아 **-1이 될 수 있는 두 끝 모두**가 확인돼야
  가드로 친다. **수치를 대장으로 옮긴 이유가 정확히 이것**이고, ⚠️ **대장은 줄 번호로 적지 않는다**
