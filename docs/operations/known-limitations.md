...
행 92**(넣으면 이 계약의 수가 라운드마다 통째로 흔들린다), 트랙 D가 모집단 밖에 둔 **잔여 열둘**,
...
| 19 | **`⚠️`와 `재개` 사이에 강조 표식(`**`)을 허용할지** | `packages/test-utils/src/resume-condition-ledger.ts:759` | 안 | ⚠️⚠️ **집혔다 → 트랙 D(GAP-094 #4) · 두 시점으로 내린다.** ① 세운 손 = 라운드 93 **리뷰**(M-3). ② 소진 = 오늘. **결정은 *넓히지도 좁히지도 않고 셋으로 갈라 각자 센다*이다** — 표식형 **18** · 강조 낀 표식형 **14** · 필드형 **44** · 잔여 **12**(합 **88** = 저장소 전수). ⚠️ **`markedSourceNeedle()`의 바이트가 불변임을 등호로 증명한다** — 넓히지 않았다는 사실이 값이다 |
...
적고, 그 수를 다시 내는 것은 트랙 E의 자다**(오늘 붙인 뒤의 수는 **40**이고, 라운드
93의 표에서는 13이었다). ⚠️⚠️ **하한을 고른 비용도 함께 적는다 — *행이 사라지거나 바늘이 지워지는
...
| 표기를 지닌 소스 파일 / 그 뿌리(걷는 파일) | 셋 · 전수 파생(표기 **11** · 괄호 **15**) | **모집단 11 · 표식형을 지닌 파일 3 · 걷는 파일 970** | ⚠️⚠️ **움직였다 — 그리고 움직인 것은 저장소가 아니라 *모집단 판정*이다.** 트랙 D가 모집단을 *표식형 하나*에서 **세 갈래의 합**으로 바꿔 파일이 **3 → 11**이 됐고, **표식형을 지닌 파일 3은 그대로**라 옛 등호가 그대로 문다. **바늘**: `packages/test-utils/src/resume-condition-ledger.ts:759`(표식형) · `:775`(강조 낀 표식형) · `:796`(필드형) 셋을 각각 대고 합으로 판정한다 |
...
번째 표다.** ⓑ ⚠️⚠️ **마지막 칸에 *바늘*이 들어왔다 — 오늘 이 표의 행은 40이고, 그 40에 전부
바늘이 곁에 섰다**(라운드 93의 표에서는 행 **37** 가운데 **13**이었고 **수만 있는 행이 24**였다 ·
오늘 수만 있는 행은 **0**이다). **그중 기계가 따라갈 수 있는 좌표꼴은 9다**(라운드 93의 표에서는
...
⚠️ **그리고 40 · 40은 *이 표의 오늘 값*이지 트랙 E의 자가 낸 수가 아니다** — 그 자가 이 표를 재는
것은 다음 라운드이고, 오늘 그 자가 낸 **37 · 37 · 13 · 2**는 **AH절의 표**를 잰 값이다.
**두 시점을 한 낱말로 적지 않는다.**
