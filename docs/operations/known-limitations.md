...
갱신: 2026-08-29 (라운드 72 트랙 F — M절 신설 · L-2/L-3 갱신) · 그 앞: 2026-08-29 (라운드 71 트랙 F — L절 신설) · 그 앞: 2026-08-29 (라운드 70 트랙 F — K절 신설) · 그 앞: 2026-08-28 (라운드 57 QA까지 반영) · 브랜치: claude/app-feature-review-design-xx71k3
...
- ⚠️ **새로 생긴 값: 저장 쪽에도 목록이 생겼다 — `OFFLINE_AWARE_SAVE_ERROR_SCREENS`(둘 → 넷).**
  ...
  스윕이 없어 **새 저장 실패 문구가 생겨도 아무도 세지 않았다.** 이제 같은 파일이 두 목록의 단일
  소스이고, 손배열은 `app/**` 스윕으로 교체됐다(조회 쪽과 **같은 형태**). 오늘의 넷:
...
- ⚠️ **이 술어는 카카오 env를 읽지 않는다.** 키의 유무는 **로그인 경로**의 사실이지 빌드 성격의
  ...
  `login.tsx:184`의 **경로 선택**(`isKakaoLoginAvailable() ? loginWithKakao() : oauthLogin("kakao")`)은
  한 글자도 바뀌지 않았다.
...
- ⚠️ **`docs/store/play-listing.md` §5 머리말이 §6과 모순된 채로 남아 있다(라운드 73이 고치지 못한 자리).**
  §5의 머리말은 그 3장을 두고 "**최소 요건(2장)을 이미 충족**하므로 **이대로 제출 가능**"이라고 적는데,
  같은 문서 §6은 같은 자산 셋을 **⛔ 재캡처/재생성 전 제출 불가**로 판정한다. 사실은 §6 쪽이다 —
  "자산이 **존재**한다"와 "자산이 **지금의 앱**이다"는 다른 이야기이고, §5의 그 문장은 앞의 것만 세고 있다.
  - **왜 이번 라운드가 고치지 않았나**: 트랙 B의 소유는 §6 자산 표뿐이고, 그 계약이 **§1~5 무변경을
    부정 단언으로 고정**했다(`store-brand-and-asset-provenance.test.ts`의 "§1~5(등록 정보 문안·촬영
    가이드)은 라운드 73이 손대지 않는다" — §6 앞쪽에 이 라운드의 흔적이 생기면 범위 이탈로 본다).
    트랙 F는 **제품 소스 0건**이라 그 계약 테스트를 갱신할 수 없으므로, §5를 고치면 계약의 문언 또는
    그 문언이 지키려는 범위 판정 중 하나를 F가 일방적으로 깨게 된다. 그래서 **§5는 무접촉**으로 두고
    모순 사실만 여기 적는다.
  - **다음 라운드의 몫**: `docs/store/**`를 소유하는 트랙이 §5 머리말을 §6 판정에 맞춰 정정하고(자산
    존재와 제출 가능은 다른 판정이라는 한 줄), 같은 트랙이 그 부정 단언을 "§1~5의 **문안**은 불변,
    **제출 가능 여부 표기**는 §6 판정을 따른다"로 다시 세운다. 문서 두 절이 서로 반대를 말하는 동안에는
    §6을 읽지 않은 사람이 §5만 보고 제출할 수 있다 — 그것이 이 줄이 남는 이유다.
...
- **오늘의 소비 집합은 열다섯이다**(`LOAD_ERROR_COPY_SITES`, `apps/admin/src/lib/load-error-copy.ts` —
  ...
  `src/admin-load-error-copy.test.ts`가 `app/**`을 훑어 이 표와 **정확히 일치**하는지 보므로, 새 조회
  화면이 그 한 벌을 부르지 않으면 그 자리가 먼저 빨개진다. **모바일이 라운드 38~72에 걸쳐 배운
  형식**("판정은 한 벌, 문구는 화면별, 그리고 목록이 그 소비를 센다")이 어드민에 도착한 것이다.
