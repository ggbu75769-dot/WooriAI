...
- ⚠️ **갱신 (2026-08-30 · 라운드 77) — 수치는 그대로이고, 그 결정이 값을 치르는 자리가 어디인지가
  이번에 보였다.** 표기 방언은 재실측해도 같고(띄어 쓴 **31건 / 파일 열여덟** — 라운드 76의 30에서
  트랙 A의 대장 문장 하나가 늘었을 뿐이다 · 붙여 쓴 **10건 / 파일 셋**) 기각 사유도 그대로다(둘 다
  어법상 허용 · **거짓이 아님**). ⚠️ **다만 통일하지 않기로 한 결정이 대가를 치르는 자리는 이제
...
- **오늘의 값.** 표에 두 줄이 편입되고(`api-error.ts` — *"이 구매 링크는 더 이상 열 수 없어요. 내려간
  링크일 수 있으니 다른 구매 링크를 확인해 주세요."* · 주소 쪽 한 줄), 화면의 `onError` 한 자리가
...
- ⚠️ **새 문장이 표기 방언의 대가를 처음으로 치렀다** — 꼬리에 `"잠시 후 다시"` 0건이고 표기는
  **띄어 쓴 쪽**이다. **통일하지 않기로 한 결정이 값을 치르는 자리는 이제 새 문장을 짓는 순간이다**
  (Q-1 갱신 참고).
- ⚠️ **접점 하나(다음 라운드의 입력).** `purchase-followup-flow.test.ts`가 이 화면의 문자열을 **끝점**
  으로 잡고 있어, 트랙 A는 그 끝점을 보존하려고 `clickLink` 블록을 옮겼다. **그 테스트를 여는 라운드는
  끝점을 문자열이 아니라 함수 이름 기준으로 바꾸는 편이 낫다.**
...
- **오늘의 값.** 연결 실패도 **같은 두 값으로만** 갈린다(`STATE_CHANGING_METHODS.has(method)` ·
  ...
  **읽기 문장은 바이트 불변**이다. 타입도 늘리지 않아(`AdminApiError(0, …)` 그대로 · **새 클래스 0건**)
  `writeErrorMessage`·`loadErrorCopy`와 화면 전부가 **무접촉**이다. `admin-api.test.ts` **18 → 27**.
...
- **오늘의 값.** 세 화면의 제출 컨트롤 **여섯 자리**가 `canEdit`(`admin` 또는 `editor` — 편집자는 검토
  ...
  **문자열은 바이트 불변**이다. **새 한국어 문구 0건 · 서버 0건**이며 `isEditor` 갈래(검토 요청 문안
  ...
  않는다). 전수 단언의 단위는 **다섯 화면**이다. `apps/admin` **505**(신설 **15**).
...
- **표기 방언(`"시도해 주세요"` vs `"시도해주세요"`) — 재실측했고 이번에도 통일하지 않는다.**
  수치는 Q-1이 남긴 그대로다(띄어 쓴 쪽 **31건 / 파일 열여덟** — 라운드 76의 30에서 트랙 A의 대장
  문장 하나가 늘었다 · 붙여 쓴 쪽 **10건 / 파일 셋**). 기각 사유도 그대로다(둘 다 어법상 허용 ·
  **거짓이 아님** · 어느 트랙의 축도 아님). **다만 그 대가를 확인한 자리가 하나 늘었다 — Q-1 갱신 참고.**
