...
| F3 | 중간(예방) | attachments에 PK 외 인덱스 0개인데 파기 캐스케이드가 30초 트랜잭션 안에서 `expense_id IN (...)` UPDATE(FK 널링)와 `child_id IN (...)` DELETE를 수행 — 첨부 볼륨이 쌓이면 배치당 풀스캔 x2가 tx 타임아웃 예산을 잠식 | **추가** `idx_attachments_expense (expense_id)`, `idx_attachments_child (child_id)` — 현재 볼륨에선 측정 이득이 작지만 30초 타임아웃 실패 모드(포이즌 행 오판)의 예방 비용이 낮다고 판정. ⚠️ **정정(GAP-067 #8)**: "첨부 볼륨이 쌓이면"은 일어나지 않는다 — `attachments`에 행을 만드는 코드가 저장소에 **0건**이고 채울 기능(사진/영수증)은 DNC-016이 MVP 밖으로 못박았다(schema.prisma의 `Attachment` 모델 주석). 이 두 인덱스와 위 캐스케이드는 **언제나 빈 표**를 상대로 돈다. 인덱스를 되돌리지 않는 이유는 표를 지우는 날 함께 정리할 것이고 되돌리는 것 자체가 또 한 번의 마이그레이션이기 때문이며, **이 행을 "첨부 데이터가 있다"의 근거로 인용하지 말 것** |
