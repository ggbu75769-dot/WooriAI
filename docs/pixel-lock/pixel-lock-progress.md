...
- Full Android phase-A check after SPL/IMP/EXP: all 9 screens remained `renderValid=true`; SET stayed `0.0498`.
- Wired `FAM-001` family screen to `FamilyPixelStyles` runtime getters.
- Rejected `FAM-001` emoji avatar candidate: score improved to `0.0847`, but Android rendered unsupported emoji as broken glyphs.
- Accepted safe `FAM-001` candidate: white outline CTA, stable initials, `DAON2025`, `scale 1 -> 0.78`, `topOffset 0 -> -20`; score `0.1180 -> 0.0776`, SET guard stayed `0.0498`, RBAC/API logic unchanged.
- Verified `npm run release:gate`: PASS at `2026-07-08T21:21:36.362Z`.
- Wired `REP-001` report screen to `ReportPixelStyles` runtime getters; baseline score remained effectively unchanged.
- Rejected `REP-001` numeric candidates: best tested `scale=0.68`, `topOffset=-81`, `horizontalOffset=30` scored `0.1234`, below the `>=0.0030` improvement acceptance threshold.
- Verified `npm run release:gate`: PASS at `2026-07-08T21:35:18.724Z`.
- Wired `ITEM-001` recommendation list screen to `ItemListPixelStyles` runtime getters.
- Accepted safe `ITEM-001` candidate: `scale 0.82 -> 0.48`, `topOffset -50 -> -30`, `horizontalOffset -30 -> 0`; score `0.1278 -> 0.1138`, SET guard stayed `0.0498`, ranking/status behavior unchanged.
- Verified `npm run release:gate`: PASS at `2026-07-08T21:57:59.022Z`.

## Latest Android Report
Generated by `npm run pixel:android -- --force` after accepted `SET-001` candidate.

> **이 표는 최신 실측이 아니다 — `docs/qa/android-release-test-report.md:111`의 9/9 PASS로 대체됨.**
> 아래 표는 튜닝 도중의 스냅샷이라 대부분 `fail`로 남아 있지만, 그 뒤 기준 프로필 AVD에서
> 9화면 전부 임계(0.05) 아래로 재측정됐다(SPL 0.0230 / HOME 0.0421 / EXP 0.0408 /
> ITEM-001 0.0466 / ITEM-002 0.0491 / REP 0.0396 / FAM 0.0362 / IMP 0.0450 / SET 0.0218).
> 현재 픽셀락 상태를 알고 싶으면 그 리포트를 보고, 아래 표는 튜닝 이력으로만 읽는다.
