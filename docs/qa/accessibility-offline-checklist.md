...
| --- | --- | --- | --- | --- |
| 1 | 잠금 오버레이 (`src/security/AppLockOverlay.tsx`) | 잠금 중 배경(Stack)의 금액·품목이 접근성 트리에서 잘리는가. | 잠금 상태에서 TalkBack이 뒤쪽 화면 내용을 읽지 못한다. | **코드 계약 없음 — 라운드 59 트랙 C 대상.** 오버레이 안쪽의 PIN 도트 숨김(`accessibilityElementsHidden`)과 안내 문구 live region만 고정돼 있다. C절 참조 |
...
| C-3 | **잠금 오버레이 TalkBack 투과** — 잠금 중 뒤쪽 화면의 금액·품목이 낭독되는가 | Stack과 오버레이가 형제라 접근성 트리가 z-order로 잘리지 않는다. A-2 #1의 코드 계약이 붙기 전까지 **투과가 실측 사실**이다. 라운드 59 트랙 C가 고친 뒤에도 기기 재확인 대상. |
...
- **C-3(잠금 오버레이 투과)은 릴리즈 전 필수 확인 항목이다** — 잠금이 켜져 있는데 금액이
  낭독되면 앱 잠금 기능 자체가 약속을 지키지 못한 것이 된다.
