...
## 4. 개선 라운드 2 — 신기능 APK 런타임 검증 (2026-07-12)

라운드 2 개선(4개 배치: 리포트 정합 / 지출 UX / 죽은 버튼·하드코딩 정리 / API 하드닝 2차) 반영 APK를 clean install 후 검증했다.

| 산출물 | 값 |
|---|---|
| APK SHA-256 | `f76e7b497085dfbf606433cded78c035d89ae4a4be0ea6819a936cea3595f688` (64,715,782 B) |
| 게이트 | `release:gate` PASS (mobile 64 / api 50 / e2e 21 / contracts 8 / domain 19 테스트) |

| # | 신기능 시나리오 | 결과 |
|---|---|---|
| 1 | 지출 시트: 날짜 변경 행 + 최근 14일 칩(오늘/어제/그제/…) + 직접 입력 | PASS (어제 선택 → 2026-07-11 반영) |
| 2 | 품목명 입력 노출, 선물 토글("선물은 지출 합계에 포함되지 않아요") | PASS |
| 3 | **선물 7,777원 저장 → 목록에 "선물 · 7월 11일" 배지, 월 합계 97,200원 불변(선물 제외)** | PASS |
| 4 | 기록 월 이동(‹ 2026년 6월 → 빈 상태, › 복귀), 합계 라벨 월 표기 | PASS |
| 5 | 기록 검색("seonmul" → 해당 1건만 필터) | PASS |
| 6 | 최근 품목 칩 3개(실데이터: 기저귀 45,900 등) | PASS |
| 7 | 홈 알림 벨 → `/notifications` 빈 상태 화면 | PASS |
| 8 | **리포트 라인 차트 실데이터 지오메트리**(2~6월 0 평탄 → 7월 상승) | PASS |
| 9 | 카테고리 카드 "전체 기간 카테고리 비중" 정직 표기, 한국어 라벨 | PASS |
| 10 | 가족 실데이터("우리 가족 / 엄마 (나) · 아빠"), DAON2025 가짜 코드 세션 제거 | PASS |
| 11 | 초대 역할 선택 다이얼로그(공동부모/보기 전용/취소) | PASS |
