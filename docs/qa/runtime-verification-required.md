...
특히 **C-3(잠금 오버레이 TalkBack 투과)** 은 실측된 결함이다: 잠금 화면과 뒤쪽 Stack이 형제라
접근성 트리가 z-order로 잘리지 않아 **잠금 중에도 금액·품목이 낭독될 수 있다**. 라운드 59 트랙 C가
`importantForAccessibility`로 잘라 낸 뒤에도 기기 재확인 대상이다(잘렸는지는 실제 낭독으로만 안다).
