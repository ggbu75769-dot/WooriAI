...
5. **512 아이콘·피처 그래픽을 다시 만들어요.** 아이콘은 `apps/mobile/assets/icon.png`(DSN-053
   ...
   `docs/store/play-listing.md` §1에서 읽고, 계보는 `graphic-assets-manifest.json`에 남아요).
6. **§0의 상태 문구와 `docs/qa/runtime-verification-required.md`의 해당 행(#103)을 실측에 맞게
   갱신해요.** (2026-09-02: §0·§6은 갱신됨 — #103 행의 상태 갱신은 표 소유 규율에 따라 후속 몫)
...
- [ ] 금융 기능(Financial features) 신고: 모든 앱이 작성해야 한다. 현재 구현은 대출·결제·투자 기능이 없지만 지출 기록·예산·예상 지출을 제공한다. [Google의 신고 항목](https://support.google.com/googleplay/android-developer/answer/13849271)은 `금융 조언`과 `기타`도 포함하므로 **"해당 기능 없음"을 미리 확정하지 않는다**. 최종 AAB와 스토어 설명을 기준으로 콘솔의 실제 선택지를 확인해 정확히 신고하고, 해당 여부가 불명확하면 초안으로 저장한 뒤 Play 지원에 문의한다. [금융 서비스 정책](https://support.google.com/googleplay/android-developer/answer/9876821)의 적용을 받는 서비스라면 계정 유형도 다시 확인한다.
...
- [ ] 문제 없으면:
  ...
  - 현재 계정(2026-09-22 생성 개인 계정): **최소 12명이 14일 연속 참여를 유지하는 비공개 테스트** 후 프로덕션 액세스 신청.
