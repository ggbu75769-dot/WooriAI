...
describe("shared contract schemas", () => {
  ...
  /**
   * 라운드 110 A 후속: **연도도 같은 이유로 묶인다.**
   *
   * 종전 이 스키마의 연도는 `\d{4}`였고 — **그때는 서버와 문자 그대로 같았다** — 위 자가 무는
   * 것은 월뿐이었다. 그런데 그 열린 연도가 R24-L5가 월에서 닫은 병을 연도에서 그대로 남기고
   * 있었다: 서버 실측으로 **12,987개 값**(`yearMonth` 11,988 + 맨 연도 999)이
   * `getSeoulMonthRange`의 `endExclusive` 0채움 부재로 Invalid Date가 되어 500이었다
   * (`0001-01` → `"1-02-01"`, `9999-12` → `"10000-01-01"`). 서버가 `[1900, 2200]`으로 묶었으니
   * 이 줄도 따라간다 — 아니면 계약이 다시 서버보다 넓어져 R24-L5가 막으려던 그 왕복이 생긴다.
   *
   * 이 자는 **두 끝을 다** 문다. 상한만 보는 자는 하한 결함(`0001-01`, 전체의 11/12)을 놓친다.
   */
  it("라운드 110 A: 연도도 서버와 같은 [1900, 2200]으로 묶인다", () => {
    // 창의 두 끝은 통과한다(경계 포함).
    for (const yearMonth of ["1900-01", "1900-12", "2200-01", "2200-12", "2026-08", "1900-01-01"]) {
      expect(listExpensesQuerySchema.parse({ yearMonth }).yearMonth).toBe(yearMonth);
    }

    // 창 밖 — 종전 정규식이 통과시켰고 서버가 500으로 터지던 값들.
    for (const yearMonth of ["9999-12", "0001-01", "0999-11", "1899-12", "2201-01", "0001-01-01"]) {
      expect(() => listExpensesQuerySchema.parse({ yearMonth }), yearMonth).toThrow();
    }
  });
});
