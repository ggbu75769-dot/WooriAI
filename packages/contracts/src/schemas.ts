 ...
 *
 * ---------------------------------------------------------------------------
 * COM-105 후속 — **링크 헬스 판정 중 실패한 것만** 앱에 싣는다(`healthStatus`).
 *
 * 종전(라운드 68 C #4 · 그때는 참): 앱 DTO는 헬스를 한 글자도 노출하지 않았고, `broken`이
 * 앱에서 받는 대우는 ⓐ 정렬 강등과 ⓑ 공유 URL 미발급 **둘뿐**이었다. 그 판단의 근거였던
 * "묵은 판정으로 판매처를 공개 비난하지 않는다"는 지금도 옳다. 다만 그때 **함께 참이었던
 * 사실**이 하나 더 있다: 우리가 도달 실패를 아는 링크를 사용자는 아무 말도 못 들은 채 눌러
 * 앱 밖에서 404를 만났고, 앱은 그 클릭을 성공으로 취급했다.
 *
 * 이제: 그 둘을 동시에 지키는 자리는 "비난하지 않는 관찰 한 줄"이다. 그래서 워커가 **실제로
 * 실패를 관찰한 두 값만** 내려보낸다.
 *
 * ⚠️ `"ok"`가 이 유니온에 **없는 것이 핵심**이다. 실으면 앱이 그것을 "확인됨"으로 그릴 수
 * 있게 되는데, 그 값은 최대 `LINK_HEALTH_INTERVAL_HOURS`(기본 24시간)만큼 묵은 데이터센터발
 * HEAD 응답 하나다 — 그것을 지금 이 사용자에게 "확인됨"이라고 말하면 그것이 허위 표시다.
 * 유니온에서 빼 두면 그 배지는 **타입상 표현 자체가 불가능**하다(모바일에서 `=== "ok"`를
 * 쓰면 컴파일 타임에 터진다).
 *
 * ⚠️ 미확인(null)도 싣지 않는다. 그것은 "괜찮다"는 뜻이 **아니라** 앱이 그 링크에 대해 아무
 * 말도 하지 않는다는 뜻이다(종전 동작 그대로 — 없는 판정을 "확인됨"으로 바꿔 말하는 자리가
 * 어디에도 생기지 않는다). 미확인을 "확인되지 않았어요"로 그리는 선택지는 값으로 재어 보고
 * 버렸다: 워커 후보 조건이 `affiliate_url IS NOT NULL`이라(worker/jobs/link-health.job.ts)
 * 오늘 시드의 링크 67건은 **전부** `affiliate_url`이 null인 일반 검색 링크다(LP-A,
 * prisma/seed-data.ts). 즉 그 문구는 오늘 67/67 = 100%의 링크에 영구히 서고, 정작 진짜 신호
 * 한 줄이 그 사이에 묻힌다.
 *
 * `broken`과 `unstable`을 **한 값으로 합치지 않는다**: 4xx·리디렉트 5홉 초과(확정된 도달
 * 실패)와 5xx·타임아웃(다음 회차에 바로 재확인되는 일시 실패)은 서로 다른 사실이라, 둘에
 * 같은 말을 하면 그중 하나는 틀린 말이 된다. 강등표가 둘을 2와 1로 가른 그 근거 그대로다.
 *
 * 가산 optional이라 이 필드를 모르는 구버전 클라이언트·구버전 서버 응답 모두 종전대로
 * 통과한다. DNC-009 무접촉: 추천 점수·정렬의 입력이 아니라 **표시 전용**이고 수수료율은
 * 여기에 들어가지 않는다.
 */
export const productLinkSchema = z
  .object({
    ...
    /** 위 머리말의 링크 헬스 절 — `"ok"`와 미확인(null)은 **이 유니온에 없다**. */
    healthStatus: z.enum(["broken", "unstable"]).optional()
  })
  .refine(
