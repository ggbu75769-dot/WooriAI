...
//     ⚠️ **두 시점 — 이 축은 라운드 90 트랙 D가 *모집단만* 넓혔다(새 축을 얹지 않았다).**
...
//      · **오늘(라운드 90 트랙 D 재실측)**: `§1-1 #N` 꼴로 가리키는 자리를 **전수**로 건다 —
//        자리 **91** · 인용된 번호 **95**(사슬 셋이 그 차이다: `#37·#40·#45` · `#38·#48` ·
//        `#56·#57`) · 서로 다른 번호 **91**(⚠️ 정찰이 값으로 적은 **87**은 자리마다 **첫 번호만**
//        셌을 때의 값이다 — 그 갈림이 이 트랙의 재실측이고, 하한은 정찰의 87로 둔다).
//        그 전수가 짝 문서 §1-1의 행으로 **실제로 풀리고**(어긋남 **0**), 표면까지 함께 적은
//        참조 **32**는 그 표면이 짝 문서의 행과 **같고**(어긋남 **0**), **역방향**으로 짝 문서가
//        접근성 표를 가리키는 자리 **8**(⚠️ 정찰의 하한은 7 — 줄바꿈을 넘는 자리 하나를
//        `:575`에서 더 찾았다)도 실재로 풀리며, 짝 문서의 번호 행 **163**은 **빠진 번호 0건**이다.
...
/** 표면까지 함께 적은 참조 — `§1-1 **#113**(표면 `브라우저`)` 꼴(오늘 32). */
...
/** 사슬을 편 전수(오늘 95) — 사슬의 둘째·셋째 번호도 좌표다. */
...
/** 짝 문서 §1-1의 범위 — `### 1-1.`부터 다음 `## `까지(0-based 인덱스 · 오늘 393~1157행). */
...
describe("ⓔ 좌표(전수) — 같은 축, 넓힌 모집단 (라운드 90 트랙 D)", () => {
  it("모집단이 0건이 아니다 (오늘 자리 91 · 번호 95 · 서로 다른 번호 91 — 하한은 정찰의 91·87)", () => {
  ...
  it("표면까지 적은 참조는 그 표면이 짝 문서의 행과 같다 (오늘 32 · 어긋남 0)", () => {
    expect(
      surfaceSites.length,
      "표면까지 함께 적은 참조가 줄었어요 — 하한은 정찰의 32입니다"
  ...
  it("유령 방지 — 짝 문서의 번호 행이 163이고 빠진 번호가 0건이다", () => {
...
describe("ⓕ 사각 — 이 그물이 못 보는 것을 값과 하한으로 적는다", () => {
  const BLIND_SPOTS = [
    ...
    },
    {
      id: "좌표-내용",
      statement:
        "ⓔ가 무는 것은 **좌표의 실재**다 — 그 번호가 짝 문서 §1-1의 행으로 풀리는가, 표면을 함께 적었다면 그 표면이 같은가까지다. **그 행의 확인 항목이 접근성 표가 말하는 그 일인지는 세지 않는다**(오늘 자리 91 · 인용 번호 95 전부가 이 사각 안에 있고, 사람이 읽어야 풀리는 판단이다). C-3 한 자리만 예외로 `잠금 오버레이`라는 낱말까지 대조한다 — 라운드 89가 그 자리에 손으로 심은 표식이고, 전수로 옮길 수 있는 형식이 아니다.",
      /** 내용까지는 못 보는 자리 = 인용된 좌표 전수(오늘 95). 모집단이 늘면 사각도 함께 는다. */
      floor: 91,
    ...
    },
    {
      id: "문서가-옮겨-적은-계약-상수",
      statement:
        "이 계약의 상수를 **문서가 손으로 옮겨 적은 자리**는 이 그물 밖이다 — 오늘 갈려 있다: 계약 상수 `MAX_DIVERGENCE = 0`인데 문서 두 자리(`docs/qa/accessibility-offline-checklist.md:1085` C-3 행 · `:1111` 수동 증거 절)는 아직 *\"끝 라운드 차이가 셋을 넘지 않는가\"* 라고 적는다(값 0 ↔ 셋 · 자리 둘). ⚠️ 이 트랙은 그 갈림을 **무는 축을 세우지 않는다**: 오늘 빨간 계약을 세우면 이 트랙이 문서를 고쳐야 하고, 그 순간 *계약이 문서를 지킨다*가 *문서가 계약을 맞춘다*로 뒤집힌다 — 고치는 손은 문서 트랙(F)이다. ⚠️ **재개 조건(사건형): 문서가 인용한 계약 상수의 정합이 한 트랙의 축이 되는 라운드가 서는 날 — 그날 F가 이미 고친 두 자리가 그 계약의 첫 모집단이다.**",
      /** 계약의 ⓒ 축을 산문으로 옮겨 적은 자리 수(오늘 둘 — :1085 · :1111). 수가 무엇이든 자리는 남는다. */
      floor: 2,
