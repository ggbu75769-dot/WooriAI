...
export const LEDGER_BLIND_SPOTS: readonly LedgerBlindSpot[] = [
  ...
  },
  {
    ...
    // 두 시점(토스 라운드 통합): 90 → 78 — T2가 route-surface.test.ts를 고쳐 쓰며 그 파일의
    // 변수 바늘 열셋이 이 갈래에서 빠졌다(넷은 파생 주어로 — derived-subject 136 → 140,
    // 나머지는 리터럴 바늘의 모집단 안 앵커로). ⚠️ 이 수는 앵커를 고쳐 쓰는 걸음마다 줄 수도
    // 있는 **양방향 수**라, 여기 적는 것은 래칫이 아니라 "오늘 잰 하한"이고 내려간 시점을
    // 값으로 남긴다(방향 규율 — 하한 단언은 그대로, 값만 오늘의 자로 내림).
    measuredLowerBound: 78,
