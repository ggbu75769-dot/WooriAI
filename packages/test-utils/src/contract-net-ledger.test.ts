...
// **`CONTRACT_NETS_BEFORE_THIS_ONE`을 export하는 파일 전수**에서 나온다(오늘 **둘**). 값은
// 소스에서 파싱하지 않고 **그 모듈을 실제로 불러** 읽는다 — 손으로 적힌 것을 무는 계약이 다시
// 텍스트만 보면 같은 자리에서 속는다.
//
// ⚠️ **걸은 파일 수는 두 시점으로 적는다**(라운드 90 리뷰 L-1). 이 자리에 처음 적힌 **906**은
// *이 파일이 생기기 직전*의 수였다 — 자기 자신을 더한 **커밋 시점의 실측은 907**(packages 47 +
// apps 860)이고, 같은 라운드의 트랙 B가 `apps/admin`에 스윕 하나를 더한 **오늘 HEAD는 908**
// (packages 47 + apps 861)이다. ⚠️ **세 값 다 계약은 초록이었다** — 아래가 무는 것은 값이 아니라
// **하한**(`SCANNED_FLOOR`)이라, 걷기가 통째로 깨졌을 때만 빨개진다. 그래서 이 수는 지켜야 할
// 약속이 아니라 *언제 몇을 걸었는가*의 기록이다.
//
...
/** 오늘 908(= packages 47 + apps 861 · 커밋 시점 907). 하한만 값으로 진다. */
const SCANNED_FLOOR = 700;
/** apps 뿌리만의 하한 — 오늘 861. 그 뿌리에서 나온 자리가 0건이라는 사실이 사각에 적혀 있다. */
 ...
 *
 * ⚠️ `scannedFiles`는 **두 시점**이다(라운드 90 리뷰 L-1): 이 파일의 커밋 시점 **907**(자기 자신
 * 포함) → 트랙 B가 어드민에 스윕 하나를 더한 **오늘 HEAD 908**. 처음 적힌 906은 자기 자신을 빼고
 * 센 값이었다. 아래 어느 단언도 이 수를 등호로 물지 않는다.
 */
...
describe("ⓕ 사각 — 이 그물이 못 보는 것을 값과 하한으로 적는다", () => {
  const BLIND_SPOTS = [
    ...
    },
    {
      id: "apps-뿌리-0건",
      statement:
        "모집단 걷기는 `apps/**`도 전수로 걷지만(오늘 861 · 커밋 시점 860) 그 뿌리에서 나온 " +
        "자리는 **0건**이다 — 오늘 그물 목록은 `packages/test-utils/src` 둘뿐이라, 걷기의 " +
        "대부분이 아직 아무것도 지키지 않는다. 뿌리를 좁히지 않는 이유는 그물이 앱 쪽에 설 수 " +
        "있기 때문이다.",
      /** apps 아래에서 걸린 소스 수(오늘 861 · 커밋 시점 860). */
