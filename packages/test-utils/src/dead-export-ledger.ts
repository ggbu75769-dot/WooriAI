...
export const LEDGER_BLIND_SPOTS: readonly LedgerBlindSpot[] = [
  ...
  },
  {
    ...
    //
    // 두 시점(라운드 110 B): 244 → 246. ⚠️ **여기서 처음으로 둘의 성질이 갈린다** — 앞의 다섯
    // 번은 전부 오독이었는데, 이번 +2는 진짜 하나 + 오독 하나다(5dedc41 워크트리와 이름 집합을
    // 정렬해 diff 한 실측):
    //   · `useSelectedChildStore` — **진짜다.** apps/mobile/src/offline/messages.ts의
    //     `LOGOUT_UNCOUNTED_TEARDOWN_STORES` 판정 표에 **실제 객체 키**로 선다. 그 표는 선택이
    //     아니라 기존 계약이 강제했다(messages.test.ts의 파생 단언: "session-teardown이
    //     import하는 모든 useXStore는 센다/세지 않는다 중 하나로 판정돼 있어야 한다").
    //     즉 이 하나는 **그물이 제 일을 한 것**이고, 낮출 이유가 없다.
    //   · `clearSessionScopedChildSelection` — 오독이다. 실제 속성/키 자리 0건(grep 실증).
    //     표면은 sync-controller.ts:1037의 주석 줄이 `머리말에 있다.` 로 끝나고 다음 줄이 그
    //     이름으로 시작하는 것 — 라운드 101 햅틱 셋 · 107 · 108이 밟은 그 `.\n이름` 이다.
    // 다섯 번째 오독이지만, 이번에는 **오독만 있는 것이 아니라는 사실**을 함께 적어 둔다:
    // 이 수를 "또 오독이겠거니" 하고 넘기면 진짜 property 자리가 느는 것을 못 본다.
    // 상품 비교 정렬 모듈이 공개한 이름 하나가 기존 속성 이름과 겹쳐 현재 실측은 247이다.
    value: 247,
    floor: 20,
    statement:
