...
export const DNC_GUARD_LEDGER: Readonly<Record<string, DncLedgerEntry>> = {
  "DNC-001": {
    ...
    reason:
      ...
      "⚠️ **라운드 85 트랙 E — 종전 이 칸이 기대던 가설('DNC-016의 스윕이 서면 그 모집단이 이 조항의 첫 가드가 된다 · 같은 축이다')을 " +
      "오늘 그 스윕이 서서 실제로 판정했고, 가설은 거짓이었다.** 이 조항이 이름으로 잠근 포지션 이탈 축은 셋(커뮤니티 · 쇼핑몰 · 일반 가계부)인데 " +
      "그 스윕이 걷는 것은 **셋 중 하나(커뮤니티)뿐**이고(packages/test-utils/src/dnc-scope-guard.ts의 DNC_001_POSITION_AXES가 그 판정을 값으로 진다), " +
      "쇼핑몰(자체 장바구니·결제·독립 카탈로그)과 일반 가계부(아이·시기 축이 빠지는 것)를 읽는 바늘은 0건이다 — " +
      "뒤엣것은 무엇이 **생기는지**가 아니라 **빠지는지**를 물어야 해서 이름을 세는 스윕과 그물의 방향이 반대다. " +
      "그리고 그 하나조차 이 조항의 가드가 아니다: 커뮤니티 라우트가 서면 빨개지는 것은 **DNC-016의 항목**이고, " +
      "이웃의 가드가 증상 하나를 잡는 것으로 이 칸을 채우지 않는다는 것이 이 대장의 판정 기준이다.",
    resumeWhen:
      "탭·핵심 루프 밖에서 포지션을 옮기는 표면이 하나라도 서는 날(커뮤니티 피드·독립 상품 카탈로그·일반 가계부 진입점 등), " +
      "또는 세 축(커뮤니티·쇼핑몰·일반 가계부)을 **전부** 읽는 단언이 서는 날. ⚠️ DNC-016의 스윕이 서는 날을 기다리던 종전 조건은 " +
      "라운드 85에 도래했고 판정 결과 셋 중 하나만 걷혔다(위 이유) — 그 하나가 늘어난다고 이 칸이 채워지는 것은 아니고, " +
      "이 조항이 잠근 것은 **포지션 문장 자체**라 그 문장을 읽는 단언이 서야 한다."
  },
  "DNC-002": {
    state: "guarded",
  ...
  "DNC-016": {
    state: "guarded",
    file: "packages/test-utils/src/dnc-scope-guard.test.ts",
    assertion: "      expect(violations.map(describeViolation), scopeFailureHint(item)).toEqual([]);",
    // ⚠️ H-2: 이 단언은 **여섯을 도는 루프 안**에 있다 — 배열이 비거나 항목이 빠지면 0회 돌고도
    // 초록이다. 그래서 모집단은 그 for 줄과 여섯의 id 전수다(라운드 85 트랙 E).
    population: [
    ...
    covers:
      "라운드 85 트랙 E가 세운 부정 스윕이다 — 범위 밖 여섯(사진/영수증 AI · 커뮤니티 · 가격 추적 · 중고 연동 · " +
      "보험/금융 제휴 · 의료 조언) **각각이 자기 뿌리와 바늘**을 값으로 갖고 항목별로 독립해서 선다. " +
      "뿌리는 아홉이고(스키마의 테이블·열거형·열거형 값·열 · API 엔드포인트 경로 · 앱·어드민 라우트 파일 · " +
      "의존성 이름 · 워커 잡 이름) 각각 이유와 함께 적혀 있으며, 계약이 뿌리의 실재와 이름 산출을 함께 확인한다. " +
      "여섯의 문구는 이 문서의 DNC-016 행에서 파싱해 대조하므로 **조항에 일곱째가 붙으면 그 스윕이 먼저 빨개진다**. " +
      "⚠️ 무는 것은 **이름**이다: 표면(테이블·라우트·엔드포인트·의존성·잡)이 생기는 순간을 잡고, 이름을 감춘 구현은 " +
      "잡지 못한다. 오늘 걸리는 자리는 product_links의 `price_snapshot_krw`·`price_checked_at` 둘뿐이며 " +
      "**이유·재개 조건·증명(이력 테이블 부재 · 주기 잡 부재)** 과 함께 면제 대장에 있고, 그 이유의 참을 같은 계약이 " +
      "소스로 다시 확인한다(정직 표시의 근거이지 가격 추적이 아니다 — 라운드 84 리뷰 L-8). " +
      "문구 축은 이 가드 밖이다: 추천 문구가 효능을 단정하지 않는지는 DNC-020이, 가져오기의 미리보기·승인은 DNC-012가 각각 문다."
  },
  "DNC-017": {
    state: "guarded",
...
/**
 * 래칫 상한 — **오늘 실측값 둘**(DNC-001 · DNC-019)이다.
 *
 * ⚠️ 정찰이 적어 준 값이 아니다. 정찰은 인용 수로 둘을 셌고, 라운드 84 트랙 B가 단언으로 다시
 * 재어 셋이 됐다(위 머리말의 "정찰과 갈린 자리 둘" 참고). 가드가 하나 서면 이 줄을 함께 내린다 —
 * 내리지 않으면 다음 사람이 "셋까지는 비어도 된다"로 읽는다.
 *
 * ⚠️ **라운드 85 트랙 E: 3 → 2.** 실제로 닫힌 것은 **DNC-016 하나**다(부정 스윕이 섰다).
