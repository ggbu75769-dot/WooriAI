...
//    실제로 요구하는 모양이고, **이 대장의 축(ⓒ)이 무는 바늘이다**. 오늘 **61**.
//  · **줄 바늘**(`line`) — 줄 어디에든 *사건형|결정형* 이라는 낱말이 있으면 센다. 오늘 **84**.
//    ⚠️ **이 수는 표기가 아니라 언급까지 센다** — 스물셋의 차이 대부분은 *"재개 조건에는 사건형과
//    결정형이 있고…"* 같은 **관례를 논하는 산문**이다. 그래서 축은 이 바늘을 쓰지 않는다.
//  · **접힘 바늘**(`window`) — 줄 바늘에 **앞뒤 한 줄**을 더해 본다. 표기가 두 줄로 접힌 자리를
//    회수한다. 손의 위치는 오늘 줄 바늘로 **12**, 접힘 바늘로 **14**다.
//
// ## ⓒ 축 — 결정형이면 손의 위치를 함께 적었을 것
//
// 오늘 괄호 바늘의 결정형 자리는 **열둘**이고 그중 **열하나**가 손의 위치를 함께 적었다. 남은
// 하나(라운드 86 Z-1의 기록)는 `DECISIVE_HAND_EXEMPTIONS`에 이유·재개 조건과 함께 선다.
//
...
// 파일은 `dead-export-ledger.ts` 하나다(⚠️ 표기 넷). 같은 조항이 그 넷에도 걸린다.
// ⚠️ **그 파일은 이 라운드 트랙 C의 소유라 이 대장은 읽기만 한다** — 그래서 소스 축의 하한은
// **오늘의 넷이 아니라 셋**이다: C가 오늘 도래한 결정형 조건(`export-const-axis`)을 소진하며 그
// 한 줄을 지울 수 있고, **도래한 조건을 지우는 것은 옳은 손이다.** 그물이 그것을 막으면 족쇄가
// 된다(`SOURCE_AXIS_FILES[].floor`).
//
...
// **형 표기가 아예 없는 산문 조건 119**이고, 그다음이 **라운드 노트(`docs/5차/**`) 221**이다.
//
// ## ⚠️ 전제 재실측 — 정찰의 다섯 수 중 넷이 그대로이고 하나는 바늘이 갈렸다
//
// 정찰(`docs/5차/round89-scout.md`)이 적은 수는 **203 · 61 · 84 · 11 · 14**다. 오늘 워킹트리에서
// 다시 세니 **203 · 61 · 84 · 11 · 12**이고, 마지막 하나는 **틀린 것이 아니라 바늘이 다르다**:
// 손의 위치가 **다음 줄로 접힌 자리 둘**을 함께 세면 정확히 **14**가 된다(`window` 바늘).
...
export const LEDGER_DOCUMENT: LedgerDocument = {
  ...
  reason:
    ...
    "오늘 자리가 각각 하나·셋뿐이고 그중 형을 밝힌 것은 하나(사건형)뿐이며 **이 계약의 축이 무는 " +
    "결정형은 0건**이라, 넣으면 축이 아무것도 지키지 못하는 뿌리가 하나 늘어난다.",
  minSites: 180
};
...
/**
 * 2026-08-31(라운드 89 트랙 D) 워킹트리의 실측 — **기록이지 계약이 아니다.**
 *
 * 계약이 무는 것은 `NOTATION_RATCHET`(하한)뿐이고, 이 표는 *"그날 얼마였나"* 를 남긴다.
 * ⚠️ 오늘은 래칫과 같은 수이지만, **같아야 한다고 계약이 묻지 않는다** — 그것이 전수 일치다.
 */
...
export const SOURCE_AXIS_FILES: readonly SourceAxisFile[] = [
  {
    ...
    floor: 3,
    floorReason:
      "그 넷 중 하나(export-const-axis의 '결정형 · 손은 안')는 오늘 도래한 조건이고 트랙 C가 " +
      "그 자리를 소진하며 지울 수 있다. ⚠️ 도래한 조건을 지우는 것은 옳은 손이므로 그물이 " +
      "그것을 막으면 족쇄가 된다 — 하한은 오늘의 넷이 아니라 셋이다."
  }
];
...
export const LEDGER_BLIND_SPOTS: readonly LedgerBlindSpot[] = [
  {
    ...
    reopenCondition:
      ...
      "그날 이 119가 줄기 시작하고, 이 대장의 하한이 그 방향을 값으로 보여 준다."
  },
  {
    id: "folded-notation",
    what:
  ...
  },
  {
    ...
    what:
      "짝 문서 둘(`runtime-verification-required.md` 하나 · `accessibility-offline-checklist.md` 셋)의 " +
      "재개 조건 자리 넷 — 그중 형을 괄호로 밝힌 것은 **하나뿐**(C-12의 사건형)이고 " +
      "⚠️ **이 계약의 축이 무는 결정형은 0건**이다.",
    why:
  ...
  },
  {
    ...
    why:
      ...
      "강조·인용이 섞인 줄에서 그것은 다른 그물의 일이다). 오늘 언급 210과 자리 203의 차이가 " +
