 ...
 *
 * ⚠️⚠️ **`remeasured`는 이 트랙이 첫 걸음을 떼기 전 워킹트리의 수다** — 오늘 이 파일 자신이 갈래
 * 픽스처를 더하며 *저장소 전수*는 그만큼 커졌고, 같은 라운드의 트랙 A~C도 자기 파일에 조건을
 * 더하고 지운다. 그 낡음은 정정하지 않는다: 이 표가 남기는 것은 *오늘 몇인가*가 아니라
 * **정찰과 이 트랙이 같은 모집단을 어떻게 다르게 셌는가**이고, 오늘 값으로 덮으면 그 대조가
 * 사라진다(라운드 89 리뷰 M-4가 `SCOUT_NEEDLE_VALUES`에 세운 판단 그대로).
 */
