...
describe("§1-1 머리말의 라운드 구간 (ⓔ 신설분 편입)", () => {
  ...
  it("마지막 구간이 이번 라운드 신설분이고, 그 행들이 실제로 표에 있다", () => {
    ...
    // ⚠️⚠️ **두 시점(라운드 95 트랙 F)**: 이 줄은 `"94"`였고 오늘 `"95"`다 — 짝 문서에 `#172`
    // (선택 상태 이중 낭독 셋 · 트랙 A)가 서면서 §1-1의 마지막 구간이 `172~172`가 됐다.
    // ⚠️ **옮기는 손은 그 행을 더하는 라운드의 F이고, 시점은 그 라운드 안이다**(미루면 이 계약이
    // 빨간 채로 선다 — 그 빨강이 곧 *행은 더했는데 머리말을 안 고쳤다*는 신호다).
    expect(last.round).toBe("95");
