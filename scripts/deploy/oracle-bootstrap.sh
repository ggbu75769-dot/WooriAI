...
# ── 8. 시드 (카테고리·준비템·관리자) — **있는 행은 고치지 않는다** ─────────────
# 시드는 기존 관리자 계정의 비밀번호/활성 상태를 절대 덮어쓰지 않는다(ADM-007).
#
# ⚠️ 두 시점(라운드 107 트랙 E · 정찰 S5 D3). 위 한 줄은 오랫동안 `admin_users` **하나에만**
# 참이었다. 콘텐츠 다섯 표(categories · item_templates · item_template_stages ·
# disclosures · product_links)는 반대였다 — 이 줄이 매 배포마다 어드민 편집분을 시드 값으로
# 되돌렸고, product_links는 되돌림이 아니라 **증식**이었다(자연키에 어드민 편집 축인 title이
# 들어 있어, 링크 이름을 고치면 다음 배포가 살아 있는 구매 CTA를 하나 더 만들었다 —
# 실측 67 → 68행). 오늘의 시드는 다섯 표 전부 "없는 행을 만들 뿐, 있는 행을 고치지 않는다"
# 이고, 링크는 안정 식별자 `product_links.seed_key`(마이그레이션 000026)로 자기 행을 찾는다.
#
# **그래서 배포마다 돌리는 것을 유지한다.** 근거 셋:
#   1. 이제 이 명령의 효과는 **추가뿐**이다 — 새 릴리즈가 준비템·카테고리·링크를 더하면
#      그것이 운영에 자동으로 서고, 이미 있는 행에는 아무 일도 일어나지 않는다.
#      "처음 한 번만"으로 바꾸면 새 콘텐츠가 운영에 영영 닿지 않아 운영자가 어드민에
#      손으로 다시 입력해야 한다(그 입력이 곧 시드 목록과 어긋나는 두 번째 진실이 된다).
#   2. 부트스트랩 스크립트는 **재실행이 정상 경로**다(런북이 그렇게 안내한다). 그 재실행이
#      안전하려면 각 단계가 멱등이어야 하고, 이 단계는 이제 실제로 멱등이다.
#   3. 시드가 무엇을 했는지 배포 로그가 값으로 말한다("신규 N · 유지 N · 덮어씀 N").
#      이미 중복 링크가 생긴 환경이면 그 쌍을 `product_links 중복:` 경고로 이름과 함께 찍는다
#      (지우지는 않는다 — 어느 행에 클릭 이력이 붙었는지를 봐야 하는 판단이다).
#
# 콘텐츠를 시드 값으로 **되돌리는** 것은 명시적 opt-in 하나뿐이다: `SEED_OVERWRITE_CONTENT=1`.
# 이 스크립트는 그 값을 절대 설정하지 않는다(운영에서 되돌리기가 필요하면 사람이 그 자리에서
# 한 번 실행한다). 로컬에서 시드 데이터를 고친 뒤 dev/test DB를 맞출 때 쓰는 값이다.
log "시드 실행 (있는 행은 어드민 편집분 그대로 — 라운드 107 E)"
