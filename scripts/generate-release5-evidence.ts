...
const aims = [
  ...
  scenario("CATALOG-PILOT-02", "editorial lead", "409 in-review items", "build worklist without publishing", "DB integration and catalog audit", "AUTOMATED_PASS", ["release5c-readiness.db.test.ts", "admin-release5.browser.test.ts", "release4-catalog-audit.json"]),
...
const findings = [
  ...
  { id: "R5-G-003", severity: "P2", actor: "release engineer", input: "catalog audit without explicit DATABASE_URL", mission: "audit the migrated isolated database", expected: "catalog audit reads a current isolated verification DB", actual: "default older DB lacked current catalog columns", rootCause: "the audit runner implicitly selected mutable wooriai_dev", fix: "provision, migrate, seed, audit, and always drop wooriai_catalog_audit_verify; keep explicit DATABASE_URL as an override", regressionTest: "pnpm catalog:audit PASS and post-run database absence", status: "FIXED", residualRisk: "explicit DATABASE_URL callers remain responsible for selecting the intended target" },
