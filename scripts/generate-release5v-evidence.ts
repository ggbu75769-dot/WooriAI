...
const claims = [
  ...
  ["R5V-C19", "4-tab repository invariant", "apps/mobile/app/(tabs)/_layout.tsx", "TabsLayout", "four visible Tabs.Screen entries", "hidden more route", "apps/mobile/src/android-native-ui-quality.test.ts", 4],
...
const findings = [
  ...
  { id: "R5V-P2-002", severity: "P2", status: "OPEN_ENVIRONMENT", expected: "default dev DB schema matches migration history", actual: "wooriai_dev reports migrations but lacks catalog columns", rootCause: "pre-existing local DB drift", sourcePath: "local PostgreSQL wooriai_dev", fix: "none; audited explicit fresh 40-migration test DB", regressionTest: "release4:verify-db PASS", residualRisk: "default catalog:audit fails until dev DB is rebuilt by owner", ...aim("release operator", "default catalog audit", "avoid treating drifted local DB as source truth") },
...
write("release5v-test-evidence.json", { schemaVersion: 1, generatedAt: now, status: "PASS", commands: [
  ["pnpm --filter api test", "PASS", "65 files / 288 tests"], ["pnpm --filter api test:e2e", "PASS", "22 files / 118 tests"], ["pnpm --filter mobile test", "PASS", "72 files / 397 tests"], ["pnpm --filter admin test", "PASS", "7 files / 34 tests"], ["pnpm --filter @wooriai/domain test", "PASS", "12 files / 74 tests"], ["pnpm --filter @wooriai/contracts test", "PASS", "4 files / 39 tests"], ["pnpm lint", "PASS", "0 warnings"], ["pnpm typecheck", "PASS", "8 workspaces"], ["pnpm release:gate", "PASS", "593 seconds; tests and builds"], ["pnpm release4:verify-db", "PASS", "fresh 40 and upgrade 12->40"], ["pnpm security:secrets", "PASS", "5 high-confidence rules"], ["pnpm security:audit", "PASS", "high/critical 0; moderate 5 reviewed"], ["pnpm catalog:audit (explicit wooriai_test)", "PASS", "408/408/0/84/0"], ["pnpm release4:contamination:export", "PASS", "Hermes production-profile export"]
