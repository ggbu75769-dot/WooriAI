...
export class ReleaseGateAlreadyRunningError extends Error {
  constructor(
    ...
    const ownerSummary = owner
      ...
      : "owner details are still being written";
    super(
...
const maximumLockAgeMs = 3 * 60 * 60_000;
...
export function acquireReleaseGateLock(
  lockPath = join(process.cwd(), ".toolcache", "release-gate.lock")
) {
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    ...
    try {
      ...
      const active = currentOwner
        ...
        && (ageMs === null || ageMs < maximumLockAgeMs);
